...
// Accounting Staging Center — Source → classification → settings → AI → review → draft JE.
const SEED_STAGING = [
 ...
 {id:'STG-004', category:'FAST Cost', type:'Cost', source_system:'FAST', source_id:'FAST-88413', entity_id:5, date:'2026-07-29', amount:6200, description:'Punch-out · Lot 101 Block A (completed unit)', payee:'Summit General Contractors', cost_code:'2HD850', status:'COMPLETED', mapping:'OK', stage:'Pending Coding'},
...
export function StagingCenter({ctx}) {
  ...
  const STAGES=['All','Pending Mapping','Pending Coding','Pending Review','Ready to Post','Posted','Exception','AI decision log'];
  const list = rows.filter(r=>(tab==='All'||r.stage===tab) && (!entity||r.entity_id===entity));
  ...
  const advance = (r)=>{
    if (r.stage==='Pending Mapping'){ toast('Mapping is missing: configure Mapping Center first (exception recorded).','bad');
      actions.ensureException({exception_type:'GL_MAPPING_MISSING', severity:'HIGH', object_type:'STAGING', object_ref:r.source_id, entity_id:r.entity_id, owner:'PROPERTY_ACCT', root_cause:'No matching staging setting'}); return; }
    const j = judge(r);
    if (r.stage==='Pending Coding'){ save(rows.map(x=>x.id===r.id?{...x, stage:'Pending Review', ai:j}:x)); toast(`AI coding complete: Dr ${j.suggested.dr} / Cr ${j.suggested.cr} (${(j.confidence*100).toFixed(0)}%)`); return; }
    if (r.stage==='Pending Review'){ save(rows.map(x=>x.id===r.id?{...x, stage:'Ready to Post', ai:j}:x)); toast('Human review approved → Ready to Post'); return; }
    if (r.stage==='Ready to Post'){
      import('./ai.js').then(m=>m.logAI({input_digest:'human', input_summary:r.source_id, entity:enOf(r.entity_id).entity_code, human_decision:'APPROVED→DraftJE', suggested:j.suggested, confidence:j.confidence, rule:j.rule_used, diff:'none (AI suggestion accepted)'}));
      ...
      toast(`Draft JE created and routed to approval (source trace: ${r.source_id})`); return; }
  };
  const actLabel = {'Pending Mapping':'Configure mapping','Pending Coding':'AI coding','Pending Review':'Approve review','Ready to Post':'Create draft JE','Posted':'✓','Exception':'Resolve'}
  return <div className="full-bleed">
    <h2 className="page-h">Accounting Staging Center</h2>
    <div className="filter-bar">
      <span className="muted sm">Source → Classification → Company Setting → AI Coding → Human Review → Draft JE → Approval → GL. Workflow steps cannot be skipped.</span>
    </div>
    ...
    {tab==='AI decision log' && (()=>{ const log=repo.load('ai_log',[]);
      return <Table exportName="ai-decision-log" pageSize={20} cols={[
        ...
        {h:'Human decision / difference',render:r=>r.human_decision?<Badge tone="ok">{r.human_decision}</Badge>:<span className="muted sm">Suggestion pending review</span>},
      ]} rows={log} empty="No AI decisions recorded."/>; })()}
    {tab!=='AI decision log' && <Table rowKey="id" pageSize={20} exportName="staging" onRow={r=>setAiRow(r)} cols={[
    ...
    ]} rows={list} empty="No source records are waiting in this stage."/>}
    {aiRow && (()=>{ const j=judge(aiRow); return <div className="src-card" style={{marginTop:14}}>
      ...
      <div className="src-grid">
        ...
        <span><i>Human review required</i><b>{j.need_human?'YES':'no'}</b></span>
        <span><i>Amount</i><b>{money(aiRow.amount)}</b></span>
      </div>
      <p className="muted sm" style={{margin:'8px 0 0'}}>Reason: {j.reason} · Evidence: {j.evidence.slice(0,70)} · AI only suggests; a human confirmation is required before a Draft JE is created.</p>
    </div>; })()}
  </div>;
}
