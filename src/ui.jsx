...
export function Table({cols, rows, onRow, empty='No data available', rowKey, features={}, pageSize=25, exportName}) {
  ...
  return <div>
    {(filterable||exportable) && <div className="grid-bar">
      {filterable && <input className="grid-search" placeholder="Filter records (view is saved automatically)" value={q} onChange={e=>{setQ(e.target.value); setPage(0); persist({q:e.target.value});}}/>}
      <span className="grid-count muted">{sorted.length} records</span>
      <span style={{flex:1}}/>
      <button className="grid-tool" onClick={()=>{setDense(d=>{persist({dense:!d}); return !d;});}} title="Row density">{dense?'Comfortable':'Compact'}</button>
