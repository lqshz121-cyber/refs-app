...
export function AuthoritativeGeneralLedgerDetail({row,returnContext,onBack}){
  return <AuthoritativeGeneralLedgerDetailView eyebrow="GENERAL LEDGER" title="Posted ledger line" subtitle="Review this posted ledger line.">
    ...
    <details className="card authoritative-return-context authoritative-gl-identifiers" aria-label="Immutable ledger identifiers"><summary>Audit identifiers</summary><p className="muted sm">Exact IDs for audit and drill-through.</p><div className="authoritative-gl-id-grid"><EvidenceIds label="Journal entry ID" ids={[row.journal_entry_id]}/><EvidenceIds label="Journal line ID" ids={[row.journal_line_id]}/><EvidenceIds label="Ledger line ID" ids={[row.ledger_line_id]}/><EvidenceIds label="Source document IDs" ids={row.source_document_ids}/></div></details>
...
export function AuthoritativeGeneralLedgerWorkspace({config,fetcher=globalThis.fetch,environment=globalThis,onBack=null,backLabel='Back to standard reports',initialAccountCode='',workspaceTitle='General Ledger',workspaceSubtitle='Review posted ledger activity.'}){
  const [query,setQuery]=useState('');const [accountCode,setAccountCode]=useState(initialAccountCode);const [offset,setOffset]=useState(0);const [detail,setDetail]=useState(null);const [state,setState]=useState({phase:'LOADING',rows:[],total:0,error:null});const opener=useRef(null);const scrollY=useRef(0);const tableX=useRef(0);
  ...
  useEffect(()=>{setQuery('');setAccountCode(initialAccountCode);setOffset(0);void load({nextOffset:0,nextQuery:'',nextAccountCode:initialAccountCode});},[config,fetcher,initialAccountCode]);
  ...
  return <AuthoritativeGeneralLedgerView eyebrow="ACCOUNTING" title={workspaceTitle} periodCaption={authoritativeGeneralLedgerPeriodCaption(rows)} subtitle={workspaceSubtitle}>
    {onBack&&<div className="qbo-report-back authoritative-gl-catalog-back"><button type="button" className="btn btn-sm btn-ghost" onClick={onBack}>{backLabel}</button></div>}
