...
export function AuthoritativeGeneralLedgerWorkspace({config,fetcher=globalThis.fetch}){
  ...
  return <section className="full-bleed authoritative-general-ledger" aria-label="Authoritative General Ledger">
    ...
    {state.phase==='READY'&&!rows.length&&<StateBlock tone="empty" title="No POSTED ledger lines returned">This scoped result is not evidence of zero activity outside the entity, period, account, query, or returned server page.</StateBlock>}
