...
export function AuthoritativeGeneralLedgerWorkspace({config,fetcher=globalThis.fetch,environment=globalThis}){
  ...
  return <AuthoritativeGeneralLedgerView eyebrow="GENERAL LEDGER · POSTED EVIDENCE" title="General Ledger" subtitle="Read-only POSTED ledger lines from the accounting API.">
    ...
    <div className="authoritative-filter-bar" aria-label="General Ledger filters"><label><span>Account code</span><input value={accountCode} onChange={event=>setAccountCode(event.target.value)} placeholder="Optional exact account"/></label><label><span>Search posted evidence</span><input value={query} onChange={event=>setQuery(event.target.value)} placeholder="Account, journal, or description"/></label><button type="button" className="btn btn-sm" disabled={loading} onClick={apply}>Apply</button>{(query||accountCode)&&<button type="button" className="btn btn-sm btn-ghost" disabled={loading} onClick={reset}>Reset</button>}<button type="button" className="btn btn-sm btn-ghost" disabled={loading} onClick={()=>void load()}>{loading?'Loading…':'Refresh evidence'}</button><span className="result-count" aria-live="polite"><b>{state.phase==='READY'?state.total:'—'}</b> POSTED lines</span></div>
