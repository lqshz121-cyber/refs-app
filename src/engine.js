...
export function validateJE(je, period) {
  ...
  je.lines.forEach((l,i)=>{ const st=subsidiaryOf(l.account_code);
    if (st && !memberOf(l)) errs.push({code:'4020', msg:`Line ${i+1}: account ${l.account_code} requires a ${st} subsidiary member.`}); });
  const t = jeTotals(je);
  if (t.debit <= 0) errs.push({code:'VAL-001', msg:'Journal entry amount is required.'});
  if (Math.abs(t.debit - t.credit) >= 0.005) errs.push({code:'4006', msg:`Journal entry is out of balance: debit ${money(t.debit)}, credit ${money(t.credit)}.`});
  je.lines.forEach((l,i)=>{
    const bothZero = (!l.debit_amount && !l.credit_amount);
    const both = (l.debit_amount>0 && l.credit_amount>0);
    if (bothZero) errs.push({code:'VAL-002', msg:`Line ${i+1}: a debit or credit amount is required.`});
    if (both) errs.push({code:'VAL-003', msg:`Line ${i+1}: debit and credit cannot both have values.`});
  ...
  if (je.je_type === 'MANUAL' && je.has_attachment === false) errs.push({code:'4010', msg:'A manual journal entry requires an attachment before posting.'});
  return errs;
}
...
export const JE_FLOW = {
  ...
  PENDING_REVIEW:   {next:'PENDING_APPROVAL', action:'Complete review', perm:'GL.JE.REVIEW', reject:'DRAFT'},
  PENDING_APPROVAL: {next:'APPROVED',         action:'Approve', perm:'GL.JE.APPROVE', reject:'DRAFT'},
  APPROVED:         {next:'POSTED',           action:'Post', perm:'GL.JE.POST'},
