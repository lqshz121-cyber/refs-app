...
import {AuthoritativeDemoSourceDocumentsView} from './authoritative-demo-source-documents-view.jsx';
...
export function AuthoritativeSourceDocumentsWorkspace({config,fetcher=globalThis.fetch}){
  ...
  if(state.detail){const detail=state.detail;return <section className="full-bleed qbo-transaction-report authoritative-source-document-detail" aria-labelledby="source-document-detail-title">
  ...
  return <AuthoritativeDemoSourceDocumentsView
    scope="Entity-scoped persisted evidence from the accounting API. Browser state, attachment bytes, provider requests, and WBS calls are not used."
    actions={<button type="button" className="btn btn-sm" onClick={load}>Refresh evidence</button>}
    metrics={<><span><i>In scope</i><b>{counts.documents}</b><small>Retained documents</small></span><span><i>Journal-linked</i><b>{counts.linked}</b><small>API journal references</small></span><span><i>Unlinked</i><b>{counts.unlinked}</b><small>No journal reference returned</small></span><span><i>Source systems</i><b>{counts.sourceSystems}</b><small>Distinct retained sources</small></span></>}>
    <section className="authoritative-source-scope" aria-label="Authoritative Source Document scope"><div><span>Entity scope</span><b>{config.entityId}</b></div><div><span>Period scope</span><b>{config.periodId}</b></div><div><span>Evidence channel</span><b>Authenticated API read</b></div></section>
    <section className="authoritative-source-intro" aria-label="Source Document evidence boundary"><div><b>Evidence register</b><p>Open one retained document to inspect its exact revision, hash, and retained line facts.</p></div><ul><li><b>Included</b><span>Persisted metadata, source identity, and line references.</span></li><li><b>Excluded</b><span>Attachment bytes and raw provider payloads.</span></li><li><b>Read-only</b><span>No import, mapping, posting, or external call.</span></li></ul></section>
  ...
  </AuthoritativeDemoSourceDocumentsView>;
}
