...
export const authoritativeSourceScopeLabels=config=>({
  entity:config?.scopePresentation?.entityLabel||'Configured entity',
  period:config?.scopePresentation?.periodLabel||'Configured period',
});
...
export function AuthoritativeSourceDocumentsWorkspace({config,fetcher=globalThis.fetch}){
  ...
  const scopeLabels=authoritativeSourceScopeLabels(config);
  ...
  return <AuthoritativeSourceDocumentsView
    scope="Persisted source evidence from the authenticated accounting API."
    ...
    <section className="authoritative-source-scope" aria-label="Authoritative Source Document scope"><div title={`Entity ID: ${config.entityId}`}><span>Entity</span><b>{scopeLabels.entity}</b></div><div title={`Period ID: ${config.periodId}`}><span>Period</span><b>{scopeLabels.period}</b></div><div><span>Evidence channel</span><b>Authenticated API read</b></div></section>
    <AuthoritativeSecondaryDisclosure label="Evidence boundaries"><section className="authoritative-source-intro" aria-label="Source Document evidence boundary"><div><b>Evidence register</b><p>Open one retained document to inspect its exact revision, hash, and retained line facts.</p></div><ul><li><b>Included</b><span>Persisted metadata, source identity, and line references.</span></li><li><b>Excluded</b><span>Attachment bytes and raw provider payloads.</span></li><li><b>Read-only</b><span>No import, mapping, posting, or external call.</span></li></ul></section></AuthoritativeSecondaryDisclosure>
