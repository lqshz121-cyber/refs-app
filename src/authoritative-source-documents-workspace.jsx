...
export function AuthoritativeSourceDocumentsWorkspace({config,fetcher=globalThis.fetch}){
  ...
  const companyReference=config?.scopePresentation?.entityDetail||config.entityId;
  const periodReference=config?.scopePresentation?.periodDetail||config.periodId;
  ...
  return <AuthoritativeDemoSourceDocumentsView
    scope="Read-only source records for the selected company. Attachments and provider requests are not displayed here."
    ...
    <section className="authoritative-source-scope" aria-label="Source Documents scope"><div title={`Company reference: ${companyReference}`}><span>Company</span><b>{companyLabel}</b></div><div title={`Period reference: ${periodReference}`}><span>Period</span><b>{periodLabel}</b></div><div><span>Access</span><b>View only</b></div></section>
