...
export function AuthoritativeSourceDocumentsWorkspace({config,fetcher=globalThis.fetch}){
  ...
  return <AuthoritativeDemoSourceDocumentsView
    ...
    <section className="authoritative-source-scope" aria-label="Authoritative Source Document scope"><div><span>Entity scope</span><b title={`Entity ID: ${config.entityId}`}>Configured entity</b></div><div><span>Period scope</span><b title={`Period ID: ${config.periodId}`}>Configured period</b></div><div><span>Evidence channel</span><b>Authenticated API read</b></div></section>
