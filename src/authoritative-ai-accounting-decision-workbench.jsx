...
export function AuthoritativeAiAccountingDecisionWorkbench({config,fetcher=globalThis.fetch,cryptoApi=globalThis.crypto,onAccountingRefresh,onOpenJournalWorkflow}){
  ...
  const retainPostedOutcome=async()=>{
    const idempotencyKey=await aiAccountingPostedOutcomeIdempotencyKey({config,decision:selected});
    if(!idempotencyKey){setCommand({phase:'BLOCKED',data:null,error:{code:'AI_POSTED_OUTCOME_REVIEW_COMMAND_INVALID',message:'Choose a Posted retained decision with its current review revision.'}});return;}
    setCommand({phase:'LOADING',data:null,error:null});
    const result=await retainAuthoritativeAiAccountingPostedOutcomeReview({config,decision:selected,idempotencyKey,fetcher});
    setCommand(result.ok?{phase:'READY',data:result.data,error:null}:{phase:'BLOCKED',data:null,error:result});
    if(result.ok){setHistory(initialHistory);await load(offset);await onAccountingRefresh?.();}
  };
  const loadPostedOutcomeHistory=async()=>{
    setHistory({phase:'LOADING',data:null,error:null});
    const result=await refreshAuthoritativeAiAccountingPostedOutcomeHistory({config,decision:selected,limit:200,fetcher});
    setHistory(result.ok?{phase:'READY',data:result.data,error:null}:{phase:'BLOCKED',data:null,error:result});
  };
  ...
  return <section className="card" aria-label="Retained AI accounting decision workflow">
    ...
    {selected?<div className="report-workbench" aria-label="Selected AI accounting decision human workflow"><div className="report-workbench-head"><div><b>{selected.packet.classification} decision</b><div className="page-subtitle">The command binds the exact retained decision hash. A concurrent or changed decision fails closed.</div></div><span className={`badge ${selected.packet.risk.risk_level==='HIGH'?'badge-danger':selected.packet.risk.risk_level==='MEDIUM'?'badge-warning':'badge-muted'}`}>{selected.packet.risk.risk_level}</span></div>
      ...
      {selected.action_flags.can_retain_posted_outcome?<div className="filter-bar"><div><b>Server-derived Posted review</b><div className="page-subtitle">Re-read the exact accepted decision, four-stage Journal workflow, Posted ledger, and retained report snapshot. The browser cannot supply or change accounting evidence.</div></div><button type="button" className="btn" onClick={retainPostedOutcome} disabled={command.phase==='LOADING'}>{command.phase==='LOADING'?'Reviewing Posted outcome...':'Retain Posted outcome review'}</button></div>:null}
      {selected.latest_posted_outcome_review?<div className="filter-bar"><div><b>Immutable Posted review history</b><div className="page-subtitle">Load only a population-complete, decision-bound history. Saturated or mismatched evidence fails closed.</div></div><button type="button" className="btn" onClick={loadPostedOutcomeHistory} disabled={history.phase==='LOADING'}>{history.phase==='LOADING'?'Loading complete history...':'Load complete review history'}</button></div>:null}
      {selected.draft_evidence?.journal_status==='DRAFT'?<StateBlock tone="ok" title="Retained AI accounting Draft" actions={typeof onOpenJournalWorkflow==='function'?<button type="button" className="btn" onClick={()=>onOpenJournalWorkflow(selected.draft_evidence)}>Continue in Journal entries</button>:null}>Journal {selected.draft_evidence.journal_entry_id} remains a Draft at revision {selected.draft_evidence.journal_revision}. The exact Draft will be re-read before the standard Journal workflow opens.</StateBlock>:null}
      {!selected.action_flags.can_accept_or_reject&&!selected.action_flags.can_create_draft&&!selected.action_flags.can_retain_posted_outcome&&selected.draft_evidence?.journal_status!=='DRAFT'?<StateBlock tone="empty" title="No decision command available">This retained decision is already decided, has progressed beyond Draft, is not yet Posted, or the current actor lacks the exact permission. Standard Journal workflow authority remains separate.</StateBlock>:null}
    </div>:null}
    {history.phase==='BLOCKED'?<StateBlock tone="blocked" title={history.error?.code||'AI_POSTED_OUTCOME_REVIEW_HISTORY_BLOCKED'}>{history.error?.message} No partial or cached review history is substituted.</StateBlock>:null}
    {history.phase==='READY'?<div className="table-wrap" role="region" tabIndex={0} aria-label="Complete immutable Posted outcome review history; scroll horizontally to view every column"><table className="tbl"><thead><tr><th>Revision</th><th>Status</th><th>Reason codes</th><th>Evidence checks</th><th>Retained trace</th></tr></thead><tbody>{history.data.rows.map(review=><tr key={review.ai_accounting_posted_outcome_review_id}><td>{review.review_revision}</td><td><b>{review.status}</b><div className="muted sm">{review.reviewed_at}</div></td><td>{review.reason_codes.length?review.reason_codes.join(', '):'None'}</td><td><div>Proposed journal {review.evidence.proposed_journal_exact?'exact':'not exact'}</div><div>Posted ledger {review.evidence.posted_ledger_exact?'exact':'not exact'}</div><div>Workflow {review.evidence.workflow_exact?'exact':'not exact'}</div><div>Report snapshot {review.evidence.report_snapshot_exact?'exact':'not exact'}</div></td><td><details><summary>Immutable hashes</summary><dl className="muted sm"><dt>Review hash</dt><dd title={review.review_hash}>{shortHash(review.review_hash)}</dd><dt>Workflow evidence</dt><dd title={review.evidence.workflow_evidence_hash}>{shortHash(review.evidence.workflow_evidence_hash)}</dd><dt>Ledger lines</dt><dd title={review.evidence.ledger_lines_hash}>{shortHash(review.evidence.ledger_lines_hash)}</dd><dt>Report snapshot</dt><dd title={review.evidence.financial_statement_snapshot_hash||''}>{shortHash(review.evidence.financial_statement_snapshot_hash)}</dd></dl></details></td></tr>)}</tbody></table></div>:null}
    ...
    {command.phase==='READY'?<StateBlock tone="ok" title={command.data?.schema_version==='AI_ACCOUNTING_POSTED_OUTCOME_REVIEW_V1'?'Posted outcome review retained':command.data?.status==='DRAFT'?'AI accounting Draft created':'Human decision retained'} actions={command.data?.status==='DRAFT'&&typeof onOpenJournalWorkflow==='function'?<button type="button" className="btn" onClick={()=>onOpenJournalWorkflow(command.data)}>Continue in Journal entries</button>:null}>{command.data?.schema_version==='AI_ACCOUNTING_POSTED_OUTCOME_REVIEW_V1'?`Review revision ${command.data.review_revision} is ${command.data.status}. The immutable evidence and review hash are retained; no Journal action was performed.`:command.data?.status==='DRAFT'?`Journal ${command.data.journal_entry_id} is a Draft at revision 0. The exact Draft will be re-read before the standard Journal workflow opens.`:`The ${command.data?.outcome?.toLowerCase()} decision is retained as immutable human evidence.`}</StateBlock>:null}
  </section>;
}
