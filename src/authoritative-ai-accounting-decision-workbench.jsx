...
import {aiAccountingDecisionCommandIdempotencyKey,aiAccountingPostedOutcomeIdempotencyKey,createAuthoritativeAiAccountingDecisionDraft,decideAuthoritativeAiAccountingDecision,refreshAuthoritativeAiAccountingDecisionQueue,retainAuthoritativeAiAccountingPostedOutcomeReview} from './accounting-api.js';
...
export function AuthoritativeAiAccountingDecisionWorkbench({config,fetcher=globalThis.fetch,onAccountingRefresh}){
  ...
  const retainPostedOutcome=async()=>{
    ...
    setCommand(result.ok?{phase:'READY',data:result.data,error:null}:{phase:'BLOCKED',data:null,error:result});
    if(result.ok){await load(offset);await onAccountingRefresh?.();}
  };
  ...
  return <section className="card" aria-label="Retained AI accounting decision workflow">
    ...
    {selected?<div className="report-workbench" aria-label="Selected AI accounting decision human workflow"><div className="report-workbench-head"><div><b>{selected.packet.classification} decision</b><div className="page-subtitle">The command binds the exact retained decision hash. A concurrent or changed decision fails closed.</div></div><span className={`badge ${selected.packet.risk.risk_level==='HIGH'?'badge-danger':selected.packet.risk.risk_level==='MEDIUM'?'badge-warning':'badge-muted'}`}>{selected.packet.risk.risk_level}</span></div>
      ...
      {!selected.action_flags.can_accept_or_reject&&!selected.action_flags.can_create_draft&&!selected.action_flags.can_retain_posted_outcome?<StateBlock tone="empty" title="No decision command available">This retained decision is already decided, has a Draft, is not yet Posted, or the current actor lacks the exact permission. Continue any Draft in Journal entries with separately authorized actors.</StateBlock>:null}
    </div>:null}
    ...
    {command.phase==='READY'?<StateBlock tone="ok" title={command.data?.schema_version==='AI_ACCOUNTING_POSTED_OUTCOME_REVIEW_V1'?'Posted outcome review retained':command.data?.status==='DRAFT'?'AI accounting Draft created':'Human decision retained'}>{command.data?.schema_version==='AI_ACCOUNTING_POSTED_OUTCOME_REVIEW_V1'?`Review revision ${command.data.review_revision} is ${command.data.status}. The immutable evidence and review hash are retained; no Journal action was performed.`:command.data?.status==='DRAFT'?`Journal ${command.data.journal_entry_id} is a Draft at revision 0. Continue in Journal entries.`:`The ${command.data?.outcome?.toLowerCase()} decision is retained as immutable human evidence.`}</StateBlock>:null}
  </section>;
}
