...
export function GLTrialBalance({ctx}) {
  ...
  if (!hasReportEntity && !drill) return <div className="full-bleed">
    ...
    {periodBar}
    <StateBlock tone="permission" title="Select an entity to run this report"><span>Financial statements, control totals, drill-downs, and evidence status are never calculated across all entities.</span><small>Choose one entity in the global entity selector to load same-entity POSTED evidence.</small></StateBlock>
  </div>;
  ...
  return <div className={drill ? 'full-bleed report-replacement-view' : 'full-bleed'}>
    ...
    <details className="gl-evidence-fold">
      <summary>Report control evidence · scope, AP / AR control bridge, bank reconciliation</summary>
    ...
    {tab==='Balance Sheet' && (()=>{ const rhs=bsSt.liabilities+bsSt.equity+bsSt.netIncome; const ok=Math.abs(bsSt.assets-rhs)<0.01;
      ...
      return <div className="stmt stmt-wide">
        ...
        {!bsTb.rows.length ? <StateBlock tone="empty" title="No posted local activity">No posted local activity through {toP} for {dimensionLabel}.</StateBlock> : <>
    ...
    {tab==='Income Statement' && (()=>{ const rev=tb.rows.filter(r=>r.type==='REVENUE'); const exp=tb.rows.filter(r=>r.type==='EXPENSE');
      ...
      return <div className="stmt stmt-wide">
        ...
        {!rev.length && !exp.length ? <StateBlock tone={incomeScopeState.state === 'NO_LOCAL_EVIDENCE_IN_SCOPE' ? 'permission' : 'empty'} title={incomeScopeState.state === 'NO_LOCAL_EVIDENCE_IN_SCOPE' ? 'Select an entity before viewing Income Statement' : incomeScopeState.state === 'NO_POSTED_LOCAL_ACTIVITY' ? 'No POSTED local activity in this Income Statement scope' : 'No revenue or expense activity in this Income Statement scope'}><span>{incomeScopeState.detail}</span><small>Scope: {fromP} to {toP} · {dimensionLabel}. CWIP, prepaid, deposits, escrow and restricted cash are not substituted for P&L activity.</small></StateBlock> : <>
...
export function Reports({ctx}) {
  ...
  if (open) return <div className="reports-library report-replacement-view">
    ...
    {['LOCAL_PREVIEW','LOCAL_CONTROL'].includes(previewMeta?.capability.state) && REPORTS[open]
      ? REPORTS[open]()
      : <StateBlock tone="empty" title="Reference only — no local workflow"><span>This report name is not part of the real-estate close scope. REFS does not connect, synchronize, create, save, share, email, print, export, or expose source data here.</span><small>Use Back to return to the Reports Center.</small></StateBlock>}
  </div>;
  return <div className="reports-library">
    ...
    </div></> : <StateBlock tone="permission" title="Select an entity to view financial results"><span>REFS does not aggregate entity balances, journal counts, or mock projections into an “All entities” report.</span><small>The report catalog remains available below; local report actions require one selected entity.</small></StateBlock>}
...
export function Assets({ctx}) {
  ...
  return <div className="full-bleed">
    <div className="accounting-page-head"><div><div className="page-eyebrow">ACCOUNTING · FIXED ASSETS / CWIP</div><h2 className="page-h">Fixed assets</h2><div className="page-subtitle">Posted local acquisition, CWIP and capitalized-interest evidence only. No asset valuation or tax depreciation is inferred.</div></div><div className="row-acts"><Btn size="sm" variant="ghost" onClick={()=>setShowHowItWorks(open=>!open)}>How it works</Btn><Btn size="sm" variant="ghost" onClick={()=>goto('gl',{route:'gl',tab:'Balance Sheet',toP:toPeriod})}>See reports</Btn><Unavailable reason="Asset creation is not adopted in this evidence-only register.">Add an asset</Unavailable></div></div>
    ...
    {tab==='Depreciation evidence' && <StateBlock tone="empty" title="No local depreciation schedule is established"><span>CWIP is not depreciated. In-service balances require retained useful-life, in-service date, accumulated-depreciation, and POSTED depreciation JE evidence before a schedule can be shown.</span></StateBlock>}
    {tab==='Disposals' && <StateBlock tone="empty" title="No retained disposal/reversal chain is available"><span>Asset disposal is not inferred from an account credit. A complete source, proceeds, accumulated depreciation, gain/loss and reversal chain is required.</span></StateBlock>}
  </div>;
}
...
export function IntegrationHub({ctx}) {
  ...
  return <div className="full-bleed integration-hub" aria-label="Integration Hub">
    ...
    <Table rowKey="batch_id" cols={[
      ...
      {h:'Notes',k:'err'},
      {h:'Actions',render:()=> <Unavailable reason="External retry and source synchronization are outside the retained local evidence scope.">No action</Unavailable>},
    ]} rows={localBatches} />
  </div>;
...
export function RuleCenter() {
  ...
  return <div><h2 className="page-h">Accounting Rule Center</h2>
    <section className="report-workbench" aria-label="Local accounting rule records" style={{marginBottom:12}}>
      <div className="report-workbench-head"><div><b>Rules</b><div className="page-subtitle">Locally controlled rule records. Rule authoring and posting automation are not REFS capabilities.</div></div></div>
      <div className="report-shelf"><label className="qbo-report-search"><input aria-label="Search local rules by name or conditions" placeholder="Search rules by name or conditions" value={query} onChange={e=>setQuery(e.target.value)} /></label></div>
      <p className="muted sm">Showing {filteredRules.length} locally controlled records. Priority is display-only; rule creation, selection, posting automation, and editing are not available in REFS.</p>
    </section>
    <p className="muted sm">Rules are controlled local records. Search affects only these records, and their state labels are local states only.</p>
    {filteredRules.length ? <Table cols={[{h:'Priority',render:r=>r.priority},{h:'Rule name',render:r=>r.id},{h:'Applied to',render:r=>r.appliedTo},{h:'Conditions',render:r=>r.conditions},{h:'Settings',render:r=>r.settings},{h:'Posting automation',render:r=><span className="muted">{r.autoPost}</span>},{h:'Status',render:r=><Badge tone="ok">{r.status}</Badge>},{h:'Actions',render:()=> <Unavailable reason="Rule editing is not a REFS action.">Edit</Unavailable>}]} rows={filteredRules} rowKey="id" /> : <StateBlock tone="empty" title="No local rules found">Try a different rule name or condition.</StateBlock>}</div>;
}
