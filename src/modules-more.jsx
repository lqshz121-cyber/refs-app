...
export function RuleCenter() {
  ...
  return <div><h2 className="page-h">Accounting Rule Center</h2>
    <section className="report-workbench" aria-label="Observed QuickBooks Rules shell" style={{marginBottom:12}}>
      ...
      <div className="qbo-drill-summary">{['Drag to reorder','Select','Priority','Rule name','Applied to','Conditions','Settings','Posting automation','Status','Actions'].map(label=><span key={label}><i>{label}</i><b>Observed column</b></span>)}</div>
      <p className="muted sm">Showing {filteredRules.length} locally controlled records. Priority is display-only; selection, posting automation, Edit, and menus remain unavailable.</p>
    </section>
    ...
    {filteredRules.length ? <Table cols={[{h:'Priority',render:r=>r.priority},{h:'Rule name',render:r=>r.id},{h:'Applied to',render:r=>r.appliedTo},{h:'Conditions',render:r=>r.conditions},{h:'Settings',render:r=>r.settings},{h:'Posting automation',render:r=><span className="muted">{r.autoPost}</span>},{h:'Status',render:r=><Badge tone="ok">{r.status}</Badge>},{h:'Actions',render:()=> <Btn size="sm" variant="ghost" disabled>Edit</Btn>}]} rows={filteredRules} rowKey="id" /> : <div className="empty-state"><b>No local rules found</b><p>Try a different rule name or condition. This does not query or modify QuickBooks.</p></div>}</div>;
}
