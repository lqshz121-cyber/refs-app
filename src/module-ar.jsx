...
import { KPI, Btn, Badge, Money, Table, Tabs, Drawer, Field, StateBlock, Unavailable } from './ui.jsx';
  ...
  return <div className="ar-workspace">
    ...
    {tab === 'Invoices' && <>
      ...
      <Table rowKey="inv_id" features={{exportable:false}} onRow={row=>openInvoiceDetail(row.inv_id, {tab:'Invoices',asOfDate})} cols={[
        ...
        {h:'Receipt evidence',render:row=>row.status==='OPEN'?<Unavailable reason={row.localEvidence.receivePaymentAllowed?'Receipt recording is unavailable here; review retained receipt or bank evidence.':`Blocked: ${row.localEvidence.sourceState}`}>Receipt unavailable</Unavailable>:<span className="muted sm">{row.pay_je_number || row.localEvidence.receiptState}</span>},
    ...
    {tab === 'Receipts' && <section className="ap-aging-shell" aria-label="Local customer payment evidence">
      ...
      ]} rows={visibleCustomerPayments}/>:<StateBlock tone="empty" title="No retained customer receipts">No retained customer receipts match this view.</StateBlock>}
...
function InvoiceDetail({invoice, onClose, goto, sourceStateTone, returnScope}) {
  ...
  return <div className="full-bleed qbo-transaction-report">
    ...
    <div className="row-acts" style={{marginTop:12}}>
      {invoice.localEvidence.sourceJournal ? <Btn size="sm" variant="ghost" onClick={()=>goto('je',{jeNumber:invoice.je_number,arReturn:invoiceEvidenceReturn})}>Open source JE</Btn> : <Unavailable reason="No AR journal entry is retained for this invoice.">No retained AR JE</Unavailable>}
      {invoice.localEvidence.paymentJournal ? <Btn size="sm" variant="ghost" onClick={()=>goto('je',{jeNumber:invoice.pay_je_number,arReturn:invoiceEvidenceReturn})}>Open receipt JE</Btn> : <Unavailable reason="No receipt journal entry is retained for this invoice.">No retained receipt JE</Unavailable>}
      {invoice.localEvidence.exactBankCredits[0] ? <Btn size="sm" variant="ghost" onClick={()=>goto('banktx',{route:'banktx',acctCode:invoice.localEvidence.exactBankCredits[0].bank_account_code,bankTxnId:invoice.localEvidence.exactBankCredits[0].bank_txn_id,arReturn:invoiceEvidenceReturn})}>Open bank evidence</Btn> : <Unavailable reason="No exact retained bank CREDIT matches this record.">No exact bank credit</Unavailable>}
...
function ReceiptDetail({receipt, onClose, goto, sourceStateTone, returnScope}) {
  ...
  return <div className="full-bleed qbo-transaction-report">
    ...
    <div className="row-acts" style={{marginTop:12}}>
      ...
      {bankCredit ? <Btn size="sm" variant="ghost" onClick={()=>goto('banktx',{route:'banktx',acctCode:bankCredit.bank_account_code,bankTxnId:bankCredit.bank_txn_id,arReturn:receiptEvidenceReturn})}>Open bank credit</Btn> : <Unavailable reason="No exact retained bank CREDIT matches this record.">No exact bank credit</Unavailable>}
    </div>
