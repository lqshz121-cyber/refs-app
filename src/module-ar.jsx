  ...
  return <div>
    ...
    {tab === 'Invoices' && <>
      ...
      <Table rowKey="inv_id" features={{exportable:false}} onRow={row=>openInvoiceDetail(row.inv_id, {tab:'Invoices',asOfDate})} cols={[
        ...
        {h:'Action',render:row=>row.status==='OPEN'?<Btn size="sm" variant="primary" disabled={!row.localEvidence.receivePaymentAllowed} title={row.localEvidence.receivePaymentAllowed?'Record a local receipt':`Blocked: ${row.localEvidence.sourceState}`} onClick={event=>{event.stopPropagation();actions.receivePayment(row.inv_id);toast('Local receipt recorded; exact bank matching remains separate.');}}>Receive local payment</Btn>:<span className="muted sm">{row.pay_je_number || row.localEvidence.receiptState}</span>},
