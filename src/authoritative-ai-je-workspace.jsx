...
export function AuthoritativeAiJeWorkspace({config,fetcher=globalThis.fetch,onAccountingRefresh,onOpenJournalWorkflow}){
  ...
  return <AuthoritativeWorkspaceView area="AI JE Workbench">
    ...
    <section className="report-workbench" aria-label="AI amortization Draft workbench">
      ...
      {command.phase==='READY'?<StateBlock tone="ok" title="Amortization Draft created" actions={typeof onOpenJournalWorkflow==='function'?<button type="button" className="btn" onClick={()=>onOpenJournalWorkflow(command.data)}>Continue in Journal entries</button>:null}>Journal {command.data.journal_entry_id} is a MANUAL Draft at revision 0. The exact Draft will be re-read before the standard Journal workflow opens; submit, review, approve, and post remain separately authorized.</StateBlock>:null}
