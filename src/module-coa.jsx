...
export function COAWorkspace({ctx}) {
  ...
  useEffect(() => {
    ...
    setCoaPage(Math.max(0, Number(navContext.coaReturn.coaPage) || 0));
  ...
  return <div>
    ...
    <section className="filter-bar accounting-filter-bar" aria-label="Chart of accounts filters" style={{marginBottom:12}}>
      <label><span className="filter-label">Filter by name or number</span><input aria-label="Filter by name or number" value={qboQuery} onChange={e=>{setQboQuery(e.target.value);setCoaPage(0);}} placeholder="Filter by name or number" /></label>
      <label><span className="filter-label">Account type</span><select aria-label="Account type" value={accountType} onChange={e=>{setAccountType(e.target.value);setCoaPage(0);}}><option value="ALL">All account types</option>{[...new Set(coa.map(account=>account.account_type).filter(Boolean))].sort().map(type=><option key={type} value={type}>{type}</option>)}</select></label>
    </section>
    ...
    <Table rowKey="account_code" cols={localAccountColumns} rows={filteredCoa} pageSize={200} page={coaPage} onPageChange={setCoaPage} />
