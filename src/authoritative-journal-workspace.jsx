...
import {exportAuthoritativeJournalUploadFile,readAuthoritativeJournalEntryDetail,readAuthoritativeJournalWorkflowCapabilities,refreshAuthoritativeJournalEntries,transitionAuthoritativeJournal} from './accounting-api.js';
...
const journalReturnScope = (entityId, journal, context) => [
  context?.entityLabel || 'Configured entity',
...
export function AuthoritativeJournalTable({ journals = [], entityId=null, view = DEFAULT_AUTHORITATIVE_LIST_VIEW, onViewChange, onOpen, capabilities=null, onWorkflowAction, workflowState=null }) {
  ...
  return <AuthoritativeJournalView>
    <section className="authoritative-journal-summary" aria-label="Journal entry queue summary">
      <button type="button" className={`journal-summary-card ${view.status==='ALL'?'journal-summary-card-on':''}`} aria-pressed={view.status==='ALL'} onClick={()=>setQueue('ALL')}><span>Entity register</span><b>{queueCounts.all}</b><small>All entries</small></button>
      ...
      <button type="button" className={`journal-summary-card ${view.status==='REVIEW_REQUIRED'?'journal-summary-card-on':''}`} aria-pressed={view.status==='REVIEW_REQUIRED'} onClick={()=>setQueue('REVIEW_REQUIRED')}><span>Needs review</span><b>{queueCounts.review}</b><small>Awaiting action</small></button>
      <button type="button" className={`journal-summary-card ${view.status==='POSTED'?'journal-summary-card-on':''}`} aria-pressed={view.status==='POSTED'} onClick={()=>setQueue('POSTED')}><span>Posted</span><b>{queueCounts.posted}</b><small>Posted entries</small></button>
    </section>
    <div className="filter-bar authoritative-list-filters" role="search" aria-label="Journal entry presentation filters">
      ...
      <span className="result-count" aria-live="polite">{page.total} {page.total===1?'result':'results'}</span>
    ...
    {!page.total ? journals.length?<StateBlock tone="empty" title="No journal entries match these filters">Try changing or resetting the filters. This result does not confirm zero ledger activity.</StateBlock>:<AuthoritativeScopeEmpty subject="Journal entries" requiresPosted/> : <div className="table-wrap authoritative-journal-table" tabIndex={0} aria-label="Journal entry list; scroll horizontally to view every column"><table className="tbl">
      <thead><tr><th>Journal</th><th>Date</th><th>Memo / description</th><th>Type</th><th>Currency</th><th>Status</th><th>Ledger lines</th><th>Details</th><th>Workflow</th></tr></thead>
      <tbody>{page.rows.map(row => {const next=nextAuthoritativeJournalWorkflowAction(row,capabilities,entityId),busy=workflowState?.journalEntryId===row.journal_entry_id&&workflowState.phase==='RUNNING';return <tr key={row.journal_entry_id}>
        ...
        <td>{row.ledger_line_count}</td><td><button id={`authoritative-journal-${row.journal_entry_id}`} type="button" className="btn btn-sm" onClick={event => onOpen(row,`authoritative-journal-${row.journal_entry_id}`,Number(event.currentTarget.closest('.table-wrap')?.scrollLeft)||0)}>View details</button></td>
...
export function AuthoritativeJournalDetail({journal,entityId,returnContext,onBack}) {
  ...
  return <section className="full-bleed qbo-transaction-report authoritative-evidence-page authoritative-journal-detail" aria-label="Journal entry evidence">
    ...
    <header className="journal-evidence-header"><div><div className="authoritative-eyebrow">JOURNAL ENTRY</div><h1>Journal entry {journal.journal_number}</h1><p className="page-subtitle">Review journal lines and posting details.</p></div><span className={`badge ${scopeMatches?'journal-detail-status':'badge-danger'}`}>{scopeMatches?journal.status:'BLOCKED'}</span></header>
    {!scopeMatches?<StateBlock tone="blocked" title="BLOCKED - immutable Journal scope mismatch">The detail does not match the entity, period, Journal ID, revision, and currency frozen in its parent return context. No line, ledger, or source evidence is asserted.</StateBlock>:<>
      <section className="journal-evidence-scope" aria-label="Journal evidence scope"><span><b>Entity</b>{returnContext?.entityLabel||'Configured entity'}</span><span><b>Period</b>{returnContext?.periodLabel||'Period unavailable'}</span><span><b>Currency</b>{journal.currency}</span><span><b>Revision</b>{journal.revision}</span><span><b>Journal date</b>{formatAuthoritativeDate(journal.journal_date)}</span></section><details className="authoritative-scope-identifiers"><summary>Scope identifiers</summary><dl><div><dt>Entity ID</dt><dd>{entityId}</dd></div><div><dt>Period ID</dt><dd>{journal.period_id}</dd></div></dl></details>
      <dl className="evidence-grid journal-evidence-grid"><div><dt>Journal</dt><dd>{journal.journal_number}</dd></div><div><dt>Journal ID</dt><dd>{journal.journal_entry_id}</dd></div><div><dt>Type</dt><dd>{journal.journal_type}</dd></div><div><dt>Journal line count</dt><dd>{lines.length}</dd></div><div><dt>Created</dt><dd>{formatAuthoritativeDate(journal.created_at)}</dd></div><div><dt>Posted</dt><dd>{journal.posted_at?formatAuthoritativeDate(journal.posted_at):'Not posted'}</dd></div><div><dt>Description</dt><dd>{journal.description||'No description returned'}</dd></div></dl>
      <section className="authoritative-journal-line-evidence" aria-label="Authoritative journal line evidence"><div className="authoritative-section-heading"><div><div className="authoritative-eyebrow">JOURNAL LINES</div><h2>Journal lines</h2><p className="page-subtitle">Ordered debit and credit lines.</p></div><span className="badge">{lines.length} {lines.length===1?'line':'lines'}</span></div>
        <div className="table-wrap authoritative-journal-line-table" tabIndex={0} aria-label="Journal line evidence; scroll horizontally to view every column"><table className="tbl"><thead><tr><th>Line</th><th>Account</th><th>Debit</th><th>Credit</th><th>Member</th><th>Dimensions</th><th>Description</th></tr></thead><tbody>{lines.map(line=><tr key={line.journal_line_id}><td>{line.line_no}</td><td>{line.account_code}</td><td>{line.debit_amount}</td><td>{line.credit_amount}</td><td>{line.member_ref||'None returned'}</td><td>{Object.keys(line.dimensions).length?JSON.stringify(line.dimensions):'None returned'}</td><td>{line.description||'None returned'}</td></tr>)}</tbody></table></div>
        <details className="authoritative-return-context authoritative-journal-line-identifiers"><summary>Audit identifiers</summary><div className="authoritative-journal-line-id-list">{lines.map(line=><div className="authoritative-journal-line-id-item" key={line.journal_line_id}><b>Line {line.line_no}</b><span><i>Journal line ID</i><code>{line.journal_line_id}</code></span><span><i>Ledger line ID</i><code>{line.ledger_line_id||'Not posted'}</code></span><span><i>Source document IDs</i><code>{line.source_document_ids.length?line.source_document_ids.join(', '):'None returned'}</code></span></div>)}</div></details>
      </section><p className="report-drill-hint authoritative-journal-readonly-note"><span className="badge badge-muted">READ ONLY</span> No editing, workflow, posting, reversing, export, or inferred source links.</p>
    </>}
  </section>;
}
...
export function AuthoritativeJournalWorkspace({ journals, config, fetcher=globalThis.fetch, environment=globalThis, initialJournalEntryId=null }) {
  ...
  const [exportState,setExportState]=useState({phase:'IDLE',data:null,error:null});
  ...
  const openEvidence=async(journal,focusId,tableX)=>{
    const returnContext=createAuthoritativeReturnContext({config,view,focusId,scrollY:Number(environment?.scrollY)||0,tableX});
    if(!returnContext)return;
    const frozenContext={...returnContext,entityLabel:config?.scopePresentation?.entityLabel||'Configured entity',periodLabel:config?.scopePresentation?.periodLabel||'Period unavailable',journalId:journal.journal_entry_id,journalRevision:journal.revision,journalCurrency:journal.currency};
  ...
  const exportApprovedUploadFile=async()=>{
    if(!environment?.URL?.createObjectURL||!environment?.document?.createElement||typeof environment?.Blob!=='function'){setExportState({phase:'BLOCKED',data:null,error:{code:'JOURNAL_UPLOAD_DOWNLOAD_UNAVAILABLE',message:'This browser cannot safely create a local verified CSV download.'}});return;}
    setExportState({phase:'LOADING',data:null,error:null});
    const result=await exportAuthoritativeJournalUploadFile({config,fetcher,cryptoApi:environment.crypto||globalThis.crypto});
    if(!result.ok){setExportState({phase:'BLOCKED',data:null,error:result});return;}
    try{const blob=new environment.Blob([result.data.content],{type:'text/csv;charset=utf-8'}),url=environment.URL.createObjectURL(blob),anchor=environment.document.createElement('a');anchor.href=url;anchor.download=result.data.filename;anchor.click();environment.URL.revokeObjectURL(url);setExportState({phase:'READY',data:result.data,error:null});}catch{setExportState({phase:'BLOCKED',data:null,error:{code:'JOURNAL_UPLOAD_DOWNLOAD_UNAVAILABLE',message:'The verified Journal upload file could not be downloaded locally.'}});}
  };
  if (detail) return <JournalDetailReadState detail={detail} entityId={config.entityId} config={config} fetcher={fetcher} onBack={() => { setView(detail.returnContext.view); setDetail(null); restoreAuthoritativeReturnContext(environment,config,detail.returnContext,{getTable:()=>environment?.document?.querySelector?.('.authoritative-journal-table')}); }}/>;
  return <div className="stack authoritative-journal-workspace"><section className="report-workbench" aria-label="Verified Journal upload export"><div className="report-workbench-head"><div><b>Journal upload file</b><div className="page-subtitle">Download only balanced APPROVED Journal Entries for this company and period. The server returns a content hash; download does not submit, post, or change any Journal Entry.</div></div><button type="button" className="btn" onClick={exportApprovedUploadFile} disabled={exportState.phase==='LOADING'}>{exportState.phase==='LOADING'?'Verifying file...':'Download approved JE CSV'}</button></div>{exportState.phase==='BLOCKED'?<StateBlock tone="blocked" title={exportState.error?.code||'JOURNAL_UPLOAD_EXPORT_BLOCKED'}>{exportState.error?.message}</StateBlock>:null}{exportState.phase==='READY'?<StateBlock tone="success" title="Verified Journal upload file downloaded">{exportState.data.filename} contains {exportState.data.rowCount} balanced Journal lines. Content hash: {exportState.data.contentHash}</StateBlock>:null}</section>{capabilityState.phase==='LOADING'&&<StateBlock tone="loading" title="Checking Journal workflow access">Reading the current actor's fixed entity-scoped permissions. No action is available while this read is pending.</StateBlock>}{capabilityState.phase==='BLOCKED'&&<StateBlock tone="blocked" title="Journal workflow access unavailable">{capabilityState.error?.message||'The capability read failed closed. No Journal workflow action is available.'}</StateBlock>}<AuthoritativeJournalTable journals={rows} entityId={config.entityId} view={view} onViewChange={setView} onOpen={openEvidence} capabilities={capabilityState.phase==='READY'?capabilityState.data:null} onWorkflowAction={runWorkflow} workflowState={workflowState}/>{workflowState?.phase==='ERROR'&&<StateBlock tone="blocked" title="Journal workflow action unavailable">{workflowState.message}</StateBlock>}{workflowState?.phase==='DONE'&&<StateBlock tone="empty" title="Authoritative Journal register refreshed">{workflowState.action} completed and the entity-scoped Journal register was re-read.</StateBlock>}<AuthoritativeSecondaryDisclosure label="External WBS evidence"><AuthoritativeWbsLivePilotObservation config={config} fetcher={fetcher} tools={WBS_LIVE_PILOT_SURFACE_TOOLS.journal} title="External WBS journal observations"/></AuthoritativeSecondaryDisclosure></div>;
}
