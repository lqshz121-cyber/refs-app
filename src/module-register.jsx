...
export function AccountRegister({ctx}) {
  ...
  if (selectedEntry) return <div className="full-bleed qbo-transaction-report" aria-label="Local account-register entry detail">
    ...
    <div className="row-acts"><Btn size="sm" variant="ghost" onClick={()=>goto('je',{jeNumber:selectedEntry.ref,registerReturn:registerReturnContext(selectedEntry.id)})}>Open retained JE</Btn>{selectedEntry.sourceTarget?<Btn size="sm" variant="ghost" onClick={()=>goto(selectedEntry.sourceTarget.route,selectedEntry.sourceTarget.context)}>Open retained source</Btn>:<Unavailable reason="No retained source document is linked to this register line.">No source drill</Unavailable>}{cashRegisterScope.master?<Btn size="sm" variant="ghost" onClick={()=>goto('bankrec',{route:'bankrec',acctCode:cashRegisterScope.master.bank_account_code,registerReturn:registerReturnContext(selectedEntry.id)})}>Open local reconcile</Btn>:<Unavailable reason="This account has no single mapped local cash reconciliation scope.">No reconcile scope</Unavailable>}</div>
  </div>;
