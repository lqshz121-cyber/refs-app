...
export function AuthoritativeLineageDrill({config,fetcher=globalThis.fetch,initial,onExit}){
  ...
  const readLedgerFromEvidence=async(evidence,ledgerLineId)=>{
    ...
    const token=beginRead();
    const result=await refreshAuthoritativeGeneralLedger({config,accountCode:evidence.account_code,query:null,limit:200,offset:0,fetcher});
    if(!requestGuard.current.isCurrent(token))return;
    if(!result.ok){fail(token,result.message);return;}
    const matches=result.rows.filter(item=>item.ledger_line_id===ledgerLineId&&item.account_code===evidence.account_code&&item.currency===evidence.currency&&ids(evidence.journal_entry_ids).includes(item.journal_entry_id)&&ids(evidence.journal_line_ids).includes(item.journal_line_id)&&includesAll(ids(evidence.source_document_ids),item.source_document_ids));
  ...
  if(read.phase==='LOADING')return <section className="full-bleed qbo-transaction-report authoritative-evidence-page"><button type="button" className="btn btn-sm btn-ghost" disabled aria-disabled="true">Reading evidence...</button><StateBlock tone="loading">Re-reading immutable lineage evidence...</StateBlock></section>;
  if(read.phase==='BLOCKED')return <section className="full-bleed qbo-transaction-report authoritative-evidence-page"><div className="qbo-report-back"><button type="button" className="btn btn-sm btn-ghost" onClick={clearBlocked}>Back to current evidence</button><ScopeLabel entityId={config.entityId} periodId={config.periodId}/></div><StateBlock tone="blocked" title="BLOCKED - immutable lineage mismatch">{read.error.message}</StateBlock></section>;
...
const ScopeLabel=({entityId,periodId})=><span><span title={`Entity ID: ${entityId}`}>Configured entity</span><span aria-hidden="true"> | </span><span title={`Period ID: ${periodId}`}>Configured period</span></span>;
const Back=({onClick,label='Back to prior evidence',context})=><div className="qbo-report-back"><button type="button" className="btn btn-sm btn-ghost" onClick={onClick}>{label}</button><ScopeLabel entityId={context.entityId} periodId={context.periodId}/></div>;
...
function EvidenceFrame({frame,back,readLedger}){const row=frame.row;return <section className="full-bleed qbo-transaction-report authoritative-evidence-page" aria-label="Rollforward or profitability lineage evidence"><Back onClick={back} context={frame.context}/><h1>{row.account_code} evidence drill</h1><p className="page-subtitle">Select one API-returned POSTED ledger line. REFS will re-read General Ledger, then require the exact Journal and Source relationships before continuing.</p><div className="qbo-toolgrid"><span><i>Account</i><b>{row.account_code}</b></span><span><i>Journal entries</i><b>{ids(row.journal_entry_ids).length}</b></span><span><i>Journal lines</i><b>{ids(row.journal_line_ids).length}</b></span><span><i>Sources</i><b>{ids(row.source_document_ids).length}</b></span></div><div className="row-acts">{ids(row.ledger_line_ids).map(id=><button key={id} type="button" className="btn btn-sm" onClick={()=>readLedger(row,id)}>Open GL line</button>)}</div></section>}
