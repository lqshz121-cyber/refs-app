...
export async function readExactAuthoritativeLedgerLine({config,accountCode=null,ledgerLineId,fetcher=globalThis.fetch,readPage=refreshAuthoritativeGeneralLedger}={}){
  ...
  let offset=0,total=null,match=null;
  while(total===null||offset<total){
    const result=await readPage({config,accountCode,query:null,limit:LEDGER_PAGE_SIZE,offset,fetcher});
    if(!result.ok)return result;
    if(total===null){total=result.total;if(total>LEDGER_RESULT_CAP)return {ok:false,message:'The General Ledger evidence exceeds the bounded lineage-read limit.'};}
    else if(result.total!==total)return {ok:false,message:'The General Ledger changed during the immutable lineage read.'};
...
export function AuthoritativeLineageDrill({config,fetcher=globalThis.fetch,initial,onExit}){
  ...
  const readLedgerFromReport=async(row,ledgerLineId)=>{
    ...
    if(item.account_code!==row.account_code||row.currency&&item.currency!==row.currency||!row.journal_entry_ids.includes(item.journal_entry_id)||!row.journal_line_ids.includes(item.journal_line_id)||!includesAll(row.source_document_ids,item.source_document_ids)){fail(token,'The General Ledger read did not return exactly one immutable line contained by the selected report row.');return;}
    push({kind:'GL',row:item,context:{entityId:config.entityId,periodId:config.periodId,journalEntryId:item.journal_entry_id,journalLineId:item.journal_line_id,ledgerLineId:item.ledger_line_id}},token);
  };
  const readLedgerFromEvidence=async(evidence,ledgerLineId)=>{
    ...
    if(!result.ok){fail(token,result.message);return;}
    const item=result.row;
    if(accountCode&&item.account_code!==accountCode||evidence.currency&&item.currency!==evidence.currency||!ids(evidence.journal_entry_ids).includes(item.journal_entry_id)||!ids(evidence.journal_line_ids).includes(item.journal_line_id)||!includesAll(ids(evidence.source_document_ids),item.source_document_ids)){fail(token,'The General Ledger read did not return exactly one immutable line contained by the selected evidence row.');return;}
    push({kind:'GL',row:item,context:{entityId:config.entityId,periodId:config.periodId,journalEntryId:item.journal_entry_id,journalLineId:item.journal_line_id,ledgerLineId:item.ledger_line_id}},token);
  };
...
const LedgerLineButtons=({values,onOpen})=>{const pageSize=25,[page,setPage]=useState(0),pageCount=Math.max(1,Math.ceil(values.length/pageSize)),safePage=Math.min(page,pageCount-1),start=safePage*pageSize,visible=values.slice(start,start+pageSize);return <div className="stack" aria-label="Retained ledger-line evidence"><div className="row-acts">{visible.map(id=><button key={id} type="button" className="btn btn-sm" onClick={()=>onOpen(id)}>Open GL line</button>)}</div>{pageCount>1&&<div className="row-acts"><button type="button" className="btn btn-sm btn-ghost" disabled={safePage===0} onClick={()=>setPage(value=>Math.max(0,value-1))}>Previous ledger lines</button><span className="muted sm">Ledger lines {start+1}-{Math.min(start+pageSize,values.length)} of {values.length}</span><button type="button" className="btn btn-sm btn-ghost" disabled={safePage===pageCount-1} onClick={()=>setPage(value=>Math.min(pageCount-1,value+1))}>Next ledger lines</button></div>}</div>;};

function JournalFrame({frame,back,readLedger,readSource}){const j=frame.journal;return <section className="full-bleed qbo-transaction-report authoritative-evidence-page" aria-label="Journal lineage evidence"><Back onClick={back} context={frame.context}/><div className="card-head"><div><h1>Journal entry {j.journal_number}</h1><p className="page-subtitle">Review posted journal lines.</p></div><span className="badge badge-muted">READ ONLY</span></div><details className="authoritative-secondary-disclosure authoritative-lineage-identifiers"><summary><span>Journal identifiers</span></summary><section><div className="detail-grid"><div><i>Journal ID</i><b>{j.journal_entry_id}</b></div><div><i>Revision</i><b>{j.revision}</b></div><div><i>Currency</i><b>{j.currency}</b></div></div></section></details><div className="table-wrap authoritative-journal-lineage-table" role="region" tabIndex={0} aria-label="Journal lineage lines; scroll horizontally"><table className="tbl"><thead><tr><th>Line</th><th>Account</th><th>Debit</th><th>Credit</th><th>Ledger</th><th>Sources</th></tr></thead><tbody>{j.lines.map(line=><tr key={line.journal_line_id}><td>{line.line_no}</td><td>{line.account_code}</td><td>{money(line.debit_amount)}</td><td>{money(line.credit_amount)}</td><td>{line.ledger_line_id?<button type="button" className="btn btn-sm" onClick={()=>readLedger(j,line)}>Open GL evidence</button>:'Not posted'}</td><td>{line.source_document_ids.length?line.source_document_ids.map(id=><button key={id} type="button" className="btn btn-sm btn-ghost" onClick={()=>readSource(id,j.journal_entry_id)}>Open source</button>):'None returned'}</td></tr>)}</tbody></table></div><p className="report-drill-hint"><span className="badge badge-muted">READ ONLY</span> No edit, workflow, posting, export, or reconstructed lineage action is available.</p></section>}
...
function ReportChooser({frame,back,open}){return <section className="full-bleed qbo-transaction-report authoritative-evidence-page" aria-label="Matching report evidence"><Back onClick={back} context={frame.context}/><h1>Reports containing this ledger line</h1><p className="page-subtitle">Choose a statement row to continue. REFS will not infer a report.</p><div className="stack">{frame.rows.map(row=><button key={`${row.statement_type}:${row.account_code}`} type="button" className="btn" onClick={()=>open(row)}>{row.statement_type} | {row.statement_section} | {row.account_code}</button>)}</div></section>}

function EvidenceFrame({frame,back,readLedger}){const row=frame.row,account=row.account_code||'Multiple retained accounts';return <section className="full-bleed qbo-transaction-report authoritative-evidence-page" aria-label="Report evidence lineage"><Back onClick={back} context={frame.context}/><h1>{account} evidence drill</h1><p className="page-subtitle">Select one posted ledger line to continue through General Ledger, Journal, and Source.</p><div className="qbo-toolgrid"><span><i>Account scope</i><b>{account}</b></span><span><i>Journal entries</i><b>{ids(row.journal_entry_ids).length}</b></span><span><i>Journal lines</i><b>{ids(row.journal_line_ids).length}</b></span><span><i>Sources</i><b>{ids(row.source_document_ids).length}</b></span></div><LedgerLineButtons values={ids(row.ledger_line_ids)} onOpen={id=>readLedger(row,id)}/></section>}

function ReportFrame({frame,back,readLedger}){const row=frame.row;return <section className="full-bleed qbo-transaction-report authoritative-evidence-page" aria-label="Report lineage evidence"><Back onClick={back} context={frame.context}/><h1>{row.statement_type} account evidence</h1><div className="qbo-toolgrid"><span><i>Section</i><b>{row.statement_section}</b></span><span><i>Account</i><b>{row.account_code}</b></span><span><i>Period debit</i><b>{money(row.period_debit)}</b></span><span><i>Period credit</i><b>{money(row.period_credit)}</b></span><span><i>Balance</i><b>{money(row.display_balance)}</b></span></div><p className="muted sm">Open a linked General Ledger line to review its details.</p><LedgerLineButtons values={ids(row.ledger_line_ids)} onOpen={id=>readLedger(row,id)}/></section>}
