...
function EvidenceFrame({frame,back,readLedger}){const row=frame.row,account=row.account_code||'Multiple retained accounts';return <section className="full-bleed qbo-transaction-report authoritative-evidence-page" aria-label="Report evidence lineage"><Back onClick={back} context={frame.context}/><h1>{account} evidence drill</h1><p className="page-subtitle">Select one API-returned POSTED ledger line. REFS will re-read General Ledger, then require the exact Journal and Source relationships before continuing.</p><div className="qbo-toolgrid"><span><i>Account scope</i><b>{account}</b></span><span><i>Journal entries</i><b>{ids(row.journal_entry_ids).length}</b></span><span><i>Journal lines</i><b>{ids(row.journal_line_ids).length}</b></span><span><i>Sources</i><b>{ids(row.source_document_ids).length}</b></span></div><LedgerLineButtons values={ids(row.ledger_line_ids)} onOpen={id=>readLedger(row,id)}/></section>}

function ReportFrame({frame,back,readLedger}){const row=frame.row;return <section className="full-bleed qbo-transaction-report authoritative-evidence-page" aria-label="Report lineage evidence"><Back onClick={back} context={frame.context}/><h1>{row.statement_type} account evidence</h1><div className="qbo-toolgrid"><span><i>Section</i><b>{row.statement_section}</b></span><span><i>Account</i><b>{row.account_code}</b></span><span><i>Period debit</i><b>{money(row.period_debit)}</b></span><span><i>Period credit</i><b>{money(row.period_credit)}</b></span><span><i>Balance</i><b>{money(row.display_balance)}</b></span></div><p className="muted sm">This statement row was returned for the exact entity and period. Open one retained ledger line to re-read its immutable GL evidence.</p><LedgerLineButtons values={ids(row.ledger_line_ids)} onOpen={id=>readLedger(row,id)}/></section>}
