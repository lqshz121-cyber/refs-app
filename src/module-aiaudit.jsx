...
// AI Audit — rules-based review across the local ledger.
export function AIAudit({ctx}) {
  ...
  const findings = useMemo(()=>{
    ...
    posted.forEach(j=>{
      ...
      if (Math.abs(dr-cr)>0.005) flag('HIGH','AI-BAL-01', j.je_number, `Journal is unbalanced: Dr ${money(dr)} ≠ Cr ${money(cr)}`, 'Return and correct the amounts', 0.99);
      ...
      j.lines.forEach((l,i)=>{
        ...
        if (st && !memberOf(l)) flag('HIGH','AI-SUB-01', `${j.je_number} line ${i+1}`, `${l.account_code} requires a subsidiary member (${st}) but none is recorded`, 'Add the member before reposting', 0.97);
        if (l.account_code==='142000') flag('MEDIUM','AI-SUSP-01', j.je_number, 'Suspense balance remains open', 'Identify the party and reclassify', 0.8);
      });
      if (j.je_type==='AUTO' && ['PAYABLE','CLOSING'].includes(j.source_system) && !j.source_doc_id && !j.rule_code)
        flag('MEDIUM','AI-SRC-01', j.je_number, 'Automated journal lacks source-document trace', 'Trace the document through Integration Hub', 0.85);
      if (j.je_date && j.period_code && j.je_date.slice(0,7)!==j.period_code)
        flag('MEDIUM','AI-CUT-01', j.je_number, `Business date ${j.je_date} differs from accounting period ${j.period_code} (cutoff risk)`, 'Confirm the accrual period or correct the accounting date', 0.88);
      if (j.source_system==='WBS_CL' && j.description.includes('Draw') && j.lines.some(l=>l.account_code.startsWith('164')&&l.debit_amount>0))
        flag('HIGH','AI-LOAN-01', j.je_number, 'Loan draw recorded as cost (should be Dr Cash / Cr Loan)', 'Reverse and regenerate using the rule', 0.95);
    });
    ...
    Object.entries(net).forEach(([m,v])=>{ if (v<-0.005 && Math.abs(v)>5000) flag('LOW','AI-291-AGE', '291001 · '+m, `Open net balance with ${m}: ${money(v)}`, 'Check whether the bank feed omitted a match (EXPA)', 0.7); });
    ...
    Object.entries(cash).forEach(([e,v])=>{ if (v<-0.01) flag('HIGH','AI-CASH-01', 'Entity '+e, `Operating cash is negative: ${money(v)}`, 'Check for omitted capital funding or receipts', 0.92); });
  ...
  return <div className="full-bleed">
    <h2 className="page-h">AI Audit</h2>
    <div className="filter-bar">
      <Btn variant="primary" onClick={()=>setRan(r=>!r)}>Run ledger audit again</Btn>
      <span className="muted sm">The rules engine scans posted journals. AI only suggests; it does not post.</span>
    </div>
    <div className="kpi-row">
      <KPI label="Posted journals scanned" value={jes.filter(j=>j.posting_status==='POSTED'&&(!entity||j.entity_id===entity)).length}/>
    ...
    <Table exportName="ai-audit-findings" pageSize={20} cols={[
      ...
      {h:'Object',k:'object'},
      {h:'Reason (AI rationale)',k:'reason'},
      {h:'Suggested Action',k:'action'},
      {h:'Confidence',num:true,render:r=>(r.conf*100).toFixed(0)+'%',csv:r=>r.conf},
      {h:'Human review',render:r=>r.needs_human?<Badge tone="warn">YES</Badge>:<Badge tone="ok">no</Badge>,csv:r=>r.needs_human?'Y':'N'},
      {h:'Owner',render:r=>r.risk==='HIGH'?'CONTROLLER':'SENIOR_ACCT'},
      {h:'Due',render:r=>r.risk==='HIGH'?'2026-08-05':'2026-08-15'},
      {h:'Resolution action',render:r=>{ const jeNum=(r.object.match(/(?:JE-|\d{14})[\w-]*/)||[])[0];
        ...
        return <span className="row-acts">
          ...
          {je && r.rule==='AI-LOAN-01' && je.posting_status==='POSTED' && <Btn size="sm" variant="danger" onClick={e=>{e.stopPropagation(); ctx.actions.reverseJE(je.je_id); resolve(r); ctx.toast('Reversal created and marked Resolved. Use Staging to regenerate under the rule.');}}>Reverse journal</Btn>}
        </span>; }},
    ...
    ]} rows={findings.filter(TABMAP[tab]||(()=>true))} empty="✓ Ledger scan passed: no exceptions found."/>
  </div>;
}
