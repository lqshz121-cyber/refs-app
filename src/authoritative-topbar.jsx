...
export function AuthoritativeTopbar({
  ...
  return <header className="topbar authoritative-topbar">
    ...
    <label className="sw authoritative-shell-scope"><span className="sr-only">Company</span><span className="authoritative-shell-select" title={entityLabel}>{entityLabel}</span></label>
    <div className="top-right authoritative-top-actions">
      <span className="period-chip authoritative-period-chip"><span className="period-label">Reporting period</span><b>{periodLabel}</b></span>
      <button type="button" className="icon-btn" aria-label="Refresh accounting records" title="Refresh accounting records" onClick={onRefresh}><span aria-hidden="true">↻</span></button>
      ...
      <span className="badge badge-ok authoritative-mode-chip">Secure</span>
