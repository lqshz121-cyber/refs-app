...
export function Dashboard({ctx}) {
  ...
  const expCats = [['Operating expense', Math.max(1,st.expense*0.42), '#2CA01C'], ['Interest expense', Math.max(1,st.expense*0.31), '#0077C5'], ['Management expense', Math.max(1,st.expense*0.17), '#FF8000'], ['Other expense', Math.max(1,st.expense*0.10), '#8A5BE0']];
  ...
  return <div>
    <div className="qbo-home-hero">
      <div><span className="qbo-eyebrow">REFS FINANCE WORKSPACE</span><h2 className="page-h">Business overview</h2><p>One place for work that needs attention, the current financial position, and a controlled path back to source records.</p></div>
    ...
    <div className="qbo-quicklinks" aria-label="Quick links">
      {[['Accounting','gl'],['Expenses & Pay Bills','ap'],['Banking','banktx'],['Auto reconciliation','autobankrec'],['Projects','cost'],['Reports','reports'],['Customer hub','ar'],['Team','approvals'],['Time','pmpickup'],['Inventory','cost'],['Sales tax','reports'],['Lending','loan'],['Payroll','close']].map(([label,route])=><button key={route} type="button" onClick={()=>goto(route)}><span>{label}</span></button>)}
    </div>
    ...
    <div className="qbo-grid">
      <div className="qbo-card" onClick={()=>goto('gl')} style={{cursor:'pointer'}}>
        <h4>Profit and loss · 2026-07</h4>
        <div className={`qbo-big ${st.netIncome<0?'num-neg':''}`}>{money(st.netIncome)}</div>
        <div className="qbo-sub">Net income · revenue {money(st.revenue)} · expenses {money(st.expense)}</div>
    ...
    <SectionTitle right={<Btn size="sm" variant="ghost" onClick={()=>goto('approvals')}>View all</Btn>}>Pending approvals ({pendingApprovals.length})</SectionTitle>
    <Table rowKey="je_id" onRow={r=>goto('je',{jeNumber:r.je_number})} cols={[
      {h:'Journal entry',k:'je_number'},{h:'Description',k:'description'},
