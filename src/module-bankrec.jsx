...
import { KPI, Btn, Badge, Money, Table, SectionTitle, StateBlock, Unavailable } from './ui.jsx';
...
export function BankRec2({ctx}) {
  ...
  if(ctx.authoritativeMode)return <section className="card"><h2 className="page-h">Reconciliation</h2>
    <StateBlock tone="error" title="RECONCILIATION_API_UNAVAILABLE">This workspace reads reconciliation evidence from the accounting API. No local browser record is substituted for it.</StateBlock></section>;
  ...
  return <div className="full-bleed">
    ...
    <p className="muted sm" style={{margin:'0 0 12px'}}>Reconciliation compares retained local book evidence with the retained statement. Account connection, feed import, and external reconciliation setup are outside REFS. POSTED is not treated as reconciled.</p>
       ...
       {h:'Evidence action',render:r=> r.match_status==='MATCHED'? (r.matched_je&&jes.some(j=>j.je_number===r.matched_je) ? <Btn size="sm" variant="ghost" onClick={()=>goto('je',{jeNumber:r.matched_je,reconciliationReturn:localReconciliationJournalReturnContext({acctCode,bankTxnId:r.bank_txn_id})})}>Open JE</Btn> : <span className="muted sm">{r.matched_je||'—'}</span>) :
        <span className="row-acts">
          {r.suggest && ['FEE','INTEREST'].includes(r.suggest)
            ? <Unavailable reason="Categorizing a bank line is not a REFS action.">Categorize</Unavailable>
            : <Btn size="sm" onClick={()=>match(r)}>Open Match review</Btn>}
          <Unavailable reason="Excluding a bank line is not a REFS action.">Exclude</Unavailable>
        </span>},
    ]} rows={txns} />
    ...
    <div style={{marginTop:14, display:'flex', gap:14, alignItems:'center'}}>
      <Unavailable reason="Sign-off is not executed from this evidence view.">Reconcile</Unavailable>
    ...
    {historyState.isEmpty && <><SectionTitle>Local reconciliation history</SectionTitle><StateBlock tone="empty" title={historyState.emptyLabel}><span>Complete the existing guarded local sign-off only when adjusted balances agree and all local activity is handled.</span><small>Local audit evidence appears here once a sign-off is retained.</small></StateBlock></>}
  </div>;
}
