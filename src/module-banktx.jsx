...
export function BankTransactions({ctx}){
  ...
  return <div className="full-bleed">
    ...
    {authorityUnavailable&&<div className="alert alert-warn"><Badge tone="warn">BANK_API_UNAVAILABLE</Badge> 权威 Accounting API 已启用，但 Bank Transaction / Reconcile 命令尚未提供后端契约；本页仅只读，未创建分录、匹配或排除任何流水。</div>}
    ...
    {tab==='For Review'&&<><div style={{display:'flex',gap:10,margin:'4px 0 10px'}}><Btn size="sm" variant="primary" disabled={authorityUnavailable} onClick={batchAccept}>Batch accept ({Object.values(checked).filter(Boolean).length})</Btn><Btn size="sm" variant="ghost" disabled={authorityUnavailable} onClick={()=>toast('Rule draft opened. It must be tested and approved before LIVE.')}>Create rule</Btn></div><Table rowKey="bank_txn_id" cols={[
      {h:'',w:34,render:r=><input type="checkbox" disabled={authorityUnavailable} checked={!!checked[r.bank_txn_id]} onClick={e=>e.stopPropagation()} onChange={e=>setChecked(c=>({...c,[r.bank_txn_id]:e.target.checked}))}/>},{h:'Date',k:'txn_date'},{h:'Bank description',k:'reference'},{h:'Amount',num:true,render:r=><Money v={r.direction==='DEBIT'?-r.amount:r.amount}/>,sortVal:r=>r.amount},
      ...
      {h:'Action',render:r=><span className="row-acts"><Btn size="sm" variant="primary" disabled={authorityUnavailable} onClick={()=>setReviewTxn(r)}>Review</Btn><Btn size="sm" variant="ghost" onClick={()=>setAiSel(aiSel?.bank_txn_id===r.bank_txn_id?null:r)}>See why</Btn><Btn size="sm" variant="ghost" disabled={authorityUnavailable} onClick={()=>{const result=actions.bankExclude(acctCode,r.bank_txn_id);toast(result?.ok?'Transaction excluded.':(result?.message||'Exclude blocked.'),result?.ok?'ok':'bad');}}>Exclude</Btn></span>},
    ...
    {tab==='Categorized'&&<Table rowKey="bank_txn_id" cols={[{h:'Date',k:'txn_date'},{h:'Description',k:'reference'},{h:'Amount',num:true,render:r=><Money v={r.direction==='DEBIT'?-r.amount:r.amount}/>},{h:'Accounting result',render:r=><span>{r.draft_je_number||r.matched_je||'—'}<div><Badge>{r.processing_type==='DRAFT_JE'?'DRAFT':'MATCHED'}</Badge></div></span>},{h:'Action',render:r=><Btn size="sm" variant="ghost" disabled={authorityUnavailable} onClick={()=>undo(r)}>Undo</Btn>}]} rows={categorized} empty="No categorized transactions."/>}
    {tab==='Excluded'&&<Table rowKey="bank_txn_id" cols={[{h:'Date',k:'txn_date'},{h:'Description',k:'reference'},{h:'Amount',num:true,render:r=><Money v={r.amount}/>},{h:'Action',render:r=><Btn size="sm" variant="ghost" disabled={authorityUnavailable} onClick={()=>undo(r)}>Restore</Btn>}]} rows={excluded} empty="No excluded transactions."/>}
