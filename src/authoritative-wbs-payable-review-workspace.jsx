...
export function AuthoritativeWbsPayableReviewWorkspace({config,fetcher=globalThis.fetch,onReviewed=()=>{}}){
  ...
  return <section className="report-workbench authoritative-wbs-payable-review" aria-label="Admitted WBS Payable review readiness">
    ...
    {state.phase==='LOADING'&&<StateBlock tone="loading" title="Loading admitted WBS Payables">Checking source, scope, mapping, separation of duties, and attachments.</StateBlock>}
    {state.phase==='BLOCKED'&&<ReviewCandidateReadBlocked error={state.error} onRetry={load}/>}
    {state.phase==='READY'&&state.rows.length===0&&<StateBlock tone="empty" title="No admitted Payables awaiting review">No signed Payables are ready for review. Unsigned pilot rows remain excluded.</StateBlock>}
    {state.phase==='READY'&&state.rows.length>0&&<div className="table-wrap authoritative-wbs-payable-table" role="region" tabIndex={0} aria-label="Admitted WBS Payables awaiting review; scroll horizontally to view every column"><table className="tbl"><thead><tr><th>Document</th><th>Accounting date</th><th>Vendor</th><th>Currency</th><th>Gross amount</th><th>Readiness</th><th>Action</th></tr></thead><tbody>{state.rows.map(row=><tr key={row.wbs_inbound_row_id}><td>{compact(row.document_number)}</td><td>{compact(row.accounting_date)}</td><td>{compact(row.vendor_name)}</td><td>{compact(row.currency)}</td><td>{compact(row.gross_amount)}</td><td><span className={`badge ${row.can_review?'badge-success':'badge-muted'}`}>{row.review_readiness}</span></td><td><button type="button" className="linklike" disabled={!row.can_review&&row.review_readiness!=='VERIFIED_ATTACHMENT_REQUIRED'||command.phase==='LOADING'} onClick={()=>open(row)}>{row.can_review?'Inspect review evidence':row.review_readiness==='VERIFIED_ATTACHMENT_REQUIRED'?'Add support evidence':'Unavailable'}</button></td></tr>)}</tbody></table></div>}
