import {ListedRecordJournal} from './listed-record-journal.jsx';
...
import {DocumentSettlementHistory} from './document-settlement-history.jsx';
...
export function AuthoritativeDocumentTable({title,documents=[],kind,onOpen}) {
  ...
  return <section aria-label={title}><div className="card-head"><div><h2>{title}</h2><p className="muted sm">{documents.length} {documents.length===1?(bill?'bill':'invoice'):(bill?'bills':'invoices')}</p></div></div>{documents.length?<div className={`table-wrap authoritative-document-table${bill?' authoritative-expense-table':''}`} role="region" tabIndex={0} aria-label={`${title}; scroll horizontally to view every column`}><table className="tbl"><thead>{bill?<tr><th scope="col">Bill date</th><th scope="col">Type</th><th scope="col">No.</th><th scope="col">Vendor</th><th scope="col">Category</th><th scope="col">Due date</th><th scope="col" className="ta-r">Bill amount</th><th scope="col" className="ta-r">Open balance</th><th scope="col">Status</th><th scope="col">Action</th></tr>:<tr><th scope="col">Invoice</th><th scope="col">Customer</th><th scope="col">Date</th><th scope="col">Due date</th><th scope="col" className="ta-r">Amount</th><th scope="col" className="ta-r">Open balance</th><th scope="col">Status</th><th scope="col">Details</th></tr>}</thead><tbody>{documents.map(row=>{
    ...
    return <tr key={key}>{bill?<><td>{row[dateKey]}</td><td>Bill</td><td>{row[number]}</td><td>{row[counterparty]}</td><td>{row.account_code||'Not available'}</td><td>{row.due_date||'Not available'}</td><td className="ta-r">{money(row.amount,row.currency)}</td><td className="ta-r">{money(row.open_balance,row.currency)}</td><td><span className="badge badge-muted authoritative-row-status">{row.status}</span></td><td><button id={focusId} type="button" className="btn btn-sm btn-ghost" onClick={event=>onOpen?.(row,focusId,Number(event.currentTarget.closest('.table-wrap')?.scrollLeft)||0)}>View details</button></td></>:<><td>{row[number]}</td><td>{row[counterparty]}</td><td>{row[dateKey]}</td><td>{row.due_date||'Not available'}</td><td className="ta-r">{money(row.amount,row.currency)}</td><td className="ta-r">{money(row.open_balance,row.currency)}</td><td><span className="badge badge-muted authoritative-row-status">{row.status}</span></td><td><button id={focusId} type="button" className="btn btn-sm btn-ghost" onClick={event=>onOpen?.(row,focusId,Number(event.currentTarget.closest('.table-wrap')?.scrollLeft)||0)}>View details</button></td></>}</tr>;
...
export function AuthoritativeDocumentDetail({document,kind,entityId,config,returnContext,onBack,currentActorAccess,accounts=[],scopes=[],fetcher=globalThis.fetch,onOpenDraft,onRefresh}){
  const bill=kind==='AP',number=bill?'bill_no':'inv_no',counterparty=bill?'vendor_name':'customer_name',dateKey=bill?'bill_date':'inv_date',title=bill?'Bill evidence':'Invoice evidence';
  const entityLabel=config?.entityId===entityId?config?.scopePresentation?.entityLabel||'Configured entity':'Configured entity',periodLabel=config?.periodId===document.period_id?config?.scopePresentation?.periodLabel||'Configured period':'Configured period',returnScope=documentReturnScope(entityLabel,returnContext?.view,document.revision,bill);
  const exactScope=returnContext?.entityId===config?.entityId&&returnContext?.periodId===config?.periodId&&returnContext?.documentId===document?.business_document_id&&returnContext?.documentRevision===Number(document?.revision)&&returnContext?.documentKind===kind&&returnContext?.documentPeriodId===document?.period_id&&document?.period_id===config?.periodId;
  return <section className="full-bleed qbo-transaction-report authoritative-document-detail" aria-label={title}><div className="qbo-report-back"><button type="button" onClick={onBack}>Back to {bill?'AP bills':'AR invoices'}</button><details className="authoritative-return-context"><summary>List filters retained</summary><span>{returnScope}</span></details></div>{!exactScope?<StateBlock tone="blocked" title="BLOCKED — immutable document scope mismatch">The selected document ID, revision, kind, or accounting period no longer matches the exact parent list scope. Return to the list; no document evidence is asserted.</StateBlock>:<><div className="gl-drill-head"><div><div className="gl-drill-crumb">{bill?'Payables':'Receivables'} / read-only evidence</div><h1>{document[number]}</h1><div className="gl-drill-account">{document[counterparty]} · {document[dateKey]}</div></div><span className="badge badge-muted">{document.status}</span></div><div className="authoritative-document-detail-summary" aria-label={`${title} summary`}><span><i>{bill?'Vendor':'Customer'}</i><b>{document[counterparty]}</b></span><span><i>Original amount</i><b>{money(document.amount,document.currency)}</b></span><span><i>Open balance</i><b>{money(document.open_balance,document.currency)}</b></span><span><i>Due date</i><b>{document.due_date||'Not available'}</b></span></div><p className="report-drill-hint">Retained document evidence.</p><NativeSettlementEntry config={config} kind={bill?'AP_PAYMENT':'AR_RECEIPT'} businessDocumentId={document.business_document_id} access={currentActorAccess} accounts={accounts} scopes={scopes} fetcher={fetcher} onOpenDraft={onOpenDraft} onRefresh={onRefresh}/><CreditAllocationHistory config={config} subjectId={document.business_document_id} kind={bill?'AP_BILL':'AR_INVOICE'} access={currentActorAccess} accounts={accounts} scopes={scopes} fetcher={fetcher} onOpenDraft={onOpenDraft} onRefresh={onRefresh}/><DocumentSettlementHistory config={config} businessDocumentId={document.business_document_id} kind={bill?'AP_PAYMENT':'AR_RECEIPT'} access={currentActorAccess} fetcher={fetcher}/><ListedRecordJournal key={`${config?.baseUrl}:${entityId}:${config?.periodId}:${document.business_document_id}:${document.revision}:${currentActorAccess?.actor_id}`} config={config} row={document} recordKind={bill?"AP_BILL":"AR_INVOICE"} access={currentActorAccess} fetcher={fetcher}/><AuthoritativeLineageBlock record={document} entityId={entityId} subject={bill?'Bill':'Invoice'}/><div className="table-wrap authoritative-document-detail-table" role="region" tabIndex={0} aria-label={`${title} fields; scroll horizontally to view every column`}><table className="tbl"><tbody><tr><th scope="row">Entity</th><td title={`Entity ID: ${entityId}`}>{entityLabel}</td><th scope="row">Currency</th><td>{document.currency}</td></tr><tr><th scope="row">Offset account</th><td>{document.account_code||'Not available'}</td><th scope="row">Posted journal</th><td>{document.je_number||'Not available'}</td></tr><tr><th scope="row">Period</th><td title={`Period ID: ${document.period_id}`}>{periodLabel}</td><th scope="row">Description</th><td>{document.description||'Not available'}</td></tr></tbody></table></div></>}</section>;
}
...
export function AuthoritativeAdjustmentSummary({title,adjustments=[],kind,onOpen}) {
  ...
  return <section aria-label={title}><div className="card-head"><div><h2>{title}</h2><p className="muted sm">{adjustments.length} {adjustments.length===1?'adjustment':'adjustments'}</p></div></div>{adjustments.length?<div className="table-wrap authoritative-adjustment-table" role="region" tabIndex={0} aria-label={`${title}; scroll horizontally to view every column`}><table className="tbl"><thead><tr><th scope="col">Adjustment</th><th scope="col">Date</th><th scope="col">Reason</th><th scope="col" className="ta-r">Amount</th><th scope="col">Status</th><th scope="col">Details</th></tr></thead><tbody>{adjustments.map(row=>{
    ...
    return <tr key={row.business_adjustment_id}><td>{row.adjustment_kind}</td><td>{row.accounting_date}</td><td>{row.reason||'Not available'}</td><td className="ta-r">{money(row.amount,row.currency)}</td><td>{row.status}</td><td><button id={focusId} type="button" className="btn btn-sm btn-ghost" onClick={event=>onOpen?.(row,focusId,Number(event.currentTarget.closest('.table-wrap')?.scrollLeft)||0)}>View details</button></td></tr>;
...
const adjustmentKinds=Object.freeze({AP:new Set(['AP_BILL_VOID','AP_VENDOR_CREDIT','AP_PAYMENT_REVERSAL']),AR:new Set(['AR_CREDIT_MEMO','AR_REFUND','AR_RECEIPT_REVERSAL'])});
export const authoritativeAdjustmentScopeMatches=(adjustment,side,config,returnContext)=>!!(
  adjustmentKinds[side]?.has(adjustment?.adjustment_kind)&&uuid.test(adjustment?.business_adjustment_id||'')&&revision(adjustment?.version)
  &&config?.entityId&&config?.periodId&&returnContext?.entityId===config.entityId&&returnContext?.periodId===config.periodId
  &&returnContext?.adjustmentId===adjustment.business_adjustment_id&&returnContext?.adjustmentVersion===Number(adjustment.version)
  &&returnContext?.adjustmentSide===side&&returnContext?.adjustmentKind===adjustment.adjustment_kind
  &&returnContext?.adjustmentPeriodId===adjustment.period_id&&adjustment.period_id===config.periodId
);
export const authoritativeAdjustmentJournalTarget=(adjustment,side,config,returnContext)=>{
  if(!authoritativeAdjustmentScopeMatches(adjustment,side,config,returnContext)||!uuid.test(adjustment?.journal_entry_id||'')||!['DRAFT','PENDING_REVIEW','REVIEWED','APPROVED','POSTED'].includes(adjustment?.journal_status)||!revision(adjustment?.journal_revision))return null;
  if(returnContext.journalEntryId!==adjustment.journal_entry_id||returnContext.journalStatus!==adjustment.journal_status||returnContext.journalRevision!==Number(adjustment.journal_revision))return null;
  return {journalEntryId:adjustment.journal_entry_id,status:adjustment.journal_status,revision:Number(adjustment.journal_revision)};
};
export function AuthoritativeAdjustmentDetail({adjustment,side,entityId,config,returnContext,onBack,onOpenJournal,currentActorAccess,accounts=[],scopes=[],fetcher=globalThis.fetch,onOpenDraft,onRefresh}){
  const payables=side==='AP',label=payables?'AP adjustment evidence':'AR adjustment evidence',entityLabel=config?.entityId===entityId?config?.scopePresentation?.entityLabel||'Configured entity':'Configured entity',periodLabel=config?.periodId===adjustment.period_id?config?.scopePresentation?.periodLabel||'Configured period':'Configured period',returnScope=documentReturnScope(entityLabel,returnContext?.view,adjustment.version,payables);
  const exactScope=authoritativeAdjustmentScopeMatches(adjustment,side,config,returnContext),journalTarget=authoritativeAdjustmentJournalTarget(adjustment,side,config,returnContext);
  return <section className="full-bleed qbo-transaction-report authoritative-document-detail" aria-label={label}><div className="qbo-report-back"><button type="button" onClick={onBack}>Back to {payables?'AP adjustments':'AR adjustments'}</button><details className="authoritative-return-context"><summary>List filters retained</summary><span>{returnScope}</span></details></div>{!exactScope?<StateBlock tone="blocked" title="BLOCKED — immutable adjustment scope mismatch">The selected adjustment ID, revision, kind, or accounting period no longer matches the exact parent list scope. Return to the list; no adjustment evidence or Journal link is asserted.</StateBlock>:<><div className="gl-drill-head"><div><div className="gl-drill-crumb">{payables?'Payables':'Receivables'} / read-only adjustment evidence</div><h1>{adjustment.adjustment_kind}</h1><div className="gl-drill-account">{adjustment.accounting_date} · {adjustment.business_adjustment_id}</div></div><span className="badge badge-muted">{adjustment.status}</span></div><div className="authoritative-document-detail-summary" aria-label={`${label} summary`}><span><i>Amount</i><b>{money(adjustment.amount,adjustment.currency)}</b></span><span><i>Period</i><b title={`Period ID: ${adjustment.period_id||'Not available'}`}>{periodLabel}</b></span><span><i>Currency</i><b>{adjustment.currency}</b></span><span><i>Revision</i><b>{adjustment.version}</b></span></div><p className="report-drill-hint">Retained adjustment evidence.</p>{journalTarget&&typeof onOpenJournal==='function'&&<button type="button" className="btn btn-sm" onClick={()=>onOpenJournal(journalTarget)}>{journalTarget.status==='POSTED'?'Open posted Journal':'Open linked Journal workflow'}</button>}{['AP_VENDOR_CREDIT','AR_CREDIT_MEMO'].includes(adjustment.adjustment_kind)&&<CreditAllocationHistory config={config} subjectId={adjustment.business_adjustment_id} kind={adjustment.adjustment_kind} access={currentActorAccess} accounts={accounts} scopes={scopes} fetcher={fetcher} onOpenDraft={onOpenDraft} onRefresh={onRefresh}/>}{['AP_VENDOR_CREDIT','AR_CREDIT_MEMO'].includes(adjustment.adjustment_kind)&&adjustment.status==='POSTED'&&<NativeCreditAllocationEntry config={config} kind={adjustment.adjustment_kind} sourceAdjustmentId={adjustment.business_adjustment_id} access={currentActorAccess} fetcher={fetcher} onRefresh={onRefresh}/>}{!payables&&adjustment.adjustment_kind==='AR_CREDIT_MEMO'&&adjustment.status==='POSTED'&&<NativeRefundEntry config={config} sourceAdjustmentId={adjustment.business_adjustment_id} access={currentActorAccess} accounts={accounts} scopes={scopes} fetcher={fetcher} onOpenDraft={onOpenDraft} onRefresh={onRefresh}/>} <ListedRecordJournal key={`${config?.baseUrl}:${entityId}:${config?.periodId}:${adjustment.business_adjustment_id}:${adjustment.version}:${currentActorAccess?.actor_id}`} config={config} row={adjustment} recordKind={adjustment.adjustment_kind} access={currentActorAccess} fetcher={fetcher}/><AuthoritativeLineageBlock record={adjustment} entityId={entityId} subject="Adjustment"/><details className="authoritative-secondary-disclosure authoritative-adjustment-fields"><summary><span>Adjustment evidence fields</span><span className="badge badge-muted">READ ONLY</span></summary><section><div className="table-wrap authoritative-document-detail-table" role="region" tabIndex={0} aria-label={`${label} fields; scroll horizontally to view every column`}><table className="tbl"><tbody><tr><th scope="row">Entity</th><td title={`Entity ID: ${entityId}`}>{entityLabel}</td><th scope="row">Document ID</th><td>{adjustment.business_document_id||'Not available'}</td></tr><tr><th scope="row">Source adjustment</th><td>{adjustment.source_adjustment_id||'Not available'}</td><th scope="row">Journal entry</th><td>{adjustment.journal_entry_id||'Not available'}</td></tr><tr><th scope="row">Journal status</th><td>{adjustment.journal_status||'Not available'}</td><th scope="row">Created at</th><td>{adjustment.created_at}</td></tr><tr><th scope="row">Reason</th><td colSpan="3">{adjustment.reason||'Not available'}</td></tr></tbody></table></div></section></details></>}</section>;
}

export function AuthoritativeDocumentWorkspace({kind,documents=[],adjustments=[],readScopes=null,view,onViewChange,onOpenDocument,onOpenAdjustment,onOpenAging,config,fetcher=globalThis.fetch,currentActorAccess,scope,accounts,onOpenDraft,onRefresh,onOpenExpenseDraft}) {
  const [receivableTab,setReceivableTab]=useState('INVOICES');
  const bill=kind==='AP';
  ...
  const tabs=bill?[]:[{id:'INVOICES',label:'Invoices'}, {id:'RECEIPTS',label:'Sales receipts'}, {id:'AGING',label:'AR Aging',focusId:'authoritative-ar-aging-launch'}, {id:'COUNTERPARTIES',label:'Counterparties',unavailable:true}];
  const activeTab=bill?state.transactionType:receivableTab;
  const selectTab=next=>{
    if(next==='AGING'){onOpenAging?.();return;}
    if(bill)change({transactionType:next});
    else if(['INVOICES','RECEIPTS'].includes(next))setReceivableTab(next);
  };
  ...
  return <AuthoritativeApArView kind={kind} readOnly={!(documentEntry||creditEntry)} className="authoritative-document-workspace stack" headerClassName={`authoritative-document-page-head${bill?' authoritative-expense-page-head':''}`} metrics={metrics} tabs={tabs} activeTab={activeTab} onSelectTab={selectTab} toolbar={bill?null:<p className="muted sm authoritative-api-scope">API read · filters do not change records.</p>}>
    ...
    {(!bill||appliedScope.length>0)&&<p className="muted sm authoritative-applied-scope" aria-live="polite">{appliedScope.length?appliedScope.join(' · '):'All records'}</p>}
    </section>
    {showDocumentList&&<section className="card" aria-label={`${workspaceLabel} document list facts`}>
    {page.total?<AuthoritativeDocumentTable title={bill?'Bills':'AR invoices'} documents={page.rows} kind={kind} onOpen={onOpenDocument}/>:sourceEmpty?<StateBlock tone="empty" title="No expenses found">Change the filters to see more results. Empty results do not confirm a zero balance.</StateBlock>:bill&&state.transactionType==='ALL'&&visibleResultCount===0?<StateBlock tone="empty" title="No expenses match these filters">Try changing or resetting the filters. This scoped result is not evidence of zero activity.</StateBlock>:documents.length?<StateBlock tone="empty" title={`No ${bill?'bills':'invoices'} match these filters`}>Try changing or resetting the filters. This result does not confirm a zero balance.</StateBlock>:<AuthoritativeScopeEmpty subject={bill?'AP bills':'AR invoices'}/>}
