...
export function AuthoritativeDocumentTable({title,documents=[],kind,onOpen}) {
  ...
  return <section aria-label={title}><div className="card-head"><div><h2>{title}</h2><p className="muted sm">{bill?`${documents.length} ${documents.length===1?'bill':'bills'}`:'Authoritative API rows only. Open a row to inspect its retained evidence.'}</p></div></div>{documents.length?<div className="table-wrap authoritative-document-table" role="region" tabIndex={0} aria-label={`${title}; scroll horizontally to view every column`}><table className="tbl"><thead><tr><th scope="col">{bill?'Bill':'Invoice'}</th><th scope="col">{bill?'Vendor':'Customer'}</th><th scope="col">Date</th><th scope="col">Due date</th><th scope="col" className="ta-r">Amount</th><th scope="col" className="ta-r">Open balance</th><th scope="col">Status</th><th scope="col">{bill?'Details':'Evidence'}</th></tr></thead><tbody>{documents.map(row=>{
    ...
    return <tr key={key}><td>{row[number]}</td><td>{row[counterparty]}</td><td>{row[dateKey]}</td><td>{row.due_date||'Not retained'}</td><td className="ta-r">{money(row.amount,row.currency)}</td><td className="ta-r">{money(row.open_balance,row.currency)}</td><td><span className="badge badge-muted authoritative-row-status">{row.status}</span></td><td><button id={focusId} type="button" className="btn btn-sm btn-ghost" onClick={event=>onOpen?.(row,focusId,Number(event.currentTarget.closest('.table-wrap')?.scrollLeft)||0)}>{bill?'View details':'Open evidence'}</button></td></tr>;
...
export function AuthoritativeAdjustmentSummary({title,adjustments=[],kind,onOpen}) {
  ...
  return <section aria-label={title}><div className="card-head"><div><h2>{title}</h2><p className="muted sm">{payables?`${adjustments.length} ${adjustments.length===1?'adjustment':'adjustments'}`:'Authoritative adjustment rows only. Open a row to inspect its retained evidence.'}</p></div></div>{adjustments.length?<div className="table-wrap authoritative-adjustment-table" role="region" tabIndex={0} aria-label={`${title}; scroll horizontally to view every column`}><table className="tbl"><thead><tr><th scope="col">Adjustment</th><th scope="col">Date</th><th scope="col">Reason</th><th scope="col" className="ta-r">Amount</th><th scope="col">Status</th><th scope="col">{payables?'Details':'Evidence'}</th></tr></thead><tbody>{adjustments.map(row=>{
    const focusId=authoritativeEvidenceKey('adjustment',row)?`authoritative-adjustment-${row.business_adjustment_id}`:undefined;
    return <tr key={row.business_adjustment_id}><td>{row.adjustment_kind}</td><td>{row.accounting_date}</td><td>{row.reason||'Not retained'}</td><td className="ta-r">{money(row.amount,row.currency)}</td><td>{row.status}</td><td><button id={focusId} type="button" className="btn btn-sm btn-ghost" onClick={event=>onOpen?.(row,focusId,Number(event.currentTarget.closest('.table-wrap')?.scrollLeft)||0)}>{payables?'View details':'Open evidence'}</button></td></tr>;
