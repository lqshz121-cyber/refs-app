...
export function AuthoritativeUnavailableWorkspace({ item, config }) {
  ...
  const periodId = config?.periodId;
  return <AuthoritativeDemoView area={`${label} unavailable workspace`} className="stack authoritative-unavailable-workspace">
    ...
    <section className="report-workbench" role="status">
      ...
      <div className="qbo-toolgrid"><span><i>Entity scope</i><b title={entityId ? `Entity ID: ${entityId}` : undefined}>{entityId ? 'Configured entity' : 'Not configured'}</b></span><span><i>Period scope</i><b title={periodId ? `Period ID: ${periodId}` : undefined}>{periodId ? 'Configured period' : 'Not configured'}</b></span><span><i>Requested workspace</i><b>{label}</b></span></div>
