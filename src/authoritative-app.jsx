...
export function AuthoritativeApp({ environment = globalThis, fetcher = globalThis.fetch }) {
  ...
  if (typeof environment?.document === 'undefined') return <main className="login-shell"><section className="login-card"><h1>REFS accounting</h1><p>Preparing your secure session.</p></section></main>;
  if (phase === 'CHECKING_RELEASE') return <main className="login-shell"><section className="login-card"><h1>Preparing your workspace</h1><p>Confirming that your accounting workspace is ready before records are shown.</p></section></main>;
  if (phase === 'CHECKING_IDENTITY') return <main className="login-shell"><section className="login-card"><h1>Confirming your sign-in</h1><p>Checking your secure session before records are shown.</p></section></main>;
  if (phase === 'LOGIN_REQUIRED' || phase === 'IDENTITY_FAILED') return <main className="login-shell"><section className="login-card">
    <h1>Sign in to REFS accounting</h1><p>Sign in securely to view the records available to your role.</p>
  ...
  return <div className="app authoritative-app">
    ...
    <div className="main">
      ...
      <main className="content">
        ...
        {phase === 'LOADING_ACCOUNTING' && <StateBlock tone="loading">Loading financial records...</StateBlock>}
        ...
        {phase === 'READY' && route === 'project-cost-cwip' && <AuthoritativeReportsWorkspace key={`project-cost-cwip-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'OPERATING_ANALYSIS',query:'',preview:'TRIAL_BALANCE'}} initialDimensionType="PROJECT" workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Project Cost & CWIP" workspaceDescription="Review project profitability, work in progress, construction financing, prepaid balances, and budget results. Detailed cost-code, vendor, and project registers will appear when they are ready for review."/>}
        {phase === 'READY' && route === 'unit-cost-ledger' && <AuthoritativeReportsWorkspace key={`unit-cost-ledger-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'OPERATING_ANALYSIS',query:'',preview:'TRIAL_BALANCE'}} initialDimensionType="UNIT" workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Unit / Lot profitability" workspaceDescription="Review income and costs for a selected unit or lot, then open the supporting entries and documents. Transfers, pricing, and allocation setup are managed outside this view."/>}
        {phase === 'READY' && route === 'property-ops-pickup' && <AuthoritativeReportsWorkspace key={`property-ops-pickup-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'OPERATING_ANALYSIS',query:'',preview:'TRIAL_BALANCE'}} initialDimensionType="PROPERTY" workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Property operating P&amp;L" workspaceDescription="Review property income and costs for the selected period, then open the supporting entries and documents. Rent pickup and journal creation are managed through their controlled workflows."/>}
        {phase === 'READY' && route === 'construction-loan' && <AuthoritativeReportsWorkspace key={`construction-loan-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'CASH_AND_CAPITAL',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Construction Loan" workspaceDescription="Review construction-loan balances, draws, repayments, and supporting ledger evidence. Lender commitments and draw management are handled through their controlled workflows."/>}
        {phase === 'READY' && route === 'amortization' && <AuthoritativeReportsWorkspace key={`amortization-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'CASH_AND_CAPITAL',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Amortization Center" workspaceDescription="Review prepaid balances and recognized expense by period with supporting ledger evidence. Schedule setup and posting follow the standard review workflow."/>}
        {phase === 'READY' && route === 'intercompany' && <AuthoritativeReportsWorkspace key={`intercompany-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'GROUP_AND_COMPARISON',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Intercompany" workspaceDescription="Review balances between the selected companies and period, including the supporting entries. Elimination and adjustment posting follow the controlled close workflow."/>}
        {phase === 'READY' && route === 'consolidation' && <AuthoritativeReportsWorkspace key={`consolidation-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'GROUP_AND_COMPARISON',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="GENERAL LEDGER" workspaceTitle="Consolidation" workspaceDescription="Review approved group statements and their supporting ledger evidence. Group setup and elimination entries follow the controlled close workflow."/>}
