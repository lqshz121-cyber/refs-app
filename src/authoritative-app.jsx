...
export function AuthoritativeApp({ environment = globalThis, fetcher = globalThis.fetch }) {
  ...
  if (phase === 'LOAD_FAILED' && error?.code === 'ACCOUNTING_API_RELEASE_MISMATCH') return <main className="login-shell"><section className="login-card"><h1>版本不匹配，等待 API 发布</h1><p>前端构建版本与 API /health/ready 返回的发布版本不一致。REFS 会在 OIDC 登录和任何会计数据读取之前停止。</p><RuntimeErrorPanel code={error.code} detail={error.message} onRetry={refresh}/></section></main>;
