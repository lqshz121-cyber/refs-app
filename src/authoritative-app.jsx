...
import { AuthoritativeWbsH1ImportWorkspace } from './authoritative-wbs-h1-import-workspace.jsx';
...
import { AuthoritativeBillPaymentsWorkspace } from './authoritative-bill-payments-workspace.jsx';
import { AuthoritativeSubsidiaryLedgerWorkspace } from './authoritative-subsidiary-ledger-workspace.jsx';
import { AuthoritativeLoanRegisterWorkspace } from './authoritative-loan-register-workspace.jsx';
import { AuthoritativeAccountingStagingWorkspace } from './authoritative-accounting-staging-workspace.jsx';
import { AuthoritativeMappingExceptionsWorkspace } from './authoritative-mapping-exceptions-workspace.jsx';
import { AuthoritativeReceiptsWorkspace } from './authoritative-receipts-workspace.jsx';
import { AuthoritativeIntegrationTransactionsWorkspace } from './authoritative-integration-transactions-workspace.jsx';
...
const internalReadOnlyRuntime = environment => Boolean(
  ...
  && environment?.__REFS_OIDC__ === null
);
...
export function AuthoritativeApp({ environment = globalThis, fetcher = globalThis.fetch }) {
  ...
  const [reportReconciliationDetail, setReportReconciliationDetail] = useState(null);
  const [reportCatalogReturn, setReportCatalogReturn] = useState(null);
  const [workflowJournalId,setWorkflowJournalId]=useState(null);
  ...
  const openAdjustmentEvidence = useCallback((side, row, focusId, tableX) => {
    ...
    setAdjustmentDetail({side,row,returnContext:{
      ...returnContext,
      adjustmentId:row.business_adjustment_id,
      adjustmentVersion:Number(row.version),
      adjustmentSide:side,
      adjustmentKind:row.adjustment_kind,
      adjustmentPeriodId:row.period_id,
      journalEntryId:row.journal_entry_id??null,
      journalStatus:row.journal_status??null,
      journalRevision:row.journal_revision==null?null:Number(row.journal_revision),
    }});
  }, [config, environment, listViews]);
  ...
  const openReportReconciliation = useCallback((focusId, catalog) => {
    const returnContext=createAuthoritativeReturnContext({config,view:DEFAULT_AUTHORITATIVE_LIST_VIEW,focusId,scrollY:Number(environment?.scrollY)||0});
    if (!returnContext) return;
    setReportReconciliationDetail({returnContext,catalog});
    setRouteState('reconciliation');
    retainRoute(environment, 'reconciliation');
  }, [config, environment]);

  const closeReportReconciliation = useCallback(() => {
    const context=reportReconciliationDetail?.returnContext;
    setReportCatalogReturn(reportReconciliationDetail?.catalog || null);
    setReportReconciliationDetail(null);
    setRouteState('reports');
    retainRoute(environment, 'reports');
    restoreAuthoritativeReturnContext(environment,config,context);
  }, [reportReconciliationDetail, environment, config]);
  ...
  const openAdjustmentJournalWorkflow = useCallback(async target => {
    if(!config||!target?.journalEntryId)return;
    const isCurrent=accountingReadGuard.current.begin('adjustment-journal',accountingReadGeneration);
    if(!isCurrent())return;
    ...
    if(!journals.journals.some(row=>row.journal_entry_id===target.journalEntryId&&row.status===target.status&&Number(row.revision)===target.revision)){
      setError({code:'ADJUSTMENT_JOURNAL_STALE',message:'The linked Journal no longer matches the adjustment list revision. Refresh the adjustment before continuing.'});return;
    }
    setData(current=>({...current,journals:journals.journals}));setSharedAccountingLoaded(true);setError(null);setRoute('journals');setWorkflowJournalId(target.journalEntryId);
  },[config,boundFetcher,setRoute,accountingReadGeneration]);
  ...
  if (phase === 'LOAD_FAILED' && error?.code === 'ACCOUNTING_API_RELEASE_MISMATCH') return <main className="login-shell"><section className="login-card"><h1>Deployment version mismatch</h1><p>The client release stamp does not match the API /health/ready release stamp. REFS stops before OIDC sign-in or any accounting-data read.</p><RuntimeErrorPanel code={error.code} detail={error.message} onRetry={refresh}/></section></main>;
  ...
  return <div className="app authoritative-app">
    ...
    <div className="main">
      ...
      <main className="content">
        ...
        {phase === 'READY' && route === 'receipts' && <AuthoritativeReceiptsWorkspace key={`receipts-${workspaceRefreshVersion}`} config={displayConfig} fetcher={boundFetcher}/>}
        {phase === 'READY' && route === 'integration-transactions' && <AuthoritativeIntegrationTransactionsWorkspace key={`integration-transactions-${workspaceRefreshVersion}`} config={displayConfig} fetcher={boundFetcher}/>}
        {phase === 'READY' && route === 'rules' && <AuthoritativeRulesWorkspace key={`rules-${workspaceRefreshVersion}`} config={displayConfig} fetcher={boundFetcher}/>}
        {phase === 'READY' && route === 'recurring-transactions' && <AuthoritativeRecurringTransactionsWorkspace key={`recurring-transactions-${workspaceRefreshVersion}`} config={displayConfig} fetcher={boundFetcher}/>}
        ...
        {phase === 'READY' && route === 'checks-payments' && <AuthoritativeBillPaymentsWorkspace key={`checks-payments-${workspaceRefreshVersion}`} config={displayConfig} access={accessState.status==='READY'?accessState.row:null} fetcher={boundFetcher} workspaceTitle="Checks & payments" workspaceDescription="Review retained vendor payment evidence for the selected company and accounting period. Check printing and payment commands remain separate and unavailable." backLabel="Back to bank transactions" onBack={()=>setRoute('bank')}/>}
        {phase === 'READY' && route === 'loan-register' && <AuthoritativeLoanRegisterWorkspace key={`loan-register-${workspaceRefreshVersion}`} config={displayConfig} access={accessState.status==='READY'?accessState.row:null} fetcher={boundFetcher} onBack={()=>setRoute('construction-loan')}/>}
        {phase === 'READY' && route === 'subsidiary-ledger' && <AuthoritativeSubsidiaryLedgerWorkspace key={`subsidiary-ledger-${workspaceRefreshVersion}`} config={displayConfig} fetcher={boundFetcher}/>}
        {phase === 'READY' && route === 'integration-hub' && <AuthoritativeWbsH1ImportWorkspace key={`integration-hub-${workspaceRefreshVersion}`} config={displayConfig} scopes={scopeCatalog} currentActorAccess={accessState.status==='READY'?accessState.row:null} fetcher={boundFetcher} environment={environment} onOpenJournalWorkflow={openWbsH1DraftWorkflow}/>}
        ...
        {phase === 'READY' && route === 'reports' && <AuthoritativeReportsWorkspace key={`reports-${workspaceRefreshVersion}-${reportsNavigationVersion}`} config={displayConfig} scopes={scopeCatalog} fetcher={boundFetcher} environment={environment} currentActorAccess={accessState.status==='READY'?accessState.row:null} initialCatalog={reportCatalogReturn||DEFAULT_AUTHORITATIVE_REPORTS_CATALOG} onOpenArAging={openReportAgingEvidence} onOpenGeneralLedger={openReportGeneralLedger} onOpenReconciliation={openReportReconciliation}/>}
        ...
        {phase === 'READY' && route === 'intercompany' && <AuthoritativeReportsWorkspace key={`intercompany-${workspaceRefreshVersion}`} config={displayConfig} scopes={scopeCatalog} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'GROUP_AND_COMPARISON',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="AUTHORITATIVE - ACCOUNTING OPERATIONS" workspaceTitle="Intercompany" workspaceDescription="Review raw reciprocal member balances, then create and route an evidence-bound two-line presentation elimination through maker, reviewer, approver, and poster controls. Member-company ledgers remain unchanged."/>}
        {phase === 'READY' && route === 'consolidation' && <AuthoritativeReportsWorkspace key={`consolidation-${workspaceRefreshVersion}`} config={displayConfig} scopes={scopeCatalog} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'GROUP_AND_COMPARISON',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="AUTHORITATIVE - GENERAL LEDGER" workspaceTitle="Consolidation" workspaceDescription="Read approved group snapshots and POSTED ledger evidence, review retained intercompany elimination batches, and post only independently approved presentation evidence. Group maintenance remains a separate controlled workflow."/>}
        ...
        {phase === 'READY' && ['month-end-close','period-management','closing-accounting'].includes(route) && <AuthoritativePeriodCloseWorkspace key={`period-close-${workspaceRefreshVersion}`} config={displayConfig} fetcher={boundFetcher} currentActorAccess={accessState.status==='READY'?accessState.row:null}/>}
