...
import { accountingApiConfig, activateAuthoritativeReadAccess, refreshAuthoritativeChartOfAccounts, refreshAuthoritativeDocuments, refreshAuthoritativeJournalEntries } from './accounting-api.js';
...
export function AuthoritativeApp({ environment = globalThis, fetcher = globalThis.fetch }) {
  ...
  const openAgingEvidence = useCallback((side, focusId) => {
    ...
    if (!returnContext) return;
    setAgingDetail({side,returnContext:{...returnContext,agingSide:String(side).toUpperCase(),agingOrigin:'RECEIVABLES'}});
  }, [config, environment, listViews]);
  ...
  const scopePresentation=useMemo(()=>authoritativeScopePresentation(config,scopeRows),[config,scopeRows]);
  ...
  return <div className="app authoritative-app">
    ...
    <div className="main">
      ...
      <main className="content">
        ...
        {phase === 'LOADING_ACCOUNTING' && <StateBlock tone="loading">Loading your financial information...</StateBlock>}
        ...
        {phase === 'READY' && route === 'ai-audit' && <AuthoritativeAiAuditWorkspace key={`ai-audit-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher}/>}
        {phase === 'READY' && route === 'wbs-autorec-evidence' && <AuthoritativeWbsTransitionWorkspace key={`wbs-autorec-evidence-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} onAccountingRefresh={async()=>{const [documents,journals]=await Promise.all([refreshAuthoritativeDocuments({config,fetcher:boundFetcher}),refreshAuthoritativeJournalEntries({config,fetcher:boundFetcher})]);if(documents.ok&&journals.ok){setData({ap:documents.ap,ar:documents.ar,journals:journals.journals});setSharedAccountingLoaded(true);}}}/>}
        ...
        {phase === 'READY' && route === 'project-cost-cwip' && <AuthoritativeReportsWorkspace key={`project-cost-cwip-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'OPERATING_ANALYSIS',query:'',preview:'TRIAL_BALANCE'}} initialDimensionType="PROJECT" workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Project Cost & CWIP" workspaceDescription="Review project profitability, construction in progress, financing, prepaid costs, and budget results for the selected period. Detailed cost-code and vendor activity will appear here when it is ready."/>}
        {phase === 'READY' && route === 'construction-loan' && <AuthoritativeReportsWorkspace key={`construction-loan-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'CASH_AND_CAPITAL',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Construction Loan" workspaceDescription="Review loan activity and balances for the selected period. Detailed lender, commitment, and draw management will appear here when it is ready."/>}
        {phase === 'READY' && route === 'amortization' && <AuthoritativeReportsWorkspace key={`amortization-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'CASH_AND_CAPITAL',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Amortization Center" workspaceDescription="Review prepaid balances and period amortization. Schedule setup and posting will appear here when they are ready."/>}
        {phase === 'READY' && route === 'intercompany' && <AuthoritativeReportsWorkspace key={`intercompany-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'GROUP_AND_COMPARISON',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="ACCOUNTING OPERATIONS" workspaceTitle="Intercompany" workspaceDescription="Review balances between companies for aligned periods. Eliminations and adjustments will appear only when they are ready for review."/>}
        {phase === 'READY' && route === 'consolidation' && <AuthoritativeReportsWorkspace key={`consolidation-${workspaceRefreshVersion}`} config={config} fetcher={boundFetcher} environment={environment} initialCatalog={{category:'GROUP_AND_COMPARISON',query:'',preview:'TRIAL_BALANCE'}} workspaceEyebrow="GENERAL LEDGER" workspaceTitle="Consolidation" workspaceDescription="Review approved group balances and performance for the selected period. Elimination entries and group maintenance will appear here when they are ready."/>}
