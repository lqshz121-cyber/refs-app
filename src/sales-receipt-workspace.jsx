...
export function SalesReceiptWorkspace({config,access,scope,fetcher=globalThis.fetch,onOpenDraft}){
 ...
 {allowed?<ReceiptList key={`${config.baseUrl}:${config.entityId}:${config.periodId}:${access.actor_id}:${version}`} config={config} access={access} fetcher={fetcher}/>:<section className="card"><h2>Sales receipts</h2><p role="status">Sales receipt access is unavailable for this company.</p></section>}</div>;
}
function ReceiptList({config,access,fetcher}){
