...
export function AuthoritativeWbsLivePilotObservation({config,fetcher=globalThis.fetch,tools=WBS_LIVE_PILOT_SURFACE_TOOLS.wbs,title='Production WBS provider observation',showRows=true}){
  ...
  const hasExactAttestationScope=observation?.scope?.company_codes?.length===1;
  ...
  const read=async event=>{
    ...
    const result=await refreshAuthoritativeWbsLivePilot({config,tool,limit:10,companyCode:scopedPayables?companyCode.trim()||null:null,dateFrom:scopedPayables?dateFrom:null,dateTo:scopedPayables?dateTo:null,fetcher});
