...
import { Badge, Btn, KPI, Table, Tabs, StateBlock } from './ui.jsx';
  ...
  return <div className="full-bleed">
    ...
    <div className="split two">
      ...
      <div className="card sticky-card">
        ...
        </> : <StateBlock tone="empty" title="No schedule line selected">Select a schedule line to review its retained amortization evidence.</StateBlock>}
      </div>
    </div>
  </div>;
}
    ...
    {tab === 'Checklist' ? <div className="card">
      ...
      <div className="card sticky-card">
        ...
        {accrualFinding ? <>
          ...
          <div className="row-acts" style={{ marginTop: 14 }}>
            <Btn variant="primary" onClick={createAccrualDraft} disabled={Boolean(state.accrual_je_id)} title={state.accrual_je_id ? 'An accrual Draft JE already exists for this candidate' : 'Create the accrual Draft JE'}>Create accrual Draft JE</Btn>
            <Btn onClick={createReversalDraft} disabled={!state.accrual_je_id || Boolean(state.reversal_je_id)} title={!state.accrual_je_id ? 'Create the accrual Draft JE first' : state.reversal_je_id ? 'A reversing Draft JE already exists' : 'Create the reversing Draft JE'}>Create reversing Draft JE</Btn>
          </div>
        </> : <StateBlock tone="empty" title="No accrual candidate">No accrual candidate was generated by the WBS mock rules.</StateBlock>}
      </div>
    </div>}
  </div>;
}
