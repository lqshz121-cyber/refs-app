...
export async function uploadVerifiedAttachment({config=accountingApiConfig(),file,idempotencyKey,fetcher=globalThis.fetch,cryptoApi=globalThis.crypto}={}){
  const metadata=validateAttachmentFile(file);
  const authoritativeConfig=normalizeConfig(config);
  if(!authoritativeConfig||typeof fetcher!=='function'||!cryptoApi?.subtle||!validIdempotency(idempotencyKey)||!metadata)return fail('ATTACHMENT_UPLOAD_INVALID','Attachment name, type, size, or command configuration is invalid.');
