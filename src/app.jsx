...
import { accountingApiConfig, refreshAuthoritativeDocuments } from './accounting-api.js';
...
function App() {
  ...
  const actions = {
    ...
    addBill: (f) => { if(accountingApiConfig())return {ok:false,code:'ACCOUNTING_API_WRITE_REQUIRED',message:'Create Bills through the authoritative accounting API.'};const auth=can('AP.INVOICE.CREATE');if(!auth)return {ok:false,code:'AP_PERMISSION_DENIED',message:'Missing permission AP.INVOICE.CREATE.'};const dup = ap.bills.find(b=>b.vendor_id===f.vendor_id && b.invoice_no.trim().toLowerCase()===f.invoice_no.trim().toLowerCase());
