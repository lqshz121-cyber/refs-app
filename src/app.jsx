...
function AuthoritativeWorkflowWorkspace({config}){
  ...
  return <main className="login-shell"><section className="login-card" aria-live="polite" style={{maxWidth:1180,width:'calc(100% - 32px)'}}><h1>REFS authoritative workspace</h1><p>Authenticated data is read directly from the accounting API. Browser-local accounting data is disabled.</p><button className="btn btn-primary" disabled={state.phase==='LOADING'} onClick={refresh}>Refresh authoritative records</button>{notice&&<p role="alert" className="muted">{notice}</p>}{state.phase==='LOADING'&&<p className="muted">Loading authoritative AP/AR records…</p>}{state.phase==='FAILED'&&<p role="alert" className="muted">{state.message||'Authoritative accounting refresh failed.'}</p>}{state.phase==='READY'&&<><AuthoritativeWorkflowTable title="Authoritative AP bills" documents={state.bills} kind="AP" onWorkflow={transition} workingJournalIds={workingJournalIds}/><AuthoritativeWorkflowAdjustmentTable title="Authoritative AP adjustments" adjustments={state.apAdjustments} onWorkflow={transition} workingJournalIds={workingJournalIds}/><AuthoritativeWorkflowTable title="Authoritative AR invoices" documents={state.invoices} kind="AR" onWorkflow={transition} workingJournalIds={workingJournalIds}/><AuthoritativeWorkflowAdjustmentTable title="Authoritative AR adjustments" adjustments={state.arAdjustments} onWorkflow={transition} workingJournalIds={workingJournalIds}/><AuthoritativeDraftForm config={config} onCreated={refresh}/><p className="muted" style={{marginTop:20}}>Workflow actions are sent with the authoritative revision. The server enforces permission and separation of duties; this browser never infers either.</p></>}</section></main>;
}
...
export { App, AuthoritativeAdjustmentSummary, AuthoritativeDocumentTable, AuthoritativeDraftForm, AuthoritativeWorkflowAdjustmentTable, AuthoritativeWorkflowTable, nextAuthoritativeWorkflowAction };
