...
import { applyPostedDocumentTransition, validateDocumentReversal } from './document-posting.js';
...
function App() {
  ...
  const applyPostedSource = je => {
    ...
    if(!result.ok){setExceptions(xs=>xs.some(e=>e.exception_type===result.code&&e.object_ref===String(je.source_object_id)&&e.status==='OPEN')?xs:[{exception_id:nextId(),exception_type:result.code,object_type:je.source_object_type,object_ref:String(je.source_object_id),status:'OPEN',occurred_date:'2026-07-31',severity:'HIGH',description:`Posted source synchronization blocked for ${je.je_number}`},...xs]);audit('SOURCE_STATUS_SYNC_BLOCKED',je.source_object_type,String(je.source_object_id),result.code);return result;}
