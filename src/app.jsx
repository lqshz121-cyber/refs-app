...
import { accountingApiConfig, applyAuthoritativeCredit, createAuthoritativeAdjustment, createAuthoritativeBusinessDocument, createAuthoritativeSettlement, refreshAuthoritativeDocuments, refreshAuthoritativeJournalEntries, transitionAuthoritativeJournal } from './accounting-api.js';
...
function AuthoritativeWorkflowAdjustmentTable({title,adjustments,onWorkflow,workingJournalIds}){
  return <section aria-label={title} style={{marginTop:20}}><h2 style={{fontSize:18,margin:'0 0 8px'}}>{title} ({adjustments.length})</h2>{!adjustments.length?<p className="muted">No authoritative adjustments were returned.</p>:<div className="table-wrap"><table className="tbl"><thead><tr><th>Kind</th><th className="ta-r">Amount</th><th>Status</th><th>Workflow</th></tr></thead><tbody>{adjustments.map(adjustment=>{const journalId=adjustment.journal_entry_id,action=nextAuthoritativeWorkflowAction(adjustment.journal_status),eligible=typeof journalId==='string'&&Number.isSafeInteger(adjustment.journal_revision)&&action,working=workingJournalIds.has(journalId);return <tr key={adjustment.business_adjustment_id}><td>{adjustment.adjustment_kind||'UNKNOWN'}</td><td className="ta-r">{authoritativeMoney(adjustment.amount,adjustment.currency)}</td><td><span className="badge badge-muted">{adjustment.status||'UNKNOWN'}</span></td><td>{eligible?<button className="btn btn-primary btn-sm" disabled={working} onClick={()=>onWorkflow(adjustment,action)}>{working?'Working…':action}</button>:<span className="muted sm">{adjustment.journal_status||'-'}</span>}</td></tr>;})}</tbody></table></div>}</section>;
}

function AuthoritativeJournalEntryTable({journals,onWorkflow,workingJournalIds}){
  return <section aria-label="Authoritative Journal Entries" style={{marginTop:20}}><h2 style={{fontSize:18,margin:'0 0 8px'}}>Authoritative Journal Entries ({journals.length})</h2>{!journals.length?<p className="muted">No authoritative Journal Entries were returned.</p>:<div className="table-wrap"><table className="tbl"><thead><tr><th>Journal</th><th>Date</th><th>Type</th><th>Description</th><th>Ledger lines</th><th>Status</th><th>Workflow</th></tr></thead><tbody>{journals.map(journal=>{const action=nextAuthoritativeWorkflowAction(journal.status),working=workingJournalIds.has(journal.journal_entry_id);return <tr key={journal.journal_entry_id}><td>{journal.journal_number||journal.journal_entry_id}</td><td>{journal.journal_date||'-'}</td><td>{journal.journal_type||'-'}</td><td>{journal.description||'-'}</td><td>{journal.ledger_line_count}</td><td><span className="badge badge-muted">{journal.status||'UNKNOWN'}</span></td><td>{action?<button className="btn btn-primary btn-sm" disabled={working} onClick={()=>onWorkflow({journal_entry_id:journal.journal_entry_id,journal_revision:journal.revision},action)}>{working?'Working…':action}</button>:<span className="muted sm">{journal.status||'-'}</span>}</td></tr>;})}</tbody></table></div>}</section>;
}

const postedAdjustments=(adjustments,kind)=>adjustments.filter(adjustment=>adjustment?.adjustment_kind===kind&&adjustment?.status==='POSTED'&&typeof adjustment.business_adjustment_id==='string');
const openDocuments=documents=>documents.filter(document=>document?.journal_status==='POSTED'&&typeof document.business_document_id==='string'&&Number(document.open_balance)>0);
const commandKey=prefix=>globalThis.crypto?.randomUUID?`${prefix}-${globalThis.crypto.randomUUID()}`:null;

function AuthoritativeCreditApplicationForm({config,kind,credits,documents,onCompleted}){
  const [form,setForm]=useState({creditId:'',documentId:'',amount:'',reason:''});
  const [state,setState]=useState({phase:'IDLE',message:''});
  const keyRef=useRef(null);
  const availableCredits=postedAdjustments(credits,kind),availableDocuments=openDocuments(documents);
  const creditLabel=kind==='AP_VENDOR_CREDIT'?'Vendor credit':'Credit memo',documentLabel=kind==='AP_VENDOR_CREDIT'?'Bill':'Invoice';
  const update=(key,value)=>setForm(current=>({...current,[key]:value}));
  const submit=async event=>{event.preventDefault();const credit=availableCredits.find(item=>item.business_adjustment_id===form.creditId),document=availableDocuments.find(item=>item.business_document_id===form.documentId),amount=Number(form.amount);if(!credit||!document||!Number.isFinite(amount)||amount<=0||!form.reason.trim()){setState({phase:'FAILED',message:`Select a posted ${creditLabel.toLowerCase()}, an open ${documentLabel.toLowerCase()}, amount, and reason.`});return;}const idempotencyKey=keyRef.current||(commandKey('UI-CREDIT-APPLY'));if(!idempotencyKey){setState({phase:'FAILED',message:'Browser cryptography is required for an idempotent credit allocation.'});return;}keyRef.current=idempotencyKey;setState({phase:'WORKING',message:'Sending the authoritative allocation command…'});const result=await applyAuthoritativeCredit({config,kind,businessAdjustmentId:credit.business_adjustment_id,businessDocumentId:document.business_document_id,amount,reason:form.reason.trim(),idempotencyKey});if(!result.ok){setState({phase:'FAILED',message:result.message||'Authoritative credit allocation was rejected.'});return;}keyRef.current=null;setForm({creditId:'',documentId:'',amount:'',reason:''});const refreshed=await onCompleted();setState(refreshed?.ok===false?{phase:'FAILED',message:'Allocation was accepted. Refresh is required to confirm the authoritative state.'}:{phase:'SUCCEEDED',message:'Allocation was accepted and authoritative records were refreshed.'});};
  return <section aria-label={`${creditLabel} application`} style={{marginTop:20}}><h2 style={{fontSize:18,margin:'0 0 8px'}}>{creditLabel} application</h2><p className="muted sm">Only POSTED credits and POSTED documents returned by the API are selectable. The server validates remaining balances, period, authorization, and idempotency.</p>{!availableCredits.length||!availableDocuments.length?<p className="muted">No eligible authoritative {creditLabel.toLowerCase()} or open {documentLabel.toLowerCase()} is currently available.</p>:<form onSubmit={submit} className="two-col" style={{alignItems:'end'}}><label>{creditLabel}<select value={form.creditId} onChange={event=>update('creditId',event.target.value)} required><option value="">Select…</option>{availableCredits.map(credit=><option key={credit.business_adjustment_id} value={credit.business_adjustment_id}>{credit.adjustment_number||credit.business_adjustment_id} — {authoritativeMoney(credit.amount,credit.currency)}</option>)}</select></label><label>{documentLabel}<select value={form.documentId} onChange={event=>update('documentId',event.target.value)} required><option value="">Select…</option>{availableDocuments.map(document=><option key={document.business_document_id} value={document.business_document_id}>{document.bill_no||document.inv_no||document.business_document_id} — {authoritativeMoney(document.open_balance,document.currency)} open</option>)}</select></label><label>Amount<input type="number" min="0.0001" step="0.0001" value={form.amount} onChange={event=>update('amount',event.target.value)} required/></label><label>Reason<input value={form.reason} onChange={event=>update('reason',event.target.value)} required/></label><button className="btn btn-primary" disabled={state.phase==='WORKING'} type="submit">{state.phase==='WORKING'?'Applying…':`Apply ${creditLabel}`}</button></form>}{state.message&&<p role="alert" className="muted">{state.message}</p>}</section>;
}

function AuthoritativeRefundForm({config,credits,onCompleted}){
  const [form,setForm]=useState({creditId:'',number:'',date:new Date().toISOString().slice(0,10),amount:'',reason:''});
  const [state,setState]=useState({phase:'IDLE',message:''});
  const keyRef=useRef(null),availableCredits=postedAdjustments(credits,'AR_CREDIT_MEMO');
  const update=(key,value)=>setForm(current=>({...current,[key]:value}));
  const submit=async event=>{event.preventDefault();const credit=availableCredits.find(item=>item.business_adjustment_id===form.creditId),amount=Number(form.amount);if(!credit||!form.number.trim()||!form.date||!Number.isFinite(amount)||amount<=0||!form.reason.trim()){setState({phase:'FAILED',message:'Select a posted credit memo and provide refund number, date, amount, and reason.'});return;}const idempotencyKey=keyRef.current||commandKey('UI-AR-REFUND');if(!idempotencyKey){setState({phase:'FAILED',message:'Browser cryptography is required for an idempotent refund command.'});return;}keyRef.current=idempotencyKey;setState({phase:'WORKING',message:'Creating the authoritative refund Draft…'});const result=await createAuthoritativeAdjustment({config,kind:'AR_REFUND',idempotencyKey,adjustment:{sourceAdjustmentId:credit.business_adjustment_id,number:form.number.trim(),date:form.date,amount,reason:form.reason.trim()}});if(!result.ok){setState({phase:'FAILED',message:result.message||'Authoritative refund was rejected.'});return;}keyRef.current=null;setForm({creditId:'',number:'',date:new Date().toISOString().slice(0,10),amount:'',reason:''});const refreshed=await onCompleted();setState(refreshed?.ok===false?{phase:'FAILED',message:'Refund Draft was accepted. Refresh is required to confirm the authoritative state.'}:{phase:'SUCCEEDED',message:'Refund Draft was created. Advance its linked journal through the authoritative workflow.'});};
  return <section aria-label="Customer refund" style={{marginTop:20}}><h2 style={{fontSize:18,margin:'0 0 8px'}}>Customer refund</h2><p className="muted sm">A refund is created as a server Draft. It cannot post until its linked journal completes the authoritative workflow.</p>{!availableCredits.length?<p className="muted">No POSTED authoritative credit memo is available for refund.</p>:<form onSubmit={submit} className="two-col" style={{alignItems:'end'}}><label>Credit memo<select value={form.creditId} onChange={event=>update('creditId',event.target.value)} required><option value="">Select…</option>{availableCredits.map(credit=><option key={credit.business_adjustment_id} value={credit.business_adjustment_id}>{credit.adjustment_number||credit.business_adjustment_id} — {authoritativeMoney(credit.amount,credit.currency)}</option>)}</select></label><label>Refund number<input value={form.number} onChange={event=>update('number',event.target.value)} required/></label><label>Refund date<input type="date" value={form.date} onChange={event=>update('date',event.target.value)} required/></label><label>Amount<input type="number" min="0.0001" step="0.0001" value={form.amount} onChange={event=>update('amount',event.target.value)} required/></label><label>Reason<input value={form.reason} onChange={event=>update('reason',event.target.value)} required/></label><button className="btn btn-primary" disabled={state.phase==='WORKING'} type="submit">{state.phase==='WORKING'?'Creating…':'Create refund Draft'}</button></form>}{state.message&&<p role="alert" className="muted">{state.message}</p>}</section>;
}

function AuthoritativeWorkflowWorkspace({config}){
  const [state,setState]=useState({phase:'LOADING',bills:[],invoices:[],apAdjustments:[],arAdjustments:[],journals:[],message:''});
  const [workingJournalIds,setWorkingJournalIds]=useState(()=>new Set());
  const [notice,setNotice]=useState('');
  const refresh=async()=>{setState(current=>({...current,phase:'LOADING',message:''}));const [result,journalResult]=await Promise.all([refreshAuthoritativeDocuments({config}),refreshAuthoritativeJournalEntries({config})]);if(!result.ok||!journalResult.ok){setState({phase:'FAILED',bills:[],invoices:[],apAdjustments:[],arAdjustments:[],journals:[],message:result.message||journalResult.message});return result.ok?journalResult:result;}setState({phase:'READY',bills:result.ap.bills,invoices:result.ar.invoices,apAdjustments:result.ap.adjustments,arAdjustments:result.ar.adjustments,journals:journalResult.journals,message:''});return result;};
  useEffect(()=>{refresh();},[]);
  ...
  return <main className="login-shell"><section className="login-card" aria-live="polite" style={{maxWidth:1180,width:'calc(100% - 32px)'}}><h1>REFS authoritative workspace</h1><p>Authenticated data is read directly from the accounting API. Browser-local accounting data is disabled.</p><button className="btn btn-primary" disabled={state.phase==='LOADING'} onClick={refresh}>Refresh authoritative records</button>{notice&&<p role="alert" className="muted">{notice}</p>}{state.phase==='LOADING'&&<p className="muted">Loading authoritative accounting records…</p>}{state.phase==='FAILED'&&<p role="alert" className="muted">{state.message||'Authoritative accounting refresh failed.'}</p>}{state.phase==='READY'&&<><AuthoritativeJournalEntryTable journals={state.journals} onWorkflow={transition} workingJournalIds={workingJournalIds}/><AuthoritativeWorkflowTable title="Authoritative AP bills" documents={state.bills} kind="AP" onWorkflow={transition} workingJournalIds={workingJournalIds}/><AuthoritativeWorkflowAdjustmentTable title="Authoritative AP adjustments" adjustments={state.apAdjustments} onWorkflow={transition} workingJournalIds={workingJournalIds}/><AuthoritativeCreditApplicationForm config={config} kind="AP_VENDOR_CREDIT" credits={state.apAdjustments} documents={state.bills} onCompleted={refresh}/><AuthoritativeWorkflowTable title="Authoritative AR invoices" documents={state.invoices} kind="AR" onWorkflow={transition} workingJournalIds={workingJournalIds}/><AuthoritativeWorkflowAdjustmentTable title="Authoritative AR adjustments" adjustments={state.arAdjustments} onWorkflow={transition} workingJournalIds={workingJournalIds}/><AuthoritativeCreditApplicationForm config={config} kind="AR_CREDIT_MEMO" credits={state.arAdjustments} documents={state.invoices} onCompleted={refresh}/><AuthoritativeRefundForm config={config} credits={state.arAdjustments} onCompleted={refresh}/><AuthoritativeDraftForm config={config} onCreated={refresh}/><p className="muted" style={{marginTop:20}}>Workflow actions are sent with the authoritative revision. The server enforces permission and separation of duties; this browser never infers either.</p></>}</section></main>;
}
...
export { App, AuthoritativeAdjustmentSummary, AuthoritativeCreditApplicationForm, AuthoritativeDocumentTable, AuthoritativeDraftForm, AuthoritativeJournalEntryTable, AuthoritativeRefundForm, AuthoritativeWorkflowAdjustmentTable, AuthoritativeWorkflowTable, nextAuthoritativeWorkflowAction };
