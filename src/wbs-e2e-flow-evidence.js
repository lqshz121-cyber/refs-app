...
export function buildWbsEndToEndFlowEvidence(snapshot = createWbsMockDataset()) {
  ...
  const flows = [
    ...
    flow({
      ...
      source: { id: 'DOC-PROPERTY-TAX-REVIEW', document_type: 'PROPERTY_TAX_STATEMENT' },
      event: { event_id: 'EVT-PROPERTY-TAX-REVIEW', event_type: 'property_expense', rule_id: 'PROPERTY_TAX_ACCRUAL_REVIEW', reason: 'Property tax support is expected in the WBS connector contract.' },
      finding: null,
      suggestedJe: null,
      postedJe: null,
      glLines: [],
      reportRows: [],
      auditTrail: [{ action: 'contract-ready', actor: 'SYSTEM_RULE_ENGINE' }],
      status: 'CONTRACT_READY_NO_SOURCE',
      blocker: true,
      nextAction: 'Wait for WBS source record, then classify as accrual or prepaid.',
    }),
    flow({
