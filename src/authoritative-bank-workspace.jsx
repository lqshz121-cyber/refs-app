import {PaymentBankMatch} from './payment-bank-match.jsx';
import React, {useEffect,useRef,useState} from 'react';
import {BankSalesReceiptDetail} from './bank-sales-receipt-detail.jsx';
import {SalesReceiptBankMatch} from './sales-receipt-bank-match.jsx';
import {createAuthoritativeReconciliationAdjustmentDraft,readAuthoritativeAdmittedBankStatement,readAuthoritativeReconciliationAdjustmentAttachmentCandidates,refreshAuthoritativeAdmittedBankStatements,refreshAuthoritativeBankTransactions,refreshAuthoritativeReconciliation,refreshAuthoritativeReconciliationScopes,refreshAuthoritativeReconciliationWorksheet,setAuthoritativeReconciliationAdjustmentClearance,setAuthoritativeReconciliationClearance,startAuthoritativeReconciliation,startAuthoritativeReconciliationFromAdmittedStatement,transitionAuthoritativeReconciliation,unmatchAuthoritativeBankPayment} from './accounting-api.js';
...
const BankReadMetadata=({count,readAt,subject})=><AuthoritativeSecondaryDisclosure label="Read evidence">
  <section className="authoritative-bank-read-metadata"><p className="muted sm" aria-label={`${subject} authoritative read diagnostics`}>
...
</AuthoritativeSecondaryDisclosure>;
...
export function AuthoritativeBankMatchReview(props){
  ...
  if(props.row.bank_match_id&&props.row.match_status==='ACTIVE')return <><PaymentBankMatchReview {...props}/>{props.row.match_source_kind==='PAYMENT'&&<PaymentBankMatch key={`${props.config.baseUrl}:${props.config.entityId}:${props.row.bank_source_id}:recovery`} {...props} recoveryOnly/>}{props.row.match_source_kind==='SALES_RECEIPT'&&<SalesReceiptBankMatch key={`${props.config.baseUrl}:${props.config.entityId}:${props.row.bank_source_id}:recovery`} {...props} recoveryOnly/>}</>;
  return <><fieldset disabled={locked}><legend>Match to</legend><label><input type="radio" name="bank-match-kind" checked={kind==='PAYMENT'} onChange={()=>setKind('PAYMENT')}/> Payment</label><label><input type="radio" name="bank-match-kind" checked={kind==='SALES_RECEIPT'} onChange={()=>setKind('SALES_RECEIPT')}/> Sales receipt</label></fieldset>{kind==='SALES_RECEIPT'?<SalesReceiptBankMatch key={`${props.config.baseUrl}:${props.config.entityId}:${props.row.bank_source_id}:${props.row.version}`} {...props} onLockChange={setLocked}/>:<PaymentBankMatchReview {...props} onLockChange={setLocked}/>}</>;
}
function PaymentBankMatchReview({row,config,fetcher,onChanged=()=>{},onLockChange}){
  ...
  if(!row.bank_match_id||row.match_status==='UNMATCHED')return <PaymentBankMatch key={`${config.baseUrl}:${config.entityId}:${row.bank_source_id}:${row.version}:${row.bank_match_id||'none'}:${row.match_status||'none'}`} row={row} config={config} fetcher={fetcher} onChanged={onChanged} onLockChange={onLockChange}/>;
...
export const BankMatchSourceEvidence=({row,config,fetcher})=>{
  if(!row?.match_source_kind)return null;
  const sale=row.match_source_kind==='SALES_RECEIPT',payment=row.match_source_kind==='PAYMENT';
  const label=sale?'Sales receipt':payment?'Payment':'Imported source';
  const id=sale?row.sales_receipt_id:payment?row.payment_occurrence_id:row.business_source_document_id;
  return <details className="bank-match-source-evidence" style={{overflowWrap:'anywhere'}}><summary>{label}{sale?` ${row.sales_receipt_number}`:' details'}</summary><dl><dt>{label} ID</dt><dd>{id}</dd>{sale&&<><dt>Receipt revision</dt><dd>{row.sales_receipt_revision}</dd></>}<dt>Journal entry</dt><dd>{row.journal_entry_id||'Not supplied'}</dd><dt>Journal line</dt><dd>{row.journal_line_id||'Not supplied'}</dd><dt>Ledger line</dt><dd>{row.ledger_line_id||'Not supplied'}</dd></dl>{sale&&config&&<BankSalesReceiptDetail key={`${config.baseUrl}:${config.entityId}:${row.bank_match_id}:${row.sales_receipt_revision}`} config={config} row={row} fetcher={fetcher}/>}</details>;
};
...
export const AuthoritativeBankDetail=({row,scope,onBack,config,fetcher,onMatchChanged})=>{
  ...
  {row.bank_match_id&&<div className="qbo-toolgrid">
    <span><i>Bank match ID</i><b>{row.bank_match_id}</b></span><span><i>{row.match_source_kind==='SALES_RECEIPT'?'Sales receipt':row.match_source_kind==='PAYMENT'?'Payment':'Business source document'}</i><b>{row.sales_receipt_number||row.payment_occurrence_id||row.business_source_document_id||'Not supplied'}</b></span>
    <span><i>Journal line</i><b>{row.journal_line_id||'Unavailable'}</b></span><span><i>Ledger line</i><b>{row.ledger_line_id||'Unavailable from the active-Match read'}</b></span><span><i>Candidate rule</i><b>{row.candidate_rule_code||'Unavailable'}</b></span>
  ...
  <BankMatchSourceEvidence row={row} config={scopeMatches?config:null} fetcher={fetcher}/>
...
export function AuthoritativeReconciliationDetail({row,scope,onBack,config,fetcher,onChanged=()=>{}}){
  ...
  const [adjustmentAttachments,setAdjustmentAttachments]=useState({phase:'IDLE',rows:[],error:null});
  ...
  const selectAdjustmentItem=async item=>{setAdjustment({item,journalNumber:'',journalDate:item.transaction_date,offsetAccountCode:'',description:`Reconciliation adjustment for ${item.external_bank_line_id}`,attachmentIds:[],phase:'READY',error:null});setAdjustmentAttachments({phase:'LOADING',rows:[],error:null});const result=await readAuthoritativeReconciliationAdjustmentAttachmentCandidates({config,fetcher});setAdjustmentAttachments(result.ok?{phase:'READY',rows:result.attachments,error:null}:{phase:'ERROR',rows:[],error:result});};
  const toggleAdjustmentAttachment=id=>setAdjustment(current=>({...current,attachmentIds:current.attachmentIds.includes(id)?current.attachmentIds.filter(value=>value!==id):[...current.attachmentIds,id]}));
  const uploadAdjustmentAttachment=async file=>{if(!file)return;setAdjustment(current=>({...current,phase:'UPLOADING',error:null}));const upload=await uploadVerifiedAttachment({config,file,idempotencyKey:`reconciliation-adjustment-${globalThis.crypto?.randomUUID?.()||Date.now()}`,fetcher});if(!upload.ok){setAdjustment(current=>({...current,phase:'READY',error:upload}));return;}const refreshed=await readAuthoritativeReconciliationAdjustmentAttachmentCandidates({config,fetcher});if(!refreshed.ok){setAdjustment(current=>({...current,phase:'READY',error:refreshed}));return;}setAdjustmentAttachments({phase:'READY',rows:refreshed.attachments,error:null});setAdjustment(current=>({...current,phase:'READY',attachmentIds:[...new Set([...current.attachmentIds,upload.attachmentId])]}));};
  const createAdjustment=async event=>{event.preventDefault();const attachmentIds=adjustment.attachmentIds;setAdjustment(current=>({...current,phase:'COMMANDING',error:null}));const result=await createAuthoritativeReconciliationAdjustmentDraft({config,reconciliationId:row.reconciliation_id,reconciliationRevision:row.version,row:adjustment.item,journalNumber:adjustment.journalNumber,journalDate:adjustment.journalDate,offsetAccountCode:adjustment.offsetAccountCode,description:adjustment.description,attachmentIds,reason,fetcher});if(result.ok){onChanged();await loadWorksheet();return;}setAdjustment(current=>({...current,phase:'READY',error:result}));};
  ...
  <section className="card" aria-label="Reconciliation worksheet">
    ...
    {(worksheet.phase==='READY'||worksheet.phase==='COMMANDING')&&<>{!hasAuthorizedWorksheetEvidence?<StateBlock tone="blocked" title="Reconciliation controls blocked">BLOCKED — The accounting API returned no authorized worksheet evidence for this statement. Clear, unclear, adjustment Draft, review, sign-off, and reopen controls are unavailable until the exact retained worksheet is returned.</StateBlock>:<><form className="filterbar" onSubmit={event=>event.preventDefault()}><label>Controller reason<input required minLength={8} maxLength={2000} value={reason} onChange={event=>setReason(event.target.value)}/></label></form><div className="table-wrap authoritative-bank-evidence-table" role="region" tabIndex={0} aria-label="Reconciliation worksheet; scroll horizontally to view every column"><table className="tbl"><thead><tr><th>Date</th><th>Bank evidence</th><th>Amount</th><th>Match</th><th>Clearance</th><th>Action</th></tr></thead><tbody>{worksheet.rows.map(item=><tr key={item.bank_source_id}><td>{item.transaction_date}</td><td><b>{item.external_bank_line_id}</b><div className="muted sm">{item.bank_source_id}</div></td><td className="num">{money(item.amount)} {item.currency}</td><td>{item.match_status==='ACTIVE'?<><b>ACTIVE</b><div className="muted sm">JE {item.journal_entry_id}</div><BankMatchSourceEvidence row={item} config={config} fetcher={fetcher}/></>:'No exact active Match'}</td><td>{item.clearance_state}</td><td>{item.clearance_state==='CLEARED'&&item.match_status==='ACTIVE'?<button type="button" className="btn btn-sm" disabled={commandInFlight||!reasonReady||!canChangeItems} onClick={()=>setClearance(item,false)}>Unclear matched item</button>:item.match_status==='ACTIVE'?<button type="button" className="btn btn-sm" disabled={commandInFlight||!reasonReady||!canChangeItems} onClick={()=>setClearance(item,true)}>Clear matched item</button>:<div className="button-row"><button type="button" className="btn btn-sm" disabled={commandInFlight||!reasonReady||!canChangeItems} onClick={()=>selectAdjustmentItem(item)}>Prepare adjustment Draft</button><p className="muted sm">Posted adjustment clearance is BLOCKED until the API returns separate posted adjustment evidence for this bank source.</p></div>}</td></tr>)}</tbody></table></div></>}{worksheet.error&&<ReadError error={worksheet.error} onRetry={loadWorksheet}/>}</>}
  </section>
  {adjustment.item&&<section className="card" aria-label="Create reconciliation adjustment Draft"><div className="card-head"><div><h2>Create controlled adjustment Draft</h2><p className="muted sm">This request binds the selected bank source, current statement revision, configured cash account, exact four-decimal source amount, a supplied offset account, and server-verified attachment evidence. It cannot approve or post a Journal Entry.</p></div><span className="badge badge-muted">DRAFT ONLY</span></div><p className="muted sm">Selected bank source {adjustment.item.external_bank_line_id} · {money(adjustment.item.amount)} {adjustment.item.currency} · {adjustment.item.bank_source_id}</p><form className="filterbar" onSubmit={createAdjustment}><label>Journal number<input required maxLength={128} value={adjustment.journalNumber} onChange={event=>setAdjustment(current=>({...current,journalNumber:event.target.value}))}/></label><label>Journal date<input required type="date" value={adjustment.journalDate} onChange={event=>setAdjustment(current=>({...current,journalDate:event.target.value}))}/></label><label>Offset account code<input required maxLength={64} value={adjustment.offsetAccountCode} onChange={event=>setAdjustment(current=>({...current,offsetAccountCode:event.target.value}))}/></label><label>Upload supporting document<input type="file" accept=".pdf,.png,.jpg,.jpeg,.csv" disabled={commandInFlight||adjustment.phase==='UPLOADING'} onChange={event=>uploadAdjustmentAttachment(event.target.files?.[0]||null)}/></label><label>Description<input maxLength={2000} value={adjustment.description} onChange={event=>setAdjustment(current=>({...current,description:event.target.value}))}/></label><button type="submit" className="btn btn-primary" disabled={commandInFlight||!reasonReady||!adjustment.attachmentIds.length||adjustment.phase==='COMMANDING'||adjustment.phase==='UPLOADING'}>Create adjustment Draft</button><button type="button" className="btn btn-sm" disabled={commandInFlight||adjustment.phase==='COMMANDING'||adjustment.phase==='UPLOADING'} onClick={()=>setAdjustment(current=>({...current,item:null,error:null}))}>Cancel</button></form><div className="authoritative-adjustment-attachments" aria-label="Verified attachment selection"><p className="muted sm">Choose one or more verified documents for this company. New uploads are scanned before they appear here.</p>{adjustmentAttachments.phase==='LOADING'&&<p className="muted sm">Loading verified attachments…</p>}{adjustmentAttachments.phase==='READY'&&<>{adjustmentAttachments.rows.length===0?<p className="muted sm">No verified attachments are available. Upload a supporting document.</p>:adjustmentAttachments.rows.map(file=><label key={file.attachment_id}><input type="checkbox" checked={adjustment.attachmentIds.includes(file.attachment_id)} onChange={()=>toggleAdjustmentAttachment(file.attachment_id)}/>{file.name} · {file.media_type} · verified {file.verified_at}</label>)}</>}{adjustmentAttachments.phase==='ERROR'&&<ReadError error={adjustmentAttachments.error} onRetry={()=>selectAdjustmentItem(adjustment.item)}/>}</div>{!reasonReady&&<p className="muted sm">Enter the controller reason above before creating a Draft.</p>}{adjustment.error&&<ReadError error={adjustment.error} onRetry={()=>{}}/>}</section>}
...
export function AuthoritativeReconciliationWorkspace({config,fetcher=globalThis.fetch,environment=globalThis,onBack=null,initialScope=null}){
  ...
  const [selected,setSelected]=useState(null);
  const [startForm,setStartForm]=useState({bankAccountRef:initialScope?.bankAccountRef||'',statementEndingDate:initialScope?.statementEndingDate||'',statementOpeningBalance:'',statementEndingBalance:'',reason:''});
  ...
  const handleAdmittedStarted=async row=>{const nextScope={bankAccountRef:row.bank_account_ref,statementEndingDate:row.statement_end_date};setScope(nextScope);setStartForm(current=>({...current,bankAccountRef:nextScope.bankAccountRef,statementEndingDate:nextScope.statementEndingDate}));await readSummary(nextScope,false);};
  const startReconciliation=async event=>{
    event.preventDefault();
    setStartState({phase:'COMMANDING',error:null});
    const result=await startAuthoritativeReconciliation({config,...startForm,fetcher});
    if(!result.ok){setStartState({phase:'ERROR',error:result});return;}
    const nextScope={bankAccountRef:startForm.bankAccountRef.trim(),statementEndingDate:startForm.statementEndingDate};
    setScope(nextScope);setStartState({phase:'READY',error:null});
    await Promise.all([loadScopes(),readSummary(nextScope,false)]);
  };
  const openEvidence=(row,focusId)=>{
  ...
  return <AuthoritativeWorkspaceView area="Reconcile" className="stack authoritative-reconciliation-workspace">{onBack&&<div className="qbo-report-back authoritative-reconciliation-catalog-back"><button type="button" className="btn btn-sm btn-ghost" onClick={onBack}>Back to standard reports</button></div>}<AuthoritativeWorkspaceHeader eyebrow="BANKING" title="Reconcile" description="Match the books to the bank records."/>
    <section className="report-workbench authoritative-reconciliation-scope-picker" aria-label="Reconciliation history"><div className="report-workbench-head"><div><b>Reconciliation history</b><div className="page-subtitle">Choose an existing statement to review.</div></div><span className="badge badge-muted">READ ONLY</span></div>
      ...
      {scopeDiscovery.phase==='READY'&&!scopeDiscovery.rows.length&&<p className="page-subtitle authoritative-reconciliation-history-empty" role="status"><b>No reconciliation history.</b> Enter a bank account and statement ending date below.</p>}
    ...
    <section className="card" aria-label="Start reconciliation">
      <div className="card-head"><div><h2>Start reconciliation</h2><p className="muted sm">Create a Draft reconciliation from the actual bank statement. The server validates the account, cutoff, balances, authority, idempotency, and accounting period before any worksheet can be opened.</p></div><span className="badge badge-muted">DRAFT ONLY</span></div>
      <form className="filterbar" onSubmit={startReconciliation}>
        <label>Bank account<input required maxLength={128} value={startForm.bankAccountRef} onChange={event=>setStartForm(current=>({...current,bankAccountRef:event.target.value}))}/></label>
        <label>Statement ending date<input required type="date" value={startForm.statementEndingDate} onChange={event=>setStartForm(current=>({...current,statementEndingDate:event.target.value}))}/></label>
        <label>Opening balance<input required inputMode="decimal" pattern="-?[0-9]{1,16}\.[0-9]{4}" placeholder="0.0000" value={startForm.statementOpeningBalance} onChange={event=>setStartForm(current=>({...current,statementOpeningBalance:event.target.value}))}/></label>
        <label>Ending balance<input required inputMode="decimal" pattern="-?[0-9]{1,16}\.[0-9]{4}" placeholder="0.0000" value={startForm.statementEndingBalance} onChange={event=>setStartForm(current=>({...current,statementEndingBalance:event.target.value}))}/></label>
        <label>Controller reason<input required minLength={8} maxLength={2000} value={startForm.reason} onChange={event=>setStartForm(current=>({...current,reason:event.target.value}))}/></label>
        <button type="submit" className="btn btn-primary" disabled={startState.phase==='COMMANDING'}>Start Draft reconciliation</button>
      </form>
      {startState.phase==='ERROR'&&<BankReadFailure error={startState.error} onRetry={()=>{}} subject="reconciliation start"/>}
      {startState.phase==='READY'&&<StateBlock tone="success">Draft reconciliation created. The exact statement is loaded below for review.</StateBlock>}
    </section>
    ...
    {state.phase==='IDLE'&&!(scopeDiscovery.phase==='READY'&&!scopeDiscovery.rows.length)&&<StateBlock tone="empty" title="Choose a statement">Select a bank account and statement ending date.</StateBlock>}
