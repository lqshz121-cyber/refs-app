...
export function AuthoritativeReportsWorkspace({config,fetcher=globalThis.fetch,environment=globalThis,initialCatalog=DEFAULT_AUTHORITATIVE_REPORTS_CATALOG,onOpenArAging=()=>{},workspaceTitle='Reports center',workspaceEyebrow='AUTHORITATIVE · REPORTING',workspaceDescription='OIDC-authenticated, entity-and-period-scoped POSTED ledger evidence. Every displayed report reads the accounting API; no browser data is used.',initialDimensionType='PROPERTY'}){
  ...
  return <AuthoritativeDemoView area={workspaceTitle} className="stack reports-library authoritative-reports-library authoritative-workbench-shell"><AuthoritativeDemoWorkspaceHeader className="reports-head" eyebrow={workspaceEyebrow} title={workspaceTitle} description={workspaceDescription} scope={<div className="report-period-chip"><span>Reporting scope</span><b>Entity {config.entityId} · Period {config.periodId}</b><small>POSTED ledger evidence</small></div>}/>
