...
export function AuthoritativeReportsWorkspace({config,fetcher=globalThis.fetch,environment=globalThis,initialCatalog=DEFAULT_AUTHORITATIVE_REPORTS_CATALOG,onOpenArAging=()=>{},workspaceTitle='Reports center',workspaceEyebrow='FINANCIAL REPORTING',workspaceDescription='Review posted ledger evidence for the selected company and period. Browser data is never used for accounting figures.',initialDimensionType='PROPERTY'}){
  ...
  return <AuthoritativeDemoReportsView className="reports-library authoritative-reports-library" eyebrow={workspaceEyebrow} title={workspaceTitle} description={workspaceDescription} scope={<div className="report-period-chip"><span>Reporting scope</span><b title={`Entity ID: ${config.entityId}; Period ID: ${config.periodId}`}>Entity {entityLabel} · Period {periodLabel}</b><small>POSTED ledger evidence</small></div>}>
    <div className="authoritative-workbench-rail" aria-label="Reports workspace structure"><span><b>1</b> Find</span><span><b>2</b> Review</span><span><b>3</b> Open full report</span></div>
    <section className="card authoritative-report-finder" aria-label="Reports catalog"><div className="card-head"><div><h2>Find a report</h2><p className="muted sm">Browse report families or use the finder. Back returns to the same category, search, and statement preview.</p></div><span className="badge badge-muted">VIEW ONLY</span></div><label>Find a report<input value={catalogSearch} maxLength="120" onChange={event=>setCatalogSearch(event.target.value)} placeholder="e.g. balance sheet, profit and loss, cash, budget"/></label>{catalogSearch.trim()&&<p className="muted sm">{matchingWorkbenchTabs.length?matchingWorkbenchTabs.map(([,label])=>label).join(' ? '):matchingShortcuts.length?`Matching statement: ${matchingShortcuts.map(([,label])=>label).join(', ')}.`:'No report category matches this search.'}</p>}</section>
    {!!catalogSearch.trim()&&!!matchingShortcuts.length&&<section className="authoritative-report-shortcuts authoritative-report-finder-results" aria-label="Matching statements"><div><span className="page-eyebrow">MATCHING STATEMENTS</span><p className="muted sm">These links open an available statement for the selected company and period.</p></div><div className="authoritative-report-shortcut-grid">{matchingShortcuts.map(([key,label,description])=><button key={key} type="button" className={`authoritative-report-shortcut ${report===key&&workbenchTab==='STATEMENTS'?'is-current':''}`} aria-pressed={report===key&&workbenchTab==='STATEMENTS'} onClick={()=>{setWorkbenchTab('STATEMENTS');setReport(key);setSelected(null);}}><span>{label}</span><small>{description}</small></button>)}</div></section>}
    <section className="authoritative-report-shortcuts authoritative-property-report-directory" aria-label="Property and project report directory"><div><span className="page-eyebrow">PROPERTY &amp; PROJECT REPORTS</span><p className="muted sm">Open a report for a selected Property, Project, or Unit. The report uses the exact selected scope.</p></div><div className="authoritative-report-shortcut-grid">{PROPERTY_REPORT_SHORTCUTS.filter(shortcut=>!catalogSearch.trim()||matchingPropertyShortcuts.includes(shortcut)).map(shortcut=>{const [key,label,description,nextTab,nextDimensionType]=shortcut;const active=workbenchTab===nextTab&&(!nextDimensionType||dimensionType===nextDimensionType);return <button key={key} type="button" className={`authoritative-report-shortcut ${active?'is-current':''}`} aria-pressed={active} onClick={()=>openPropertyReport(shortcut)}><span>{label}</span><small>{description}</small></button>;})}</div></section>
    <div className="rep-grid" role="tablist" aria-label="Reports workbench">{(catalogSearch.trim()?matchingWorkbenchTabs:REPORT_WORKBENCH_TABS).map(([key,label,description])=><button type="button" role="tab" aria-selected={workbenchTab===key} className={`rep-card ${workbenchTab===key?'rep-on':''}`} key={key} onClick={()=>{setWorkbenchTab(key);setSelected(null);}}><span className="rep-main"><span className="rep-name">{label}</span><span className="rep-desc">{description}</span></span><span className="rep-tag"><span>VIEW</span><span aria-hidden="true">›</span></span></button>)}</div>
    {!matchingWorkbenchTabs.length&&catalogSearch.trim()&&<StateBlock tone="empty" title="No report category matches">Clear the report finder to view all report families. No report data has been inferred or loaded from a browser cache.</StateBlock>}
    <section className="report-workbench" aria-label={`${REPORT_WORKBENCH_TABS.find(([key])=>key===workbenchTab)?.[1]} report workspace`}><div className="report-workbench-head"><div><b>{REPORT_WORKBENCH_TABS.find(([key])=>key===workbenchTab)?.[1]}</b><div className="page-subtitle">{REPORT_WORKBENCH_TABS.find(([key])=>key===workbenchTab)?.[2]} Figures are based on retained posted ledger evidence for this scope.</div></div>{workbenchTab==='STATEMENTS'?<button type="button" className="btn btn-sm btn-ghost" disabled={state.phase==='LOADING'} onClick={load}>Refresh statement evidence</button>:<span className="badge badge-muted">VIEW ONLY</span>}</div>
