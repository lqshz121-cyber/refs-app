...
export function AuthoritativeReportsWorkspace({config,fetcher=globalThis.fetch,environment=globalThis,initialCatalog=DEFAULT_AUTHORITATIVE_REPORTS_CATALOG,onOpenArAging=()=>{},workspaceTitle='Reports center',workspaceEyebrow='AUTHORITATIVE · REPORTING',workspaceDescription='OIDC-authenticated, entity-and-period-scoped POSTED ledger evidence. Every displayed report reads the accounting API; no browser data is used.',initialDimensionType='PROPERTY'}){
  ...
  return <div className="stack reports-library authoritative-reports-library authoritative-workbench-shell"><header className="accounting-page-head reports-head"><div><div className="page-eyebrow">{workspaceEyebrow}</div><h1 className="page-h">{workspaceTitle}</h1><p className="page-subtitle">{workspaceDescription}</p></div><div className="report-period-chip"><span>Reporting scope</span><b>Entity {config.entityId} · Period {config.periodId}</b><small>POSTED ledger evidence</small></div></header>
