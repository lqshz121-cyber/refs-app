...
import {refreshAuthoritativeBudgetVsActual,refreshAuthoritativeCashFlowClassification,refreshAuthoritativeConsolidation,refreshAuthoritativeConstructionLoanRollforward,refreshAuthoritativeCwipRollforward,refreshAuthoritativeDimensionProfitability,refreshAuthoritativeFinancialStatementPeriodComparison,refreshAuthoritativeFinancialStatements,refreshAuthoritativeIntercompanyReconciliation,refreshAuthoritativePrepaidRollforward} from './accounting-api.js';
...
const REPORT_WORKBENCH_TABS=Object.freeze([
  ...
  ['OPERATING_ANALYSIS','Operating analysis','Property, project, or unit profitability and budget-versus-actual.'],
...
const ConsolidationDetail=({row,returnContext,onBack})=><section className="full-bleed qbo-transaction-report" aria-label="Consolidation evidence">
  <div className="qbo-report-back"><button type="button" className="btn btn-sm btn-ghost" onClick={onBack}>Back to consolidation</button><span>Entity {returnContext?.entityId} | Period {returnContext?.periodId}</span></div>
...
const RollforwardDetail=({title,row,returnContext,onBack})=>{
  ...
  <div className="qbo-report-back"><button type="button" className="btn btn-sm btn-ghost" onClick={onBack}>Back to {title.toLowerCase()}</button><span>Entity {returnContext?.entityId} | Period {returnContext?.periodId}</span></div>
...
const CwipRollforwardDetail=({row,returnContext,onBack})=><section className="full-bleed qbo-transaction-report authoritative-cwip-detail" aria-label="CWIP rollforward account evidence">
  <div className="qbo-report-back"><button type="button" className="btn btn-sm btn-ghost" onClick={onBack}>Back to CWIP rollforward</button><span>Entity {returnContext?.entityId} | Period {returnContext?.periodId}</span></div>
...
const ComparisonDetail=({row,returnContext,onBack})=><section className="full-bleed qbo-transaction-report" aria-label="Prior-period comparison evidence">
  <div className="qbo-report-back"><button type="button" className="btn btn-sm btn-ghost" onClick={onBack}>Back to prior-period comparison</button><span>Entity {returnContext?.entityId} | Period {returnContext?.periodId}</span></div>
...
export function AuthoritativeReportsWorkspace({config,fetcher=globalThis.fetch,environment=globalThis,initialCatalog=DEFAULT_AUTHORITATIVE_REPORTS_CATALOG,onOpenArAging=()=>{}}){
  ...
  return <div className="stack reports-library authoritative-reports-library"><header className="accounting-page-head reports-head"><div><div className="page-eyebrow">AUTHORITATIVE · REPORTING</div><h1 className="page-h">Reports center</h1><p className="page-subtitle">OIDC-authenticated, entity-and-period-scoped POSTED ledger evidence. Every displayed report reads the accounting API; no browser data is used.</p></div><div className="report-period-chip"><span>Reporting scope</span><b>Entity {config.entityId} · Period {config.periodId}</b><small>POSTED ledger evidence</small></div></header>
