...
export function AuthoritativeReportsWorkspace({config,fetcher=globalThis.fetch,environment=globalThis,initialCatalog=DEFAULT_AUTHORITATIVE_REPORTS_CATALOG,onOpenArAging=()=>{},workspaceTitle='Reports center',workspaceEyebrow='REPORTING',workspaceDescription='Posted reports from the accounting API.',initialDimensionType='PROPERTY',initialDimensionRef=''}){
  ...
  return <AuthoritativeReportsView className="reports-library authoritative-reports-library" eyebrow={workspaceEyebrow} title={workspaceTitle} description={workspaceDescription} scope={<div className="report-period-chip"><span>Reporting scope</span><b title={`Entity ID: ${config.entityId}; Period ID: ${config.periodId}`}>Entity {entityLabel} · Period {periodLabel}</b><small>POSTED ledger evidence</small></div>}>
    ...
    {workbenchTab==='CASH_AND_CAPITAL'&&<section className="card" aria-label="AI amortization schedule proposals">
      ...
      <div className="qbo-filter-grid"><button type="button" className="btn" onClick={loadAmortizationSchedules}>Load proposed schedules</button></div>
    ...
    {workbenchTab==='OPERATING_ANALYSIS'&&<section className="card" aria-label="Dimension profitability evidence">
      ...
      <div className="qbo-filter-grid"><label>Dimension type<select value={dimensionType} onChange={event=>{setDimensionType(event.target.value);setDimensionState({phase:'IDLE',rows:[],error:null,scope:null});}}>{DIMENSION_TYPES.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label><label>Exact reference<input value={dimensionRef} maxLength="160" onChange={event=>setDimensionRef(event.target.value)} placeholder={`e.g. ${dimensionType}-01`}/></label><button type="button" className="btn" disabled={!dimensionRef.trim()} onClick={loadDimension}>Load profitability evidence</button></div>
