...
const StatementSnapshotDetail=({row,returnContext,onBack,onOpenLineage})=><section className="full-bleed qbo-transaction-report" aria-label="Financial statement snapshot lineage evidence">
  ...
  <div className="card-head"><div><h2>{row.statement_type} / {row.account_code}</h2><p className="muted sm">Immutable snapshot <code>{row.financial_statement_snapshot_id}</code> v{row.version}; row hash <code>{row.row_hash}</code>.</p></div><span className="badge badge-muted">GET ONLY</span></div>
  <div className="detail-grid"><EvidenceIds label="Journal entries" ids={row.journal_entry_ids}/><EvidenceIds label="Journal lines" ids={row.journal_line_ids}/><EvidenceIds label="Ledger lines" ids={row.ledger_line_ids}/><EvidenceIds label="Source documents" ids={row.source_document_ids}/></div>
  <EvidenceDrillAction row={row} onOpenLineage={onOpenLineage}/>
</section>;
...
export const AuthoritativeFullStatementReport=({report,rows,returnContext,onBack,onRefresh,onOpenEvidence,loading=false,tableRef})=>{
  ...
  return <section className="full-bleed qbo-transaction-report authoritative-evidence-page authoritative-full-statement" aria-label={`${title} full report`}>
    ...
    <header className="accounting-page-head"><div><div className="page-eyebrow">POSTED EVIDENCE | API GET</div><h1 className="page-h">{title}</h1><p className="page-subtitle">Read-only statement evidence from the accounting API.</p></div><div className="report-period-chip"><span>Evidence scope</span><b><ScopeLabel context={returnContext}/></b><small>READ ONLY</small></div></header>
    <div className="authoritative-full-statement-actions"><button type="button" className="btn btn-sm btn-ghost" disabled={loading} onClick={onRefresh}>{loading?'Loading…':'Refresh statement evidence'}</button><span className="badge badge-muted">GET ONLY</span></div>
    {!rows.length?<AuthoritativeScopeEmpty subject="POSTED ledger lines" requiresPosted/>:<div ref={tableRef} className={`table-wrap reports-workbench-table authoritative-full-statement-table ${report==='TRIAL_BALANCE'?'trial-balance-table':''}`} role="region" tabIndex={0} aria-label={`${title} rows; scroll horizontally to view every column`}><table className="tbl"><thead><tr><th>Section</th><th>Account</th><th>Period debit</th><th>Period credit</th><th>Balance</th><th>Evidence</th></tr></thead><tbody>{rows.map((row,index)=>{const focusId=`authoritative-full-report-${row.statement_type}-${row.account_code}`;const beginsSection=index===0||rows[index-1].statement_section!==row.statement_section;return <React.Fragment key={`${row.statement_type}:${row.account_code}`}>{beginsSection&&<tr className="report-section-row"><th colSpan="6" scope="rowgroup">{row.statement_section}</th></tr>}<tr><td>{row.statement_section}</td><td><b>{row.account_code}</b><div className="muted sm">{row.account_name}</div></td><td className="num">{money(row.period_debit)}</td><td className="num">{money(row.period_credit)}</td><td className="num">{money(row.display_balance)}</td><td><button id={focusId} type="button" className="btn btn-sm" onClick={()=>onOpenEvidence(row,focusId)}>Open evidence</button></td></tr></React.Fragment>;})}</tbody></table></div>}
  </section>;
};
...
export function AuthoritativeReportsWorkspace({config,fetcher=globalThis.fetch,environment=globalThis,initialCatalog=DEFAULT_AUTHORITATIVE_REPORTS_CATALOG,onOpenArAging=()=>{},workspaceTitle='Reports center',workspaceEyebrow='REPORTING',workspaceDescription='Posted reports from the accounting API.',initialDimensionType='PROPERTY',initialDimensionRef=''}){
  ...
  const openFullStatement=(focusId='authoritative-open-full-statement',nextReport=report)=>{
    ...
    if(base)setSelected({kind:'FULL_STATEMENT',rows:state.rows.filter(row=>row.statement_type===nextReport),returnContext:{...base,report:nextReport,workbenchTab,reportsCatalog:normalizeAuthoritativeReportsCatalog({category:workbenchTab,query:catalogSearch,preview:nextReport})}});
  };
  const openEvidenceFromFullStatement=(row,focusId)=>{
    const parent=selected;
    if(!parent||parent.kind!=='FULL_STATEMENT')return;
    const base=createAuthoritativeReturnContext({config,view:DEFAULT_AUTHORITATIVE_LIST_VIEW,focusId,scrollY:Number(environment?.scrollY)||0});
    if(base)setSelected({kind:'STATEMENT',row,returnContext:{...parent.returnContext,...base,reportAccountCode:row.account_code||null,reportSection:row.statement_section||null,reportDimensionType:row.dimension_type||null,reportDimensionRef:row.dimension_ref||null,tableX:Number(fullStatementTableRef.current?.scrollLeft)||0,parentFullStatement:parent}});
  };
  const closeEvidence=()=>{
  ...
  return <AuthoritativeReportsView className="reports-library authoritative-reports-library" eyebrow={workspaceEyebrow} title={workspaceTitle} description={workspaceDescription} scope={<div className="report-period-chip"><span>Reporting scope</span><b title={`Entity ID: ${config.entityId}; Period ID: ${config.periodId}`}>Entity {entityLabel} · Period {periodLabel}</b><small>POSTED ledger evidence</small></div>}>
    <section className="authoritative-report-finder" aria-label="Authoritative reports catalog"><label><span>Search reports</span><input value={catalogSearch} maxLength="120" onChange={event=>setCatalogSearch(event.target.value)} placeholder="Balance sheet, profit and loss, cash…"/></label>{catalogSearch.trim()&&<span className="authoritative-report-match-summary">{totalCatalogMatches?`${totalCatalogMatches} ${totalCatalogMatches===1?'match':'matches'}`:'No matching report'}</span>}</section>
    {!!catalogSearch.trim()&&!!matchingShortcuts.length&&<section className="authoritative-report-shortcuts authoritative-report-finder-results" aria-label="Matching API-backed statements"><div><span className="page-eyebrow">MATCHING STATEMENTS</span></div><div className="authoritative-report-shortcut-grid">{matchingShortcuts.map(([key,label,description])=><button key={key} type="button" className={`authoritative-report-shortcut ${report===key&&workbenchTab==='STATEMENTS'?'is-current':''}`} disabled={state.phase==='LOADING'} onClick={()=>openFullStatement(`authoritative-search-result-${key}`,key)}><span>{label}</span><small>{description}</small></button>)}</div></section>}
    <details className="authoritative-secondary-disclosure authoritative-property-report-directory"><summary><span>Property &amp; project reports</span><span className="badge badge-muted">{PROPERTY_REPORT_SHORTCUTS.length}</span></summary><section className="authoritative-report-shortcuts" aria-label="Property, project, unit, and lot report directory"><div className="authoritative-report-shortcut-grid">{PROPERTY_REPORT_SHORTCUTS.filter(shortcut=>!catalogSearch.trim()||matchingPropertyShortcuts.includes(shortcut)).map(shortcut=>{const [key,label,description,nextTab,nextDimensionType]=shortcut;const active=workbenchTab===nextTab&&(!nextDimensionType||dimensionType===nextDimensionType);return <button key={key} type="button" className={`authoritative-report-shortcut ${active?'is-current':''}`} aria-pressed={active} onClick={()=>openPropertyReport(shortcut)}><span>{label}</span><small>{description}</small></button>;})}</div></section></details>
    <div className="rep-grid" role="tablist" aria-label="Report categories">{(catalogSearch.trim()?matchingWorkbenchTabs:REPORT_WORKBENCH_TABS).map(([key,label])=><button type="button" role="tab" aria-selected={workbenchTab===key} className={`rep-card ${workbenchTab===key?'rep-on':''}`} key={key} onClick={()=>{setWorkbenchTab(key);setSelected(null);}}><span className="rep-name">{label}</span></button>)}</div>
    {!totalCatalogMatches&&catalogSearch.trim()&&<StateBlock tone="empty" title="No API-backed report category matches">Clear the report finder to view all report families. No report data has been inferred or loaded from a browser cache.</StateBlock>}
    <section className="report-workbench" aria-label={`${REPORT_WORKBENCH_TABS.find(([key])=>key===workbenchTab)?.[1]} report workspace`}><div className="report-workbench-head"><b>{REPORT_WORKBENCH_TABS.find(([key])=>key===workbenchTab)?.[1]}</b>{workbenchTab==='STATEMENTS'&&<button type="button" className="btn btn-sm btn-ghost" disabled={state.phase==='LOADING'} onClick={load}>{state.phase==='LOADING'?'Loading…':'Refresh'}</button>}</div>
    {workbenchTab==='STATEMENTS'&&<>
    <section className="authoritative-report-shortcuts authoritative-core-report-shortcuts" aria-label="Core statement shortcuts"><div><span className="page-eyebrow">FAVORITES</span></div><div className="authoritative-report-shortcut-grid">{REPORT_LIBRARY_SHORTCUTS.map(([key,label,description])=>{const focusId=`authoritative-favorite-${key}`;return <button id={focusId} key={key} type="button" className="authoritative-report-shortcut" disabled={state.phase==='LOADING'} onClick={()=>openFullStatement(focusId,key)}><span>{label}</span><small>{description}</small></button>;})}<button id="authoritative-report-ar-aging" type="button" className="authoritative-report-shortcut" onClick={()=>onOpenArAging('authoritative-report-ar-aging',normalizeAuthoritativeReportsCatalog({category:workbenchTab,query:catalogSearch,preview:report}))}><span>Accounts receivable aging</span><small>Review open receivables by age.</small></button></div></section>
    <details className="authoritative-secondary-disclosure authoritative-statement-snapshot"><summary><span>Statement snapshot</span><span className="badge badge-muted">GET ONLY</span></summary><section className="card" aria-label="Financial statement snapshot version evidence"><div className="card-head"><div><h2>Approved version</h2><p className="muted sm">Immutable statement evidence with retained GL, Journal, and source identifiers.</p></div></div><div className="qbo-filter-grid"><button type="button" className="btn" disabled={statementSnapshotState.phase==='LOADING'} onClick={loadStatementSnapshot}>{statementSnapshotState.phase==='LOADING'?'Loading…':'Load statement snapshot'}</button></div>
    ...
    {workbenchTab==='GROUP_AND_COMPARISON'&&<section className="card" aria-label="Prior-period comparison evidence">
      ...
      <div className="qbo-filter-grid"><label>Prior period ID<input value={priorPeriodId} maxLength="36" onChange={event=>{setPriorPeriodId(event.target.value.trim());setComparisonState({phase:'IDLE',rows:[],error:null,scope:null});}} placeholder="UUID of an earlier period"/></label><button type="button" className="btn" disabled={comparisonState.phase==='LOADING'||!priorPeriodId||priorPeriodId===config.periodId} onClick={loadComparison}>{comparisonState.phase==='LOADING'?'Loading…':'Load comparison evidence'}</button></div>
    ...
    {workbenchTab==='CASH_AND_CAPITAL'&&<section className="card" aria-label="CWIP rollforward evidence">
      ...
      <div className="qbo-filter-grid"><button type="button" className="btn" disabled={cwipState.phase==='LOADING'} onClick={loadCwip}>{cwipState.phase==='LOADING'?'Loading…':'Load CWIP rollforward evidence'}</button></div>
    ...
    {workbenchTab==='CASH_AND_CAPITAL'&&<section className="card" aria-label="Construction loan rollforward evidence">
      ...
      <div className="qbo-filter-grid"><button type="button" className="btn" disabled={constructionLoanState.phase==='LOADING'} onClick={loadConstructionLoan}>{constructionLoanState.phase==='LOADING'?'Loading…':'Load construction-loan evidence'}</button></div>
    ...
    {workbenchTab==='CASH_AND_CAPITAL'&&<section className="card" aria-label="Prepaid rollforward evidence">
      ...
      <div className="qbo-filter-grid"><button type="button" className="btn" disabled={prepaidState.phase==='LOADING'} onClick={loadPrepaid}>{prepaidState.phase==='LOADING'?'Loading…':'Load prepaid evidence'}</button></div>
