...
import {approveAuthoritativeFinancialStatementSnapshot,cancelAuthoritativeIntercompanyElimination,createAuthoritativeIntercompanyElimination,financialStatementSnapshotApprovalIdempotencyKey,financialStatementSnapshotPrepareIdempotencyKey,intercompanyEliminationCommandIdempotencyKey,postAuthoritativeIntercompanyElimination,prepareAuthoritativeFinancialStatementSnapshot,readAuthoritativeFinancialStatementSnapshotProposal,readAuthoritativeIntercompanyElimination,readAuthoritativeIntercompanyEliminationOptions,refreshAuthoritativeAiAmortizationSchedules,refreshAuthoritativeBudgetVsActual,refreshAuthoritativeCashFlowClassification,refreshAuthoritativeChartOfAccounts,refreshAuthoritativeConsolidation,refreshAuthoritativeConstructionLoanRollforward,refreshAuthoritativeCwipRollforward,refreshAuthoritativeDimensionProfitability,refreshAuthoritativeDocuments,refreshAuthoritativeFinancialStatementPeriodComparison,refreshAuthoritativeFinancialStatementSnapshot,refreshAuthoritativeFinancialStatementSnapshotProposalQueue,refreshAuthoritativeFinancialStatements,refreshAuthoritativeGeneralLedger,refreshAuthoritativeIntercompanyEliminations,refreshAuthoritativeIntercompanyReconciliation,refreshAuthoritativePrepaidRollforward,transitionAuthoritativeIntercompanyElimination} from './accounting-api.js';
import {alignedIntercompanyCounterpartyScopes,authorizedIntercompanyEliminationScopes,intercompanyEliminationScopeKey} from './intercompany-elimination-ui-model.js';
...
const UNAVAILABLE_REPORT_SHORTCUTS=Object.freeze([
  ...
  ['CONTRACTOR_1099_BALANCE_SUMMARY','1099 Contractor Balance Summary','Requires permission-scoped contractor tax identity and authoritative balance totals.',['contractor balance summary','1099 contractor summary']],
  ['CONTRACTOR_1099_BALANCE_DETAIL','1099 Contractor Balance Detail','Requires permission-scoped contractor tax identity and an authoritative as-of transaction ledger.',['contractor balance detail','1099 contractor detail']],
  ...
  ['EXCEPTIONS_TO_CLOSING_DATE','Exceptions to Closing Date','Requires authoritative closing-date exception and transaction-history facts.',['closing date exceptions','exceptions after close']],
  ...
  ['BILL_PAYMENT_LIST','Bill Payment List','Requires authoritative AP payment rows with vendor and payment-reference facts.',['bill payments','vendor payment list']],
  ...
  ['CLASS_LIST','Class List','Requires an authoritative permission-scoped class master-data reader.',['class master list','classes list']],
  ...
  ['CUSTOMER_CONTACT_LIST','Customer Contact List','Requires a permission-scoped customer master and contact-data reader.',['customer contacts','client contact list']],
  ['CUSTOMER_PHONE_LIST','Customer Phone List','Requires a permission-scoped customer master and phone-number reader.',['customer phone report','client phone list']],
  ...
  ['ESTIMATES_BY_CUSTOMER','Estimates by Customer','Requires authoritative customer-grouped estimate status and acceptance facts.',['customer estimates','estimate list']],
  ['ESTIMATES_PROGRESS_INVOICING_SUMMARY_BY_CUSTOMER','Estimates & Progress Invoicing Summary by Customer','Requires authoritative estimate-to-progress-invoice allocations and remaining balances.',['progress invoicing summary','estimate progress invoicing customer']],
  ['ESTIMATES_VS_ACTUALS_BY_PROJECT_V4','Estimates vs. actuals by project v4','Requires authoritative project estimates, accepted change orders, and actual cost and income facts.',['estimate actual project','project estimate actuals']],
  ...
  ['PRODUCT_ITEM_PROFITABILITY_BY_CUSTOMER','Product/Item Profitability by Customer','Requires authoritative customer and product/service line facts with server-defined profitability calculations.',['product item profitability by customer','product profitability by customer','item profitability by customer']],
  ['PROFIT_AND_LOSS_BY_MONTH','Profit and Loss by Month','Requires authoritative monthly statement columns and period totals.',['p&l by month','monthly income statement']],
  ...
  ['PRODUCT_SERVICE_LIST','Product/Service List','Requires an authoritative product and service master-data reader.',['product service list','item list']],
  ['INVENTORY_VALUATION_SUMMARY','Inventory Valuation Summary','Requires authoritative item quantities, costing, and as-of valuation totals.',['inventory value summary','stock valuation summary']],
  ['INVENTORY_VALUATION_DETAIL','Inventory Valuation Detail','Requires authoritative inventory transactions, quantities, costs, and running valuation facts.',['inventory value detail','stock valuation detail']],
  ['PHYSICAL_INVENTORY_WORKSHEET','Physical Inventory Worksheet','Requires authoritative item quantities and a retained physical-count snapshot.',['physical inventory sheet','inventory count worksheet']],
  ['PAYMENT_METHOD_LIST','Payment Method List','Requires an authoritative permission-scoped payment-method master reader.',['payment methods','payment type list']],
  ['INVENTORY_STATUS','Inventory Status','Requires authoritative on-hand, committed, ordered, available, and reorder quantities.',['inventory availability','stock status']],
  ['SALES_BY_PRODUCT_SERVICE_SUMMARY','Sales by Product/Service Summary','Requires authoritative product/service-grouped sales totals.',['sales by item summary','product service sales summary']],
  ['SALES_BY_PRODUCT_SERVICE_DETAIL','Sales by Product/Service Detail','Requires authoritative product/service sales transaction-line facts.',['sales by item detail','product service sales detail']],
  ['SALES_BY_CUSTOMER_SUMMARY','Sales by Customer Summary','Requires authoritative customer-grouped sales totals.',['customer sales summary','sales by client summary']],
  ['SALES_BY_CUSTOMER_DETAIL','Sales by Customer Detail','Requires authoritative customer-grouped sales transaction-line facts.',['customer sales detail','sales by client detail']],
  ['SALES_BY_CUSTOMER_TYPE_DETAIL','Sales by Customer Type Detail','Requires authoritative customer-type assignments and grouped sales transaction lines.',['customer type sales detail','sales by client type detail']],
  ['SALES_BY_CLASS_SUMMARY','Sales by Class Summary','Requires authoritative class assignments and grouped sales totals.',['class sales summary','sales summary by class']],
  ['SALES_BY_CLASS_DETAIL','Sales by Class Detail','Requires authoritative class-assigned sales transaction-line facts.',['class sales detail','sales detail by class']],
  ['SALES_TAX_LIABILITY_REPORT','Sales Tax Liability Report','Requires authoritative jurisdiction-scoped taxable sales and tax-collected facts.',['sales tax liability','tax liability report']],
  ...
  ['PURCHASES_BY_CLASS_DETAIL','Purchases by Class Detail','Requires authoritative class-grouped purchase transaction and line facts.',['class purchase detail','purchases by class']],
  ...
  ['RECURRING_TEMPLATE_LIST','Recurring Template List','Requires an authoritative recurring-template master and schedule reader.',['recurring templates','scheduled template list']],
  ...
  ['RECENT_EDITED_TIME_ACTIVITIES','Recent/Edited Time Activities','Requires authoritative time-activity rows joined to immutable modification history.',['recent edited time activities','recent time activities','edited time activities']],
  ...
  ['PERFORMANCE_CENTER','Performance center','Requires permission-scoped chart definitions and immutable metric snapshots.',['business performance dashboard','custom charts']],
  ['FINANCIAL_SUMMARY','Financial summary','Requires an immutable generated-summary snapshot with source report IDs, period scope, generation version, and audit evidence.',['review summary','smart financial summary']],
  ['OPEN_PURCHASE_ORDER_BY_PROJECT','Open Purchase Order By Project','Requires authoritative project-scoped purchase-order lines and received and open balances.',['open po by project','project purchase orders']],
  ['WORK_IN_PROGRESS_BY_PROJECT_V3','Work in progress by project','Requires authoritative project contract, cost-to-date, percentage-complete, earned-revenue, billing, and backlog facts.',['project work in progress','wip by project']],
  ...
  ['TIME_ACTIVITIES_BY_CUSTOMER_DETAIL','Time Activities by Customer Detail','Requires authoritative customer-grouped time-activity rows and billing facts.',['time activity customer detail','customer time activities']],
  ['TIME_ACTIVITIES_BY_EMPLOYEE_DETAIL','Time Activities by Employee Detail','Requires permission-scoped employee-grouped time-activity rows and billing facts.',['time activity employee detail','employee time activities']],
  ['EMPLOYEE_CONTACT_LIST','Employee Contact List','Requires a permission-scoped employee master and field-level contact-data reader.',['employee contacts','staff contact list']],
  ['TIMESHEET_DETAIL_BY_EMPLOYEE','Timesheet Detail by Employee','Requires permission-scoped employee timesheet rows with start, end, duration, running-total, customer, and description facts.',['employee timesheet detail','timesheets by employee']],
  ['TIME_SUMMARY_BY_PAY_TYPE','Time Summary by Pay Type','Requires payroll-scoped pay-type classification and server-calculated time totals.',['pay type time summary','time by pay type']],
  ['UNBILLED_CHARGES','Unbilled charges','Requires authoritative unbilled charge rows and balance facts.',['unbilled charge report','unbilled expenses']],
  ['UNBILLED_TIME','Unbilled time','Requires authoritative time-activity rows, rates, duration, and billing-state facts.',['unbilled time report','unbilled hours']],
  ...
  ['VENDOR_PHONE_LIST','Vendor Phone List','Requires a permission-scoped vendor master and phone-number reader.',['vendor phone report','supplier phone list']],
...
const REPORT_ICON_NAMES=Object.freeze({
  TRIAL_BALANCE:'lines',DETAIL_1099:'document',CONTRACTOR_1099_BALANCE_SUMMARY:'shield',CONTRACTOR_1099_BALANCE_DETAIL:'shield',ACCOUNTS_RECEIVABLE_AGING_DETAIL:'calendar',ACCOUNTS_RECEIVABLE_AGING_SUMMARY:'calendar',ACCOUNTS_PAYABLE_AGING_DETAIL:'calendar',ACCOUNTS_PAYABLE_AGING_SUMMARY:'calendar',ACCOUNT_LIST:'book',ADJUSTED_TRIAL_BALANCE:'lines',AUDIT_LOG:'shield',BALANCE_SHEET:'book',BALANCE_SHEET_COMPARISON:'book',BALANCE_SHEET_DETAIL:'book',BALANCE_SHEET_SUMMARY:'book',BILL_APPROVAL_STATUS:'wallet',BILLS_AND_APPLIED_PAYMENTS:'wallet',BILL_PAYMENT_LIST:'wallet',CHECK_DETAIL:'bank',CLASS_LIST:'layers',COLLECTIONS_REPORT:'wallet',CUSTOMER_BALANCE_DETAIL:'wallet',CUSTOMER_BALANCE_SUMMARY:'wallet',CUSTOMER_CONTACT_LIST:'wallet',CUSTOMER_PHONE_LIST:'wallet',DEPOSIT_DETAIL:'bank',EMPLOYEE_CONTACT_LIST:'shield',ESTIMATES_BY_CUSTOMER:'wallet',ESTIMATES_PROGRESS_INVOICING_SUMMARY_BY_CUSTOMER:'wallet',ESTIMATES_VS_ACTUALS_BY_PROJECT_V4:'bars',EXCEPTIONS_TO_CLOSING_DATE:'shield',FINANCIAL_SUMMARY:'bars',INCOME_STATEMENT:'bars',INCOME_BY_CUSTOMER_SUMMARY:'bars',INVOICE_APPROVAL_STATUS:'wallet',INVOICE_LIST:'wallet',INVENTORY_STATUS:'book',INVENTORY_VALUATION_DETAIL:'book',INVENTORY_VALUATION_SUMMARY:'book',INVOICES_AND_RECEIVED_PAYMENTS:'wallet',JOURNAL_REPORT:'book',PAYMENT_METHOD_LIST:'wallet',PERFORMANCE_CENTER:'bars',PHYSICAL_INVENTORY_WORKSHEET:'document',PROFIT_AND_LOSS_BY_CLASS:'bars',PROFIT_AND_LOSS_BY_TAG_GROUP:'bars',PROFIT_AND_LOSS_BY_CUSTOMER:'bars',PRODUCT_ITEM_PROFITABILITY_BY_CUSTOMER:'bars',PROFIT_AND_LOSS_BY_MONTH:'bars',PROFIT_AND_LOSS_PERCENT_TOTAL_INCOME:'bars',PROFIT_AND_LOSS_COMPARISON:'bars',PROFIT_AND_LOSS_DETAIL:'bars',PRODUCT_SERVICE_LIST:'document',PURCHASES_BY_CLASS_DETAIL:'wallet',PURCHASES_BY_PRODUCT_SERVICE_DETAIL:'wallet',PURCHASES_BY_VENDOR_DETAIL:'wallet',QUARTERLY_PROFIT_AND_LOSS_SUMMARY:'bars',RECENT_EDITED_TIME_ACTIVITIES:'cycle',RECENT_TRANSACTIONS:'book',SALES_BY_CLASS_DETAIL:'bars',SALES_BY_CLASS_SUMMARY:'bars',SALES_BY_CUSTOMER_DETAIL:'bars',SALES_BY_CUSTOMER_SUMMARY:'bars',SALES_BY_CUSTOMER_TYPE_DETAIL:'bars',SALES_BY_PRODUCT_SERVICE_DETAIL:'bars',SALES_BY_PRODUCT_SERVICE_SUMMARY:'bars',SALES_TAX_LIABILITY_REPORT:'shield',EXPENSES_BY_VENDOR_SUMMARY:'wallet',INVALID_JOURNAL_TRANSACTIONS:'book',OPEN_INVOICES:'wallet',OPEN_PURCHASE_ORDER_BY_PROJECT:'wallet',WORK_IN_PROGRESS_BY_PROJECT_V3:'cycle',OPEN_PURCHASE_ORDER_LIST_BY_VENDOR:'wallet',STATEMENT_LIST:'document',TERMS_LIST:'document',TIME_ACTIVITIES_BY_CUSTOMER_DETAIL:'wallet',TIME_ACTIVITIES_BY_EMPLOYEE_DETAIL:'wallet',TIMESHEET_DETAIL_BY_EMPLOYEE:'calendar',TIME_SUMMARY_BY_PAY_TYPE:'shield',UNBILLED_CHARGES:'wallet',UNBILLED_TIME:'wallet',UNPAID_BILLS:'wallet',TRANSACTION_DETAIL_BY_ACCOUNT:'book',TRANSACTION_DRILLDOWN_REPORT:'book',TRANSACTION_LIST_BY_CUSTOMER:'wallet',TRANSACTION_LIST_BY_DATE:'book',TRANSACTION_LIST_BY_TAG_GROUP:'book',TRANSACTION_LIST_WITH_SPLITS:'book',TRANSACTION_LIST_BY_VENDOR:'wallet',VENDOR_CONTACT_LIST:'wallet',VENDOR_PHONE_LIST:'wallet',VENDOR_BALANCE_DETAIL:'wallet',VENDOR_BALANCE_SUMMARY:'wallet',CASH_FLOW:'exchange',STATEMENT_OF_CASH_FLOWS:'exchange',GENERAL_LEDGER:'book',GENERAL_LEDGER_LIST:'book',
  RECURRING_TEMPLATE_LIST:'cycle',RECONCILIATION_REPORTS:'check',STATEMENTS:'book',CASH_AND_CAPITAL:'bank',OPERATING_ANALYSIS:'bars',GROUP_AND_COMPARISON:'layers',
...
export function AuthoritativeReportsWorkspace({config,scopes=[],fetcher=globalThis.fetch,environment=globalThis,currentActorAccess=null,initialCatalog=DEFAULT_AUTHORITATIVE_REPORTS_CATALOG,onOpenArAging=()=>{},onOpenGeneralLedger=()=>{},onOpenReconciliation=()=>{},workspaceTitle='Standard reports',workspaceEyebrow='',workspaceDescription='',initialDimensionType='PROPERTY',initialDimensionRef=''}){
  ...
  const [snapshotWorkflowState,setSnapshotWorkflowState]=useState({phase:'IDLE',rows:[],total:0,limit:20,offset:0,complete:false,error:null});
  const [snapshotProposalState,setSnapshotProposalState]=useState({phase:'IDLE',data:null,error:null});
  const [snapshotCommandState,setSnapshotCommandState]=useState({phase:'IDLE',message:null,error:null});
  ...
  const [eliminationOptionsState,setEliminationOptionsState]=useState({phase:'IDLE',data:null,error:null});
  const [eliminationRegisterState,setEliminationRegisterState]=useState({phase:'IDLE',data:null,error:null});
  const [eliminationBatch,setEliminationBatch]=useState(null);
  const [eliminationReason,setEliminationReason]=useState('');
  const [eliminationSourceScopeKey,setEliminationSourceScopeKey]=useState('');
  const [eliminationCounterpartyScopeKey,setEliminationCounterpartyScopeKey]=useState('');
  const [eliminationCommandState,setEliminationCommandState]=useState({phase:'IDLE',error:null,message:null});
  ...
  const actorPermissions=Array.isArray(currentActorAccess?.permissions)?currentActorAccess.permissions:[];
  const currentSession=!!currentActorAccess&&!currentActorAccess.session_refresh_required;
  const authorizedEliminationScopes=useMemo(()=>authorizedIntercompanyEliminationScopes(scopes),[scopes]);
  const reportingEliminationScope=authorizedEliminationScopes.find(scope=>scope.entity_id===config.entityId&&scope.period_id===config.periodId)||null;
  const eliminationSourceScope=authorizedEliminationScopes.find(scope=>intercompanyEliminationScopeKey(scope)===eliminationSourceScopeKey)||null;
  const alignedCounterpartyScopes=useMemo(()=>alignedIntercompanyCounterpartyScopes(authorizedEliminationScopes,eliminationSourceScope),[authorizedEliminationScopes,eliminationSourceScope]);
  const eliminationCounterpartyScope=alignedCounterpartyScopes.find(scope=>intercompanyEliminationScopeKey(scope)===eliminationCounterpartyScopeKey)||null;
  const canPrepareSnapshot=currentSession&&(actorPermissions.includes('*')||actorPermissions.includes('GL.REPORT.SNAPSHOT.PREPARE'));
  const canApproveSnapshot=currentSession&&(actorPermissions.includes('*')||actorPermissions.includes('GL.REPORT.SNAPSHOT.APPROVE'));
  ...
  const loadSnapshotProposal=async proposalId=>{setSnapshotProposalState({phase:'LOADING',data:null,error:null});const result=await readAuthoritativeFinancialStatementSnapshotProposal({config,proposalId,fetcher});setSnapshotProposalState(result.ok?{phase:'READY',data:result.data,error:null}:{phase:authoritativeReadFailurePhase(result),data:null,error:result});return result;};
  const loadSnapshotWorkflow=async(nextOffset=0)=>{setSnapshotWorkflowState(current=>({...current,phase:'LOADING',rows:[],offset:nextOffset,error:null}));const result=await refreshAuthoritativeFinancialStatementSnapshotProposalQueue({config,limit:20,offset:nextOffset,fetcher});setSnapshotWorkflowState(result.ok?{phase:'READY',rows:result.data.rows,total:result.data.total_count,limit:result.data.limit,offset:result.data.offset,complete:result.data.population_complete,error:null}:{phase:authoritativeReadFailurePhase(result),rows:[],total:0,limit:20,offset:nextOffset,complete:false,error:result});return result;};
  const prepareSnapshot=async()=>{
    if(!canPrepareSnapshot||state.phase!=='READY'||!state.rows.length||typeof environment?.confirm!=='function')return;
    if(!environment.confirm(`Prepare an immutable financial statement snapshot proposal for ${periodLabel}? A different authorized actor must approve it.`))return;
    setSnapshotCommandState({phase:'LOADING',message:null,error:null});
    const idempotencyKey=await financialStatementSnapshotPrepareIdempotencyKey({config,statementRows:state.rows,cryptoApi:environment?.crypto});
    const result=await prepareAuthoritativeFinancialStatementSnapshot({config,statementRows:state.rows,idempotencyKey,fetcher,cryptoApi:environment?.crypto});
    if(!result.ok){setSnapshotCommandState({phase:'BLOCKED',message:null,error:result});return;}
    setSnapshotCommandState({phase:'READY',message:result.data.idempotent?'Existing proposal recovered.':'Immutable proposal prepared.',error:null});
    await loadSnapshotWorkflow(0);await loadSnapshotProposal(result.data.financial_statement_snapshot_proposal_id);
  };
  const approveSnapshot=async()=>{
    const proposal=snapshotProposalState.data;
    if(!canApproveSnapshot||proposal?.can_approve!==true||typeof environment?.confirm!=='function')return;
    if(!environment.confirm(`Approve statement snapshot proposal ${proposal.financial_statement_snapshot_proposal_id}? This freezes report evidence and does not post a Journal Entry.`))return;
    setSnapshotCommandState({phase:'LOADING',message:null,error:null});
    const idempotencyKey=await financialStatementSnapshotApprovalIdempotencyKey({config,proposal,cryptoApi:environment?.crypto});
    const result=await approveAuthoritativeFinancialStatementSnapshot({config,proposal,idempotencyKey,fetcher,cryptoApi:environment?.crypto});
    if(!result.ok){setSnapshotCommandState({phase:'BLOCKED',message:null,error:result});return;}
    setSnapshotCommandState({phase:'READY',message:result.data.idempotent?'Existing approval recovered.':`Snapshot v${result.data.version} approved.`,error:null});
    await loadSnapshotWorkflow(0);await loadSnapshotProposal(proposal.financial_statement_snapshot_proposal_id);await loadStatementSnapshot();
  };
  ...
  const loadEliminationOptions=async()=>{if(!eliminationSourceScope||!eliminationCounterpartyScope)return;setEliminationOptionsState({phase:'LOADING',data:null,error:null});const result=await readAuthoritativeIntercompanyEliminationOptions({config,groupRef:consolidationGroupRef.trim(),sourceEntityId:eliminationSourceScope.entity_id,sourcePeriodId:eliminationSourceScope.period_id,counterpartyEntityId:eliminationCounterpartyScope.entity_id,counterpartyPeriodId:eliminationCounterpartyScope.period_id,fetcher});setEliminationOptionsState(result.ok?{phase:'READY',data:result.data,error:null}:{phase:authoritativeReadFailurePhase(result),data:null,error:result});};
  const loadEliminationRegister=async()=>{setEliminationRegisterState({phase:'LOADING',data:null,error:null});const result=await refreshAuthoritativeIntercompanyEliminations({config,fetcher});setEliminationRegisterState(result.ok?{phase:'READY',data:result.data,error:null}:{phase:authoritativeReadFailurePhase(result),data:null,error:result});};
  const selectEliminationBatch=async batchId=>{const result=await readAuthoritativeIntercompanyElimination({config,batchId,fetcher});if(result.ok)setEliminationBatch(result.data);else setEliminationCommandState({phase:'BLOCKED',error:result,message:null});};
  const createElimination=async option=>{setEliminationCommandState({phase:'LOADING',error:null,message:null});const idempotencyKey=await intercompanyEliminationCommandIdempotencyKey({config,option,action:'CREATE',reason:eliminationReason,cryptoApi:environment?.crypto});if(!idempotencyKey){setEliminationCommandState({phase:'BLOCKED',error:{message:'Browser cryptography could not prepare a stable command identity.'},message:null});return;}const result=await createAuthoritativeIntercompanyElimination({config,option,reason:eliminationReason,idempotencyKey,fetcher,cryptoApi:environment?.crypto});if(!result.ok){setEliminationCommandState({phase:'BLOCKED',error:result,message:null});return;}setEliminationBatch(result.data);setEliminationCommandState({phase:'READY',error:null,message:result.data.idempotent?'Existing Draft recovered.':'Elimination Draft created.'});setEliminationReason('');await loadEliminationRegister();await loadEliminationOptions();};
  const runEliminationAction=async action=>{if(!eliminationBatch)return;if(action==='POST'&&typeof environment?.confirm==='function'&&!environment.confirm(`Post elimination batch ${eliminationBatch.intercompany_elimination_batch_id}? This writes the approved consolidation presentation evidence.`))return;setEliminationCommandState({phase:'LOADING',error:null,message:null});const idempotencyKey=await intercompanyEliminationCommandIdempotencyKey({config,batch:eliminationBatch,action,reason:action==='POST'?null:eliminationReason,cryptoApi:environment?.crypto});if(!idempotencyKey){setEliminationCommandState({phase:'BLOCKED',error:{message:'Browser cryptography could not prepare a stable command identity.'},message:null});return;}const common={config,batch:eliminationBatch,idempotencyKey,fetcher,cryptoApi:environment?.crypto},result=action==='CANCEL'?await cancelAuthoritativeIntercompanyElimination({...common,reason:eliminationReason}):action==='POST'?await postAuthoritativeIntercompanyElimination(common):await transitionAuthoritativeIntercompanyElimination({...common,action,reason:eliminationReason});if(!result.ok){setEliminationCommandState({phase:'BLOCKED',error:result,message:null});return;}setEliminationBatch(result.data);setEliminationCommandState({phase:'READY',error:null,message:`Batch is now ${result.data.status}.`});setEliminationReason('');await loadEliminationRegister();if(action==='POST')await loadConsolidation();};
  ...
  return <AuthoritativeReportsView className="reports-library authoritative-reports-library" eyebrow={workspaceEyebrow} title={workspaceTitle} description={workspaceDescription} scope={<div className="report-period-chip"><span>Reporting scope</span><b title={`Entity ID: ${config.entityId}; Period ID: ${config.periodId}`}>Entity {entityLabel} · Period {periodLabel}</b><small>Posted only</small></div>}>
    <section className="authoritative-report-finder" aria-label="Reports catalog"><label><span>Search reports</span><input value={catalogSearch} maxLength="120" aria-describedby="authoritative-report-search-summary" onChange={event=>setCatalogSearch(event.target.value)} placeholder="Type report name here"/></label><span id="authoritative-report-search-summary" className={`authoritative-report-match-summary${searching?'':' sr-only'}`} aria-live="polite" aria-atomic="true">{searching?(totalCatalogMatches?`${totalCatalogMatches} ${totalCatalogMatches===1?'match':'matches'}`:'No reports found'):''}</span></section>
    ...
    <details className="authoritative-secondary-disclosure authoritative-statement-snapshot"><summary><span>Statement snapshot</span><span className="badge badge-muted">CONTROLLED WORKFLOW</span></summary><section className="card" aria-label="Financial statement snapshot version evidence"><div className="card-head"><div><h2>Prepare and approve</h2><p className="muted sm">Prepare immutable POSTED-ledger evidence, then require a different authorized actor to approve it. This workflow never posts or changes a Journal Entry.</p></div></div>
      <div className="row-acts"><button type="button" className="btn btn-sm btn-ghost" disabled={snapshotWorkflowState.phase==='LOADING'} onClick={()=>loadSnapshotWorkflow(0)}>{snapshotWorkflowState.phase==='LOADING'?'Loading…':'Load proposal queue'}</button><button type="button" className="btn btn-sm" disabled={!canPrepareSnapshot||state.phase!=='READY'||!state.rows.length||snapshotCommandState.phase==='LOADING'} onClick={prepareSnapshot}>Prepare immutable proposal</button></div>
      {!currentSession&&<StateBlock tone="blocked" title="Current workflow access required">Sign in with a current report-snapshot preparer or approver session. Stale or absent permissions never enable a command.</StateBlock>}
      {currentSession&&!canPrepareSnapshot&&!canApproveSnapshot&&<p className="muted sm">This session can read approved report evidence but cannot prepare or approve a statement snapshot.</p>}
      {snapshotWorkflowState.phase==='LOADING'&&<StateBlock tone="loading">Loading the authoritative proposal queue...</StateBlock>}
      <ReadError state={snapshotWorkflowState} onRetry={()=>loadSnapshotWorkflow(snapshotWorkflowState.offset)}/>
      {snapshotWorkflowState.phase==='READY'&&!snapshotWorkflowState.rows.length&&<StateBlock tone="empty" title="No snapshot proposals">Prepare from the current complete financial statement evidence when the period is ready for Controller approval.</StateBlock>}
      {snapshotWorkflowState.phase==='READY'&&!!snapshotWorkflowState.rows.length&&<div className="table-wrap authoritative-snapshot-workflow-table" role="region" tabIndex={0} aria-label="Financial statement snapshot proposals; scroll horizontally to view every column"><table className="tbl"><thead><tr><th>Status</th><th>Prepared evidence</th><th>Approval</th><th>Details</th></tr></thead><tbody>{snapshotWorkflowState.rows.map(proposal=><tr key={proposal.financial_statement_snapshot_proposal_id}><td><b>{proposal.status==='PENDING_APPROVAL'?'Pending approval':'Approved'}</b><div className="muted sm">{proposal.currency} · {proposal.row_count} rows</div></td><td><code>{proposal.snapshot_hash}</code><div className="muted sm">Prepared by {proposal.prepared_by}</div></td><td>{proposal.approved_by?<span>{proposal.approved_by}<span className="muted sm"> · v{proposal.version}</span></span>:<span className="muted sm">Distinct approver required</span>}</td><td><button type="button" className="btn btn-sm" onClick={()=>loadSnapshotProposal(proposal.financial_statement_snapshot_proposal_id)}>Review evidence</button></td></tr>)}</tbody></table></div>}
      {snapshotWorkflowState.phase==='READY'&&snapshotWorkflowState.total>snapshotWorkflowState.limit&&<div className="pager"><button type="button" className="btn btn-sm btn-ghost" disabled={snapshotWorkflowState.offset===0} onClick={()=>loadSnapshotWorkflow(Math.max(0,snapshotWorkflowState.offset-snapshotWorkflowState.limit))}>Previous proposals</button><span className="muted sm">{snapshotWorkflowState.offset+1}–{Math.min(snapshotWorkflowState.total,snapshotWorkflowState.offset+snapshotWorkflowState.rows.length)} of {snapshotWorkflowState.total}</span><button type="button" className="btn btn-sm btn-ghost" disabled={snapshotWorkflowState.complete} onClick={()=>loadSnapshotWorkflow(snapshotWorkflowState.offset+snapshotWorkflowState.limit)}>Next proposals</button></div>}
      {snapshotProposalState.phase==='LOADING'&&<StateBlock tone="loading">Loading canonical proposal rows...</StateBlock>}
      <ReadError state={snapshotProposalState}/>
      {snapshotProposalState.phase==='READY'&&snapshotProposalState.data&&<section className="card inset" aria-label="Selected statement snapshot proposal"><div className="card-head"><div><h3>{snapshotProposalState.data.status==='PENDING_APPROVAL'?'Pending snapshot proposal':'Approved snapshot proposal'}</h3><p className="muted sm"><code>{snapshotProposalState.data.financial_statement_snapshot_proposal_id}</code> · snapshot <code>{snapshotProposalState.data.snapshot_hash}</code></p></div><button type="button" className="btn btn-sm" disabled={!canApproveSnapshot||snapshotProposalState.data.can_approve!==true||snapshotCommandState.phase==='LOADING'} onClick={approveSnapshot}>Approve snapshot</button></div><p className="muted sm">Prepared by {snapshotProposalState.data.prepared_by}. {snapshotProposalState.data.can_approve?'The server confirmed this actor is a distinct eligible approver.':'Approval is unavailable to this actor or this proposal is already approved.'}</p><div className="table-wrap authoritative-snapshot-workflow-table" role="region" tabIndex={0} aria-label="Canonical snapshot proposal rows; scroll horizontally to view every column"><table className="tbl"><thead><tr><th>Statement / account</th><th className="ta-r">Balance</th><th>Details</th></tr></thead><tbody>{snapshotProposalState.data.rows.map(row=><tr key={`${row.statement_type}:${row.account_code}`}><td><b>{row.account_code}</b><div className="muted sm">{row.statement_type} / {row.statement_section}</div></td><td className="num">{money(row.display_balance)}</td><td><code>{row.row_hash}</code><div className="muted sm">JE {row.journal_entry_ids.length} · GL {row.ledger_line_ids.length} · Source {row.source_document_ids.length}</div></td></tr>)}</tbody></table></div></section>}
      {snapshotCommandState.phase==='LOADING'&&<StateBlock tone="loading">Recording the immutable workflow receipt...</StateBlock>}
      {snapshotCommandState.phase==='BLOCKED'&&<StateBlock tone="blocked" title={snapshotCommandState.error?.code||'Snapshot workflow blocked'}>{snapshotCommandState.error?.message||'The command did not produce an authoritative receipt.'}</StateBlock>}
      {snapshotCommandState.phase==='READY'&&<StateBlock tone="ready" title="Workflow receipt retained">{snapshotCommandState.message}</StateBlock>}
      <hr/><div className="card-head"><div><h2>Approved version</h2><p className="muted sm">Immutable statement evidence with retained GL, Journal, and source identifiers.</p></div></div><div className="qbo-filter-grid"><button type="button" className="btn" disabled={statementSnapshotState.phase==='LOADING'} onClick={loadStatementSnapshot}>{statementSnapshotState.phase==='LOADING'?'Loading…':'Load statement snapshot'}</button></div>
    ...
    {workbenchTab==='GROUP_AND_COMPARISON'&&<section className="card" aria-label="Intercompany elimination workflow">
      <div className="card-head"><div><h2>Intercompany elimination batches</h2><p className="muted sm">Creates a balanced two-line presentation elimination from the exact reciprocal DUE_FROM/DUE_TO evidence. Member-company ledgers and the raw reconciliation mismatch remain unchanged.</p></div><span className="badge badge-warn">CONTROLLED WORKFLOW</span></div>
      <p><b>Reporting scope:</b> {reportingEliminationScope?`${reportingEliminationScope.entity_code} — ${reportingEliminationScope.entity_name} · ${reportingEliminationScope.period_code} (${reportingEliminationScope.period_start} to ${reportingEliminationScope.period_end})`:`${config.entityId} · ${config.periodId}`}</p>
      <div className="qbo-filter-grid"><label>Source member company and period<select aria-label="Source member company and period" value={eliminationSourceScopeKey} onChange={event=>{setEliminationSourceScopeKey(event.target.value);setEliminationCounterpartyScopeKey('');setEliminationOptionsState({phase:'IDLE',data:null,error:null});}}><option value="">Select an authorized member scope</option>{authorizedEliminationScopes.map(scope=><option key={intercompanyEliminationScopeKey(scope)} value={intercompanyEliminationScopeKey(scope)}>{scope.entity_code} — {scope.entity_name} · {scope.period_code} ({scope.period_start} to {scope.period_end})</option>)}</select></label><label>Counterparty member company and aligned period<select aria-label="Counterparty member company and aligned period" value={eliminationCounterpartyScopeKey} disabled={!eliminationSourceScope} onChange={event=>{setEliminationCounterpartyScopeKey(event.target.value);setEliminationOptionsState({phase:'IDLE',data:null,error:null});}}><option value="">Select a different aligned member scope</option>{alignedCounterpartyScopes.map(scope=><option key={intercompanyEliminationScopeKey(scope)} value={intercompanyEliminationScopeKey(scope)}>{scope.entity_code} — {scope.entity_name} · {scope.period_code} ({scope.period_start} to {scope.period_end})</option>)}</select></label><label>Explicit audit reason<input value={eliminationReason} minLength="8" maxLength="2000" onChange={event=>setEliminationReason(event.target.value)} placeholder="Enter the evidence reviewed and decision basis" /></label><button type="button" className="btn" disabled={eliminationOptionsState.phase==='LOADING'||!consolidationGroupRef.trim()||!eliminationSourceScope||!eliminationCounterpartyScope} onClick={loadEliminationOptions}>{eliminationOptionsState.phase==='LOADING'?'Loading…':'Load safe Draft options'}</button><button type="button" className="btn" disabled={eliminationRegisterState.phase==='LOADING'} onClick={loadEliminationRegister}>{eliminationRegisterState.phase==='LOADING'?'Loading…':'Refresh batch register'}</button></div>
      <p className="muted sm">The reporting period must be the selected PRIMARY period. Only the backend can expose Create, Submit, Review, Approve, Cancel, or Post. Post is never automatic and requires a fourth distinct authorized actor.</p>
      <ReadError state={eliminationOptionsState} onRetry={loadEliminationOptions}/><ReadError state={eliminationRegisterState} onRetry={loadEliminationRegister}/><ReadError state={eliminationCommandState}/>
      {eliminationCommandState.message&&<StateBlock tone="success">{eliminationCommandState.message}</StateBlock>}
      {eliminationOptionsState.phase==='READY'&&!eliminationOptionsState.data.options.length&&<StateBlock tone="empty" title="No safe elimination Draft options">No reciprocal, normal-sign, unmapped-to-existing-elimination source is currently eligible.</StateBlock>}
      {eliminationOptionsState.phase==='READY'&&!!eliminationOptionsState.data.options.length&&<div className="table-wrap" role="region" tabIndex={0} aria-label="Safe intercompany elimination Draft options"><table className="tbl"><thead><tr><th>Member accounts</th><th className="ta-r">Raw balances</th><th className="ta-r">Matched elimination</th><th className="ta-r">Raw mismatch</th><th>Action</th></tr></thead><tbody>{eliminationOptionsState.data.options.map(option=><tr key={option.canonical_source_scope_hash}><td><b>{option.source_account_code}</b> {option.source_classification}<div className="muted sm">{option.counterparty_account_code} {option.counterparty_classification}</div></td><td className="num">{money(option.source_closing_balance)} / {money(option.counterparty_closing_balance)}</td><td className="num">{money(option.matched_amount)} {option.currency}</td><td className="num">{money(option.raw_mismatch)} {option.currency}</td><td><button type="button" className="btn btn-sm" disabled={!eliminationOptionsState.data.action_flags.can_create_draft||eliminationCommandState.phase==='LOADING'||eliminationReason.trim().length<8} onClick={()=>createElimination(option)}>Create Draft</button></td></tr>)}</tbody></table></div>}
      {eliminationRegisterState.phase==='READY'&&!eliminationRegisterState.data.rows.length&&<StateBlock tone="empty" title="No retained elimination batches">No batch has been created for this reporting period.</StateBlock>}
      {eliminationRegisterState.phase==='READY'&&!!eliminationRegisterState.data.rows.length&&<div className="table-wrap" role="region" tabIndex={0} aria-label="Intercompany elimination batch register"><table className="tbl"><thead><tr><th>Batch</th><th>Status</th><th className="ta-r">Matched</th><th className="ta-r">Raw mismatch</th><th>Details</th></tr></thead><tbody>{eliminationRegisterState.data.rows.map(batch=><tr key={batch.intercompany_elimination_batch_id}><td><code>{batch.intercompany_elimination_batch_id}</code></td><td>{batch.status}<div className="muted sm">Revision {batch.revision}</div></td><td className="num">{money(batch.matched_amount)} {batch.currency}</td><td className="num">{money(batch.raw_mismatch)} {batch.currency}</td><td><button type="button" className="btn btn-sm" onClick={()=>selectEliminationBatch(batch.intercompany_elimination_batch_id)}>View details</button></td></tr>)}</tbody></table></div>}
      {eliminationBatch&&<section className="qbo-subpanel" aria-label="Selected intercompany elimination batch"><h3>Batch {eliminationBatch.intercompany_elimination_batch_id}</h3><p><span className="badge">{eliminationBatch.status}</span> <span className={eliminationBatch.source_current?'badge badge-ok':'badge badge-warn'}>{eliminationBatch.source_current?'Source current':'Source stale or cancelled'}</span></p><p className="muted sm">Raw member balances: {money(eliminationBatch.source_closing_balance)} and {money(eliminationBatch.counterparty_closing_balance)}. Matched presentation amount: {money(eliminationBatch.matched_amount)}. Raw mismatch remains {money(eliminationBatch.raw_mismatch)}.</p><div className="table-wrap"><table className="tbl"><thead><tr><th>Line</th><th>Presentation</th><th>Entry</th><th className="ta-r">Debit</th><th className="ta-r">Credit</th></tr></thead><tbody>{eliminationBatch.lines.map(line=><tr key={line.line_no}><td>{line.line_no}</td><td>{line.presentation_account_code} {line.presentation_side}<div className="muted sm">{line.member_entity_id} · {line.source_account_code}</div></td><td>{line.entry_side}</td><td className="num">{money(line.debit_amount)}</td><td className="num">{money(line.credit_amount)}</td></tr>)}</tbody></table></div><div className="qbo-filter-grid">{eliminationBatch.action_flags.can_submit&&<button type="button" className="btn" disabled={eliminationCommandState.phase==='LOADING'||eliminationReason.trim().length<8} onClick={()=>runEliminationAction('SUBMIT')}>Submit</button>}{eliminationBatch.action_flags.can_review&&<button type="button" className="btn" disabled={eliminationCommandState.phase==='LOADING'||eliminationReason.trim().length<8} onClick={()=>runEliminationAction('REVIEW')}>Review</button>}{eliminationBatch.action_flags.can_approve&&<button type="button" className="btn" disabled={eliminationCommandState.phase==='LOADING'||eliminationReason.trim().length<8} onClick={()=>runEliminationAction('APPROVE')}>Approve</button>}{eliminationBatch.action_flags.can_cancel&&<button type="button" className="btn" disabled={eliminationCommandState.phase==='LOADING'||eliminationReason.trim().length<8} onClick={()=>runEliminationAction('CANCEL')}>Cancel</button>}{eliminationBatch.action_flags.can_post&&<button type="button" className="btn btn-primary" disabled={eliminationCommandState.phase==='LOADING'} onClick={()=>runEliminationAction('POST')}>Post presentation elimination</button>}</div>{eliminationBatch.status==='POSTED'&&<p className="muted sm">Consolidation readback returned {eliminationBatch.consolidation_results.length} bound presentation rows. If member ledger evidence changes later, these rows become BLOCKED_STALE_INTERCOMPANY_ELIMINATION_SOURCE until a newer consolidation snapshot is published.</p>}</section>}
    </section>}
