...
export function AuthoritativeAccessStatus({state}){
  ...
  return <details className={`authoritative-access-status ${row.session_refresh_required?'authoritative-access-stale':''}`}>
    <summary><b>Access</b> {row.session_refresh_required?'Changed - sign in again':'Session permissions current'}</summary>
    <div className="authoritative-access-detail">
      ...
      {row.session_refresh_required&&<p>Configured access changed after this session was issued. Sign out and sign in again before relying on the new permissions.</p>}
