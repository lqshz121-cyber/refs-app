...
export function APWorkspace({ctx}) {
  ...
  useEffect(() => {
    ...
    if (navContext.billQueueView) {
      const returnedScope = localExpenseDetailReturnScope({
  ...
  }, [navContext?.route, navContext?.tab, navContext?.vendorId, navContext?.creditKey, navContext?.billId, navContext?.billNo, navContext?.jeNumber, navContext?.paymentBillDetail, navContext?.billQueueView, navContext?.query, navContext?.statusFilter, navContext?.transactionType, navContext?.dateRange, navContext?.fromDate, navContext?.toDate, navContext?.categoryCode, bills]);
...
function VendorWorkspace({bills, journals, bankTransactions, initialVendorId, initialQuery, onOpenJournal, onOpenBill, onOpenVendor}) {
  ...
  return <div>
    ...
    <Table rowKey="vendor_id" features={{exportable:false}} cols={[
      ...
      {h:'Action',render:r=>{const balance=r.localEvidence.open_balance; return <span className="row-acts"><Btn size="sm" variant="ghost" onClick={event=>{event.stopPropagation();setSelectedVendorId(r.vendor_id);}}>View evidence</Btn><Btn size="sm" variant="ghost" onClick={event=>{event.stopPropagation();onOpenVendor(r.vendor_id,'Bills');}}>Open local bills</Btn><Btn size="sm" variant="ghost" disabled={!balance} title={balance ? 'Open the AP aging view for this vendor' : 'This vendor has no retained open balance to age'} onClick={event=>{event.stopPropagation();onOpenVendor(r.vendor_id,'AP Aging');}}>Open aging</Btn></span>;}},
...
function BillDetail({bill, onClose, onOpenPayment, agingReturn, billReturnScope, vendorReturnId, vendorReturnSearch, actionQueueReturn, ctx}) {
  ...
  return <div className="full-bleed qbo-transaction-report" aria-label="Local bill evidence detail">
    ...
    {(bill.je_number||bill.pay_je_number) && <div className="row-acts" style={{margin:'10px 0'}}>
      {bill.je_number&&<Btn size="sm" variant="ghost" onClick={()=>goto('je',{jeNumber:bill.je_number,expenseReturn:{route:'ap',tab:'Bills',billId:bill.bill_id,...evidenceReturnContext}})}>Open AP JE</Btn>}
      {bill.pay_je_number&&<Btn size="sm" variant="ghost" onClick={onOpenPayment}>Open payment detail</Btn>}
      {bill.pay_je_number&&<Btn size="sm" variant="ghost" onClick={()=>goto('je',{jeNumber:bill.pay_je_number,expenseReturn:{route:'ap',tab:'Bills',billId:bill.bill_id,...evidenceReturnContext}})}>Open payment JE</Btn>}
      {bill.voidEvidence?.apReversals?.[0]&&<Btn size="sm" variant="ghost" onClick={()=>goto('je',{jeNumber:bill.voidEvidence.apReversals[0].je_number,expenseReturn:{route:'ap',tab:'Bills',billId:bill.bill_id,...evidenceReturnContext}})}>Open reversal JE</Btn>}
    </div>}
    ...
    {trace.canOpenSourceDocument && <div className="row-acts" style={{margin:'10px 0'}}><Btn size="sm" variant="ghost" onClick={()=>goto('sourcedocs',{route:'sourcedocs',docId:trace.sourceDocId,jeNumber:trace.apJournal.je_number,sourceSystem:trace.apJournal.source_system,expenseReturn:{route:'ap',tab:'Bills',billId:bill.bill_id,...evidenceReturnContext}})}>Open local source document</Btn></div>}
...
function ExpenseReviewExceptionDetail({exception, onClose, onOpenSource, ctx}) {
  ...
  return <div className="full-bleed qbo-transaction-report" aria-label="Local expense review exception detail">
    ...
    <div className="row-acts" style={{marginTop:12}}>
      ...
      <Unavailable reason="Exception resolution is not executed from this evidence view.">Resolve locally</Unavailable>
    </div>
  </div>;
}
...
function VendorCreditDetail({credit, agingReturn, onClose, onOpenBill, ctx}) {
  ...
  return <div className="full-bleed qbo-transaction-report" aria-label="Local vendor credit evidence detail">
    ...
    <div className="row-acts" style={{margin:'12px 0'}}>
      ...
      {credit.bill ? <Btn size="sm" variant="ghost" onClick={()=>onOpenBill(credit.bill.bill_id)}>Open linked Bill</Btn> : <Unavailable reason="This credit has no retained linked bill.">No linked local Bill</Unavailable>}
      {bankAccountCode ? <Btn size="sm" variant="ghost" onClick={()=>goto('bankrec',{route:'bankrec',acctCode:bankAccountCode,bankTxnId:bankLink?.bank_txn_id || null})}>Open local reconcile evidence</Btn> : <Unavailable reason="No retained bank evidence is mapped to this record.">No retained bank drill</Unavailable>}
    </div>
