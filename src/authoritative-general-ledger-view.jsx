...
export function AuthoritativeGeneralLedgerView({eyebrow,title,subtitle,periodCaption,actions,children}){
  return <section className="full-bleed authoritative-general-ledger authoritative-workbench-shell authoritative-general-ledger-presentation" aria-label="Authoritative General Ledger">
    <div className="accounting-page-head"><div><div className="page-eyebrow">{eyebrow}</div><h1 className="page-h">{title}</h1>{periodCaption&&<p className="authoritative-report-period-caption">{periodCaption}</p>}<p className="page-subtitle">{subtitle}</p></div><div className="row-acts"><span className="badge badge-muted">READ ONLY</span>{actions}</div></div>
    {children}
  </section>;
}
