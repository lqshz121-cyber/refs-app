...
const wbsTestRangeImportResult=value=>{
  if(!exactObjectKeys(value,['bank','date_from','date_to','page_size','payables','status','test_only'])||value.status!=='WBS_TEST_RANGE_IMPORT_COMPLETE'||value.test_only!==true||!validDate(value.date_from)||!validDate(value.date_to)||value.date_from>value.date_to||!Number.isSafeInteger(value.page_size)||value.page_size<1||value.page_size>10)return null;
  const payable=value.payables,bank=value.bank;
  if(!exactObjectKeys(payable,['imported_count','page_count','posted_count','record_count','replayed_count'])||!['imported_count','page_count','posted_count','record_count','replayed_count'].every(field=>Number.isSafeInteger(payable[field])&&payable[field]>=0&&payable[field]<=500)||payable.page_count>50||payable.posted_count!==payable.imported_count+payable.replayed_count)return null;
  if(!exactObjectKeys(bank,['bank_source_ids','provider_page_count','reconciliations','record_count'])||!Number.isSafeInteger(bank.provider_page_count)||bank.provider_page_count<0||bank.provider_page_count>50||!Number.isSafeInteger(bank.record_count)||bank.record_count<0||bank.record_count>500||!Array.isArray(bank.reconciliations)||bank.reconciliations.length>6||bank.reconciliations.some(row=>!exactObjectKeys(row,['bank_account_ref','period_code','reconciliation_id','transaction_count'])||!/^2026-0[1-6]$/.test(row.period_code||'')||row.bank_account_ref!==`WBS_TEST_BANK_${row.period_code.replace('-','_')}`||!UUID.test(row.reconciliation_id||'')||!Number.isSafeInteger(row.transaction_count)||row.transaction_count<1||row.transaction_count>500)||bank.reconciliations.reduce((sum,row)=>sum+row.transaction_count,0)!==bank.record_count||!Array.isArray(bank.bank_source_ids)||bank.bank_source_ids.length!==bank.record_count||bank.bank_source_ids.some(id=>!UUID.test(id))||new Set(bank.bank_source_ids).size!==bank.bank_source_ids.length)return null;
  return Object.freeze({...value,payables:Object.freeze({...payable}),bank:Object.freeze({...bank,reconciliations:Object.freeze(bank.reconciliations.map(row=>Object.freeze({...row}))),bank_source_ids:Object.freeze([...bank.bank_source_ids])})});
};

export async function importAuthoritativeWbsTestRange({config,companyCode,dateFrom='2026-01-01',dateTo='2026-06-30',pageSize=10,maxPages=50,fetcher=globalThis.fetch,cryptoApi=globalThis.crypto}={}){
  const company=typeof companyCode==='string'?companyCode.trim():'';
  if(config?.wbsTestImportMode!=='ENABLED'||typeof fetcher!=='function'||!wbsScopeText(company,64)||dateFrom!=='2026-01-01'||dateTo!=='2026-06-30'||!Number.isSafeInteger(pageSize)||pageSize<1||pageSize>10||!Number.isSafeInteger(maxPages)||maxPages<1||maxPages>50||typeof cryptoApi?.subtle?.digest!=='function')return {ok:false,code:'WBS_TEST_RANGE_IMPORT_COMMAND_INVALID',message:'Range import requires TEST_ONLY mode, the exact 2026 H1 WBS scope, and paging bounded to 500 rows per source view.'};
  let idempotencyKey;try{const canonical=JSON.stringify(['RANGE',company,dateFrom,dateTo,pageSize,maxPages]);idempotencyKey=`wbs-test-range-${hex(await cryptoApi.subtle.digest('SHA-256',new TextEncoder().encode(canonical)))}`;}catch{return {ok:false,code:'WBS_TEST_RANGE_IMPORT_COMMAND_INVALID',message:'The browser could not derive the stable range-import identity.'};}
  const authorization=await authoritativeBearerHeaders(config);if(!authorization)return authenticationRequired();
  const body={companyCode:company,dateFrom,dateTo,pageSize,maxPages};
  try{const response=await fetcher(`${config.baseUrl}/api/v1/entities/${config.entityId}/wbs/test-import/range`,{method:'POST',credentials:'include',cache:'no-store',headers:{accept:'application/json','content-type':'application/json','idempotency-key':idempotencyKey,...authorization},body:JSON.stringify(body)});if(!response.ok)return await failure(response,'WBS_TEST_RANGE_IMPORT');const envelope=await response.json(),data=wbsTestRangeImportResult(envelope?.data);if(envelope?.ok!==true||data===null||data.date_from!==dateFrom||data.date_to!==dateTo||data.page_size!==pageSize)return {ok:false,code:'WBS_TEST_RANGE_IMPORT_PROTOCOL',message:'The range-import endpoint returned an invalid, mismatched, or non-test result.'};return {ok:true,data};}catch{return unreachable('The browser could not run the paged WBS test range import; no HTTP response was produced.');}
}
