...
const wbsTestMonthImportResult=value=>{
  if(!exactObjectKeys(value,['bank','date_from','date_to','page_size','payables','period_code','status','test_only'])||value.status!=='WBS_TEST_MONTH_IMPORT_COMPLETE'||value.test_only!==true||!/^2026-0[1-6]$/.test(value.period_code||'')||value.date_from!==`${value.period_code}-01`||value.date_to!==new Date(Date.UTC(2026,Number(value.period_code.slice(5,7)),0)).toISOString().slice(0,10)||value.page_size!==10)return null;
  ...
  if(!exactObjectKeys(bank,['bank_source_ids','provider_page_count','reconciliation','record_count'])||!Number.isSafeInteger(bank.provider_page_count)||bank.provider_page_count<0||bank.provider_page_count>1000||!Number.isSafeInteger(bank.record_count)||bank.record_count<0||bank.record_count>10000||!Array.isArray(bank.bank_source_ids)||bank.bank_source_ids.length!==bank.record_count||bank.bank_source_ids.some(id=>!UUID.test(id||''))||new Set(bank.bank_source_ids).size!==bank.bank_source_ids.length||(bank.record_count===0?reconciliation!==null:!exactObjectKeys(reconciliation,['bank_account_ref','period_code','period_id','reconciliation_id','transaction_count'])||reconciliation.period_code!==value.period_code||reconciliation.bank_account_ref!==`WBS_TEST_BANK_${value.period_code.replace('-','_')}`||!UUID.test(reconciliation.period_id||'')||!UUID.test(reconciliation.reconciliation_id||'')||reconciliation.transaction_count!==bank.record_count))return null;
