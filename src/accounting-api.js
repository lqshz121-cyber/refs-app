...
const AI_FINDING_ACTION_FIELDS=['ai_finding_action_id','finding_kind','finding_id','finding_hash','owner','due_date','revision','assigned_by','assigned_at','can_create_draft','can_review','can_approve','can_post'];
const aiFindingAction=row=>exactObjectKeys(row,AI_FINDING_ACTION_FIELDS)&&UUID.test(row.ai_finding_action_id||'')&&AI_ANALYSIS_CATEGORIES.has(row.finding_kind)&&UUID.test(row.finding_id||'')&&SHA256.test(row.finding_hash||'')&&nullableBoundedText(row.owner,128)&&validDate(row.due_date)&&Number.isSafeInteger(Number(row.revision))&&Number(row.revision)>=0&&nullableBoundedText(row.assigned_by,128)&&validTimestamp(row.assigned_at)&&row.can_create_draft===false&&row.can_review===false&&row.can_approve===false&&row.can_post===false?Object.freeze({...row,revision:Number(row.revision)}):null;
