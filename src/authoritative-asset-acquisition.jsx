...
export function AuthoritativeAssetAcquisition({config,assetId,fetcher=globalThis.fetch,onOpenJournalWorkflow}){
 ...
 const load=async()=>{if(pending.current)return;const token=++generation.current;setOpen(true);setState({phase:'LOADING'});setError(null);const result=await readAuthoritativeAcquisitionOptions({config,assetId,fetcher});if(token!==generation.current)return;
  ...
  setState({phase:'READY',data:result.data});setNumber(current=>current||('FA-'+result.data.asset_tag).slice(0,100));setDate(current=>current||result.data.source.accounting_date);
 };
 ...
 return <section aria-label="Asset acquisition">
  ...
  {open?<div className="report-workbench"><div className="report-workbench-head"><h3 ref={heading} tabIndex={-1}>Record acquisition</h3><button type="button" className="btn" onClick={close} disabled={saving}>Close acquisition</button></div>
   {saved?<div role="status"><h4 ref={resultHeading} tabIndex={-1}>Acquisition draft saved</h4><p>The invoice and attachments are linked to the journal. Continue with its review and approval.</p>{onOpenJournalWorkflow?<button type="button" className="btn" onClick={()=>onOpenJournalWorkflow(saved)}>Open journal</button>:null}</div>:<>
   ...
   {options?<><dl className="fixed-assets-detail-grid">{[['Asset',options.asset_tag],['Invoice',options.source.document_no||'Source invoice'],['Original cost',options.currency+' '+options.cost_basis],['Accounting period',options.period.period_code],['Asset account',options.asset_account_code],['Liability account',options.liability_account_code],['Vendor',options.vendor_ref||'Not recorded']].map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
