...
export function AuthoritativeAssetAcquisition({config,assetId,fetcher=globalThis.fetch,onOpenJournalWorkflow}){
 ...
 const generation=useRef(0),pending=useRef(false),launch=useRef(null),heading=useRef(null),resultHeading=useRef(null),journalNumberRef=useRef(null);
 useEffect(()=>()=>{generation.current++;},[]);
 useEffect(()=>{if(open)heading.current?.focus();},[open]);
 useEffect(()=>{if(state.phase==='LOADING')heading.current?.focus();},[state.phase]);
 useEffect(()=>{if(saved)resultHeading.current?.focus();},[saved]);
 useEffect(()=>{if(newDraft)journalNumberRef.current?.focus();},[newDraft]);
 ...
 return <section aria-label="Asset acquisition">
  ...
  {open?<div className="report-workbench"><div className="report-workbench-head"><h3 ref={heading} tabIndex={-1}>Record acquisition</h3><button type="button" className="btn" onClick={close} disabled={saving}>Close acquisition</button></div>
   ...
   {options?<><dl className="fixed-assets-detail-grid">{[['Asset',options.asset_tag],['Invoice',options.source.document_no||'Source invoice'],['Original cost',options.currency+' '+options.cost_basis],['Accounting period',options.period.period_code],['Source accounting date',options.source.accounting_date],['Placed in service',options.placed_in_service_date],['Asset account',options.asset_account_code],['Liability account',options.liability_account_code],['Vendor',options.vendor_ref||'Not recorded']].map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
