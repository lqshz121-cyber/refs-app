...
import { copyJEAsDraft, createReclassDraft, createRecurringTemplate, createReversal, rejectJETransition, saveJEDraft, transitionJE, validateJETransition } from './src/je-workflow.js';
...
const canAll=()=>true;
const manualDraft={je_id:7001,je_number:'JE-7001',entity_id:4,period_code:'2026-07',je_date:'2026-07-31',je_type:'MANUAL',source_system:'MAN',posting_status:'DRAFT',created_by:'maker',has_attachment:true,description:'Monthly accrual',revision:0,history:[{a:'CREATE',by:'maker',at:'2026-07-31'}],lines:[{account_code:'651000',debit_amount:100,credit_amount:0},{account_code:'111000',debit_amount:0,credit_amount:100,member:'Operating Cash_BA-003'}]};
expectJE('JE illegal Draft to Posted jump is blocked',validateJETransition({je:manualDraft,next:'POSTED',user:maker,period:{status:'OPEN'},can:canAll}).code==='JE_ILLEGAL_TRANSITION');
expectJE('JE manual attachment gate blocks submission',validateJETransition({je:{...manualDraft,has_attachment:false},next:'PENDING_REVIEW',user:maker,period:{status:'OPEN'},can:canAll}).code==='4010');
const submitted=transitionJE({je:manualDraft,next:'PENDING_REVIEW',user:maker,period:{status:'OPEN'},can:canAll});
const reviewed=transitionJE({je:submitted.je,next:'PENDING_APPROVAL',user:reviewer,period:{status:'OPEN'},can:canAll});
expectJE('JE Draft to Review to Approval queue is sequential',submitted.ok&&reviewed.ok&&reviewed.je.reviewer==='reviewer');
expectJE('JE maker cannot approve own entry',transitionJE({je:reviewed.je,next:'APPROVED',user:maker,period:{status:'OPEN'},can:canAll}).code==='JE_SOD_MAKER');
const approved=transitionJE({je:reviewed.je,next:'APPROVED',user:approver,period:{status:'OPEN'},can:canAll});
expectJE('JE approver recorded at action boundary',approved.ok&&approved.je.approver==='approver');
expectJE('JE maker cannot post own entry',transitionJE({je:approved.je,next:'POSTED',user:maker,period:{status:'OPEN'},can:canAll}).code==='JE_SOD_MAKER');
expectJE('JE approver cannot also post entry',transitionJE({je:approved.je,next:'POSTED',user:approver,period:{status:'OPEN'},can:canAll}).code==='JE_SOD_APPROVER_POSTER');
const posted=transitionJE({je:approved.je,next:'POSTED',user:poster,period:{status:'OPEN'},can:canAll});
expectJE('JE separate poster completes Posted state',posted.ok&&posted.je.posted_by==='poster');
expectJE('JE Posted cannot Cancel Post or move backward',validateJETransition({je:posted.je,next:'APPROVED',user:poster,period:{status:'OPEN'},can:canAll}).code==='JE_IMMUTABLE');
expectJE('JE closed period blocks workflow',validateJETransition({je:manualDraft,next:'PENDING_REVIEW',user:maker,period:{period_code:'2026-07',status:'CLOSED'},can:canAll}).code==='4005');
...
expectJE('JE reject returns review item to Draft with history',rejectJETransition({je:submitted.je,user:reviewer,can:canAll}).je?.posting_status==='DRAFT');
const autoNoTrace={...manualDraft,je_type:'AUTO',source_system:'BANK',source_doc_id:null,rule_code:null};
expectJE('JE automatic source trace is mandatory',validateJETransition({je:autoNoTrace,next:'PENDING_REVIEW',user:maker,period:{status:'OPEN'},can:canAll}).code==='JE_AUTO_TRACE_MISSING');
const tracedAuto={...manualDraft,je_type:'AUTO',source_system:'BANK',source_doc_id:'BANK-7001',rule_code:'R-BANK-01',setting_used:'SET-BANK',mapping_used:'MAP-BANK'};
const protectedAuto=saveJEDraft({current:tracedAuto,draft:{...tracedAuto,source_doc_id:'TAMPERED',rule_code:'TAMPERED',setting_used:null,mapping_used:null},user:maker});
expectJE('JE automatic source, setting, mapping and rule trace are immutable on Save',protectedAuto.ok&&protectedAuto.je.source_doc_id==='BANK-7001'&&protectedAuto.je.rule_code==='R-BANK-01'&&protectedAuto.je.setting_used==='SET-BANK'&&protectedAuto.je.mapping_used==='MAP-BANK');
