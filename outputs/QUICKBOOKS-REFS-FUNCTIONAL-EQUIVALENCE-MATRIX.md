...
| Capability | Newly observed QuickBooks evidence | REFS implementation | Status |
|---|---|---|---|
| Expenses entry point and scoped Bill queue | Read-only QBO homepage audit showed the `Expenses & Pay Bills` quick link and a separate `Record expense` create action. Neither action was invoked. The home surface also shows unrelated payment, payroll, sales, and promotional actions. | The REFS Expenses workspace retains Bills, Payments, AP Aging, and Vendors; Bills keeps read-only search, transaction type, date, status, vendor, and category filters plus a full-page evidence drill and exact Back scope. The fixed table now always exposes the retained Due date alongside date, payee, category, amount, approval, and local proof. | PARTIAL — local implementation and offline verifiers passed. The populated QBO Expenses queue, its permissions, detailed filter behavior, audit history, responsive layout, and all mutation behavior remain unobserved. |
