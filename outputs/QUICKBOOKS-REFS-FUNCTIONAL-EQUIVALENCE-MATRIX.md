...
- **REFS implementation:** Secondary-navigation labels use a compact readable type treatment. The header retains the full `Public demonstration data` explanation on wide screens, a clear `Demo data` label on compact desktop/tablet, and `Demo` on phone while preserving the full accessible name. Compact desktop now uses the signed-in person's short name rather than clipping a longer role-bearing display name; the full identity remains accessible. The tablet off-canvas navigation uses a 320px readable panel so long workspace names are not truncated. At phone width, the signed-in chip retains both avatar and Sign out inside an 82px no-overflow footprint.
...
## REFS authoritative consolidation and elimination evidence (2026-08-10)

- **Observed product evidence:** No QBO cross-company consolidation behavior was invoked or inferred.
- **REFS implementation:** The authoritative Reports workspace reads only a separately approved immutable consolidation snapshot. Every member entity and aligned period, presentation-account mapping, source/receipt/snapshot hash, and elimination evidence reference is explicit. Only same-currency POSTED ledger lines contribute. A missing member grant, period/currency alignment, posted member evidence, mapping, or elimination evidence produces a BLOCKED result rather than a zero or inferred conclusion.
- **Hard boundary:** This is a read-only report. It never creates a consolidation, an elimination journal, an adjustment, a mapping, or a budget. The current live environment has no approved consolidation snapshot, so an empty result is expected and is not a financial conclusion.
- **Status:** **VERIFIED LOCAL / PARTIAL LIVE** — PostgreSQL 18 fresh migration and a two-member scope test cover denied-member, missing-elimination, approved-zero-elimination and zero-ELIMINATION-JE paths. Authenticated current-release browser evidence and real approved group data remain pending.
- **Verification:** `npm.cmd run test:authoritative-reports`; `node --test server/tests/financial-statements-read-contract.test.mjs`; `POSTGRES_IMAGE=postgres:18-alpine npm.cmd run test:postgres:fresh`; `git diff --check`.

## Authority site: A/R Aging full-page read-only evidence and return (2026-08-11)

- **Observed QBO evidence:** Standard Reports exposes Accounts Receivable Aging as a report surface with an explicit return to the report catalog, as-of scope, a ready/empty result, and a horizontally dense table. QBO save, email, print, export, customize, payments, and provider connection controls were visible but not used and are outside REFS scope.
- **Authority implementation:** The authenticated Receivables workspace now opens A/R Aging as a full-page surface. It freezes the existing invoices-and-receipts list context, preserves it through Back, displays immutable entity/period/as-of scope, uses only the existing accounting API GET readers, and keeps horizontal overflow inside the table. API scope/authentication/protocol failures render `BLOCKED — authoritative evidence unavailable`; an empty response is explicitly not a zero-balance assertion.
- **Boundary and gap:** This is not a claim of production data equivalence. Browser/OIDC state is required at runtime; authenticated browser/API E2E against a deployed authority endpoint remains unverified. No save/email/print/export/customize, mutation, provider call, local storage, seed, or demo state is introduced.
- **Verification:** `npm.cmd run test:authoritative-aging` (exit 0); `npm.cmd run test:authoritative-full-shell` (exit 0); `npm.cmd run build` (exit 0); `npm.cmd run test:visual` (exit 0); `git diff --check` (exit 0).

## Authority site: statement refresh retains the report workspace (2026-08-11)

- **Observed QBO evidence:** Read-only QBO Balance Sheet opens from Standard Reports into a separate page with an explicit `Back to standard reports` path, report period, date range, accounting method, loading state, and a refresh control. Save, Customize, Email, Print, Export, and Insights were visible but not used.
...
## Accounting: Chart of Accounts and Register evidence boundary (2026-08-11)

- **Observed QBO evidence:** The authenticated, read-only Chart of Accounts page showed the account-list surface, `Run report`, `New account`, batch actions, pagination, and the columns Name, Account Type, QuickBooks Balance, Bank Balance, and Action. Bank-type rows displayed both QuickBooks and bank balances; non-bank rows did not show a bank balance. `View register` appeared on observed account rows. None of the actions was selected.
- **REFS adaptation:** The authority site must keep local chart-of-account and register/GL evidence scoped to the authenticated entity and account. Cash accounts may enter the separate reconciliation evidence workflow; non-cash accounts remain GL/Register evidence only. An authority COA/Register GET read model is not yet wired, so the current authority route must remain unavailable rather than fall back to legacy/demo rows.
- **Boundary and gap:** QBO account creation, batch editing, account edits/deletes/merges, feeds, imports, exports, and its bank balance are not copied. QBO register detail, permissions, populated account variants, and responsive behavior remain unverified. This is observation and scope guidance, not a QBO-equivalence claim.

## Authority site: COA/Register contract is visible before the reader exists (2026-08-11)

- **Implementation:** The visible Chart of Accounts authority route now describes the exact signed-in read contract required before it can render: immutable account identity/revision/type/normal side/local-balance evidence and an account/date/query/page-scoped Register reader with retained GL, Journal Entry, and source-document references. It also makes the REFS boundary explicit: only cash accounts may continue to the separate authorised Reconciliation reader; non-cash accounts remain Register/GL evidence.
- **Boundary and gap:** The route still fails closed with no browser, demonstration, local-storage, or inferred balances. It adds no QBO account creation, batch actions, export/print, feed, or bank balance feature. The required authority API endpoint and authenticated deployed E2E are not yet available, so this remains **PARTIAL**, not a COA/Register implementation or QBO-equivalence claim.
- **Verification:** `npm.cmd run test:authoritative-full-shell`; `npm.cmd run test:authoritative-bank`; `git diff --check`.

## Authority site: Expenses list fact detail now exposes its evidence boundary (2026-08-11)

- **Observed QBO evidence:** The authenticated, read-only Expenses page displayed an `Expenses` heading, `New transaction`, Transaction Type, Filter, a `Dates: Last 12 months` control, disabled export/print, and the English empty state `No expenses found` with a filter-adjustment hint. Its filter popover exposed Status, Delivery method, Date, From, To, Reset, and Apply. The Expenses & Bills navigation contained Expense transactions, Vendors, Bills, and Bill payments. No QBO create, apply, print, export, or payment action was used.
- **Authority implementation:** A full-page Bill or Invoice evidence view now displays its immutable document ID and list revision plus the exact query/page context that Back restores. It explicitly renders `Authoritative lineage unavailable` if the existing list reader has not returned a receipt, mapping, audit, journal-line, or ledger-line chain; it does not infer one from list facts.
- **Boundary and gap:** The authority AP list retains only its API-backed Status, date range, search, and AP offset-account filter. QBO Delivery method, payment promotion, provider controls, New transaction, print/export, and the broader Bills navigation are not copied. QBO populated list detail and responsive row behavior remain unverified, so this remains **PARTIAL** and is not a QBO-equivalence claim.
- **Verification:** `npm.cmd run test:authoritative-documents`; `npm.cmd run test:authoritative-full-shell`; `npm.cmd run build`; `git diff --check`.

## Authority site: Journal list fact detail retains its immutable ID (2026-08-11)

- **Observed QBO evidence:** The authenticated QBO Journal Entry surface showed a journal number and date, a line grid with Account, Debits, Credits, Description, Name, and Class, total debit/credit values, Memo, Attachments, History, and editing controls (add/duplicate/delete lines, Save, Save and new, recurring, export). It was inspected without typing, saving, attaching, exporting, or changing a line.
- **Authority implementation:** The existing full-page Journal evidence surface now exposes the API-returned immutable Journal ID beside its display number, status, revision, dates, and list-return context. Its existing `Authoritative lineage unavailable` panel remains the fail-closed outcome when the list reader lacks Journal Lines, ledger-line IDs, source documents, mappings, or audit events.
- **Boundary and gap:** No QBO line editor, totals editor, memo/attachment write, History mutation, Save, recurring, or export control is copied. QBO populated account lines, audit history, permissions, and mobile/modal behavior remain unverified. This is **PARTIAL** evidence adaptation, not Journal Entry equivalence.
- **Verification:** `npm.cmd run test:authoritative-journals`; `npm.cmd run build`; `git diff --check`.

## Authority site: Financial-statement detail fails closed on incomplete lineage (2026-08-11)

- **Observed QBO evidence:** The authenticated, read-only Standard Reports catalog showed core report links and a `Back to standard reports` route convention. No populated report row, account drill, source document, or Journal Entry drill was selected, so their lineage and permission behavior remains unobserved.
- **Authority implementation:** A full-page financial-statement account detail now shows `POSTED EVIDENCE` only when the API supplies Journal Entry, Journal Line, ledger-line, and source-document IDs together. A partial row remains visible as the scoped report fact, but shows `BLOCKED — authoritative lineage unavailable`; its existing Back path remains available and no inferred Journal Entry or source navigation is offered.
- **Boundary and gap:** This does not remove an existing Controller workflow or change reports data. It is a rendering guard over OIDC/API read data only; it adds no refresh command, adjustment, payment, mapping, export, save, customize, or provider/WBS call. QBO populated report-drill behavior is unverified, so this remains **PARTIAL**, not an equivalence claim.
- **Verification:** `npm.cmd run test:authoritative-reports`; `npm.cmd run build`; `git diff --check` (authenticated deployed E2E remains pending).
