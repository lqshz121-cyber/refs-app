...
assert.match(markup,/ACCOUNTING/);assert.match(markup,/Chart of Accounts/);assert.match(markup,/Search/);assert.match(markup,/Filter by name or number/);assert.match(markup,/Status/);assert.match(markup,/Currency/);assert.match(markup,/Rows per page/);assert.match(markup,/READ ONLY/);assert.match(markup,/Loading accounts and posted balances/);assert.doesNotMatch(markup,/GENERAL LEDGER \| ACCOUNT MASTER|Account name or number|Master status|Posted currency|Loading authoritative account master|Accounts returned|Exact API snapshot/,'COA must use concise accounting copy without repeating implementation labels or summary cards');
...
assert.match(source,/Review accounts and posted balances\./);assert.match(source,/'Loading…':'Refresh'/);assert.doesNotMatch(source,/Review accounts and posted balances by currency|Entity-scoped account master facts|All retained currencies|No entity-scoped account-master row/);
