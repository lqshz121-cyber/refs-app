...
import {AuthoritativeDemoAccountRegisterView,AuthoritativeDemoChartOfAccountsView} from '../src/authoritative-demo-coa-view.jsx';
...
const presentation=fs.readFileSync(path.join(process.cwd(),'src','authoritative-demo-coa-view.jsx'),'utf8');
...
assert.match(source,/AuthoritativeDemoChartOfAccountsView/);assert.match(source,/AuthoritativeDemoAccountRegisterView/);assert.match(source,/Chart of Accounts reading path/);assert.match(source,/Account register reading path/);
assert.match(presentation,/demo-coa-presentation/);assert.match(presentation,/demo-account-register-presentation/);
assert.doesNotMatch(presentation,/localStorage|from ['"]\.\/repo|from ['"]\.\/seed|from ['"]\.\/data|accounting-api|legacy-demo-app/);
assert.match(renderToStaticMarkup(<AuthoritativeDemoChartOfAccountsView><div>API slot</div></AuthoritativeDemoChartOfAccountsView>),/API slot/);
assert.match(renderToStaticMarkup(<AuthoritativeDemoAccountRegisterView><div>API register slot</div></AuthoritativeDemoAccountRegisterView>),/API register slot/);
