...
const presentation=fs.readFileSync(path.join(process.cwd(),'src','authoritative-demo-source-documents-view.jsx'),'utf8');
...
assert.match(source,/AuthoritativeDemoSourceDocumentsView/,'Source Documents must use the mechanically extracted demo register presentation');
assert.match(presentation,/full-bleed.*demo-source-documents-presentation/,'Source Documents must retain the demo full-page register frame');
...
const presentationMarkup=renderToStaticMarkup(<AuthoritativeDemoSourceDocumentsView scope="API scope" metrics={<span>1</span>} actions={<button>Refresh</button>}><div>API slot</div></AuthoritativeDemoSourceDocumentsView>);
