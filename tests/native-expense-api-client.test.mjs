...
const options={schema_version:'NATIVE_EXPENSE_CREATE_OPTIONS_V1',entity_id:entityId,period_id:periodId,vendors:[{vendor_ref:'VENDOR-1',vendor_name:'Vendor One'}],bank_cash_accounts:[{bank_member_ref:'BANK-1',bank_name:'Main Bank',cash_account_code:'100100',cash_account_name:'Cash'}],expense_accounts:[{expense_account_code:'610100',expense_account_name:'Utilities'}],attachments:[{attachment_id:attachmentId,name:'Invoice.pdf',media_type:'application/pdf',size_bytes:'1200',content_hash:'sha256:'+'a'.repeat(64),verified_at:'2026-09-13T00:00:00.000000Z'}]};
...
test('expense client blocks stale selections and invalid successful receipts before accepting a save',async()=>{
 ...
 for(const patch of [{input:{...input,vendorRef:'VENDOR-404'}},{input:{...input,amount:'0.0000'}},{input:{...input,attachmentIds:[randomUUID()] }},{input:{...input,reason:'short'}},{input:{...input,date:'not-a-date'}},{options:{...options,entity_id:randomUUID()}},{config:{}}])assert.equal((await createAuthoritativeNativeExpense({config,options,input,fetcher,cryptoApi:webcrypto,...patch})).ok,false);assert.equal(calls,0);
