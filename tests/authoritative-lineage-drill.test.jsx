...
const liveReport={...report};delete liveReport.currency;
assert.equal(reportRowContainsLedger(liveReport,gl),true,'the live financial-statement contract has no row currency and must bind through period plus immutable journal, line, ledger, and source identities');
assert.equal(reportRowContainsLedger({...report,currency:'EUR'},gl),false,'an explicitly returned report currency must match the ledger currency');
assert.equal(reportRowContainsLedger({...report,currency:''},gl),false,'a malformed explicit report currency must not be treated as an omitted contract field');
for(const changed of [{period_id:entityId},{account_code:'620000'},{journal_entry_ids:[]},{journal_line_ids:[]},{ledger_line_ids:[]},{source_document_ids:[]}])assert.equal(reportRowContainsLedger({...liveReport,...changed},gl),false,'report lineage must retain the exact scope and complete immutable ledger relationship');
