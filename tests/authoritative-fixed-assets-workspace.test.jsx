...
import {AuthoritativeFixedAssetsWorkspace,localAccountingDate,canCreateAssetAcquisition,canCreateAssetDepreciation,canCreateAssetDisposal} from '../src/authoritative-fixed-assets-workspace.jsx';
...
const disposalSource=fs.readFileSync('src/authoritative-asset-disposal.jsx','utf8');for(const label of ['Record asset disposal','Disposal source','Proceeds account','Bank / customer member','Gain / loss account','Calculated gain / loss','Save disposal draft'])assert.match(disposalSource,new RegExp(label));
assert.doesNotMatch(disposalSource,/localStorage|parseFloat|\bNumber\(/);assert.match(disposalSource,/source\.gross_amount/);assert.match(disposalSource,/options\.snapshot\.carrying_value/);
...
const disposalAccess={...makerAccess,row:{...makerAccess.row,permissions:[...makerAccess.row.permissions.filter(p=>p!=='FIXED_ASSET.ACQUISITION.DRAFT'),'FIXED_ASSET.DISPOSAL.DRAFT']}};
assert.equal(canCreateAssetDisposal(depreciationConfig,disposalAccess),true);assert.equal(canCreateAssetDisposal(makerConfig,disposalAccess),false,'selected period required');
for(const permission of disposalAccess.row.permissions)assert.equal(canCreateAssetDisposal(depreciationConfig,{...disposalAccess,row:{...disposalAccess.row,permissions:disposalAccess.row.permissions.filter(p=>p!==permission)}}),false,'missing '+permission);
