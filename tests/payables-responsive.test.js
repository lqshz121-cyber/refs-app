...
assert.match(styles,/@media\(max-width:900px\)\{[\s\S]*?\.expense-toolbar>\*\{min-width:0;max-width:100%;\}[\s\S]*?\.expense-toolbar>label:not\(\.expense-search\)>select\{width:100%;max-width:100%;\}/,
  'each Payables toolbar item and its Vendor/date select must be able to shrink inside its grid track');
