...
import {accountingApiConfig,applyAuthoritativeCredit,createAuthoritativeAdjustment,createAuthoritativeBankPaymentMatch,createAuthoritativeBusinessDocument,createAuthoritativeSettlement,refreshAuthoritativeBankTransactions,refreshAuthoritativeBankMatchCandidates,refreshAuthoritativeDocuments,refreshAuthoritativeJournalEntries,refreshAuthoritativeReconciliation,refreshAuthoritativeReconciliationWorksheet,setAuthoritativeReconciliationClearance,startAuthoritativeReconciliation,transitionAuthoritativeReconciliation,transitionAuthoritativeJournal,unmatchAuthoritativeBankPayment,refreshAuthoritativeAging,refreshAuthoritativeControlTotals} from '../src/accounting-api.js';
...
(async()=>{
  ...
  assert.equal(candidateRead.ok,true);assert.equal(candidateRead.candidates[0].occurrence_version,1);assert.equal(candidateRead.candidates[0].amount,125.25);assert.match(candidateCall.url,/\/bank\/transactions\/.+\/match-candidates$/);assert.equal(candidateCall.options.method,'GET');assert.equal(candidateCall.options.headers.authorization,`Bearer ${accessToken}`);assert.equal('body' in candidateCall.options,false);
