...
assert.match(workspaceMarkup,/<label>Transaction Type <select aria-label="Transaction Type"><option value="ALL" selected="">All transactions<\/option>/,'the mixed AP state must use the observed compact QBO transaction-type selector');
...
assert.match(workspaceSource,/bill\?<label>Transaction Type <select aria-label="Transaction Type"/,'Expenses must use one QBO-style transaction-type control rather than a second tab navigation');
assert.match(workspaceMarkup,/>Vendor credit<\/option>/);
...
assert.doesNotMatch(workspaceMarkup,/role="tablist" aria-label="Expenses evidence views"|class="tab tab-on"/,'Expenses must not retain the wide evidence-tab strip beside its transaction-type selector');
assert.match(workspaceMarkup,/>Vendor credit<\/option>/);
assert.match(workspaceMarkup,/AP Aging/);
assert.match(workspaceMarkup,/<option value="EXPENSE" disabled="">Expense — Not available<\/option>[\s\S]*?<option value="BILL_PAYMENT" disabled="">Bill payment — Not available<\/option>[\s\S]*?<option value="CHECK" disabled="">Check — Not available<\/option>[\s\S]*?<option value="PURCHASE_ORDER" disabled="">Purchase order — Not available<\/option>/,'unsupported QBO expense types must remain discoverable and fail closed inside the same selector');
assert.match(workspaceMarkup,/<option value="RECENTLY_PAID" disabled="">Recently paid — Not available<\/option>[\s\S]*?<option value="VENDOR_CREDITS">Vendor credit<\/option>[\s\S]*?<option value="ITEM_RECEIPT" disabled="">Item Receipt — Not available<\/option>[\s\S]*?<option value="RECEIPT_REMINDER" disabled="">Expense \(Receipt reminder\) — Not available<\/option>/,'the selector must preserve the complete observed QBO type order without inventing unavailable readers');
assert.doesNotMatch(workspaceMarkup,/More transaction types|Unavailable expense transaction types/,'the observed type list must not be repeated as a verbose disclosure');
...
assert.doesNotMatch(workspaceMarkup,/Vendors unavailable/,'Vendors remains an independent Expenses navigation route rather than a transaction type');
...
assert.doesNotMatch(workspaceMarkup,/class="tab-unavailable" role="tab" aria-selected="false" aria-disabled="true" aria-label="Vendors unavailable"/,'Vendors must not be presented as a disabled peer of transaction types');
...
assert.match(workspaceMarkup,/<summary aria-label="Filter, 6 active filters">Filter<\/summary>[\s\S]*?<label>Find <input[^>]*value="Evidence"/,'Expenses Find must stay inside the staged Filter rather than permanently lengthening the toolbar');
assert.doesNotMatch(workspaceMarkup,/>Search <input/,'Expenses must not retain a second always-visible query field');
assert.match(workspaceMarkup,/<summary aria-label="Filter, 6 active filters">Filter<\/summary>/,'Expenses must keep the observed concise QBO Filter label while retaining the active-filter count for assistive technology');
assert.doesNotMatch(workspaceMarkup,/>Filter \(5\)</,'Expenses must not expose its internal active-filter count in the compact visible toolbar');
assert.doesNotMatch(workspaceMarkup,/<summary>More filters/,'Expenses must not retain the longer disclosure label');
assert.match(workspaceMarkup,/<summary aria-label="Filter, 6 active filters">Filter<\/summary><div[^>]*><label>Find <input[\s\S]*?<label>Status <select/,'Expenses Find and Status must stay inside the observed compact Filter disclosure');
assert.equal((workspaceMarkup.match(/<label>Status <select/g)||[]).length,1,'Expenses must not duplicate Status in the primary toolbar');
...
assert.doesNotMatch(emptyExpenseMarkup,/authoritative-applied-scope|All records/,'default Expenses must not repeat a no-op scope line below its compact controls');
assert.match(emptyExpenseMarkup,/No expenses found/);
assert.match(emptyExpenseMarkup,/Change the filters to see more results\. Empty results do not confirm a zero balance\./);
...
assert.match(detail,/Retained document evidence\./);
assert.doesNotMatch(detail,/Record payment|Receive payment/);
...
assert.match(workspaceSource,/<label>\{bill\?'Payee':'Customer'\} <select value=\{filterDraft\.counterparty\}[\s\S]*?\{bill&&\(accountCodes\.length>0\?<label>Category <select value=\{filterDraft\.accountCode\}/,'the compact disclosure must stage the observed Expenses Payee, shared counterparty behavior, and AP-only Category');
