...
for(const boundary of ['VIEW ONLY','No WBS observation loaded','No credential, raw business identifier, accounting record, or action is exposed'])assert.match(dashboard,new RegExp(boundary,'i'));
assert.match(dashboard,/No WBS observation loaded/);
assert.match(dashboard,/Refresh WBS data/);
for(const liveFact of ['WBS data connection','Last update','Records returned','Test entity','Data source'])assert.match(dashboard,new RegExp(liveFact));

const payables=render(WBS_LIVE_PILOT_SURFACE_TOOLS.payables);
assert.match(payables,/View:<\/b> Payables/);
...
assert.match(payables,/title="Available after you have exception-evidence access and at least one WBS Payables row\."/,'the disabled exception-retain button must explain what is needed');
...
assert.match(bank,/View:<\/b> Bank transactions/);
...
assert.match(journal,/View:<\/b> Journal entries/);
