...
for(const boundary of ['VIEW ONLY','SIGNATURE NEEDED','NO JOURNAL CREATED'])assert.match(dashboard,new RegExp(boundary,'i'));
assert.match(dashboard,/No WBS records loaded/);
assert.match(dashboard,/Refresh WBS records/);
for(const liveFact of ['WBS records received','Latest refresh','Records received','Test entity','WBS service'])assert.match(dashboard,new RegExp(liveFact));

const payables=render(WBS_LIVE_PILOT_SURFACE_TOOLS.payables);
assert.match(payables,/Records to view:<\/b> Payables/);
...
for(const retainedUi of ['Retained WBS Payable review rows','Company scope status','View retained rows','View details','AWAITING SIGNED DELIVERY','GL / REPORT','Next owner'])assert.match(source,new RegExp(retainedUi));
...
assert.match(source,/row\.accounting_date\|\|'Not provided'/,'a missing provider accounting date must be explicit rather than presented as a false zero or generic unavailable state');
...
assert.match(fs.readFileSync('src/authoritative-wbs-live-pilot-observation.jsx','utf8'),/WBS company code.*WBS observation date from.*WBS observation date to/s,'the authoritative UI must expose the exact Provider-native company and date scope');
...
assert.match(fs.readFileSync('src/authoritative-wbs-live-pilot-observation.jsx','utf8'),/!hasExactAttestationScope.*Company assignment needed/s,'mixed or unresolved company results must remain visible and retainable only as exception evidence');
...
assert.match(bank,/Records to view:<\/b> Bank transactions/);
...
assert.match(journal,/Records to view:<\/b> Journal entries/);
