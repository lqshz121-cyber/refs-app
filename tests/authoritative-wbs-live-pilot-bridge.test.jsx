...
for(const progressCopy of ['completedMonths','MONTH_COMPLETE','of 6 months complete','Completed 2026 H1 import months'])assert.match(source,new RegExp(progressCopy),'the long-running H1 import must show resumable month-by-month progress');
...
async function testRangeImportClientContract(){
  const runtime=accountingApiConfig({__REFS_ACCOUNTING_API__:{...config,wbsTestImportMode:'ENABLED'}}),requests=[],progress=[];
  const monthData=index=>{const month=index+1,periodCode=`2026-${String(month).padStart(2,'0')}`,dateTo=new Date(Date.UTC(2026,month,0)).toISOString().slice(0,10),periodId=`${String(index+20).padStart(8,'0')}-0000-4000-8000-000000000001`,reconciliationId=`${String(index+30).padStart(8,'0')}-0000-4000-8000-000000000001`;return {status:'WBS_TEST_MONTH_IMPORT_COMPLETE',period_code:periodCode,date_from:`${periodCode}-01`,date_to:dateTo,page_size:10,payables:{provider_page_count:124,h1_record_count:1237,record_count:1,imported_count:1,replayed_count:0,posted_count:1},bank:{provider_page_count:1,record_count:1,reconciliation:{bank_account_ref:`WBS_TEST_BANK_${periodCode.replace('-','_')}`,period_code:periodCode,period_id:periodId,reconciliation_id:reconciliationId,transaction_count:1},bank_source_count:1},test_only:true};};
  const result=await importAuthoritativeWbsTestRange({config:runtime,companyCode:'WBPA',dateFrom:'2026-01-01',dateTo:'2026-06-30',onProgress:value=>progress.push(value),fetcher:async(url,options)=>{const index=requests.length;requests.push({url,options});return {ok:true,status:201,headers:{get:()=> 'application/json'},json:async()=>({ok:true,data:monthData(index)})};}});
  assert.equal(result.ok,true,JSON.stringify(result));assert.equal(requests.length,6);assert.equal(result.data.status,'WBS_TEST_H1_IMPORT_COMPLETE');assert.equal(result.data.payables.posted_count,6);assert.equal(result.data.bank.reconciliations.length,6);assert.ok(requests.every(request=>/\/wbs\/test-import\/range$/.test(request.url)&&/^wbs-test-month-[0-9a-f]{64}$/.test(request.options.headers['idempotency-key'])));assert.deepEqual(JSON.parse(requests[0].options.body),{companyCode:'WBPA',dateFrom:'2026-01-01',dateTo:'2026-01-31',pageSize:10,maxPages:1000});assert.deepEqual(progress.map(value=>[value.phase,value.periodCode,value.completedMonths.length]),[['MONTH_START','2026-01',0],['MONTH_COMPLETE','2026-01',1],['MONTH_START','2026-02',1],['MONTH_COMPLETE','2026-02',2],['MONTH_START','2026-03',2],['MONTH_COMPLETE','2026-03',3],['MONTH_START','2026-04',3],['MONTH_COMPLETE','2026-04',4],['MONTH_START','2026-05',4],['MONTH_COMPLETE','2026-05',5],['MONTH_START','2026-06',5],['MONTH_COMPLETE','2026-06',6]]);
  const invalidProgress=await importAuthoritativeWbsTestRange({config:runtime,companyCode:'WBPA',onProgress:true,fetcher:async()=>{throw new Error('must fail before fetch');}});assert.equal(invalidProgress.code,'WBS_TEST_RANGE_IMPORT_COMMAND_INVALID');
  const resumedRequests=[];let completedMonth=0;const resumed=await importAuthoritativeWbsTestRange({config:runtime,companyCode:'WBPA',fetcher:async(_url,options)=>{resumedRequests.push(options);if(resumedRequests.length===1){const complete=monthData(0);return {ok:true,status:201,headers:{get:()=> 'application/json'},json:async()=>({ok:true,data:{...complete,status:'WBS_TEST_MONTH_IMPORT_PARTIAL',bank:{provider_page_count:201,record_count:2001,reconciliation:null,bank_source_count:0,checkpoint:{stage_id:'00000999-0000-4000-8000-000000000001',next_chunk_index:20,chunk_count:21,transaction_count:2001}}}})};}const data=monthData(completedMonth++);return {ok:true,status:201,headers:{get:()=> 'application/json'},json:async()=>({ok:true,data})};}});assert.equal(resumed.ok,true,JSON.stringify(resumed));assert.equal(resumedRequests.length,7);assert.equal(resumedRequests[0].headers['idempotency-key'],resumedRequests[1].headers['idempotency-key']);
