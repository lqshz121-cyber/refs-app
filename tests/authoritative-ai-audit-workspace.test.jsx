...
for(const token of ['Review accounting exceptions','SAVED EVIDENCE','SOURCE HISTORY','FINANCE REVIEW REQUIRED','NO AUTOMATIC POSTING','Loading review items'])assert.match(markup,new RegExp(token,'i'));
const source=fs.readFileSync('src/authoritative-ai-audit-workspace.jsx','utf8');
for(const token of ['refreshAuthoritativeAiWbsExceptionFindings','No entry or posting is created automatically','Set a due date during review','findingLabel'])assert.match(source,new RegExp(token,'i'));
