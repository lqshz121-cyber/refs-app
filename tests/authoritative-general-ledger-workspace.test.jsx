...
test('General Ledger workspace is a retained read-only, contained-table surface',()=>{
  ...
  const reportsMarkup=renderToStaticMarkup(<AuthoritativeGeneralLedgerWorkspace config={displayConfig} fetcher={async()=>new Response(JSON.stringify({ok:true,data:[]}),{status:200})} onBack={()=>{}}/>);
  const accountReportMarkup=renderToStaticMarkup(<AuthoritativeGeneralLedgerWorkspace config={displayConfig} fetcher={async()=>new Response(JSON.stringify({ok:true,data:[]}),{status:200})} initialAccountCode="111000" workspaceTitle="Account QuickReport" workspaceSubtitle="Review posted activity for this account." backLabel="Back to chart of accounts" onBack={()=>{}}/>);
  ...
  assert.match(accountReportMarkup,/value="111000"/,'a COA report must start with the selected exact account code');assert.match(accountReportMarkup,/Account QuickReport/);assert.match(accountReportMarkup,/Review posted activity for this account\./);assert.match(accountReportMarkup,/Back to chart of accounts/,'a COA report must return to its exact parent');
