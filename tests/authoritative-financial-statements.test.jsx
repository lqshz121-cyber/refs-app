...
import {AuthoritativeFullStatementReport,AuthoritativeReportDetail,AuthoritativeReportsWorkspace,AuthoritativeStatementOfCashFlowsReport,DimensionProfitabilitySummary,FinancialStatementSummary,DEFAULT_AUTHORITATIVE_REPORTS_CATALOG,authoritativeReportLineageConfig,authoritativeReportPeriodCaption,authoritativeReportRangeCaption,findAuthoritativeGeneralLedgerShortcut,findAuthoritativeMappingBackedReportShortcuts,findAuthoritativePropertyReportShortcuts,findAuthoritativeReportShortcuts,findAuthoritativeUnavailableReportShortcuts,normalizeAuthoritativeReportsCatalog,restoreAuthoritativeReportTablePosition} from '../src/authoritative-reports-workspace.jsx';
...
async function main(){
  ...
  assert.match(markup,/REPORTING/);assert.match(markup,/Reports center/);assert.match(markup,/Review posted financial reports\./);assert.match(markup,/Reporting scope/);assert.match(markup,/Posted only/);assert.doesNotMatch(markup,/POSTED ledger evidence/,'the Reports scope chip must use concise status copy instead of an implementation-oriented ledger label');assert.match(markup,/Core statements/);assert.match(markup,/Cash &amp; capital/);assert.match(markup,/Property &amp; project analysis/);assert.match(markup,/Group &amp; comparison/);assert.match(markup,/Search reports/);assert.match(markup,/Core statement shortcuts/);assert.match(markup,/Trial Balance/);assert.match(markup,/Balance Sheet/);assert.match(markup,/Profit and Loss/);assert.match(markup,/Cash activity/);assert.doesNotMatch(markup,/Cash movement evidence|direct cash evidence|Posted reports from the accounting API|>Cash Flow</);assert.match(markup,/rep-grid/);assert.match(markup,/rep-card/);assert.match(markup,/report-workbench/);
  ...
  assert.match(profitLossSearchMarkup,/1 match/);assert.match(profitLossSearchMarkup,/MATCHING STATEMENTS/);assert.match(profitLossSearchMarkup,/Profit and Loss/);
  ...
  assert.equal(authoritativeReportPeriodCaption('INCOME_STATEMENT',[{...row,period_start:'2026-02-31'}]),'','an impossible calendar date must not produce a Profit and Loss range');
  ...
  assert.match(cashFlowReport,/Statement of Cash Flows full report/);assert.match(cashFlowReport,/January 1–August 20, 2026/);assert.match(cashFlowReport,/Rows 1-25 of 30/);assert.match(cashFlowReport,/Statement of Cash Flows pages/);assert.match(cashFlowReport,/Back to standard reports/);assert.doesNotMatch(cashFlowReport,/610026/,'the first cash-flow report page must not render rows from the next page');
  assert.equal(authoritativeReportRangeCaption('2026-01-01','2026-08-20'),'January 1–August 20, 2026');assert.equal(authoritativeReportRangeCaption('2026-08-20','2026-01-01'),'','reversed cash-flow scope must not produce a range');assert.equal(authoritativeReportRangeCaption('2026-02-31','2026-08-20'),'','invalid cash-flow dates must not produce a range');
