...
async function main(){
  ...
  assert.match(markup,/REPORTING/);assert.match(markup,/Reports center/);assert.match(markup,/Posted reports from the accounting API/);assert.match(markup,/Reporting scope/);assert.match(markup,/POSTED ledger evidence/);assert.match(markup,/Core statements/);assert.match(markup,/Cash &amp; capital/);assert.match(markup,/Property &amp; project analysis/);assert.match(markup,/Group &amp; comparison/);assert.match(markup,/Search reports/);assert.match(markup,/Core statement shortcuts/);assert.match(markup,/Trial Balance/);assert.match(markup,/Balance Sheet/);assert.match(markup,/Income Statement/);assert.match(markup,/Cash movement evidence/);assert.doesNotMatch(markup,/>Cash Flow</);assert.match(markup,/Read only/);assert.match(markup,/rep-grid/);assert.match(markup,/rep-card/);assert.match(markup,/report-workbench/);
  ...
  assert.match(markup,/>Refresh</);assert.doesNotMatch(markup,/Each read is a real API GET/,'the compact Reports surface must not repeat implementation notes above the report data');assert.doesNotMatch(markup,/class="rep-desc"/,'report categories must be compact tabs rather than tall explanatory cards');
  ...
  assert.match(workspace,/disabled=\{state\.phase==='LOADING'\} onClick=\{load\}>Refresh<\/button>/,'statement refresh must remain an API-read control that retains the current workspace state');
