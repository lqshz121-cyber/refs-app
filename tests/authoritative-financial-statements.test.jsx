...
import {AuthoritativeFullStatementReport,AuthoritativeReportDetail,AuthoritativeReportsWorkspace,AuthoritativeStatementOfCashFlowsReport,DimensionProfitabilitySummary,FinancialStatementSummary,DEFAULT_AUTHORITATIVE_REPORTS_CATALOG,authoritativeReportLineageConfig,authoritativeReportPeriodCaption,findAuthoritativeMappingBackedReportShortcuts,findAuthoritativePropertyReportShortcuts,findAuthoritativeReportShortcuts,normalizeAuthoritativeReportsCatalog,restoreAuthoritativeReportTablePosition} from '../src/authoritative-reports-workspace.jsx';
...
async function main(){
  ...
  const cashFlowReport=renderToStaticMarkup(<AuthoritativeStatementOfCashFlowsReport state={{phase:'READY',rows:cashFlowRows,complete:true}} returnContext={{entityId,periodId}} page={0} onPageChange={()=>{}} onBack={()=>{}} onRefresh={()=>{}} onOpenEvidence={()=>{}}/>);
  assert.match(cashFlowReport,/Statement of Cash Flows full report/);assert.match(cashFlowReport,/Rows 1-25 of 30/);assert.match(cashFlowReport,/Statement of Cash Flows pages/);assert.match(cashFlowReport,/Back to Reports/);assert.doesNotMatch(cashFlowReport,/610026/,'the first cash-flow report page must not render rows from the next page');
