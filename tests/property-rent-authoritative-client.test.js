...
test('Property Rent client rejects malformed MONEY4 and accepts exact period-scoped queue evidence',async()=>{
  let result=await refreshAuthoritativeWbsPropertyRentPickup({config,fetcher:async(url)=>{assert.match(url,new RegExp(`periodId=${config.periodId}`));return response([row]);}});
  ...
  const reviewed={...row,wbs_property_rent_review_evidence_id:id(6),period_id:id(99),mapping_snapshot_id:id(7),mapping_snapshot_hash:hash('c'),mapping_version:3,workflow_status:'READY_FOR_DRAFT',revision:1,reviewed_by:'rent-reviewer',reviewed_at:'2026-08-16T00:00:00.000Z',can_review:false,can_create_draft:true};
  result=await refreshAuthoritativeWbsPropertyRentPickup({config,fetcher:async()=>response([reviewed])});assert.equal(result.ok,false);assert.equal(result.code,'PROPERTY_RENT_PICKUP_PROTOCOL');
});
...
test('Property Rent client performs zero HTTP calls for stale action capabilities',async()=>{
  ...
  const crossPeriod={...row,wbs_property_rent_review_evidence_id:id(6),period_id:id(99),mapping_snapshot_id:id(7),mapping_snapshot_hash:hash('c'),mapping_version:3,workflow_status:'READY_FOR_DRAFT',revision:1,reviewed_by:'rent-reviewer',reviewed_at:'2026-08-16T00:00:00.000Z',can_review:false,can_create_draft:true};
  assert.equal((await createAuthoritativeWbsPropertyRentDraft({config,evidence:crossPeriod,reason:'Reject cross period rent Draft',idempotencyKey:'rent-draft-0003',fetcher})).ok,false);
  assert.equal((await reviewAuthoritativeWbsPropertyRent({config,evidence:row,periodId:config.periodId,reason:'Reject\ncontrol characters',idempotencyKey:'rent-review-0003',fetcher})).ok,false);
