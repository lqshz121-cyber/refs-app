...
const checksMarkup=renderToStaticMarkup(<AuthoritativeBillPaymentsWorkspace config={{baseUrl:'https://fixture.example',tenantId:'11111111-1111-4111-8111-111111111111',entityId:'22222222-2222-4222-8222-222222222222',periodId:'33333333-3333-4333-8333-333333333333'}} workspaceTitle="Checks & payments" workspaceDescription="Review retained vendor payment evidence. Check printing remains unavailable." backLabel="Back to bank transactions"/>);
assert.match(checksMarkup,/Checks &amp; payments/);assert.match(checksMarkup,/Back to bank transactions/);assert.match(checksMarkup,/Check printing remains unavailable/);
assert.doesNotMatch(checksMarkup,/Print checks|Void check|Create payment|Release payment/);
