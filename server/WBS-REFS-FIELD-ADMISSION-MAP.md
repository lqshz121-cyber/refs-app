...
| AutoRec Detail / `fast_auto_payment_detail` | `pd_guid`, signed Deposit or Payment, company/currency receipt scope, valid business and posting date, vendor/project/cost, reviewer/log; `pd_pvguid` is an **OBSERVED** high-coverage Detail-to-PB relation candidate and may become a `pd_guid -> pb_guid` trace only when one same-snapshot signed relation receipt and an approved relation policy bind it; `pd_cbid` remains relation trace only; `MB_Id`/`MB_BusinessId` relation only when signed and detail-compatible | Optional business-side evidence; relation/audit trace retained in the review candidate | Reusing incurred/clear date for a missing/invalid posting date; using `pd_batchguid`, `MB_BatchGuId`, an unsigned/unscope-checked `pd_pvguid`, `pd_cbid`, Seq No., or Ref No. as a PB/bank/allocation key; WBS Release/Incur state transition |
