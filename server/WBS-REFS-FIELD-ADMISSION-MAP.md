...
| --- | --- | --- | --- |
| Payable Report / `account_book_payable_info` | Provider immutable payable key (observed table primary key is `uuid`). The live report also visibly exposes `GuId`, Payable No., Match/Pay/Posting Status, account code/name, invoice number/date/description, amount, incurred/pay-due/clear/check dates, vendor, owner/company/division/project/activity/unit/cost dimensions, journal code/number, check trace and business type. **Posting Date is a separate visible accounting-date field.** Treat every display field as observed trace until a signed receipt binds it to the immutable provider key/version. | Transaction **business side** only: Raw -> Normalized -> reviewed Staging -> AutoRec review candidate | Substituting incurred date for a missing/invalid posting date; promoting visible `GuId`, Payable No., `cb_id`, long ID, journal/check number, memo, or display reference into an immutable bank/match key without provider proof; direct Draft/Post |
