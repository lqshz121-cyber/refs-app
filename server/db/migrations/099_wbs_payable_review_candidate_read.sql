...
-- Until an exact row-binding migration is installed, no entity-level clean
-- attachment is eligible for a Payable row.  Migration 100 replaces only
-- this helper and keeps the public candidate contract fail closed.
CREATE FUNCTION refs_read_wbs_payable_attachment_choices(
...
BEGIN
  ...
  WITH base AS (
    SELECT r.*,receipt.receipt_hash,receipt.payload_ref,imp.wbs_snapshot_import_id,imp.created_by AS imported_by,
      snapshot_receipt.receipt_hash provider_receipt_hash,
    ...
    LEFT JOIN LATERAL public.refs_read_wbs_payable_attachment_choices(
      b.tenant_id,b.entity_id,b.wbs_inbound_row_id,b.source_version,
      b.receipt_hash,b.provider_receipt_hash,b.evidence_digest
    ) attachments ON true
  ), final AS (
    SELECT s.*,
