...
BEGIN
  ...
  WITH candidates AS MATERIALIZED (
    ...
    SELECT po.created_at,po.payment_occurrence_id,jsonb_build_object(
      ...
      'cash_account_code',cash_line.cash_account_code,'bank_member_ref',cash_line.bank_member_ref,
    ...
    LEFT JOIN LATERAL (
      SELECT jl.account_code AS cash_account_code,jl.member_ref AS bank_member_ref
      FROM journal_line jl
      WHERE jl.tenant_id=po.tenant_id AND jl.entity_id=po.entity_id AND jl.period_id=po.period_id
        AND jl.journal_entry_id=journal.journal_entry_id AND jl.line_no=2
        AND jl.debit_amount=0 AND jl.credit_amount=po.amount
      ORDER BY jl.journal_line_id LIMIT 1
    ) cash_line ON true
    LEFT JOIN LATERAL (
  ...
  RETURN jsonb_build_object(
    'schema_version','BILL_PAYMENT_REGISTER_V2','entity_id',p_entity,'period_id',p_period,
