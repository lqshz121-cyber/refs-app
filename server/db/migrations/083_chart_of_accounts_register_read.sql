...
-- Read models for the authoritative Chart of Accounts and account register.
-- They deliberately use the existing account master and POSTED ledger only:
-- neither read has an authoring, export, reconciliation, or provider side effect.
...
BEGIN
  ...
  SELECT selected_period.period_id,selected_period.period_code,selected_period.starts_on,selected_period.ends_on,
    a.account_code,a.account_name,s.currency,s.journal_date,s.journal_entry_id,s.journal_number,s.journal_line_id,s.ledger_line_id,
