...
-- Closed-period WBS rows must be evaluated against the matching policy that
-- governed their accounting date, rather than whichever policy is effective
-- when an auditor opens the read model.  RETIRED snapshots remain evidence
-- only; the composition layer checks their effective window per candidate.
...
BEGIN
 ...
 FROM mapping_snapshot WHERE tenant_id=p_tenant AND entity_id=p_entity AND family='WBS_AUTOREC_MATCH' AND status IN ('APPROVED','RETIRED')
