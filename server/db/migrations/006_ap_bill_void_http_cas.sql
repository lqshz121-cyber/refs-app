...
CREATE OR REPLACE FUNCTION refs_ap_bill_void_hash(
  p_tenant uuid,p_entity uuid,p_bill uuid,p_period uuid,p_expected_version bigint,p_journal_number text,p_journal_date date,p_reason text
) RETURNS text
...
CREATE OR REPLACE FUNCTION refs_create_ap_bill_void(
  p_tenant uuid,p_entity uuid,p_bill uuid,p_period uuid,p_expected_version bigint,p_journal_number text,p_journal_date date,
...
BEGIN
  ...
  computed_hash:=refs_ap_bill_void_hash(p_tenant,p_entity,p_bill,p_period,p_expected_version,p_journal_number,p_journal_date,p_reason);
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'AP bill not found' USING ERRCODE='P0002'; END IF;
  IF bill.version<>p_expected_version THEN RAISE EXCEPTION 'AP bill revision conflict' USING ERRCODE='40001'; END IF;
  IF bill.document_kind<>'AP_BILL' OR bill.status<>'APPROVED' OR bill.open_balance<>bill.gross_amount OR bill.source_document_id IS NULL OR bill.posted_journal_entry_id IS NULL THEN
  ...
  response:=jsonb_build_object('business_adjustment_id',adjustment_id,'business_document_id',p_bill,'journal_entry_id',journal_id,'status','DRAFT','revision',0,'business_document_version',p_expected_version,'idempotent',false);
...
REVOKE EXECUTE ON FUNCTION refs_ap_bill_void_hash(uuid,uuid,uuid,uuid,bigint,text,date,text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION refs_create_ap_bill_void(uuid,uuid,uuid,uuid,bigint,text,date,text,text,text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION refs_ap_bill_void_hash(uuid,uuid,uuid,uuid,bigint,text,date,text) TO refs_app;
GRANT EXECUTE ON FUNCTION refs_create_ap_bill_void(uuid,uuid,uuid,uuid,bigint,text,date,text,text,text) TO refs_app;
