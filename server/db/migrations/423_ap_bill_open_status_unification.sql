-- 423: unify the "fully open, nothing applied" AP bill status (T11-G1 / N15-G2).
-- Native bills created through refs_create_business_document (048) post as
-- OPEN with attachment evidence and no source_document; the void command (006)
-- and its posting reducer (010) accepted only the legacy fixture status
-- APPROVED with a source_document, so no natively created bill could ever be
-- voided. The AP payment reversal reducer (023) also reopened a bill to
-- APPROVED while AR (016) reopens to OPEN. Owner decision 2026-09-16: accept
-- APPROVED or OPEN as "fully open", accept attachment evidence in place of a
-- source_document (void JE mirrors the original's journal_type and attachment
-- links), and reopen to OPEN. Patched in place via pg_get_functiondef so the
-- later scope fixes (028/031/032) stay intact. No data is rewritten.
BEGIN;
DO $$
...
BEGIN
  ...
  IF fn=old THEN RAISE EXCEPTION '423 void command: expected text not found' USING ERRCODE='55000'; END IF;
  old:=fn;
  fn:=replace(fn,
...
BEGIN
  ...
  IF fn=old THEN RAISE EXCEPTION '423 void reducer: expected text not found' USING ERRCODE='55000'; END IF;
  EXECUTE fn;
END $$;
DO $$
DECLARE fn text; old text;
BEGIN
  ...
  IF fn=old THEN RAISE EXCEPTION '423 payment reversal reducer: expected text not found' USING ERRCODE='55000'; END IF;
  EXECUTE fn;
END $$;
COMMIT;
