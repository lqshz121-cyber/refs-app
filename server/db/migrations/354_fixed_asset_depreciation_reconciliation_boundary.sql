BEGIN;
CREATE OR REPLACE FUNCTION refs_read_ai_fixed_asset_posted_reconciliation(p_tenant uuid,p_entity uuid,p_period uuid)
...
BEGIN
  ...
  RETURN QUERY WITH p AS (
    ...
    SELECT r.*,p.period_id,p.period_code,p.starts_on,p.ends_on,
      (schedule.data->>'expected_period_depreciation')::numeric(20,4) expected_period,
      (schedule.data->>'expected_accumulated_depreciation')::numeric(20,4) expected_accumulated
    FROM refs_read_ai_fixed_asset_depreciation_source(p_tenant,p_entity,p_period) AS schedule(data)
    JOIN fixed_asset_register_evidence r ON r.tenant_id=p_tenant AND r.entity_id=p_entity
      AND r.fixed_asset_register_evidence_id=(schedule.data->>'fixed_asset_register_evidence_id')::uuid
    CROSS JOIN p
  ), posted AS (
    ...
    SELECT e.*,COALESCE(p.period_expense,0) posted_period,COALESCE(p.accumulated,0) posted_accumulated,COALESCE(p.je_ids,ARRAY[]::uuid[]) je_ids,COALESCE(p.jl_ids,ARRAY[]::uuid[]) jl_ids,COALESCE(p.ll_ids,ARRAY[]::uuid[]) ll_ids FROM expected e JOIN posted p USING(fixed_asset_register_evidence_id)
  )
