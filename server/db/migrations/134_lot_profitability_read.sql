...
-- Lot profitability is deliberately a separate exact-dimension report rather
-- than a fallback from unit, project, property, or a source-document label.
-- It reads immutable POSTED evidence only and retains the same drill-back IDs
-- used by the other financial-statement reports.
...
BEGIN
  ...
  IF p_lot_ref IS NULL OR p_lot_ref<>btrim(p_lot_ref)
      OR length(p_lot_ref) NOT BETWEEN 1 AND 160
      OR p_lot_ref ~ '[[:cntrl:]]' THEN
  ...
  WITH selected_period AS (
    ...
    WHERE l.tenant_id=p_tenant AND l.entity_id=p_entity
      ...
      AND l.dimensions @> jsonb_build_object('lot_ref',p_lot_ref)
  ), classified_lines AS (
    SELECT l.*,CASE
  ...
  SELECT p.period_id,p.period_code,to_char(p.starts_on,'YYYY-MM-DD'),to_char(p.ends_on,'YYYY-MM-DD'),
    'LOT'::text,p_lot_ref,'LOT_PROFITABILITY'::text,r.statement_section,
