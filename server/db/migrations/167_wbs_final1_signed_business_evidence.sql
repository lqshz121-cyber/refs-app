...
ALTER TABLE wbs_final1_retained_evidence_admission DROP CONSTRAINT wbs_final1_retained_evidence_admission_check2;
...
CREATE FUNCTION refs_canonical_jsonb_text(p_value jsonb) RETURNS text
LANGUAGE plpgsql IMMUTABLE STRICT SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE v_result text;
BEGIN
  CASE jsonb_typeof(p_value)
    WHEN 'object' THEN
      SELECT '{'||COALESCE(string_agg(to_jsonb(key)::text||':'||refs_canonical_jsonb_text(value),',' ORDER BY key COLLATE "C"),'')||'}'
        INTO v_result FROM jsonb_each(p_value);
    WHEN 'array' THEN
      SELECT '['||COALESCE(string_agg(refs_canonical_jsonb_text(value),',' ORDER BY ordinal),'')||']'
...
CREATE FUNCTION refs_canonical_jsonb_hash(p_value jsonb) RETURNS text
LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,public,pg_temp AS $$
  SELECT 'sha256:'||encode(digest(convert_to(refs_canonical_jsonb_text(p_value),'UTF8'),'sha256'),'hex')
$$;

CREATE FUNCTION refs_wbs_final1_control_totals_hash(p_control_totals jsonb) RETURNS text
LANGUAGE sql IMMUTABLE STRICT SET search_path=pg_catalog,public,pg_temp AS $$
  SELECT refs_canonical_jsonb_hash(p_control_totals)
$$;

CREATE FUNCTION refs_wbs_final1_control_totals_valid(p_control_totals jsonb,p_hash text) RETURNS boolean
LANGUAGE plpgsql IMMUTABLE SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE v_item jsonb;v_prior text:='';v_count integer;v_sum integer:=0;
BEGIN
  IF p_control_totals IS NULL OR p_hash IS NULL THEN RETURN false; END IF;
  IF jsonb_typeof(p_control_totals)<>'object' OR (SELECT count(*) FROM jsonb_object_keys(p_control_totals))<>2
     OR NOT (p_control_totals ?& ARRAY['row_count','currency_totals'])
     OR jsonb_typeof(p_control_totals->'row_count')<>'number'
     OR p_control_totals->>'row_count' !~ '^[1-9][0-9]{0,2}$' THEN RETURN false; END IF;
  v_count:=(p_control_totals->>'row_count')::integer;
  IF v_count<1 OR v_count>500 OR jsonb_typeof(p_control_totals->'currency_totals')<>'array'
     OR jsonb_array_length(p_control_totals->'currency_totals')<1
     OR jsonb_array_length(p_control_totals->'currency_totals')>500 THEN RETURN false; END IF;
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_control_totals->'currency_totals') LOOP
    IF jsonb_typeof(v_item)<>'object' OR (SELECT count(*) FROM jsonb_object_keys(v_item))<>3
       OR NOT (v_item ?& ARRAY['currency','row_count','amount_total'])
       OR v_item->>'currency' !~ '^[A-Z]{3}$' OR (v_item->>'currency') COLLATE "C"<=v_prior COLLATE "C"
       OR jsonb_typeof(v_item->'row_count')<>'number' OR v_item->>'row_count' !~ '^[1-9][0-9]{0,2}$'
       OR (v_item->>'row_count')::integer>500
       OR v_item->>'amount_total' !~ '^(0|[1-9][0-9]{0,17})\.[0-9]{4}$' THEN RETURN false; END IF;
    v_prior:=v_item->>'currency';v_sum:=v_sum+(v_item->>'row_count')::integer;
  END LOOP;
  RETURN v_sum=v_count AND p_hash ~ '^sha256:[0-9a-f]{64}$' AND p_hash=refs_wbs_final1_control_totals_hash(p_control_totals);
END;
$$;

DO $$
DECLARE v_control_totals jsonb:='{"row_count":1,"currency_totals":[{"currency":"USD","row_count":1,"amount_total":"10.0000"}]}'::jsonb;
BEGIN
  IF refs_canonical_jsonb_text(v_control_totals) IS DISTINCT FROM '{"currency_totals":[{"amount_total":"10.0000","currency":"USD","row_count":1}],"row_count":1}'
     OR refs_wbs_final1_control_totals_hash(v_control_totals) IS DISTINCT FROM 'sha256:faa6c295db3c0d8e097f0f897b7da3102ae098551023cc9e55bba1ebd14011e1' THEN
    RAISE EXCEPTION 'Canonical Final-1 control totals implementation differs from the frozen V1 vector' USING ERRCODE='22023';
  END IF;
END $$;
...
CREATE TABLE wbs_final1_signed_control_total (
  ...
  CHECK(refs_wbs_final1_control_totals_valid(control_totals,control_totals_hash))
);
...
CREATE FUNCTION refs_record_wbs_final1_signed_control_total(p_tenant uuid,p_entity uuid,p_admission uuid,p_control_totals jsonb,p_hash text) RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE v_actor text:=refs_current_actor();v_domain text;v_existing wbs_final1_signed_control_total;v_audit uuid:=gen_random_uuid();v_payload jsonb;v_recomputed_control_totals jsonb;v_row_count integer;
BEGIN
  ...
  IF NOT refs_wbs_final1_control_totals_valid(p_control_totals,p_hash) THEN RAISE EXCEPTION 'Signed control totals are malformed or hash-mismatched' USING ERRCODE='22023'; END IF;
  v_row_count:=(p_control_totals->>'row_count')::integer;
  SELECT domain INTO v_domain FROM wbs_final1_retained_evidence_admission WHERE tenant_id=p_tenant AND entity_id=p_entity AND wbs_final1_retained_evidence_admission_id=p_admission AND row_count=v_row_count AND retained_by=v_actor FOR SHARE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Exact Final-1 admission is missing for signed controls' USING ERRCODE='P0002'; END IF;
  WITH population AS (
    SELECT d.currency,abs(d.gross_amount)::numeric(22,4) gross_amount FROM wbs_final1_retained_source_row r JOIN source_document d ON d.tenant_id=r.tenant_id AND d.entity_id=r.entity_id AND d.source_document_id=r.source_document_id WHERE r.tenant_id=p_tenant AND r.entity_id=p_entity AND r.wbs_final1_retained_evidence_admission_id=p_admission
    UNION ALL
    SELECT d.currency,abs(d.gross_amount)::numeric(22,4) gross_amount FROM wbs_final1_signed_business_source_row r JOIN source_document d ON d.tenant_id=r.tenant_id AND d.entity_id=r.entity_id AND d.source_document_id=r.source_document_id WHERE r.tenant_id=p_tenant AND r.entity_id=p_entity AND r.wbs_final1_retained_evidence_admission_id=p_admission
  ), grouped AS (SELECT currency,count(*)::integer row_count,sum(gross_amount)::numeric amount_total FROM population GROUP BY currency), currency_totals AS (
    SELECT jsonb_agg(jsonb_build_object('currency',currency,'row_count',row_count,'amount_total',amount_total::numeric(22,4)::text) ORDER BY currency COLLATE "C") value FROM grouped
  )
  SELECT jsonb_build_object('row_count',(SELECT count(*)::integer FROM population),'currency_totals',value) INTO v_recomputed_control_totals FROM currency_totals;
  IF v_recomputed_control_totals IS DISTINCT FROM p_control_totals OR refs_wbs_final1_control_totals_hash(v_recomputed_control_totals) IS DISTINCT FROM p_hash THEN RAISE EXCEPTION 'Signed control totals differ from the exact persisted source population' USING ERRCODE='23514'; END IF;
  ...
  IF FOUND THEN
    IF v_existing.domain IS DISTINCT FROM v_domain OR v_existing.control_totals IS DISTINCT FROM p_control_totals OR v_existing.control_totals_hash IS DISTINCT FROM p_hash OR v_existing.created_by IS DISTINCT FROM v_actor THEN RAISE EXCEPTION 'Signed control total replay conflicts with retained evidence' USING ERRCODE='23505'; END IF;
  ...
  END IF;
  INSERT INTO wbs_final1_signed_control_total(tenant_id,entity_id,wbs_final1_retained_evidence_admission_id,domain,control_totals,control_totals_hash,created_by) VALUES(p_tenant,p_entity,p_admission,v_domain,p_control_totals,p_hash,v_actor);
  v_payload:=jsonb_build_object('schema_version','WBS_FINAL1_SIGNED_CONTROL_TOTAL_V1','admission_id',p_admission,'domain',v_domain,'control_totals',p_control_totals,'control_totals_hash',p_hash,'can_create_transaction',false,'can_create_draft',false,'can_post',false);
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE v_actor text:=refs_current_actor();v_domain text:=p_delivery->>'domain';v_tool text:=p_delivery->>'source_tool';v_source_module text;v_company text:=p_delivery->>'company_code';v_rows jsonb:=p_plan->'evidence_rows';v_row jsonb;v_expected integer:=0;v_period uuid;v_period_count integer;v_import uuid:=gen_random_uuid();v_admission uuid:=(p_delivery->>'admission_id')::uuid;v_raw uuid;v_doc uuid;v_line uuid;v_business_row uuid;v_amount numeric(22,4);v_date date;v_status source_status;v_payload jsonb;v_response jsonb;v_idem idempotency_receipt;v_computed_control_totals jsonb;v_computed_hash text;v_mapping_count integer;
BEGIN
  PERFORM refs_assert_scope(p_tenant,p_entity,'WBS.SNAPSHOT.IMPORT');
  IF v_actor IS NULL THEN RAISE EXCEPTION 'Authenticated Final-1 service actor missing' USING ERRCODE='42501'; END IF;
  ...
  v_source_module:=CASE v_domain WHEN 'BANK' THEN 'bankFeed' WHEN 'COST' THEN 'cost_general_ledger' WHEN 'PROPERTY' THEN 'pmCharge' END;
  ...
  SELECT jsonb_build_object('row_count',jsonb_array_length(v_rows),'currency_totals',jsonb_agg(jsonb_build_object('currency',currency,'row_count',row_count,'amount_total',amount_total::numeric(22,4)::text) ORDER BY currency COLLATE "C")) INTO v_computed_control_totals FROM (SELECT value->>'currency' currency,count(*)::integer row_count,sum(abs((value->>'gross_amount')::numeric(22,4))) amount_total FROM jsonb_array_elements(v_rows) GROUP BY value->>'currency') q;
  v_computed_hash:=refs_wbs_final1_control_totals_hash(v_computed_control_totals);
  IF v_computed_control_totals IS DISTINCT FROM p_delivery->'control_totals' OR v_computed_hash IS DISTINCT FROM p_delivery->>'control_totals_hash' THEN RAISE EXCEPTION 'Final-1 business signed controls differ from normalized rows' USING ERRCODE='23514'; END IF;
  FOR v_row IN SELECT value FROM jsonb_array_elements(v_rows) LOOP
    ...
    IF v_date<(p_delivery->>'date_from')::date OR v_date>(p_delivery->>'date_to')::date THEN RAISE EXCEPTION 'Final-1 business row date differs from the signed delivery range' USING ERRCODE='23514'; END IF;
  ...
  FOR v_row IN SELECT value FROM jsonb_array_elements(v_rows) LOOP
    v_raw:=gen_random_uuid();v_doc:=gen_random_uuid();v_line:=gen_random_uuid();v_business_row:=gen_random_uuid();v_amount:=(v_row->>'gross_amount')::numeric(22,4);v_date:=(v_row->>'business_date')::date;
    ...
    INSERT INTO raw_event(raw_event_id,tenant_id,entity_id,import_batch_id,source_system,source_module,source_entity_id,source_record_id,source_version,event_type,occurred_at,payload_hash,payload_ref,correlation_id) VALUES(v_raw,p_tenant,p_entity,v_import,'WBS',v_source_module,v_company,v_row->>'source_record_id',v_row->>'source_version','UPSERT',v_date::timestamptz,v_row->>'raw_row_hash',(p_artifacts#>>'{package,storage_ref}')||'#versionId='||(p_artifacts#>>'{package,storage_version}'),p_idempotency_key);
    INSERT INTO source_document(source_document_id,tenant_id,entity_id,raw_event_id,source_system,source_module,source_entity_id,source_record_id,source_version,document_type,document_no,business_date,accounting_date,currency,gross_amount,status,source_ref,payload_hash) VALUES(v_doc,p_tenant,p_entity,v_raw,'WBS',v_source_module,v_company,v_row->>'source_record_id',v_row->>'source_version','WBS_FINAL1_'||v_domain,NULL,v_date,v_date,'USD',v_amount,v_status,(p_artifacts#>>'{package,storage_ref}')||'#versionId='||(p_artifacts#>>'{package,storage_version}')||'&row_ordinal='||(v_row->>'source_row_ordinal'),v_row->>'raw_row_hash');
  ...
  END LOOP;
  PERFORM refs_record_wbs_final1_signed_control_total(p_tenant,p_entity,v_admission,v_computed_control_totals,v_computed_hash);
  v_response:=jsonb_build_object('status','WBS_FINAL1_RETAINED_SOURCE_EVIDENCE','admission_id',v_admission,'domain',v_domain,'row_count',jsonb_array_length(v_rows),'control_totals',v_computed_control_totals,'control_totals_hash',v_computed_hash,'signature_verified',true,'can_write_wbs',false,'can_propose_amortization',false,'can_create_draft',false,'can_review',false,'can_approve',false,'can_post',false,'idempotent',false);
...
BEGIN
  ...
  PERFORM refs_record_wbs_final1_signed_control_total(p_tenant,p_entity,(v_result->>'admission_id')::uuid,p_delivery->'control_totals',p_delivery->>'control_totals_hash');
  RETURN v_result;
END;
$$;
...
REVOKE ALL ON FUNCTION refs_canonical_jsonb_text(jsonb),refs_canonical_jsonb_hash(jsonb),refs_wbs_final1_control_totals_hash(jsonb),refs_wbs_final1_control_totals_valid(jsonb,text),refs_assert_wbs_final1_signed_artifacts(jsonb,jsonb),refs_record_wbs_final1_signed_control_total(uuid,uuid,uuid,jsonb,text),refs_wbs_final1_business_evidence_hash(uuid,uuid,jsonb,jsonb,jsonb),refs_retain_wbs_final1_business_evidence(uuid,uuid,jsonb,jsonb,jsonb,text,text),refs_retain_wbs_final1_source_evidence_with_signed_controls(uuid,uuid,jsonb,jsonb,jsonb,text,text) FROM PUBLIC;
REVOKE ALL ON FUNCTION refs_canonical_jsonb_text(jsonb),refs_canonical_jsonb_hash(jsonb),refs_wbs_final1_control_totals_hash(jsonb),refs_wbs_final1_control_totals_valid(jsonb,text),refs_assert_wbs_final1_signed_artifacts(jsonb,jsonb),refs_record_wbs_final1_signed_control_total(uuid,uuid,uuid,jsonb,text) FROM refs_app;
REVOKE ALL ON FUNCTION refs_retain_wbs_final1_source_evidence(uuid,uuid,jsonb,jsonb,jsonb,text,text) FROM refs_app;
GRANT EXECUTE ON FUNCTION refs_wbs_final1_business_evidence_hash(uuid,uuid,jsonb,jsonb,jsonb),refs_retain_wbs_final1_business_evidence(uuid,uuid,jsonb,jsonb,jsonb,text,text),refs_retain_wbs_final1_source_evidence_with_signed_controls(uuid,uuid,jsonb,jsonb,jsonb,text,text) TO refs_app;
