...
BEGIN
  ...
  IF journal_count=1 THEN
    ...
    workflow_matches:=workflow_audit_count=4 AND workflow_outbox_count=4
      ...
      AND (SELECT count(*)=1 FROM outbox_event o WHERE o.tenant_id=p_tenant AND o.entity_id=p_entity AND o.aggregate_type='JOURNAL_ENTRY' AND o.aggregate_id=je.journal_entry_id AND o.event_type='JOURNAL_POSTED' AND o.payload_hash=refs_jsonb_hash(o.payload) AND o.payload->>'journal_entry_id'=je.journal_entry_id::text AND (o.payload->>'posting_batch_id')::uuid IN (SELECT posting_batch_id FROM ledger_line WHERE tenant_id=p_tenant AND entity_id=p_entity AND journal_entry_id=je.journal_entry_id));
