...
DECLARE
  ...
  journal_line_count integer; proposed_line_count integer; ledger_count integer; posting_batch_count integer; workflow_audit_count integer; workflow_outbox_count integer;
  snapshot_count integer; snapshot_bad_row_count integer; snapshot_numeric_mismatch_count integer; expected_delta_count integer; covered_delta_count integer;
  ...
  status text; evidence jsonb; review_hash text; exact_posting_batch uuid; review_id uuid:=gen_random_uuid(); review_time timestamptz:=clock_timestamp(); event_payload jsonb; response jsonb;
BEGIN
  ...
  IF journal_count=1 THEN
    ...
    workflow_matches:=workflow_audit_count=4 AND workflow_outbox_count=4
      ...
      AND (SELECT count(*)=1 FROM outbox_event o WHERE o.tenant_id=p_tenant AND o.entity_id=p_entity AND o.aggregate_type='JOURNAL_ENTRY' AND o.aggregate_id=je.journal_entry_id AND o.event_type='JOURNAL_SUBMIT' AND o.payload_hash=refs_jsonb_hash(o.payload) AND o.payload=jsonb_build_object('journal_entry_id',je.journal_entry_id,'status','PENDING_REVIEW','revision',1))
      AND (SELECT count(*)=1 FROM outbox_event o WHERE o.tenant_id=p_tenant AND o.entity_id=p_entity AND o.aggregate_type='JOURNAL_ENTRY' AND o.aggregate_id=je.journal_entry_id AND o.event_type='JOURNAL_REVIEW' AND o.payload_hash=refs_jsonb_hash(o.payload) AND o.payload=jsonb_build_object('journal_entry_id',je.journal_entry_id,'status','PENDING_APPROVAL','revision',2))
      AND (SELECT count(*)=1 FROM outbox_event o WHERE o.tenant_id=p_tenant AND o.entity_id=p_entity AND o.aggregate_type='JOURNAL_ENTRY' AND o.aggregate_id=je.journal_entry_id AND o.event_type='JOURNAL_APPROVE' AND o.payload_hash=refs_jsonb_hash(o.payload) AND o.payload=jsonb_build_object('journal_entry_id',je.journal_entry_id,'status','APPROVED','revision',3))
      AND (SELECT count(*)=1 FROM outbox_event o WHERE o.tenant_id=p_tenant AND o.entity_id=p_entity AND o.aggregate_type='JOURNAL_ENTRY' AND o.aggregate_id=je.journal_entry_id AND o.event_type='JOURNAL_POSTED' AND o.payload_hash=refs_jsonb_hash(o.payload) AND o.payload=jsonb_build_object('journal_entry_id',je.journal_entry_id,'posting_batch_id',exact_posting_batch));
    ...
    IF je.status='POSTED' THEN
      ...
      IF snapshot_count>0 THEN
        ...
        WITH period_scope AS (
          SELECT starts_on,ends_on FROM accounting_period WHERE tenant_id=p_tenant AND entity_id=p_entity AND period_id=je.period_id
        ), ledger_totals AS (
          SELECT ll.account_code,
            COALESCE(sum(ll.debit_amount) FILTER(WHERE j.journal_date<ps.starts_on),0)::numeric(20,4) opening_debit,
            COALESCE(sum(ll.credit_amount) FILTER(WHERE j.journal_date<ps.starts_on),0)::numeric(20,4) opening_credit,
            COALESCE(sum(ll.debit_amount) FILTER(WHERE j.journal_date BETWEEN ps.starts_on AND ps.ends_on),0)::numeric(20,4) period_debit,
            COALESCE(sum(ll.credit_amount) FILTER(WHERE j.journal_date BETWEEN ps.starts_on AND ps.ends_on),0)::numeric(20,4) period_credit,
            COALESCE(sum(ll.debit_amount),0)::numeric(20,4) ending_debit,
            COALESCE(sum(ll.credit_amount),0)::numeric(20,4) ending_credit
          FROM ledger_line ll JOIN journal_entry j ON j.tenant_id=ll.tenant_id AND j.entity_id=ll.entity_id AND j.journal_entry_id=ll.journal_entry_id
          CROSS JOIN period_scope ps
          WHERE ll.tenant_id=p_tenant AND ll.entity_id=p_entity AND j.status='POSTED' AND j.journal_date<=ps.ends_on
          GROUP BY ll.account_code
        )
        SELECT count(*) INTO snapshot_numeric_mismatch_count
        FROM financial_statement_snapshot_row r LEFT JOIN ledger_totals l ON l.account_code=r.account_code
        WHERE r.financial_statement_snapshot_id=snapshot.financial_statement_snapshot_id AND (
          l.account_code IS NULL OR r.opening_debit<>l.opening_debit OR r.opening_credit<>l.opening_credit OR
          r.period_debit<>l.period_debit OR r.period_credit<>l.period_credit OR r.ending_debit<>l.ending_debit OR r.ending_credit<>l.ending_credit OR
          r.display_balance<>CASE r.statement_type
            WHEN 'TRIAL_BALANCE' THEN l.ending_debit-l.ending_credit
            WHEN 'BALANCE_SHEET' THEN CASE WHEN r.statement_section='ASSETS' THEN l.ending_debit-l.ending_credit ELSE l.ending_credit-l.ending_debit END
            WHEN 'INCOME_STATEMENT' THEN CASE WHEN r.statement_section='REVENUE' THEN l.period_credit-l.period_debit ELSE l.period_debit-l.period_credit END
            WHEN 'CASH_FLOW' THEN l.period_debit-l.period_credit
          END::numeric(20,4));
        snapshot_hash_matches:=snapshot_bad_row_count=0 AND snapshot_numeric_mismatch_count=0 AND snapshot.snapshot_hash=refs_jsonb_hash(snapshot_rows) AND snapshot.ledger_evidence_hash=refs_jsonb_hash(jsonb_build_object('statement_rows',snapshot_rows));
