...
-- Count and page the scoped ledger before expanding source lineage. At large
-- period volumes the prior correlated source-link read ran for every matching
-- ledger row even though the API returns at most 200 rows.
...
BEGIN
  ...
  END IF;
  SELECT ap.period_id,ap.period_code,ap.starts_on,ap.ends_on INTO selected_period
  ...
  WITH scoped AS (
    ...
    SELECT n.* FROM numbered n ORDER BY n.journal_date,n.posted_at,n.ledger_line_id LIMIT p_limit OFFSET p_offset
  )
  SELECT selected_period.period_id,selected_period.period_code,selected_period.starts_on,selected_period.ends_on,
    p.account_code,p.account_name,p.currency,p.journal_date,p.journal_entry_id,p.journal_number,p.journal_line_id,p.ledger_line_id,
    p.member_ref,p.description,p.debit_amount::numeric(20,4),p.credit_amount::numeric(20,4),
    ARRAY(SELECT DISTINCT link.source_document_id FROM public.source_link link
      WHERE link.tenant_id=p_tenant AND link.entity_id=p_entity AND link.journal_entry_id=p.journal_entry_id
        AND link.source_document_id IS NOT NULL ORDER BY link.source_document_id)::uuid[],p.total_count
  FROM paged p ORDER BY p.journal_date,p.posted_at,p.ledger_line_id;
END;
$$;
