...
CREATE TABLE wbs_insurance_pc_mapping_pre_admission_observation (
 ...
 immutable_version uuid NOT NULL,
 ...
 CHECK(observation_document->>'schema_version'='REFS_INSURANCE_PRE_ADMISSION_OBSERVATION_V1'
   ...
   AND observation_document->>'admission_id'=admission_id::text
   AND observation_document->>'immutable_version'=immutable_version::text
   AND observation_document->>'receipt_hash'=receipt_hash
   AND (observation_document->>'date_from')::date=date_from
   AND (observation_document->>'date_to')::date=date_to
...
BEGIN
 ...
 FOR item IN SELECT value FROM jsonb_array_elements(p_rows) LOOP
  ...
  INSERT INTO wbs_insurance_pc_mapping_pre_admission_row(tenant_id,entity_id,observation_id,row_ordinal,pc_code,observed_row_count,row_hash) VALUES(p_tenant,p_entity,stored.observation_id,ordinal,item->>'pc_code',(item->>'observed_row_count')::bigint,item->>'row_hash') ON CONFLICT DO NOTHING;
  IF NOT EXISTS(SELECT 1 FROM wbs_insurance_pc_mapping_pre_admission_row r WHERE r.tenant_id=p_tenant AND r.entity_id=p_entity AND r.observation_id=stored.observation_id AND r.row_ordinal=ordinal AND r.pc_code=item->>'pc_code' AND r.observed_row_count=(item->>'observed_row_count')::bigint AND r.row_hash=item->>'row_hash') THEN RAISE EXCEPTION 'Insurance pre-admission aggregate row replay differs' USING ERRCODE='23505';END IF;
  ordinal:=ordinal+1;
 END LOOP;
...
BEGIN
 ...
 SELECT * INTO prop FROM wbs_insurance_pc_mapping_proposal WHERE tenant_id=p_tenant AND entity_id=p_entity AND proposal_id=p_proposal FOR UPDATE;IF NOT FOUND THEN RAISE EXCEPTION 'Insurance PC mapping proposal not found' USING ERRCODE='P0002';END IF;
...
CREATE FUNCTION refs_read_wbs_insurance_pc_mapping_trace(p_tenant uuid,p_entity uuid,p_pc_code text,p_accounting_date date) RETURNS jsonb LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$ DECLARE matched integer;result jsonb;BEGIN PERFORM refs_assert_scope(p_tenant,p_entity,'WBS.INSURANCE.PC_MAPPING.VIEW');IF p_pc_code IS NULL OR p_pc_code<>btrim(p_pc_code) OR length(p_pc_code) NOT BETWEEN 1 AND 128 OR p_accounting_date IS NULL THEN RAISE EXCEPTION 'Insurance PC mapping trace selection is invalid' USING ERRCODE='22023';END IF;SELECT count(*)::integer INTO matched FROM wbs_insurance_pc_mapping_decision_trace t JOIN wbs_insurance_pc_company_mapping_decision d ON d.tenant_id=t.tenant_id AND d.entity_id=t.entity_id AND d.wbs_insurance_pc_company_mapping_decision_id=t.wbs_insurance_pc_company_mapping_decision_id JOIN wbs_insurance_pc_mapping_approval a ON a.tenant_id=t.tenant_id AND a.entity_id=t.entity_id AND a.approval_id=t.approval_id WHERE t.tenant_id=p_tenant AND t.entity_id=p_entity AND t.pc_code=p_pc_code AND d.approval_status='APPROVED' AND d.effective_from<=p_accounting_date AND (d.effective_to IS NULL OR d.effective_to>=p_accounting_date);IF matched<>1 THEN RETURN jsonb_build_object('pc_code',p_pc_code,'accounting_date',to_char(p_accounting_date,'YYYY-MM-DD'),'match_count',matched,'mapping_status',CASE WHEN matched=0 THEN 'MISSING' ELSE 'AMBIGUOUS' END);END IF;SELECT jsonb_build_object('pc_code',d.pc_code,'accounting_date',to_char(p_accounting_date,'YYYY-MM-DD'),'match_count',1,'mapping_status','CONTROLLER_APPROVED','company_code',d.company_code,'effective_from',to_char(d.effective_from,'YYYY-MM-DD'),'effective_to',CASE WHEN d.effective_to IS NULL THEN NULL ELSE to_jsonb(to_char(d.effective_to,'YYYY-MM-DD')) END,'mapping_approval_id',a.approval_id,'observation_hash',t.observation_hash,'proposal_hash',t.proposal_hash,'decision_hash',a.decision_hash,'company_mapping_hash',t.company_mapping_hash,'catalog_decision_id',d.wbs_company_catalog_controller_decision_id,'approved_by',d.decided_by,'approved_at',to_char(d.decided_at AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')) INTO result FROM wbs_insurance_pc_mapping_decision_trace t JOIN wbs_insurance_pc_company_mapping_decision d ON d.tenant_id=t.tenant_id AND d.entity_id=t.entity_id AND d.wbs_insurance_pc_company_mapping_decision_id=t.wbs_insurance_pc_company_mapping_decision_id JOIN wbs_insurance_pc_mapping_approval a ON a.tenant_id=t.tenant_id AND a.entity_id=t.entity_id AND a.approval_id=t.approval_id WHERE t.tenant_id=p_tenant AND t.entity_id=p_entity AND t.pc_code=p_pc_code AND d.approval_status='APPROVED' AND d.effective_from<=p_accounting_date AND (d.effective_to IS NULL OR d.effective_to>=p_accounting_date);RETURN jsonb_strip_nulls(result);END $$;

CREATE FUNCTION refs_read_wbs_insurance_pc_mapping_admission_resume(p_tenant uuid,p_entity uuid,p_observation uuid,p_expected_observation_hash text,p_expected_approval uuid,p_expected_decision_hash text,p_expected_company_mapping_hash text) RETURNS jsonb
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,public,pg_temp AS $$
DECLARE obs wbs_insurance_pc_mapping_pre_admission_observation;approval wbs_insurance_pc_mapping_approval;result jsonb;
BEGIN
 -- Provider M2M uses its ingress permission. It never receives Controller VIEW.
 PERFORM refs_assert_scope(p_tenant,p_entity,'WBS.SNAPSHOT.IMPORT');
 SELECT * INTO obs FROM wbs_insurance_pc_mapping_pre_admission_observation WHERE tenant_id=p_tenant AND entity_id=p_entity AND observation_id=p_observation FOR SHARE;
 IF NOT FOUND OR obs.observation_hash IS DISTINCT FROM p_expected_observation_hash THEN RAISE EXCEPTION 'Insurance admission observation changed' USING ERRCODE='40001';END IF;
 SELECT * INTO approval FROM wbs_insurance_pc_mapping_approval WHERE tenant_id=p_tenant AND entity_id=p_entity AND approval_id=p_expected_approval AND observation_hash=p_expected_observation_hash FOR SHARE;
 IF NOT FOUND OR approval.decision_hash IS DISTINCT FROM p_expected_decision_hash OR approval.company_mapping_hash IS DISTINCT FROM p_expected_company_mapping_hash OR approval.effective_from>obs.date_from OR (approval.effective_to IS NOT NULL AND approval.effective_to<obs.date_to) THEN RAISE EXCEPTION 'Insurance mapping approval is absent, changed, or outside the artifact range' USING ERRCODE='40001';END IF;
 result:=jsonb_build_object('admission_id',obs.admission_id,'immutable_version',obs.immutable_version,'receipt_hash',obs.receipt_hash,'date_from',to_char(obs.date_from,'YYYY-MM-DD'),'date_to',to_char(obs.date_to,'YYYY-MM-DD'),'observation',obs.observation_document,'approval',jsonb_build_object('mapping_approval_id',approval.approval_id,'canonical_mapping_decision_hash',approval.decision_hash,'parent_company_mapping_hash',approval.company_mapping_hash,'proposal_hash',approval.proposal_hash,'status','APPROVED','revoked',false));
 RETURN result;
END $$;
