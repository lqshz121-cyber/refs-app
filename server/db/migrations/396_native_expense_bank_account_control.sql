...
-- Direct expenses use the same approved bank-to-cash-GL control as cash transfers.
-- Renaming preserves the previous implementation for a guarded rollback.
ALTER FUNCTION refs_create_native_expense(uuid,uuid,uuid,text,text,text,text,text,date,char,numeric,text,uuid[],text)
  RENAME TO refs_create_native_expense_395;
...
ALTER FUNCTION refs_read_native_expense_create_options(uuid,uuid,uuid)
  RENAME TO refs_read_native_expense_create_options_395;
...
REVOKE ALL ON FUNCTION refs_create_native_expense_395(uuid,uuid,uuid,text,text,text,text,text,date,char,numeric,text,uuid[],text),refs_read_native_expense_create_options_395(uuid,uuid,uuid) FROM PUBLIC,refs_app;
