...
-- A signed, complete production export can legitimately have zero rows for a
-- scoped view.  Preserve that fact as an immutable observation; it never
-- deletes, replaces, or posts accounting data.
...
BEGIN
  ...
  IF p_captured_at>clock_timestamp()+interval '5 minutes' OR upper(btrim(p_environment)) NOT IN ('SANDBOX','PRODUCTION')
     ...
     OR jsonb_typeof(p_receipts)<>'array' OR (jsonb_array_length(p_receipts)=0 AND upper(btrim(p_environment))<>'PRODUCTION') THEN
