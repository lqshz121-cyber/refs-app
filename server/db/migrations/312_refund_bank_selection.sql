...
BEGIN
  IF p_kind IS NULL OR p_kind NOT IN ('AP_PAYMENT','AR_RECEIPT','AR_REFUND') THEN
    RAISE EXCEPTION 'Unsupported settlement kind' USING ERRCODE='22023';
  END IF;
  PERFORM refs_assert_scope(p_tenant,p_entity,CASE p_kind WHEN 'AP_PAYMENT' THEN 'AP.PAYMENT.CREATE' WHEN 'AR_RECEIPT' THEN 'AR.RECEIPT.CREATE' ELSE 'AR.REFUND.CREATE' END);
