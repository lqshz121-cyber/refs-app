...
-- A staged checkpoint is durable state.  Keep the exact original row array so
-- a resumed worker cannot substitute an unpersisted chunk and defer discovery
-- of the change until the final accounting transaction.
ALTER TABLE wbs_test_bank_import_stage
  ADD COLUMN expected_rows jsonb;
ALTER TABLE wbs_test_bank_import_stage
  ADD CONSTRAINT wbs_test_bank_import_stage_expected_rows_shape
  CHECK(expected_rows IS NULL OR jsonb_typeof(expected_rows)='array');
...
BEGIN
  ...
  IF position(old_columns IN definition)=0 OR position(old_values IN definition)=0 OR position(old_anchor IN definition)=0
     OR position(old_conflict IN definition)=0 OR position(old_response IN definition)=0 THEN
    RAISE EXCEPTION 'Unexpected staged WBS Bank begin definition before checkpoint-integrity upgrade' USING ERRCODE='22023';
  END IF;
  ...
  definition:=replace(definition,old_conflict,new_conflict);
  definition:=replace(definition,old_response,new_response);
  EXECUTE definition;
END;
$migration$;
...
BEGIN
  ...
  IF position(old_declare IN definition)=0 OR position(old_anchor IN definition)=0 OR position(old_replay IN definition)=0 OR position(old_append IN definition)=0 THEN
    RAISE EXCEPTION 'Unexpected staged WBS Bank append definition before checkpoint-integrity upgrade' USING ERRCODE='22023';
  END IF;
  definition:=replace(definition,old_declare,new_declare);
  definition:=replace(definition,old_anchor,new_anchor);
  definition:=replace(definition,old_replay,new_replay);
  definition:=replace(definition,old_append,new_append);
  EXECUTE definition;
END;
$migration$;
...
BEGIN
  ...
  IF position(old_replay IN definition)=0 OR position(old_response IN definition)=0 THEN
    RAISE EXCEPTION 'Unexpected staged WBS Bank finalize definition before checkpoint-integrity upgrade' USING ERRCODE='22023';
  END IF;
  definition:=replace(definition,old_replay,new_replay);
  definition:=replace(definition,old_response,new_response);
  EXECUTE definition;
