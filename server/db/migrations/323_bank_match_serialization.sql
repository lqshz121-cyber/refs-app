...
CREATE OR REPLACE FUNCTION refs_create_bank_payment_match(
...
BEGIN
  ...
  IF p_request_hash IS DISTINCT FROM refs_bank_match_hash(p_tenant,p_entity,p_bank_source,p_payment_occurrence,p_expected_bank_version,p_expected_occurrence_version,p_reason) THEN
  ...
  IF p_expected_bank_version IS NULL OR p_expected_bank_version<0 OR p_expected_occurrence_version IS NULL OR p_expected_occurrence_version<0
     OR p_bank_source IS NULL OR p_payment_occurrence IS NULL OR COALESCE(length(p_idempotency_key),0) NOT BETWEEN 8 AND 200
  ...
  IF receipt.actor_id IS DISTINCT FROM actor THEN RAISE EXCEPTION 'Bank matching retry belongs to another actor' USING ERRCODE='42501'; END IF;
  IF receipt.request_hash IS DISTINCT FROM p_request_hash THEN RAISE EXCEPTION 'Idempotency key reused with a different request' USING ERRCODE='23505'; END IF;
  IF receipt.status='SUCCEEDED' THEN RETURN receipt.response_body||jsonb_build_object('idempotent',true); END IF;

  SELECT bank_account_ref INTO account_ref FROM bank_source
  ...
  -- Serialize with reconciliation sign-off before taking bank/match row locks.
  -- Do not take reconciliation row locks here: sign-off owns its row first.
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Bank transaction was not found in the selected entity' USING ERRCODE='P0002'; END IF;
  IF bank_row.bank_account_ref IS DISTINCT FROM account_ref THEN RAISE EXCEPTION 'Bank account changed during matching review' USING ERRCODE='40001'; END IF;
...
CREATE OR REPLACE FUNCTION refs_unmatch_bank_payment(
...
DECLARE bank_row bank_source; account_ref text;
...
BEGIN
  ...
  IF actor IS NULL THEN RAISE EXCEPTION 'Authenticated actor missing' USING ERRCODE='42501'; END IF;
  IF p_request_hash IS DISTINCT FROM refs_bank_unmatch_hash(p_tenant,p_entity,p_bank_source,p_bank_match,p_expected_match_version,p_reason) THEN
  ...
  IF p_expected_match_version IS NULL OR p_expected_match_version<0
     OR p_bank_source IS NULL OR p_bank_match IS NULL OR COALESCE(length(p_idempotency_key),0) NOT BETWEEN 8 AND 200
     OR COALESCE(length(btrim(p_reason)),0) NOT BETWEEN 8 AND 2000 THEN
  ...
  -- Do not take reconciliation row locks here: sign-off owns its row first.
  PERFORM pg_advisory_xact_lock(hashtextextended(p_tenant::text||':'||p_entity::text||':'||account_ref,0));
  SELECT * INTO bank_row FROM bank_source WHERE tenant_id=p_tenant AND entity_id=p_entity AND bank_source_id=p_bank_source FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Bank transaction was not found in the selected entity' USING ERRCODE='P0002'; END IF;
  IF bank_row.bank_account_ref IS DISTINCT FROM account_ref THEN RAISE EXCEPTION 'Bank account changed during matching review' USING ERRCODE='40001'; END IF;
  IF EXISTS(SELECT 1 FROM reconciliation WHERE tenant_id=p_tenant AND entity_id=p_entity AND bank_account_ref=account_ref
    AND status='RECONCILED' AND statement_ending_date>=bank_row.transaction_date) THEN
