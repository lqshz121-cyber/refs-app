...
-- Extend the closed idempotency scope allowlist rather than accepting an
-- arbitrary caller-supplied operation scope.
...
DECLARE actor text:=refs_current_actor(); schedule ai_amortization_schedule; schedule_line ai_amortization_schedule_line;
...
BEGIN
  ...
  END IF;
  PERFORM refs_reserve_idempotency(p_tenant,'AI_AMORTIZATION_DRAFT:'||p_entity,p_idempotency_key,p_request_hash,actor);
  SELECT response_body INTO create_result FROM idempotency_receipt WHERE tenant_id=p_tenant AND operation_scope='AI_AMORTIZATION_DRAFT:'||p_entity AND idempotency_key=p_idempotency_key AND status='SUCCEEDED';
  IF create_result IS NOT NULL THEN RETURN create_result||jsonb_build_object('idempotent',true); END IF;
