...
DECLARE actor text:=refs_current_actor();pair unit_transfer_pair;idem idempotency_receipt;action text:=upper(p_action);target_status journal_status;pair_target_status text;source_hash text;target_hash text;source_result jsonb;target_result jsonb;payload jsonb;
BEGIN
 ...
 pair_target_status:=CASE action WHEN 'SUBMIT' THEN 'PENDING_REVIEW_PAIR' WHEN 'REVIEW' THEN 'PENDING_APPROVAL_PAIR' WHEN 'APPROVE' THEN 'APPROVED_PAIR' ELSE 'DRAFT_PAIR' END;
 target_status:=replace(pair_target_status,'_PAIR','')::journal_status;
 INSERT INTO unit_transfer_internal_gate VALUES(pg_backend_pid(),txid_current(),p_tenant,p_pair,'JOURNAL',pair.source_journal_entry_id,target_status);
 ...
 INSERT INTO unit_transfer_internal_gate VALUES(pg_backend_pid(),txid_current(),p_tenant,p_pair,'JOURNAL',pair.target_journal_entry_id,target_status);
 ...
 INSERT INTO unit_transfer_internal_gate VALUES(pg_backend_pid(),txid_current(),p_tenant,p_pair,'PAIR',p_pair,pair_target_status);
 UPDATE unit_transfer_pair SET status=pair_target_status,revision=revision+1 WHERE tenant_id=p_tenant AND unit_transfer_pair_id=p_pair;
 payload:=jsonb_build_object('schema_version','UNIT_TRANSFER_TRANSITION_RECEIPT_V1','unit_transfer_pair_id',p_pair,'status',pair_target_status,'revision',pair.revision+1,'source_journal',source_result,'target_journal',target_result,'idempotent',false);
 ...
 INSERT INTO outbox_event(tenant_id,entity_id,aggregate_type,aggregate_id,event_type,payload,payload_hash) VALUES(p_tenant,pair.source_entity_id,'UNIT_TRANSFER_PAIR',p_pair,'UNIT_TRANSFER_'||action,payload||jsonb_build_object('entity_id',pair.source_entity_id),refs_jsonb_hash(payload||jsonb_build_object('entity_id',pair.source_entity_id)));
 INSERT INTO outbox_event(tenant_id,entity_id,aggregate_type,aggregate_id,event_type,payload,payload_hash) VALUES(p_tenant,pair.target_entity_id,'UNIT_TRANSFER_PAIR',p_pair,'UNIT_TRANSFER_'||action,payload||jsonb_build_object('entity_id',pair.target_entity_id),refs_jsonb_hash(payload||jsonb_build_object('entity_id',pair.target_entity_id)));
