BEGIN;
DO $$ BEGIN IF EXISTS(SELECT 1 FROM fixed_asset_source_consumption) THEN RAISE EXCEPTION 'Retained source consumption prevents rollback' USING ERRCODE='55006';END IF;END;$$;
DROP TRIGGER fixed_asset_source_journal_link_guard ON source_link;
DROP FUNCTION refs_guard_acquisition_source_journal_link();
DROP TRIGGER fixed_asset_source_attachment_guard ON source_link;
DROP TRIGGER fixed_asset_source_post_guard ON journal_entry;
DROP TRIGGER fixed_asset_source_consumption_insert ON fixed_asset_acquisition_posting;
DROP FUNCTION refs_guard_acquisition_source_attachment();
DROP FUNCTION refs_guard_consumed_asset_source_post();
DROP FUNCTION refs_consume_fixed_asset_source();
...
BEGIN
 ...
 RETURN payload;
END;$$;
DROP TABLE fixed_asset_source_consumption;
ALTER TABLE fixed_asset_acquisition_posting DROP CONSTRAINT fixed_asset_posting_exact_binding;
ALTER TABLE fixed_asset_acquisition_binding DROP CONSTRAINT fixed_asset_binding_source_identity,DROP CONSTRAINT fixed_asset_binding_post_identity,DROP CONSTRAINT fixed_asset_binding_scoped_asset;
ALTER TABLE fixed_asset_register_evidence DROP CONSTRAINT fixed_asset_register_scope_identity;
COMMIT;
