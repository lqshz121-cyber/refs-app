...
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM fixed_asset_disposal_evidence WHERE accumulated_impairment<>0 OR cardinality(impairment_ledger_line_ids)>0)
 OR EXISTS(SELECT 1 FROM audit_event WHERE event_type='FIXED_ASSET_DISPOSAL_REVIEWED' AND metadata->>'carrying_version'='DISPOSAL_CARRYING_V2') THEN
  RAISE EXCEPTION 'Cannot remove impaired disposal carrying model with retained evidence' USING ERRCODE='55006';
 END IF;
END; $$;
ALTER TABLE fixed_asset_disposal_evidence DROP CONSTRAINT fixed_asset_disposal_evidence_carrying_value_check;
ALTER TABLE fixed_asset_disposal_evidence DROP COLUMN impairment_ledger_line_ids,DROP COLUMN accumulated_impairment;
ALTER TABLE fixed_asset_disposal_evidence ADD CHECK(carrying_value=disposed_cost-accumulated_depreciation);
...
BEGIN
  ...
  IF EXISTS(SELECT 1 FROM ledger_line l JOIN journal_entry j ON j.tenant_id=l.tenant_id AND j.entity_id=l.entity_id AND j.journal_entry_id=l.journal_entry_id AND j.status='POSTED'
    WHERE l.tenant_id=p_tenant AND l.entity_id=p_entity AND l.dimensions->>'fixed_asset_register_evidence_id'=p_register::text AND j.journal_date<=p_date
      AND l.account_code IN(SELECT DISTINCT accumulated_impairment_account_code FROM fixed_asset_impairment_assessment_evidence WHERE tenant_id=p_tenant AND entity_id=p_entity AND fixed_asset_register_evidence_id=p_register)
    GROUP BY l.account_code HAVING sum(l.debit_amount-l.credit_amount)<>0) THEN
    RAISE EXCEPTION 'Outstanding Posted impairment requires reconciled disposal carrying evidence' USING ERRCODE='23514';
  END IF;
