...
BEGIN
  ...
  WITH period_scope AS (
    SELECT period_id,starts_on,ends_on FROM accounting_period
    ...
    SELECT count(*)::integer AS month_count FROM wbs_test_import_draft d JOIN period_scope p ON p.period_id=d.period_id
    WHERE d.tenant_id=p_tenant AND d.entity_id=p_entity
  ), h1 AS (
    SELECT count(*)::integer AS h1_count FROM wbs_test_import_draft d
    WHERE d.tenant_id=p_tenant AND d.entity_id=p_entity
  ), bank AS (
    ...
    FROM period_scope p LEFT JOIN wbs_controlled_test_bank_import b
      ON b.tenant_id=p_tenant AND b.entity_id=p_entity AND b.period_id=p.period_id AND b.company_code=p_company_code
      AND b.bank_account_ref='WBS_TEST_BANK_'||replace(p_period_code,'-','_')
  )
  SELECT jsonb_build_object('period_id',p.period_id::text,'starts_on',p.starts_on::text,'ends_on',p.ends_on::text,
    'h1_count',h.h1_count,'month_count',q.month_count,'import_count',b.import_count,'row_count',b.row_count,
    'reconciliation_id',b.reconciliation_id) INTO result
