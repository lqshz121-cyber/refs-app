...
REVOKE EXECUTE ON FUNCTION refs_transition_reconciliation_adjustment_aware(uuid,uuid,uuid,text,bigint,text,text,text) FROM refs_app;
DROP FUNCTION refs_transition_reconciliation_adjustment_aware(uuid,uuid,uuid,text,bigint,text,text,text);
ALTER FUNCTION refs_transition_reconciliation_adjustment_aware_legacy_092(uuid,uuid,uuid,text,bigint,text,text,text)
  RENAME TO refs_transition_reconciliation_adjustment_aware;
...
REVOKE EXECUTE ON FUNCTION refs_start_reconciliation_from_wbs_statement(uuid,uuid,uuid,text,text,text) FROM refs_app;
REVOKE EXECUTE ON FUNCTION refs_wbs_statement_reconciliation_start_hash(uuid,uuid,uuid,text) FROM refs_app;
DROP FUNCTION refs_start_reconciliation_from_wbs_statement(uuid,uuid,uuid,text,text,text);
DROP FUNCTION refs_wbs_statement_reconciliation_start_hash(uuid,uuid,uuid,text);
...
DROP TRIGGER reconciliation_adjustment_draft_wbs_statement_scope_guard ON reconciliation_adjustment_draft;
DROP FUNCTION refs_guard_reconciliation_wbs_statement_adjustment();
DROP TRIGGER reconciliation_item_wbs_statement_scope_guard ON reconciliation_item;
DROP FUNCTION refs_guard_reconciliation_wbs_statement_item();
DROP TRIGGER reconciliation_wbs_statement_scope_guard ON reconciliation;
DROP FUNCTION refs_guard_reconciliation_wbs_statement_scope();
DROP INDEX reconciliation_wbs_bank_statement_receipt_uq;
ALTER TABLE reconciliation DROP CONSTRAINT reconciliation_wbs_bank_statement_receipt_fk;
ALTER TABLE reconciliation DROP COLUMN wbs_bank_statement_receipt_id;
