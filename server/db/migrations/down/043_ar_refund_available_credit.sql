...
DECLARE actor text:=refs_current_actor(); receipt idempotency_receipt; source_adj business_adjustment; period_row accounting_period; entity_row entity; computed_hash text; refunded numeric(20,4); journal_id uuid:=gen_random_uuid(); adjustment_id uuid:=gen_random_uuid(); response jsonb; event_payload jsonb;
BEGIN
 ...
 IF refunded+p_amount>source_adj.amount THEN RAISE EXCEPTION 'AR refund exceeds available posted credit' USING ERRCODE='23514'; END IF;
