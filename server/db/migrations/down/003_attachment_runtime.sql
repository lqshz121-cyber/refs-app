...
DROP FUNCTION IF EXISTS refs_complete_attachment_cleanup(uuid,uuid,uuid,boolean,text);
...
ALTER TABLE attachment DROP CONSTRAINT IF EXISTS attachment_verified_entity_ck,DROP CONSTRAINT IF EXISTS attachment_reservation_window_ck,
  DROP CONSTRAINT IF EXISTS attachment_entity_fk,DROP COLUMN IF EXISTS cleaned_at,DROP COLUMN IF EXISTS cleanup_claimed_at,DROP COLUMN IF EXISTS cleanup_attempts,DROP COLUMN IF EXISTS cleanup_status,
