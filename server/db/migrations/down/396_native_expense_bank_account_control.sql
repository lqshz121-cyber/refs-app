...
BEGIN
  IF EXISTS(SELECT 1 FROM expense) THEN
    RAISE EXCEPTION 'Existing Expenses prevent rollback of their bank-to-cash-GL control boundary' USING ERRCODE='55000';
  END IF;
END;
$$;
...
ALTER FUNCTION refs_create_native_expense_395(uuid,uuid,uuid,text,text,text,text,text,date,char,numeric,text,uuid[],text)
  RENAME TO refs_create_native_expense;
ALTER FUNCTION refs_read_native_expense_create_options_395(uuid,uuid,uuid)
