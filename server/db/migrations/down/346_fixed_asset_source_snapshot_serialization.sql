BEGIN;
DO $$ BEGIN IF EXISTS(SELECT 1 FROM fixed_asset_acquisition_binding) THEN RAISE EXCEPTION 'Retained acquisition evidence requires snapshot serialization' USING ERRCODE='55006';END IF;END;$$;
...
BEGIN
 ...
 RETURN NEW;
END;$$;
DROP FUNCTION refs_serialize_asset_source(uuid,uuid,uuid);
DROP TABLE fixed_asset_source_serialization;
COMMIT;
