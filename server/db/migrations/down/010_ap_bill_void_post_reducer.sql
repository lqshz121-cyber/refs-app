...
BEGIN
  ...
  IF adj.adjustment_kind='AP_VENDOR_CREDIT' THEN
    ...
    UPDATE business_document bd SET posted_credit_adjustments=bd.posted_credit_adjustments+pending.amount,open_balance=bd.open_balance-pending.amount,
      status=CASE WHEN bd.open_balance-pending.amount=0 THEN 'PAID' ELSE 'PARTIALLY_PAID' END,version=bd.version+1,updated_at=clock_timestamp()
    FROM pending WHERE bd.tenant_id=NEW.tenant_id AND bd.entity_id=NEW.entity_id AND bd.business_document_id=pending.business_document_id;
