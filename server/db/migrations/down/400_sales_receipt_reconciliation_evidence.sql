...
DO $$
BEGIN
  IF EXISTS(SELECT 1 FROM reconciliation_item i JOIN bank_match m ON m.tenant_id=i.tenant_id AND m.entity_id=i.entity_id AND m.bank_match_id=i.bank_match_id WHERE m.sales_receipt_id IS NOT NULL) THEN
    RAISE EXCEPTION 'Sales receipt reconciliation evidence prevents destructive rollback' USING ERRCODE='55000';
  END IF;
END;
$$;
DROP FUNCTION refs_set_reconciliation_clearance_385(uuid,uuid,uuid,uuid,bigint,bigint,boolean,text,text,text);
DROP FUNCTION refs_transition_reconciliation_adjustment_aware_385(uuid,uuid,uuid,text,bigint,text,text,text);
CREATE FUNCTION refs_set_reconciliation_clearance_385(
...
END;
$$;
CREATE FUNCTION refs_transition_reconciliation_adjustment_aware_385(
