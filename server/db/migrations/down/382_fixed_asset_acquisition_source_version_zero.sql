BEGIN;
LOCK TABLE fixed_asset_acquisition_binding IN ACCESS EXCLUSIVE MODE;
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM fixed_asset_acquisition_binding WHERE source_document_version = 0) THEN
    RAISE EXCEPTION 'Retained version-zero acquisition bindings require non-negative source version support' USING ERRCODE='55006';
  END IF;
END $$;
