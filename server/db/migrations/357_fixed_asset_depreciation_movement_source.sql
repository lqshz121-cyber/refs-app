...
BEGIN
 ...
 SELECT l.ledger_line_id,jsonb_build_object('ledger_line_id',l.ledger_line_id,'journal_line_id',l.journal_line_id,'journal_entry_id',l.journal_entry_id,'posting_batch_id',l.posting_batch_id,'accounting_period_id',l.period_id,
  ...
  'source_binding_status',CASE WHEN exact_source.source_count=1 THEN exact_source.status ELSE 'BLOCKED_MISSING_EXACT_SOURCE_BINDING' END,
  'source_document_id',CASE WHEN exact_source.source_count=1 THEN exact_source.source_document_id END,
  'source_payload_hash',CASE WHEN exact_source.source_count=1 THEN exact_source.source_payload_hash END,
  'source_document_version',CASE WHEN exact_source.source_count=1 THEN exact_source.source_document_version END,
  'source_link_id',CASE WHEN exact_source.source_count=1 THEN exact_source.source_link_id END,
  'disposal_binding_id',CASE WHEN exact_source.source_count=1 THEN exact_source.disposal_binding_id END,
  'acquisition_binding_id',CASE WHEN exact_source.source_count=1 THEN exact_source.acquisition_binding_id END,
  'depreciation_binding_id',CASE WHEN exact_source.source_count=1 THEN exact_source.depreciation_binding_id END,
  'depreciation_period_id',CASE WHEN exact_source.source_count=1 THEN exact_source.depreciation_period_id END,
  'depreciation_register_evidence_hash',CASE WHEN exact_source.source_count=1 THEN exact_source.depreciation_register_evidence_hash END,
  'depreciation_schedule_snapshot_hash',CASE WHEN exact_source.source_count=1 THEN exact_source.depreciation_schedule_snapshot_hash END,
  'depreciation_policy_snapshot_id',CASE WHEN exact_source.source_count=1 THEN exact_source.depreciation_policy_snapshot_id END,
  'depreciation_policy_snapshot_hash',CASE WHEN exact_source.source_count=1 THEN exact_source.depreciation_policy_snapshot_hash END,
  'depreciation_expected_amount',CASE WHEN exact_source.source_count=1 THEN exact_source.depreciation_expected_amount END,
 ...
 LEFT JOIN LATERAL(
  SELECT retained.*,count(*) OVER() source_count FROM (
    SELECT 'EXACT_DISPOSAL_SOURCE'::text status,b.source_document_id,b.source_payload_hash,b.source_document_version,b.source_link_id,b.binding_id disposal_binding_id,NULL::uuid acquisition_binding_id,NULL::uuid depreciation_binding_id,NULL::uuid depreciation_period_id,NULL::text depreciation_register_evidence_hash,NULL::text depreciation_schedule_snapshot_hash,NULL::uuid depreciation_policy_snapshot_id,NULL::text depreciation_policy_snapshot_hash,NULL::text depreciation_expected_amount
   FROM fixed_asset_disposal_source_binding b
   ...
   WHERE b.tenant_id=p_tenant AND b.entity_id=p_entity AND b.fixed_asset_register_evidence_id=p_asset AND b.journal_entry_id=l.journal_entry_id
   UNION ALL
    SELECT 'EXACT_ACQUISITION_SOURCE',b.source_document_id,b.source_payload_hash,b.source_document_version,b.source_link_id,NULL::uuid,b.binding_id,NULL::uuid,NULL::uuid,NULL::text,NULL::text,NULL::uuid,NULL::text,NULL::text
   ...
   UNION ALL
    SELECT 'EXACT_DEPRECIATION_SOURCE',b.source_document_id,b.source_payload_hash,b.source_document_version,b.source_link_id,NULL::uuid,NULL::uuid,b.binding_id,b.accounting_period_id,b.register_evidence_hash,b.schedule_snapshot_hash,b.policy_snapshot_id,b.policy_snapshot_hash,b.expected_amount::text
   FROM fixed_asset_depreciation_binding b
   JOIN fixed_asset_depreciation_posting posted ON posted.tenant_id=b.tenant_id AND posted.entity_id=b.entity_id AND posted.fixed_asset_register_evidence_id=b.fixed_asset_register_evidence_id AND posted.accounting_period_id=b.accounting_period_id AND posted.binding_id=b.binding_id AND posted.journal_entry_id=b.journal_entry_id
   JOIN source_link s ON s.source_link_id=b.source_link_id AND s.tenant_id=b.tenant_id AND s.entity_id=b.entity_id AND s.link_type='FIXED_ASSET_DEPRECIATION_TO_JE' AND s.source_document_id=b.source_document_id AND s.journal_entry_id=b.journal_entry_id
   WHERE b.tenant_id=p_tenant AND b.entity_id=p_entity AND b.fixed_asset_register_evidence_id=p_asset AND b.journal_entry_id=l.journal_entry_id
  ) retained LIMIT 1
 ...
 LEFT JOIN LATERAL(SELECT a.audit_event_id,count(*) OVER() event_count FROM audit_event a WHERE a.tenant_id=p_tenant AND a.entity_id=p_entity AND a.object_id=l.journal_entry_id AND a.event_type='JOURNAL_POSTED' AND a.object_type='JOURNAL_ENTRY' AND a.action='POST' AND a.permission_used='GL.JE.POST' AND a.actor_id=l.posted_by AND a.actor_type='USER' ORDER BY a.audit_event_id LIMIT 1) audit ON true
 ) SELECT jsonb_build_object('schema_version','FIXED_ASSET_MOVEMENTS_V3','tenant_id',p_tenant,'entity_id',p_entity,'fixed_asset_register_evidence_id',p_asset,'as_of_date',p_as_of,'basis','POSTED_PRIMARY_LEDGER','rows',coalesce((SELECT jsonb_agg(row_data ORDER BY ledger_line_id) FROM rows),'[]'::jsonb),'next_cursor',CASE WHEN (SELECT count(*) FROM candidates)>p_limit THEN (SELECT ledger_line_id FROM page ORDER BY ledger_line_id DESC LIMIT 1) ELSE NULL END) INTO result;
