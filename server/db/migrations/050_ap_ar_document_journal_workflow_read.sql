...
-- Native AP/AR documents are backed by a standard Journal Entry.  Expose its
-- authoritative workflow identity and revision to the authenticated reader;
-- the browser must never infer workflow state from its local document cache.
...
RETURNS TABLE(
  business_document_id uuid, document_number text, counterparty_ref text,
  counterparty_name text, currency char(3), accounting_date date, due_date date,
  gross_amount numeric(20,4), open_balance numeric(20,4), status text,
  posted_journal_entry_id uuid, version bigint, offset_account_code text,
  description text, created_at timestamptz, updated_at timestamptz,
  journal_entry_id uuid, journal_status text, journal_revision bigint, period_id uuid
)
...
BEGIN
  ...
  RETURN QUERY
    SELECT d.business_document_id,d.document_number,d.counterparty_ref,d.counterparty_name,d.currency,
      ...
      journals.journal_entry_id,journals.status::text,journals.revision,journals.period_id
    FROM public.business_document d
    LEFT JOIN public.journal_entry journals ON journals.tenant_id=d.tenant_id AND journals.entity_id=d.entity_id
