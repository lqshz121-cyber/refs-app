BEGIN;
ALTER TABLE fixed_asset_acquisition_binding ADD COLUMN attachment_ids uuid[], ADD COLUMN attachment_snapshot_hash text;
CREATE FUNCTION refs_asset_acquisition_attachment_snapshot(p_tenant uuid,p_entity uuid,p_source uuid) RETURNS text LANGUAGE sql STABLE SET search_path=pg_catalog,public,pg_temp AS $$
 WITH evidence AS (SELECT DISTINCT a.attachment_id,a.content_hash,a.storage_ref,a.storage_version,a.scan_status,a.finalization_status
 ...
 WHERE sl.tenant_id=p_tenant AND sl.entity_id=p_entity AND sl.source_document_id=p_source AND sl.link_type='SOURCE_ATTACHMENT')
 SELECT CASE WHEN count(*)>0 AND bool_and(scan_status='CLEAN' AND finalization_status='VERIFIED_CLEAN') THEN refs_jsonb_hash(jsonb_agg(to_jsonb(evidence) ORDER BY attachment_id)) ELSE NULL END FROM evidence
$$;
REVOKE EXECUTE ON FUNCTION refs_asset_acquisition_attachment_snapshot(uuid,uuid,uuid) FROM PUBLIC,refs_app;
