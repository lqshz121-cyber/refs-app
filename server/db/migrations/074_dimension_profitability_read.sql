...
-- This report reads only immutable, POSTED ledger dimensions.  It deliberately
-- does not derive a property, project, or unit from descriptions, bank account
-- names, source-document headers, or a missing dimension.
CREATE INDEX ledger_line_dimension_profitability_gin_idx
  ON ledger_line USING gin (dimensions jsonb_path_ops);

CREATE FUNCTION refs_get_dimension_profitability(
  p_tenant uuid,
  p_entity uuid,
  p_period uuid,
  p_dimension_type text,
  p_dimension_ref text
)
RETURNS TABLE(
  period_id uuid,
  period_code text,
...
BEGIN
  ...
  IF p_dimension_type NOT IN ('PROPERTY','PROJECT','UNIT') THEN
  ...
  IF p_dimension_ref IS NULL OR p_dimension_ref<>btrim(p_dimension_ref)
      OR length(p_dimension_ref) NOT BETWEEN 1 AND 160
      OR p_dimension_ref ~ '[[:cntrl:]]' THEN
  ...
  dimension_key:=CASE p_dimension_type
    WHEN 'PROPERTY' THEN 'property_ref'
    WHEN 'PROJECT' THEN 'project_ref'
    ELSE 'unit_ref'
  END;
  report_type:=CASE p_dimension_type
    WHEN 'PROPERTY' THEN 'PROPERTY_PNL'
    WHEN 'PROJECT' THEN 'PROJECT_PNL'
    ELSE 'UNIT_PROFITABILITY'
  END;
  ...
  WITH selected_period AS (
    ...
    WHERE l.tenant_id=p_tenant AND l.entity_id=p_entity
      AND j.status='POSTED' AND j.journal_date BETWEEN p.starts_on AND p.ends_on
      AND l.dimensions @> jsonb_build_object(dimension_key,p_dimension_ref)
  ), classified_lines AS (
    SELECT l.*,CASE
      WHEN l.account_code LIKE '4%' THEN 'REVENUE'
      WHEN l.account_code ~ '^[5-9]' THEN 'EXPENSES'
      ELSE NULL
    ...
    WHERE l.statement_section IS NOT NULL
    GROUP BY l.account_code,a.account_name,l.statement_section
  ), account_evidence AS (
    SELECT a.*,ARRAY(
      ...
      WHERE sl.tenant_id=p_tenant AND sl.entity_id=p_entity
        ...
        AND sl.journal_entry_id=ANY(a.journal_entry_ids)
  ...
  SELECT p.period_id,p.period_code,to_char(p.starts_on,'YYYY-MM-DD'),to_char(p.ends_on,'YYYY-MM-DD'),
    p_dimension_type,p_dimension_ref,report_type,r.statement_section,
    'POSTED_LEDGER_DIMENSION_EXACT'::text,
    r.account_code,COALESCE(r.account_name,'Unmapped account'),
    r.period_debit,r.period_credit,
    CASE WHEN r.statement_section='REVENUE' THEN r.period_credit-r.period_debit
         ELSE r.period_debit-r.period_credit END::numeric(20,4),
    r.journal_entry_ids,r.journal_line_ids,r.ledger_line_ids,r.source_document_ids
  FROM account_evidence r
  CROSS JOIN selected_period p
  ORDER BY r.statement_section,r.account_code;
END;
$$;
