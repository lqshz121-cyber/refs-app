BEGIN;
-- Keep the released candidate contract while avoiding a full ledger-first join.
CREATE OR REPLACE FUNCTION refs_read_payment_bank_candidates(p_tenant uuid,p_entity uuid,p_bank_source uuid,p_after uuid DEFAULT NULL,p_limit integer DEFAULT 50)
...
BEGIN
  ...
  WITH candidates AS MATERIALIZED (
    SELECT po.payment_occurrence_id,po.version::text occurrence_revision,po.occurrence_kind,po.business_document_id,
      po.source_document_id,evidence.document_number,evidence.counterparty_ref,evidence.counterparty_name,po.period_id,
      to_char(po.accounting_date,'YYYY-MM-DD') accounting_date,po.currency,po.amount::text amount,
      bank_row.bank_account_ref bank_member_ref,evidence.journal_entry_id,evidence.journal_number,evidence.journal_revision::text journal_revision,
      evidence.journal_line_id,evidence.account_code cash_account_code,evidence.ledger_line_id,(bank_row.transaction_date-po.accounting_date) date_delta_days
    FROM public.payment_occurrence po
    CROSS JOIN LATERAL (
      SELECT bd.document_number,bd.counterparty_ref,bd.counterparty_name,
        je.journal_entry_id,je.journal_number,je.revision journal_revision,
        jl.journal_line_id,jl.account_code,ll.ledger_line_id
      ...
      JOIN public.journal_line jl ON jl.tenant_id=je.tenant_id AND jl.entity_id=je.entity_id AND jl.journal_entry_id=je.journal_entry_id
      ...
      JOIN public.ledger_line ll ON ll.tenant_id=jl.tenant_id AND ll.entity_id=jl.entity_id
        AND ll.journal_entry_id=jl.journal_entry_id AND ll.journal_line_id=jl.journal_line_id
      WHERE je.tenant_id=po.tenant_id AND je.entity_id=po.entity_id AND je.journal_entry_id=po.posted_journal_entry_id
        AND je.status='POSTED' AND je.period_id=po.period_id AND je.currency=po.currency AND je.journal_date=po.accounting_date
      AND 1=(SELECT count(*)
      ...
      -- Preserve parameterized per-occurrence verification. Without this
      -- boundary the planner may join every cash ledger line before LIMIT.
      OFFSET 0
    ) evidence
    WHERE po.tenant_id=p_tenant AND po.entity_id=p_entity AND po.status='POSTED'
      ...
      AND po.payment_occurrence_id>=COALESCE(p_after,'00000000-0000-0000-0000-000000000000'::uuid)
