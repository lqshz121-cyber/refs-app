...
CREATE INDEX payment_occurrence_bill_payment_page_idx ON payment_occurrence
  (tenant_id,entity_id,period_id,created_at DESC,payment_occurrence_id DESC)
  WHERE occurrence_kind='AP_PAYMENT';
CREATE INDEX audit_event_payment_occurrence_trace_idx ON audit_event
  (tenant_id,entity_id,object_id,event_type,occurred_at,audit_event_id)
  WHERE object_type='PAYMENT_OCCURRENCE';

CREATE FUNCTION refs_read_bill_payment_register(
