...
CREATE FUNCTION refs_resolve_wbs_test_bank_match_fixture(p_tenant uuid,p_entity uuid)
RETURNS TABLE(
  period_id uuid,
  bank_source_id uuid,
  bank_version bigint,
  ...
  WITH bank_choice AS (
    ...
    JOIN LATERAL (
      ...
      WHERE bd.tenant_id=b.tenant_id AND bd.entity_id=b.entity_id
        ...
        AND (bd.status IN ('OPEN','PARTIALLY_PAID') OR (active.bank_match_id IS NOT NULL AND bd.business_document_id=active.business_source_document_id AND bd.status='PAID'))
        AND bd.currency=b.currency AND (active.bank_match_id IS NOT NULL OR bd.open_balance>=-b.amount)
        AND sd.document_type='WBS_TEST_PAYABLE'
        AND bd.document_number LIKE 'WBS-TEST-%'
        AND (active.bank_match_id IS NULL OR bd.business_document_id=active.business_source_document_id)
      ORDER BY CASE WHEN bd.business_document_id=active.business_source_document_id THEN 0 ELSE 1 END,
