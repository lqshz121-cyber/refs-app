...
CREATE FUNCTION refs_bind_wbs_test_bank_match_payment_source(
...
BEGIN
  ...
  SELECT * INTO source FROM public.source_document
    WHERE tenant_id=p_tenant AND entity_id=p_entity AND source_document_id=bill.source_document_id
      AND source_system='WBS' AND source_module='payable' AND document_type='WBS_TEST_PAYABLE' AND status='POSTED'
    FOR SHARE;
