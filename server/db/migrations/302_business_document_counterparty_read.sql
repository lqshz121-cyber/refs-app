...
CREATE INDEX member_master_active_ref_page_idx
  ON member_master(tenant_id,entity_id,member_ref COLLATE "C") WHERE active;

CREATE FUNCTION refs_read_business_document_counterparties(
...
BEGIN
  IF p_kind IS NULL OR p_kind NOT IN ('AP_BILL','AR_INVOICE') THEN
    RAISE EXCEPTION 'Unsupported business document kind' USING ERRCODE='22023';
  END IF;
  PERFORM refs_assert_scope(p_tenant,p_entity,CASE p_kind WHEN 'AP_BILL' THEN 'AP.BILL.CREATE' ELSE 'AR.INVOICE.CREATE' END);
  IF p_query IS NULL OR length(p_query)>128 OR p_query<>btrim(p_query)
    ...
    RAISE EXCEPTION 'Counterparty page is invalid' USING ERRCODE='22023';
  END IF;

  WITH candidates AS MATERIALIZED (
    SELECT m.member_ref,m.member_type,m.display_name
      FROM member_master m
     WHERE m.tenant_id=p_tenant AND m.entity_id=p_entity AND m.active
       AND ((p_kind='AP_BILL' AND m.member_type='VENDOR')
         OR (p_kind='AR_INVOICE' AND m.member_type IN ('CUSTOMER','AFFILIATE')))
  ...
  RETURN jsonb_build_object('schema_version','BUSINESS_DOCUMENT_COUNTERPARTIES_V1',
    'entity_id',p_entity,'document_kind',p_kind,'query',p_query,'after_ref',p_after_ref,
    'limit',p_limit,'rows',v_rows,'next_ref',v_next);
END;
$$;

REVOKE ALL ON FUNCTION refs_read_business_document_counterparties(uuid,uuid,text,text,text,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION refs_read_business_document_counterparties(uuid,uuid,text,text,text,integer) TO refs_app;
