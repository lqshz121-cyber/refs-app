...
DECLARE actor text:=refs_current_actor(); receipt idempotency_receipt; evidence wbs_cost_cwip_review_evidence; staging staging_item; source source_document; mapping mapping_snapshot;
DECLARE amount numeric(20,4); cwip_account text; offset_account text; lines jsonb; journal_number text; description text; auto_hash text; response jsonb;
BEGIN
  ...
  INSERT INTO idempotency_receipt(tenant_id,operation_scope,idempotency_key,request_hash,status,actor_id)
    VALUES(p_tenant,'CREATE_WBS_COST_CWIP_DRAFT:'||p_entity,p_idempotency_key,p_request_hash,'IN_PROGRESS',actor)
    ON CONFLICT(tenant_id,operation_scope,idempotency_key) DO NOTHING;
  SELECT * INTO receipt FROM idempotency_receipt WHERE tenant_id=p_tenant
    AND operation_scope='CREATE_WBS_COST_CWIP_DRAFT:'||p_entity AND idempotency_key=p_idempotency_key FOR UPDATE;
  IF receipt.request_hash<>p_request_hash THEN RAISE EXCEPTION 'Idempotency key reused with a different Cost-to-CWIP Draft request' USING ERRCODE='23505'; END IF;
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Cost-to-CWIP Draft staging evidence is missing' USING ERRCODE='23514'; END IF;
  ...
  IF NOT FOUND THEN RAISE EXCEPTION 'Cost-to-CWIP Draft source evidence is missing' USING ERRCODE='23514'; END IF;
  SELECT * INTO mapping FROM mapping_snapshot WHERE tenant_id=p_tenant AND entity_id=p_entity AND mapping_snapshot_id=evidence.mapping_snapshot_id FOR SHARE;
  IF NOT FOUND THEN RAISE EXCEPTION 'Cost-to-CWIP Draft mapping evidence is missing' USING ERRCODE='23514'; END IF;
  IF staging.status IS DISTINCT FROM 'READY_FOR_DRAFT' OR staging.version IS DISTINCT FROM 0 THEN RAISE EXCEPTION 'Cost-to-CWIP Draft staging state is incomplete or changed (status %, version %)',staging.status,staging.version USING ERRCODE='23514'; END IF;
  IF source.status IS DISTINCT FROM 'READY_FOR_DRAFT' OR source.document_type IS DISTINCT FROM 'WBS_COST_CWIP' THEN RAISE EXCEPTION 'Cost-to-CWIP Draft source state is incomplete or changed' USING ERRCODE='23514'; END IF;
  IF mapping.snapshot_hash IS DISTINCT FROM refs_jsonb_hash(jsonb_build_object('input_keys',mapping.input_keys,'output_rules',mapping.output_rules)) THEN RAISE EXCEPTION 'Cost-to-CWIP Draft mapping state is incomplete or changed' USING ERRCODE='23514'; END IF;
  ...
  response:=refs_create_auto_journal(p_tenant,p_entity,staging.staging_item_id,evidence.period_id,staging.version,journal_number,description,lines,'WBS-COST-CWIP:'||p_idempotency_key,auto_hash)||jsonb_build_object('journal_type','AUTO','wbs_cost_cwip_review_evidence_id',p_review,'mapping_snapshot_id',evidence.mapping_snapshot_id);
  UPDATE idempotency_receipt SET status='SUCCEEDED',response_status=201,response_body=response,completed_at=clock_timestamp()
    WHERE tenant_id=p_tenant AND operation_scope='CREATE_WBS_COST_CWIP_DRAFT:'||p_entity AND idempotency_key=p_idempotency_key;
  RETURN response;
END;
$$;
