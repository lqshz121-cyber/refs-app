BEGIN;
ALTER TABLE fixed_asset_disposal_evidence ADD COLUMN accumulated_impairment numeric(20,4) NOT NULL DEFAULT 0 CHECK(accumulated_impairment>=0),ADD COLUMN impairment_ledger_line_ids uuid[] NOT NULL DEFAULT '{}';
DO $$ DECLARE names text[]; BEGIN
 SELECT array_agg(conname) INTO names FROM pg_constraint WHERE conrelid='fixed_asset_disposal_evidence'::regclass AND contype='c'
   AND regexp_replace(pg_get_constraintdef(oid),'[[:space:]()]','','g')='CHECKcarrying_value=disposed_cost-accumulated_depreciation';
 IF coalesce(cardinality(names),0)<>1 THEN RAISE EXCEPTION 'Expected exactly one legacy disposal carrying constraint' USING ERRCODE='55000';END IF;
 EXECUTE format('ALTER TABLE fixed_asset_disposal_evidence DROP CONSTRAINT %I',names[1]);
END; $$;
ALTER TABLE fixed_asset_disposal_evidence ADD CONSTRAINT fixed_asset_disposal_evidence_carrying_value_check CHECK(carrying_value=disposed_cost-accumulated_depreciation-accumulated_impairment AND carrying_value>=0);
