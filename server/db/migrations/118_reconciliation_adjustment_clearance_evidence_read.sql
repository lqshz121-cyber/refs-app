...
-- Preserve the exact prior reader for a reversible migration.  The new reader
-- is deliberately a read-only proof surface; the clearance command still
-- re-checks every invariant under its own transaction locks.
ALTER FUNCTION refs_list_reconciliation_worksheet(uuid,uuid,uuid)
  RENAME TO refs_list_reconciliation_worksheet_117;
REVOKE ALL ON FUNCTION refs_list_reconciliation_worksheet_117(uuid,uuid,uuid) FROM PUBLIC,refs_app;
