...
-- A resolution idempotency key belongs to one resolver as well as one exact
-- canonical action/version/reason payload.  An equally privileged second
-- resolver must not receive or reuse the first resolver's receipt.
...
BEGIN
  ...
  IF idem.actor_id<>actor OR idem.request_hash<>p_request_hash THEN RAISE EXCEPTION 'Idempotency key reused by a different resolver or AI finding resolution payload' USING ERRCODE='23505'; END IF;
