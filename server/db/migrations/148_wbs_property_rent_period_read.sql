...
CREATE FUNCTION refs_list_wbs_property_rent_pickup(p_tenant uuid,p_entity uuid,p_period uuid,p_limit integer DEFAULT 50)
RETURNS TABLE(
...
BEGIN
 IF refs_current_tenant() IS DISTINCT FROM p_tenant OR NOT refs_entity_allowed(p_entity) OR NOT (
  refs_entity_has_permission(p_entity,'WBS.PROPERTY.REVIEW') OR refs_entity_has_permission(p_entity,'WBS.PROPERTY.RENT.REVIEW') OR refs_entity_has_permission(p_entity,'WBS.PROPERTY.RENT.DRAFT')
 ...
 IF NOT EXISTS(SELECT 1 FROM accounting_period p WHERE p.tenant_id=p_tenant AND p.entity_id=p_entity AND p.period_id=p_period) THEN RAISE EXCEPTION 'Property Rent pickup period is outside entity scope' USING ERRCODE='42501';END IF;
 RETURN QUERY
 SELECT a.wbs_property_rent_source_admission_id,r.wbs_property_rent_review_evidence_id,d.wbs_property_rent_draft_evidence_id,
  ...
  (r.wbs_property_rent_review_evidence_id IS NOT NULL AND d.wbs_property_rent_draft_evidence_id IS NULL AND refs_current_actor()<>r.reviewed_by AND refs_current_actor()<>a.admitted_by AND refs_entity_has_permission(p_entity,'WBS.PROPERTY.RENT.DRAFT') AND refs_entity_has_permission(p_entity,'AR.INVOICE.CREATE') AND refs_entity_has_permission(p_entity,'GL.JE.AUTO.CREATE')),false
 ...
 JOIN accounting_period requested ON requested.tenant_id=a.tenant_id AND requested.entity_id=a.entity_id AND requested.period_id=p_period
 ...
 WHERE a.tenant_id=p_tenant AND a.entity_id=p_entity AND ((r.wbs_property_rent_review_evidence_id IS NULL AND sd.accounting_date BETWEEN requested.starts_on AND requested.ends_on) OR r.period_id=p_period)
 ORDER BY a.admitted_at DESC,a.wbs_property_rent_source_admission_id DESC LIMIT p_limit;
END $$;

REVOKE ALL ON FUNCTION refs_list_wbs_property_rent_pickup(uuid,uuid,uuid,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION refs_list_wbs_property_rent_pickup(uuid,uuid,uuid,integer) TO refs_app;
COMMIT;
