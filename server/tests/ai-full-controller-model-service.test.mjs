...
const resultFor=(facts,isMemo=false)=>{const findings=isMemo?facts.chunk_responses.flatMap(item=>item.controller_actions.flatMap(action=>action.finding_ids)):facts.findings.map(item=>item.finding_id);return {traceId:'trace',providerRequestId:'provider-1',model:'controlled-model',elapsedMs:5,result:{headline:isMemo?'Controller Memo':'Chunk review',narrative:'Only retained evidence is summarized.',risk_summary:isMemo?{high:1,medium:1,low:0}:facts.findings[0].evidence.risk_level==='HIGH'?{high:1,medium:0,low:0}:{high:0,medium:1,low:0},controller_actions:[{category:'VENDOR_REVIEW',finding_ids:findings,action:'Human review only.'}],action_flags:flags}};};
