...
import {buildWbsAutoRecDetailCaseBinding,buildWbsMcpReadonlySnapshot} from '../runtime/wbs-mcp-inbound-lineage.mjs';
...
test('signed producer snapshot reaches review/Draft/G11 proof while only a receipt-bound AutoRec detail case can enter review',async()=>{
  ...
  const bankEnvelope=envelope('list_bank_transactions',[{cb_id:'22222222-2222-4222-8222-222222222222',company_code:'COMPANY-A',currency:'USD',account_code:'BANK-1',debtor:'0.0000',lender:'100.0000',set_date:'2026-08-09',posting_date:'2026-08-09',payee_no:'VENDOR-1'}]);
  const unlinkedPd='33333333-3333-4333-8333-333333333333',boundPd='44444444-4444-4444-8444-444444444444',pbGuid='55555555-5555-4555-8555-555555555555';
  const detailEnvelope=envelope('list_autorec_details',[{pd_guid:unlinkedPd,pd_pv_guid:'RELATION-ONLY',cb_id:'RELATION-ONLY',company_code:'COMPANY-A',currency:'USD',biz_type:'WB',deposit:'0.0000',payment:'100.0000',incurred_date:'2026-08-09',posting_date:'2026-08-09',vendor_no:'VENDOR-1',project_guid:'PROJECT-1',cost_code:'COST-1',description:'Unlinked detail'},{pd_guid:boundPd,pd_pv_guid:'RELATION-ONLY',cb_id:'RELATION-ONLY',company_code:'COMPANY-A',currency:'USD',biz_type:'WB',deposit:'0.0000',payment:'100.0000',incurred_date:'2026-08-09',posting_date:'2026-08-09',vendor_no:'VENDOR-1',project_guid:'PROJECT-1',cost_code:'COST-1',description:'Bound detail'}]);
  ...
  const caseEnvelope=envelope('list_autorec_banks',[{pb_guid:pbGuid,company_code:'COMPANY-A',ah_id:'BANK-1',quantity:'1',released_quantity:'0',pay_amount:'100.0000',released:'0.0000',incurred:'0.0000',debit_amount:'0.0000'}]);
  const traceEnvelope=envelope('trace_by_key',[{relation_id:'detail-case-1',relation_type:'DETAIL_TO_CASE',source_key_type:'pd_guid',source_key_value:boundPd,related_key_type:'pb_guid',related_key_value:pbGuid}]);
  traceEnvelope.scope={...traceEnvelope.scope,trace_key_type:'pd_guid',trace_key_value:boundPd};
  const detailBinding=buildWbsAutoRecDetailCaseBinding({detailEnvelope,bankEnvelope:caseEnvelope,traceEnvelope,lookup:{key_type:'pd_guid',key_value:boundPd},relationPolicy:{status:'APPROVED',mapping_type:'WBS_AUTOREC_DETAIL_CASE_RELATION',policy_id:'detail-case-policy-1',version:'1',snapshot_hash:'sha256:'+'c'.repeat(64),relation_type:'DETAIL_TO_CASE',scope:{company_key:'COMPANY-A',currency:'USD'}}});
  ...
  const unsigned=buildWbsMcpReadonlySnapshot({
    ...
    environment:'PRODUCTION',delivery,detachedSignature:{key_id:keyId,algorithm:'Ed25519',value:'placeholder'},bankDirectionConventions:bankRules,payableDirectionConventions:payableRules,autoRecDetailDirectionConventions:detailRules,autoRecDetailCaseBindings:[detailBinding]
  });
  ...
  assert.equal(prepared.staging.length,3);assert.equal(prepared.exceptions.length,1);
  ...
  const detail={...raw('AUTOREC_PAYMENT_DETAIL'),stage:'STAGING_REVIEWED',receipt_id:'receipt-detail',staging_item_id:'stg-detail',raw_event_id:'raw-detail',source_document_id:'doc-detail',bill_no:boundPd,project_ref:'PROJECT-1',project_code:'PROJECT-1',account_before:'600000',account_after:'291001',review_event_id:'review-detail'};
  const detailReview=buildAutoReconciliationReviewRequest({bankStaging:bank,businessStaging:detail,dateWindowDays:0,dateMatchBasis:'BUSINESS_AND_ACCOUNTING'});
  assert.equal(detailReview.status,'REVIEW_REQUIRED');assert.equal(detailReview.trace.business_external_relation_trace_hash,detail.external_trace_hash);
