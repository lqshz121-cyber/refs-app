...
test('Render staging manifest declares every production startup secret and uses locked frontend installs',async()=>{
  ...
  for(const [key,value] of [['NODE_ENV','production'],['REFS_HTTP_HOST','0.0.0.0'],['REFS_PG_REQUIRED','"1"'],['REFS_DEPLOYMENT_ENV','internal-test'],['REFS_INTERNAL_TEST_MODE','ENABLED'],['REFS_INTERNAL_TEST_TENANT_ID','6fb25daf-0799-4805-bede-be54230da33c'],['REFS_INTERNAL_TEST_ENTITY_ID','ca8d23c7-0ea6-4860-8e3e-caf9a3e22ce3'],['REFS_INTERNAL_TEST_ACTOR_ID','refs-internal-readonly'],['REFS_ATTACHMENT_MODE','DISABLED'],['REFS_WBS_INGEST_MODE','DISABLED'],['REFS_WBS_LIVE_PILOT_MODE','ENABLED'],['REFS_WBS_TEST_IMPORT_MODE','DISABLED'],['REFS_CONTROLLED_TEST_AI_WORKFLOW_MODE','DISABLED'],['REFS_AI_MODE','DISABLED']])assert.ok(hasFixed(internalApi.body,key,value),`internal read-only API is missing ${key}=${value}`);
  for(const key of ['DATABASE_URL','MIGRATION_DATABASE_URL','CONTEXT_ISSUER_DATABASE_URL','GRANT_SYNC_DATABASE_URL','WBS_CF_ACCESS_CLIENT_ID','WBS_CF_ACCESS_CLIENT_SECRET','WBS_REFS_AUTH'])assert.match(internalApi.body,new RegExp(`- key: ${key}\\r?\\n\\s+fromService: \\{ type: web, name: refs-accounting-api-staging, envVarKey: ${key} \\}`),`internal read-only API must inherit ${key} only from staging API`);
