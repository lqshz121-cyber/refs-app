...
test('Render staging manifest declares every production startup secret and uses locked frontend installs',async()=>{
  ...
  const api=serviceSection(manifest,'refs-accounting-api-staging'),integrationApi=serviceSection(integrations,'refs-accounting-api-integrations-staging'),worker=serviceSection(integrations,'refs-attachment-cleanup-staging'),outboxWorker=assertOutboxCoverage(manifest,{apiName:'refs-accounting-api-staging',workerName:'refs-outbox-dispatch-staging',fixedTenant:true}),integrationOutboxWorker=assertOutboxCoverage(integrations,{apiName:'refs-accounting-api-integrations-staging',workerName:'refs-outbox-dispatch-integrations-staging'}),web=serviceSection(manifest,'refs-app'),internalWeb=serviceSection(manifest,'refs-internal-test');
  ...
  assert.equal(api.type,'web');assert.equal(integrationApi.type,'web');assert.equal(worker.type,'worker');assert.equal(outboxWorker.type,'worker');assert.equal(integrationOutboxWorker.type,'worker');assert.equal(web.type,'web');assert.equal(internalWeb.type,'web');
  ...
  assert.equal((manifest.match(/autoDeployTrigger: off/g)||[]).length,4,'Stage 1 coordinates API, its outbox consumer, static client, and isolated internal test client');
