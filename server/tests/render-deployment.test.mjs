...
test('Render staging manifest declares every production startup secret and uses locked frontend installs',async()=>{
  ...
  for(const [key,value] of [['NODE_ENV','production'],['REFS_HTTP_HOST','0.0.0.0'],['REFS_PG_REQUIRED','"1"'],['REFS_DEPLOYMENT_ENV','internal-test'],['REFS_INTERNAL_TEST_MODE','ENABLED'],['REFS_INTERNAL_TEST_TENANT_ID','6fb25daf-0799-4805-bede-be54230da33c'],['REFS_INTERNAL_TEST_ENTITY_ID','ca8d23c7-0ea6-4860-8e3e-caf9a3e22ce3'],['REFS_INTERNAL_TEST_ACTOR_ID','refs-internal-readonly'],['REFS_INTERNAL_TEST_PROFILE','FULL_WORKFLOW'],['REFS_INTERNAL_TEST_READER_ACTOR_ID','refs-internal-reader'],['REFS_INTERNAL_TEST_MAKER_ACTOR_ID','refs-internal-maker'],['REFS_INTERNAL_TEST_SUBMITTER_ACTOR_ID','refs-internal-submitter'],['REFS_INTERNAL_TEST_REVIEWER_ACTOR_ID','refs-internal-reviewer'],['REFS_INTERNAL_TEST_APPROVER_ACTOR_ID','refs-internal-approver'],['REFS_INTERNAL_TEST_POSTER_ACTOR_ID','refs-internal-poster'],['REFS_INTERNAL_TEST_RECONCILIATION_STARTER_ACTOR_ID','refs-internal-reconciliation-starter'],['REFS_INTERNAL_TEST_CLEARER_ACTOR_ID','refs-internal-clearer'],['REFS_INTERNAL_TEST_PAYMENT_MAKER_ACTOR_ID','refs-internal-payment-maker'],['REFS_INTERNAL_TEST_REVERSAL_MAKER_ACTOR_ID','refs-internal-reversal-maker'],['REFS_INTERNAL_TEST_ALLOCATOR_ACTOR_ID','refs-internal-allocator'],['REFS_INTERNAL_TEST_REOPENER_ACTOR_ID','refs-internal-reopener'],['REFS_INTERNAL_TEST_PERIOD_CLOSER_ACTOR_ID','refs-internal-period-closer'],['REFS_INTERNAL_TEST_PERIOD_REOPENER_ACTOR_ID','refs-internal-period-reopener'],['REFS_INTERNAL_TEST_CASH_TRANSFER_RECONCILER_ACTOR_ID','refs-internal-cash-transfer-reconciler'],['REFS_ATTACHMENT_MODE','DISABLED'],['REFS_WBS_INGEST_MODE','DISABLED'],['REFS_WBS_LIVE_PILOT_MODE','ENABLED'],['REFS_WBS_TEST_IMPORT_MODE','DISABLED'],['REFS_CONTROLLED_TEST_AI_WORKFLOW_MODE','DISABLED'],['REFS_AI_MODE','DISABLED']])assert.ok(hasFixed(internalApi.body,key,value),`internal full-test API is missing ${key}=${value}`);
  for(const key of ['DATABASE_URL','MIGRATION_DATABASE_URL','CONTEXT_ISSUER_DATABASE_URL','GRANT_SYNC_DATABASE_URL','WBS_CF_ACCESS_CLIENT_ID','WBS_CF_ACCESS_CLIENT_SECRET','WBS_REFS_AUTH'])assert.match(internalApi.body,new RegExp(`- key: ${key}\\r?\\n\\s+fromService: \\{ type: web, name: refs-accounting-api-staging, envVarKey: ${key} \\}`),`internal full-test API must inherit ${key} only from staging API`);
...
test('isolated internal-test Blueprint creates only the full workflow API and browser client',async()=>{
  ...
  assert.match(api.body,/rootDir: server/);assert.match(api.body,/buildCommand: npm ci/);assert.match(api.body,/preDeployCommand: npm run db:up/);assert.match(api.body,/startCommand: npm start/);assert.match(api.body,/healthCheckPath: \/health\/ready/);assert.ok(hasFixed(api.body,'REFS_PG_MIGRATION_STATEMENT_TIMEOUT_MS','"600000"'));
