...
test('Render staging manifest declares every production startup secret and uses locked frontend installs',async()=>{
  ...
  assert.ok(hasFixed(api.body,'REFS_ATTACHMENT_MODE','DISABLED'));assert.ok(hasFixed(api.body,'REFS_WBS_INGEST_MODE','DISABLED'));assert.ok(hasFixed(api.body,'REFS_WBS_LIVE_PILOT_MODE','ENABLED'));
