...
test('LiteLLM gateway redacts secrets and emits a server-only structured request',async()=>{
  ...
  const output=await gateway.analyzeJson({traceId:'refs-test-trace',traceName:'ai-audit-explanation',actorId:'oidc|controller',facts:{entity_id:'entity-1',amount:'100.00',api_key:'do-not-send',nested:{authorization:'secret'}},systemInstruction:'Explain only the retained facts. Never create, approve, or post accounting entries.',jsonSchema:{name:'accounting_analysis',schema:{type:'object',additionalProperties:false,properties:{risk_level:{type:'string'},reason:{type:'string'}},required:['risk_level','reason']}}});
  assert.equal(request.url,'https://igateway.wbm3.com/v1/chat/completions');assert.equal(request.options.headers.authorization,'Bearer sk-virtual-key-12345');const body=JSON.parse(request.options.body);assert.equal(body.temperature,0);assert.equal(body.metadata.trace_id,'refs-test-trace');assert.equal(JSON.parse(body.messages[1].content).facts.api_key,'[REDACTED]');assert.equal(JSON.parse(body.messages[1].content).facts.nested.authorization,'[REDACTED]');assert.equal(output.providerRequestId,'chatcmpl-trace-1');assert.deepEqual(output.result,{risk_level:'HIGH',reason:'Review retained source evidence.'});assert.equal(output.model,'gpt-4.1-mini');assert.equal(output.elapsedMs,5);
});
