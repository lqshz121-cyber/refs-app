...
const costCwipReviewSql=await readFile(new URL('../db/migrations/130_wbs_cost_cwip_review.sql',import.meta.url),'utf8');
const costCwipDraftSql=await readFile(new URL('../db/migrations/131_wbs_cost_cwip_draft.sql',import.meta.url),'utf8');
...
const journalEntryReadSql=await readFile(new URL('../db/migrations/057_journal_entry_read.sql',import.meta.url),'utf8');
...
test('migration manifest freezes normalized up and down artifacts including append-only WBS inbound scope',async()=>{
  ...
  assert.equal(names[0],'001_wbs_accounting_core.sql');
  assert.equal(new Set(names).size,names.length,'migration filenames must be unique');
  for(const name of ['064_journal_entry_read_type_fix.sql','065_bank_match_candidate_read.sql','066_reconciliation_worksheet_read.sql','067_reconciliation_adjustment_draft.sql','068_reconciliation_adjustment_post_guard.sql','061_wbs_inbound_multireceipt_atomic_persistence.sql','062_wbs_inbound_single_receipt_audit_fix.sql','063_wbs_trace_relation_evidence.sql','064_wbs_control_metric_snapshot.sql','065_wbs_autorec_observed_state_evidence.sql','066_wbs_cost_gl_metric_cardinality.sql','067_wbs_control_reconciliation_read.sql','068_wbs_control_reconciliation_mapping_type.sql','069_wbs_autorec_mapping_trace_read.sql','070_wbs_autorec_historical_mapping_read.sql','071_wbs_autorec_matching_policy_read.sql','072_wbs_autorec_historical_matching_policy_read.sql','073_wbs_autorec_execution_reservation.sql'])assert.ok(names.includes(name),`missing migration: ${name}`);
...
test('Journal Entry read is introduced by a forward scoped permission migration and reverses symmetrically',()=>{
  assert.match(journalEntryReadSql,/GL\.JE\.VIEW/);
  assert.match(journalEntryReadSql,/CREATE FUNCTION refs_list_journal_entries/);
  assert.match(journalEntryReadSql,/refs_assert_scope\(p_tenant,p_entity,'GL\.JE\.VIEW'\)/);
  assert.match(journalEntryReadSql,/GRANT EXECUTE ON FUNCTION refs_list_journal_entries/);
  assert.doesNotMatch(journalEntryReadSql,/INSERT INTO journal_entry|INSERT INTO ledger_line/);
  assert.match(journalEntryReadDown,/DROP FUNCTION refs_list_journal_entries/);
  assert.match(journalEntryReadDown,/DELETE FROM permission_catalog WHERE permission_code='GL\.JE\.VIEW'/);
  assert.match(journalEntryReadTypeFixSql,/CREATE OR REPLACE FUNCTION refs_list_journal_entries/);
  assert.match(journalEntryReadTypeFixSql,/j\.status::text/);
...
test('AUTO reversal evidence predicate is extended only by a 頉勳啀 migration',()=>{
