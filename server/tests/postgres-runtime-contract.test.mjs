...
test('migration manifest freezes normalized up and down artifacts including append-only WBS inbound scope',async()=>{
  ...
  assert.equal(MIGRATION_MANIFEST.at(-4).name,'064_journal_entry_read_type_fix.sql');
  assert.equal(MIGRATION_MANIFEST.at(-3).name,'065_bank_match_candidate_read.sql');
  assert.equal(MIGRATION_MANIFEST.at(-2).name,'066_reconciliation_worksheet_read.sql');
  assert.equal(MIGRATION_MANIFEST.at(-1).name,'067_reconciliation_adjustment_draft.sql');
