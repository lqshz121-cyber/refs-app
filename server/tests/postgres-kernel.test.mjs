...
pgTest('WBS Final-1 Controller167 persists five-domain signed controls and exact business evidence with zero accounting action',async()=>{
  ...
  for(const domain of ['BANK','COST','PROPERTY']){
    ...
    const persistedBusinessSource=(await adminPool.query('SELECT r.source_tool,d.source_module FROM wbs_final1_signed_business_source_row r JOIN source_document d ON d.tenant_id=r.tenant_id AND d.entity_id=r.entity_id AND d.source_document_id=r.source_document_id WHERE r.tenant_id=$1 AND r.entity_id=$2 AND r.wbs_final1_retained_evidence_admission_id=$3',[ids.tenantId,ids.entityId,delivery.admission_id])).rows[0];
    assert.deepEqual(persistedBusinessSource,{source_tool,source_module:{BANK:'bankFeed',COST:'cost_general_ledger',PROPERTY:'pmCharge'}[domain]});
