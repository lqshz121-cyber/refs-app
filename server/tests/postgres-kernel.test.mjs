...
pgTest('authoritative Audit Log is permissioned, entity scoped, redacted and keyset paged',async()=>{
  ...
  await adminPool.query(`INSERT INTO audit_event(tenant_id,entity_id,event_type,object_type,object_id,action,actor_id,actor_type,permission_used,request_id,correlation_id,idempotency_key,occurred_at,before_hash,after_hash,reason,metadata) VALUES
    ($1,$2,'AUDIT_LOG_FIXTURE','JOURNAL_ENTRY',$3,'POST','Bearer top-secret','USER','GL.JE.POST','raw-request-new','raw-correlation-new','raw-idempotency-new','2026-08-28T12:00:01.000Z',$5,$6,'raw reason',jsonb_build_object('raw','secret')),
    ($1,$2,'AUDIT_LOG_FIXTURE','JOURNAL_ENTRY',$4,'REVIEW','reviewer','USER','GL.JE.REVIEW','raw-request-old','raw-correlation-old',NULL,'2026-08-28T12:00:00.000Z',NULL,$5,'older reason',jsonb_build_object('older',true))`,[ids.tenantId,ids.entityId,randomUUID(),randomUUID(),hash('audit-before'),hash('audit-after')]);
  const first=await reader.readAuthoritativeAuditLog({tenantId:ids.tenantId,entityId:ids.entityId,limit:1,eventType:'AUDIT_LOG_FIXTURE'});
  assert.equal(first.schema_version,'AUTHORITATIVE_AUDIT_LOG_PAGE_V1');assert.equal(first.scope.entity_id,ids.entityId);assert.equal(first.total_count,2);assert.equal(first.read_count,1);assert.equal(first.events.length,1);assert.equal(first.has_more,true);assert.ok(first.next_cursor);assert.deepEqual(first.redaction,{metadata_excluded:true,reason_excluded:true,request_references_hashed:true});assert.deepEqual(first.action_flags,{can_post:false,can_review:false,can_approve:false,can_create_draft:false});
  const event=first.events[0];assert.equal(event.actor_id,'[REDACTED]');assert.match(event.request_ref_hash,/^sha256:[0-9a-f]{64}$/);assert.match(event.correlation_ref_hash,/^sha256:[0-9a-f]{64}$/);assert.equal(Object.hasOwn(event,'metadata'),false);assert.equal(Object.hasOwn(event,'reason'),false);assert.equal(Object.hasOwn(event,'request_id'),false);
  const second=await reader.readAuthoritativeAuditLog({tenantId:ids.tenantId,entityId:ids.entityId,limit:1,eventType:'AUDIT_LOG_FIXTURE',cursorAt:first.next_cursor.occurred_at,cursorId:first.next_cursor.audit_event_id});assert.equal(second.events.length,1);assert.notEqual(second.events[0].audit_event_id,event.audit_event_id);assert.equal(second.has_more,false);
...
pgTest('WBS H1 control reconciliation binds six exact provider-signed Payables periods to one Posted ledger population and rejects drift',async()=>{
  const ids=await seed({status:'DRAFT',attachmentStatus:null}),periods=[];
  for(let month=1;month<=6;month++){
    const periodId=randomUUID(),periodCode=`2026-${String(month).padStart(2,'0')}`,periodStart=`${periodCode}-01`;
    const periodEnd=new Date(Date.UTC(2026,month,0)).toISOString().slice(0,10);
    await adminPool.query("INSERT INTO accounting_period(period_id,tenant_id,entity_id,period_code,starts_on,ends_on,status,ledger_code) VALUES($1,$2,$3,$4,$5,$6,'OPEN','PRIMARY')",[periodId,ids.tenantId,ids.entityId,periodCode,periodStart,periodEnd]);
    periods.push({periodId,periodCode,periodStart,periodEnd,postingDate:`${periodCode}-15`,businessId:`WBS-H1-BUSINESS-${month}`});
    await installApprovedAiSettingsFixture({pool:adminPool,ids:{...ids,periodId},companyCode:ids.sourceEntityId,settingsVersion:month});
  }
  const importer=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'wbs-h1-provider-importer',['WBS.SNAPSHOT.IMPORT'])}),sources=[];
  for(const [index,period] of periods.entries())sources.push(await retainFinal1PayableFixture({pool:adminPool,kernel:importer,ids,companyCode:ids.sourceEntityId,amount:'100.0000',businessId:period.businessId,postingDate:period.postingDate,invoiceDate:period.postingDate,dateFrom:period.periodStart,dateTo:period.periodEnd,invoiceNo:`WBS-H1-${period.periodCode}`,idempotencyKey:`wbs-h1-final1-${index+1}`,ensureCompanyMapping:index===0}));
  const controlRows=periods.flatMap((period,index)=>[
    {id:index*2+1,com_code:ids.sourceEntityId,posting_date:period.postingDate,cb_id:period.businessId,business_guid:period.businessId,journal_no:'',sort:1,account:'610000',account_code:'H1 expense',debtor:'100.0000',lender:'0',payee:'VENDOR-1',pj_code:'P-1',cost_code:'C-1',unit:'U-1',come_from:'PAYABLE',source:'WBS',review:'Y',closed:'N'},
  ...
  for(const [index,period] of periods.entries()){
    ...
    await adminPool.query("INSERT INTO journal_entry(journal_entry_id,tenant_id,entity_id,period_id,journal_number,journal_type,status,journal_date,currency,created_by) VALUES($1,$2,$3,$4,$5,'MANUAL','DRAFT',$6,'USD','maker')",[journalId,ids.tenantId,ids.entityId,period.periodId,`WBS-H1-${index+1}`,period.postingDate]);
    await adminPool.query("INSERT INTO journal_line(journal_line_id,tenant_id,entity_id,period_id,journal_entry_id,line_no,account_code,debit_amount,credit_amount,member_ref) VALUES($1,$2,$3,$4,$5,1,'610000',100,0,NULL),($6,$2,$3,$4,$5,2,'291001',0,100,'VENDOR-1')",[debitLineId,ids.tenantId,ids.entityId,period.periodId,journalId,creditLineId]);
    await adminPool.query("SET session_replication_role='replica'");await adminPool.query("UPDATE journal_entry SET status='POSTED',reviewed_by='reviewer',approved_by='approver',posted_by='poster',posted_at=now() WHERE journal_entry_id=$1",[journalId]);await adminPool.query("SET session_replication_role='origin'");
    await adminPool.query("INSERT INTO posting_batch(posting_batch_id,tenant_id,entity_id,period_id,idempotency_key,request_hash,posted_by) VALUES($1,$2,$3,$4,$5,$6,'poster')",[batchId,ids.tenantId,ids.entityId,period.periodId,`wbs-h1-post-${index+1}`,hash(`wbs-h1-post-${index+1}`)]);
    const ledgerRows=(await adminPool.query("INSERT INTO ledger_line(tenant_id,entity_id,period_id,posting_batch_id,journal_entry_id,journal_line_id,account_code,member_ref,currency,debit_amount,credit_amount,dimensions,posted_at) SELECT tenant_id,entity_id,period_id,$1,journal_entry_id,journal_line_id,account_code,member_ref,'USD',debit_amount,credit_amount,dimensions,now() FROM journal_line WHERE tenant_id=$2 AND entity_id=$3 AND journal_entry_id=$4 RETURNING ledger_line_id,journal_line_id",[batchId,ids.tenantId,ids.entityId,journalId])).rows;
    await adminPool.query("INSERT INTO source_link(tenant_id,entity_id,link_type,source_document_id,journal_entry_id,created_by) VALUES($1,$2,'SOURCE_TO_JE',$3,$4,'formal-posting')",[ids.tenantId,ids.entityId,source.sourceDocumentId,journalId]);
    for(const ledger of ledgerRows)await adminPool.query("INSERT INTO source_link(tenant_id,entity_id,link_type,journal_entry_id,journal_line_id,posting_batch_id,ledger_line_id,created_by) VALUES($1,$2,'JE_LINE_TO_LEDGER',$3,$4,$5,$6,'formal-posting')",[ids.tenantId,ids.entityId,journalId,ledger.journal_line_id,batchId,ledger.ledger_line_id]);
  }
  const reconciler=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'wbs-h1-reconciliation-controller',['WBS.H1.ACCOUNTING.RECONCILE','AI.ACCOUNTING.SETTINGS.VIEW','WBS.AUTOREC.VIEW'])});
  const settingsHash=await reconciler.inSession(async client=>(await client.query("SELECT refs_jsonb_hash(jsonb_agg(refs_read_wbs_ai_approved_entity_period_settings($1,$2,ap.period_id) ORDER BY ap.period_code)) hash FROM accounting_period ap WHERE ap.tenant_id=$1 AND ap.entity_id=$2 AND ap.ledger_code='PRIMARY' AND ap.period_code~'^2026-0[1-6]$'",[ids.tenantId,ids.entityId])).rows[0].hash);
  const controlShape=(await adminPool.query("SELECT (SELECT count(*)::int FROM wbs_h1_accounting_evidence_line WHERE run_id=$1 AND NOT excluded_from_h1 AND come_from='PAYABLE') line_count,(SELECT count(*)::int FROM wbs_h1_accounting_evidence_line WHERE run_id=$1 AND NOT excluded_from_h1 AND come_from='PAYABLE' AND (completeness_status<>'COMPLETE' OR account_code IS NULL OR business_guid IS NULL)) invalid_count,(SELECT COALESCE(sum(row_count),0)::int FROM wbs_h1_accounting_module_receipt WHERE run_id=$1 AND module_code='PAYABLE') receipt_count,(SELECT jsonb_object_agg(completeness_status,n) FROM (SELECT completeness_status,count(*)::int n FROM wbs_h1_accounting_evidence_line WHERE run_id=$1 GROUP BY completeness_status) s) statuses",[controlRunId])).rows[0];assert.deepEqual(controlShape,{line_count:12,invalid_count:0,receipt_count:12,statuses:{COMPLETE:12}});
  const receipt=await reconciler.retainWbsH1AccountingControlReconciliation({tenantId:ids.tenantId,entityId:ids.entityId,controlRunId,expectedControlReceiptHash:controlReceipt.receipt_hash,expectedSettingsBundleHash:settingsHash,reason:'Controller retained exact provider-to-ledger H1 reconciliation.',idempotencyKey:'wbs-h1-reconcile-273-positive'});
  assert.equal(receipt.status,'MATCHED');assert.equal(receipt.actual_source_count,6);assert.equal(receipt.admission_proofs.length,6);assert.match(receipt.source_population_hash,/^sha256:[0-9a-f]{64}$/);assert.deepEqual([receipt.can_create_draft,receipt.can_review,receipt.can_approve,receipt.can_post],[false,false,false,false]);
  assert.deepEqual((await adminPool.query("SELECT actor_id,actor_type,permission_used FROM audit_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='WBS_H1_ACCOUNTING_CONTROL_RECONCILIATION_RETAINED'",[ids.tenantId,ids.entityId])).rows,[{actor_id:'wbs-h1-reconciliation-controller',actor_type:'USER',permission_used:'WBS.H1.ACCOUNTING.RECONCILE'}]);
  const list=await reconciler.listWbsH1AccountingControlReconciliations({tenantId:ids.tenantId,entityId:ids.entityId,limit:1,offset:0});assert.equal(list.total_count,1);assert.equal(list.page_complete,true);assert.equal(list.cursor_next,null);
  const firstLine=(await adminPool.query('SELECT source_document_line_id,amount FROM source_document_line WHERE tenant_id=$1 AND entity_id=$2 AND source_document_id=$3',[ids.tenantId,ids.entityId,sources[0].sourceDocumentId])).rows[0];
  await adminPool.query("SET session_replication_role='replica'");await adminPool.query('UPDATE source_document_line SET amount=amount+1 WHERE source_document_line_id=$1',[firstLine.source_document_line_id]);await adminPool.query("SET session_replication_role='origin'");
  await assert.rejects(reconciler.retainWbsH1AccountingControlReconciliation({tenantId:ids.tenantId,entityId:ids.entityId,controlRunId,expectedControlReceiptHash:controlReceipt.receipt_hash,expectedSettingsBundleHash:settingsHash,reason:'Reject a provider source amount drift before retaining anything.',idempotencyKey:'wbs-h1-reconcile-273-amount-drift'}),error=>error.code==='23514');
  await adminPool.query("SET session_replication_role='replica'");await adminPool.query('UPDATE source_document_line SET amount=$2 WHERE source_document_line_id=$1',[firstLine.source_document_line_id,firstLine.amount]);await adminPool.query("SET session_replication_role='origin'");
  const firstAdmission=sources[0].delivery.admission_id;await adminPool.query("SET session_replication_role='replica'");await adminPool.query("UPDATE wbs_final1_retained_evidence_admission SET date_from='2025-12-01' WHERE wbs_final1_retained_evidence_admission_id=$1",[firstAdmission]);await adminPool.query("SET session_replication_role='origin'");
  await assert.rejects(reconciler.retainWbsH1AccountingControlReconciliation({tenantId:ids.tenantId,entityId:ids.entityId,controlRunId,expectedControlReceiptHash:controlReceipt.receipt_hash,expectedSettingsBundleHash:settingsHash,reason:'Reject a cross-H1 provider admission without hiding its retained row.',idempotencyKey:'wbs-h1-reconcile-273-cross-range'}),error=>error.code==='23514');
  await adminPool.query("SET session_replication_role='replica'");await adminPool.query('UPDATE wbs_final1_retained_evidence_admission SET date_from=$2 WHERE wbs_final1_retained_evidence_admission_id=$1',[firstAdmission,periods[0].periodStart]);await adminPool.query("SET session_replication_role='origin'");
  const originalRefs=(await adminPool.query('SELECT external_dimension_refs FROM source_document_line WHERE source_document_line_id=$1',[firstLine.source_document_line_id])).rows[0].external_dimension_refs;await adminPool.query("SET session_replication_role='replica'");await adminPool.query("UPDATE source_document_line SET external_dimension_refs=jsonb_set(external_dimension_refs,'{signed_invoice_date}','\"2026-01-16\"'::jsonb) WHERE source_document_line_id=$1",[firstLine.source_document_line_id]);await adminPool.query("SET session_replication_role='origin'");
  await assert.rejects(reconciler.retainWbsH1AccountingControlReconciliation({tenantId:ids.tenantId,entityId:ids.entityId,controlRunId,expectedControlReceiptHash:controlReceipt.receipt_hash,expectedSettingsBundleHash:settingsHash,reason:'Reject signed invoice date drift against the authoritative source dates.',idempotencyKey:'wbs-h1-reconcile-273-date-drift'}),error=>error.code==='23514');
  await adminPool.query("SET session_replication_role='replica'");await adminPool.query('UPDATE source_document_line SET external_dimension_refs=$2::jsonb WHERE source_document_line_id=$1',[firstLine.source_document_line_id,JSON.stringify(originalRefs)]);await adminPool.query("SET session_replication_role='origin'");
  const pendingAttachmentId=randomUUID();await adminPool.query("INSERT INTO attachment(attachment_id,tenant_id,entity_id,name,media_type,size_bytes,content_hash,storage_ref,storage_version,uploaded_by,uploaded_at,scan_status,finalization_status) VALUES($1,$2,$3,'pending-support.pdf','application/pdf',10,$4,$5,'v1','wbs-provider',now(),'PENDING','PENDING')",[pendingAttachmentId,ids.tenantId,ids.entityId,hash('pending-support'),`s3://pending/${pendingAttachmentId}`]);await adminPool.query("SET session_replication_role='replica'");await adminPool.query("INSERT INTO source_link(tenant_id,entity_id,link_type,source_document_id,attachment_id,created_by) VALUES($1,$2,'SOURCE_ATTACHMENT',$3,$4,'wbs-provider')",[ids.tenantId,ids.entityId,sources[0].sourceDocumentId,pendingAttachmentId]);await adminPool.query("SET session_replication_role='origin'");
  await assert.rejects(reconciler.retainWbsH1AccountingControlReconciliation({tenantId:ids.tenantId,entityId:ids.entityId,controlRunId,expectedControlReceiptHash:controlReceipt.receipt_hash,expectedSettingsBundleHash:settingsHash,reason:'Reject an added pending attachment from the exact source evidence set.',idempotencyKey:'wbs-h1-reconcile-273-pending-attachment'}),error=>error.code==='23514');
  await adminPool.query("SET session_replication_role='replica'");await adminPool.query('DELETE FROM source_link WHERE tenant_id=$1 AND entity_id=$2 AND source_document_id=$3 AND attachment_id=$4',[ids.tenantId,ids.entityId,sources[0].sourceDocumentId,pendingAttachmentId]);await adminPool.query('DELETE FROM attachment WHERE attachment_id=$1',[pendingAttachmentId]);await adminPool.query("SET session_replication_role='origin'");
  const draftId=randomUUID();await adminPool.query("INSERT INTO journal_entry(journal_entry_id,tenant_id,entity_id,period_id,journal_number,journal_type,status,journal_date,currency,created_by) VALUES($1,$2,$3,$4,'WBS-H1-EXTRA-DRAFT','MANUAL','DRAFT','2026-01-20','USD','other-maker')",[draftId,ids.tenantId,ids.entityId,periods[0].periodId]);await adminPool.query("INSERT INTO journal_line(tenant_id,entity_id,period_id,journal_entry_id,line_no,account_code,debit_amount,credit_amount,member_ref) VALUES($1,$2,$3,$4,1,'610000',1,0,NULL),($1,$2,$3,$4,2,'291001',0,1,'VENDOR-1')",[ids.tenantId,ids.entityId,periods[0].periodId,draftId]);await adminPool.query("INSERT INTO source_link(tenant_id,entity_id,link_type,source_document_id,journal_entry_id,created_by) VALUES($1,$2,'SOURCE_TO_JE',$3,$4,'other-maker')",[ids.tenantId,ids.entityId,sources[0].sourceDocumentId,draftId]);
  await assert.rejects(reconciler.retainWbsH1AccountingControlReconciliation({tenantId:ids.tenantId,entityId:ids.entityId,controlRunId,expectedControlReceiptHash:controlReceipt.receipt_hash,expectedSettingsBundleHash:settingsHash,reason:'Reject an extra Draft accounting link for the same source.',idempotencyKey:'wbs-h1-reconcile-273-extra-draft'}),error=>error.code==='23514');
...
async function trustedSession(ids,actorId='poster',permissions=['GL.JE.POST']){
  let fixtureAuthority='ANALYSIS';
  if(permissions.length){
    const policy=(await adminPool.query(`SELECT requested.permission,
      service.permission_code IS NOT NULL service_only,human.authority_class
...
pgTest('WBS TEST IMPORT retains an unsigned Payable before an independent human Draft and standard posting workflow',async()=>{
  ...
  assert.equal((await adminPool.query('SELECT status::text FROM source_document WHERE tenant_id=$1 AND entity_id=$2 AND source_document_id=$3',[ids.tenantId,ids.entityId,retained.source_document_id])).rows[0].status,'READY_FOR_DRAFT');
  ...
  const clientRead=await refreshAuthoritativeDocuments({config:{baseUrl:'https://accounting.test',entityId:ids.entityId,periodId:ids.periodId,getAccessToken:async()=>`test-token-${'a'.repeat(32)}`},fetcher:async url=>{
    ...
    const ap=path.endsWith('/ap/bills'),data=ap?listedBills:[];
    return {ok:true,status:200,json:async()=>({ok:true,data,scope:ap?listedBillPage.scope:{...listedBillPage.scope,total_count:0}})};
  }});
...
pgTest('provider-signed Payable admission atomically reaches Review Draft four-role Post and same-JE reports',async()=>{
  ...
  await assert.rejects(reviewerAsMaker.reviewAiWbsPayableDraftProposal({...ids,proposalId:proposed.ai_wbs_payable_draft_proposal_id,decision:'ACCEPTED',reason:'The recommended lines match the reviewed evidence',idempotencyKey:'ai-wbs-payable-proposal-review-pg-reviewer'}),error=>error.code==='42501');
  ...
  await assert.rejects(reviewerAsMaker.createWbsPayableApDraft(draftArgs),error=>error.code==='42501');
...
pgTest('provider-signed Bank source survives exact Match Unmatch adjustment Post reconciliation and same-source reports',async()=>{
  ...
  await assert.rejects(rotatedStarterReviewer.transitionReconciliation({...ids,reconciliationId:started.reconciliation_id,action:'REVIEW',expectedVersion:2,reason:'A rotated starter identity must remain separated from review',idempotencyKey:'wbs-statement-starter-review-sod-001'}),error=>error.code==='42501');
  ...
  await assert.rejects(rotatedClearerSigner.transitionReconciliation({...ids,reconciliationId:started.reconciliation_id,action:'SIGN_OFF',expectedVersion:3,reason:'A rotated clearance identity must remain separated from sign off',idempotencyKey:'wbs-statement-clearer-signoff-sod-001'}),error=>error.code==='42501');
  ...
  await assert.rejects(rotatedMatcherReopener.transitionReconciliation({...ids,reconciliationId:started.reconciliation_id,action:'REOPEN',expectedVersion:4,reason:'A rotated match identity must remain separated from reopen',idempotencyKey:'wbs-statement-matcher-reopen-sod-001'}),error=>error.code==='42501');
...
pgTest('formal IAM grant sync reconciles and revokes desired state with version, idempotency and audit',async()=>{
  const ids=await seed();const actor='iam-subject',validUntil=new Date(Date.now()+60*60*1000).toISOString();
  const sync=new PostgresGrantSync(grantSyncPool,{principalProvider:async()=>({trusted:true,serviceId:'platform-iam-sync'})});
  const first=await sync.reconcile({tenantId:ids.tenantId,actorId:actor,entityId:ids.entityId,permissions:['GL.JE.POST','AP.VIEW'],authorityClass:'POST',validUntil,expectedVersion:0,idempotencyKey:'grant-sync-0001'});
  const replay=await sync.reconcile({tenantId:ids.tenantId,actorId:actor,entityId:ids.entityId,permissions:['AP.VIEW','GL.JE.POST'],authorityClass:'POST',validUntil,expectedVersion:0,idempotencyKey:'grant-sync-0001'});
  assert.equal(first.version,1);assert.equal(replay.idempotent,true);
  const revoked=await sync.reconcile({tenantId:ids.tenantId,actorId:actor,entityId:ids.entityId,permissions:[],authorityClass:'ANALYSIS',validUntil,expectedVersion:1,idempotencyKey:'grant-sync-0002'});
  ...
  await assert.rejects(sync.reconcile({tenantId:ids.tenantId,actorId:actor,entityId:ids.entityId,permissions:['ROOT.ALL'],authorityClass:'ANALYSIS',validUntil,expectedVersion:2,idempotencyKey:'grant-sync-0003'}),error=>error.code==='22023');
  const other=await seed();
  await assert.rejects(sync.reconcile({tenantId:ids.tenantId,actorId:actor,entityId:other.entityId,permissions:['AP.VIEW'],authorityClass:'ANALYSIS',validUntil,expectedVersion:0,idempotencyKey:'grant-sync-0004'}),error=>error.code==='42501');
  const spoofed=new PostgresGrantSync(grantSyncPool,{principalProvider:async()=>({trusted:true,serviceId:'runtime-request'})});
  await assert.rejects(spoofed.reconcile({tenantId:ids.tenantId,actorId:actor,entityId:ids.entityId,permissions:['AP.VIEW'],authorityClass:'ANALYSIS',validUntil,expectedVersion:2,idempotencyKey:'grant-sync-0005'}),error=>error.code==='GRANT_SYNC_PRINCIPAL_DENIED');
  const runtime=await runtimePool.connect();
  ...
  await sync.reconcile({tenantId:ids.tenantId,actorId:actor,entityId:ids.entityId,permissions:['AP.VIEW'],authorityClass:'ANALYSIS',validUntil,expectedVersion:2,idempotencyKey:'grant-sync-0006'});
...
pgTest('finite human role sync enforces exact replacement, service-only deny, expiry, and context SoD',async()=>{
  ...
  const sync=new PostgresGrantSync(grantSyncPool,{principalProvider:async()=>({trusted:true,serviceId:'platform-iam-sync'})});
  for(const sql of [
    "SELECT refs_reconcile_actor_grants($1,'legacy-v1',$2,ARRAY['WBS.SNAPSHOT.IMPORT'],0,'legacy-v1-direct-0001',refs_grant_request_hash($1,'legacy-v1',$2,ARRAY['WBS.SNAPSHOT.IMPORT'],0))",
    "SELECT refs_upgrade_stage1_wbs_autorec_read($1,'legacy-wrapper',$2,'legacy-read-wrapper-0001','sha256:'||repeat('0',64),1)",
    "SELECT refs_upgrade_stage1_wbs_operator_attest($1,'legacy-wrapper',$2,'legacy-attest-wrapper-0001','sha256:'||repeat('0',64),2)",
    "SELECT refs_upgrade_stage1_controlled_test_workflow($1,'legacy-wrapper',$2,'legacy-test-wrapper-0001','sha256:'||repeat('0',64),3)"
  ])await assert.rejects(grantSyncPool.query(sql,[ids.tenantId,ids.entityId]),error=>error.code==='42501');
  assert.equal((await adminPool.query("SELECT count(*)::int n FROM runtime_actor_grant WHERE tenant_id=$1 AND entity_id=$2 AND actor_id LIKE 'legacy-%'",[ids.tenantId,ids.entityId])).rows[0].n,0);
  const draft=await sync.reconcile({tenantId:ids.tenantId,actorId:actor,entityId:ids.entityId,permissions:['AP.VIEW','AP.BILL.CREATE'],authorityClass:'DRAFT',validUntil,expectedVersion:0,idempotencyKey:'finite-human-draft-0001'});
  assert.equal(draft.authority_class,'DRAFT');assert.equal(draft.valid_until,validUntil);assert.equal(draft.version,1);
  const replay=await sync.reconcile({tenantId:ids.tenantId,actorId:actor,entityId:ids.entityId,permissions:['AP.BILL.CREATE','AP.VIEW'],authorityClass:'DRAFT',validUntil,expectedVersion:0,idempotencyKey:'finite-human-draft-0001'});
  assert.equal(replay.idempotent,true);
  const active=(await adminPool.query(`SELECT permission,authority_class,to_char(valid_until AT TIME ZONE 'UTC','YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') valid_until
    FROM runtime_actor_grant WHERE tenant_id=$1 AND entity_id=$2 AND actor_id=$3 AND revoked_at IS NULL ORDER BY permission`,[ids.tenantId,ids.entityId,actor])).rows;
  assert.deepEqual(active,[{permission:'AP.BILL.CREATE',authority_class:'DRAFT',valid_until:validUntil},{permission:'AP.VIEW',authority_class:'DRAFT',valid_until:validUntil}]);
  await assert.rejects(sync.reconcile({tenantId:ids.tenantId,actorId:'auth0|mixed',entityId:ids.entityId,permissions:['GL.JE.CREATE','GL.JE.SUBMIT'],authorityClass:'DRAFT',validUntil,expectedVersion:0,idempotencyKey:'finite-human-mixed-0001'}),error=>error.code==='42501');
  await assert.rejects(sync.reconcile({tenantId:ids.tenantId,actorId:'auth0|payment-reversal-mixed',entityId:ids.entityId,permissions:['AP.PAYMENT.CREATE','AP.PAYMENT.REVERSE'],authorityClass:'PAYMENT',validUntil,expectedVersion:0,idempotencyKey:'finite-human-reversal-mixed-0001'}),error=>error.code==='42501');
  await assert.rejects(sync.reconcile({tenantId:ids.tenantId,actorId:'auth0|service-leak',entityId:ids.entityId,permissions:['WBS.SNAPSHOT.IMPORT'],authorityClass:'DRAFT',validUntil,expectedVersion:0,idempotencyKey:'finite-human-service-0001'}),error=>error.code==='42501');
  await assert.rejects(sync.reconcile({tenantId:ids.tenantId,actorId:'service-post-bypass',entityId:ids.entityId,permissions:['GL.JE.POST'],authorityClass:'SERVICE',validUntil:null,expectedVersion:0,idempotencyKey:'service-post-bypass-0001'}),error=>error.code==='42501');
  await assert.rejects(sync.reconcile({tenantId:ids.tenantId,actorId:'service-mixed-bypass',entityId:ids.entityId,permissions:['WBS.SNAPSHOT.IMPORT','GL.JE.POST'],authorityClass:'SERVICE',validUntil:null,expectedVersion:0,idempotencyKey:'service-mixed-bypass-0001'}),error=>error.code==='42501');
  await assert.rejects(sync.reconcile({tenantId:ids.tenantId,actorId:'missing-policy',entityId:ids.entityId,permissions:['AP.VIEW'],expectedVersion:0,idempotencyKey:'missing-grant-policy-0001'}),error=>error.code==='GRANT_SYNC_POLICY_REQUIRED');
  ...
  const issuedClient=await runtimePool.connect();try{await issuedClient.query('BEGIN');await issuedClient.query('SET LOCAL ROLE refs_app');await issuedClient.query('SELECT refs_bootstrap_context($1)',[issued.contextToken]);assert.equal((await issuedClient.query("SELECT refs_entity_has_permission($1,'GL.JE.REVIEW') allowed",[ids.entityId])).rows[0].allowed,false);await issuedClient.query('ROLLBACK');}finally{issuedClient.release();}
  await adminPool.query("UPDATE runtime_actor_grant SET revoked_at=NULL WHERE tenant_id=$1 AND entity_id=$2 AND actor_id=$3 AND permission='GL.JE.REVIEW'",[ids.tenantId,ids.entityId,actor]);
  assert.equal((await adminPool.query('SELECT count(*)::int n FROM runtime_auth_context WHERE token_hash=$1 AND expires_at<=($2::timestamptz)',[issuedHash,reviewUntil])).rows[0].n,1);
  const exactAuthorities=[['AP.PAYMENT.CREATE','PAYMENT'],['AP.PAYMENT.REVERSE','REVERSAL'],['AP.VENDOR_CREDIT.CREATE','ADJUSTMENT'],['AP.VENDOR_CREDIT.APPLY','ALLOCATION'],['AR.RECEIPT.CREATE','RECEIPT'],['AR.RECEIPT.REVERSE','REVERSAL'],['AR.CREDIT_MEMO.CREATE','ADJUSTMENT'],['AR.CREDIT_MEMO.APPLY','ALLOCATION'],['AR.REFUND.CREATE','REFUND'],['WBS.H1.ACCOUNTING.RECONCILE','RECONCILE']];
  for(const [index,[permission,authorityClass]] of exactAuthorities.entries()){
    const roleActor=`auth0|exact-authority-${index}`;
    const result=await sync.reconcile({tenantId:ids.tenantId,actorId:roleActor,entityId:ids.entityId,permissions:['AP.VIEW',permission],authorityClass,validUntil,expectedVersion:0,idempotencyKey:`exact-authority-${String(index).padStart(2,'0')}-0001`});
    assert.equal(result.authority_class,authorityClass);
  }
...
pgTest('WBS test importer grants eleven exact finite SoD roles without mixed authority',async()=>{
  ...
  const rows=(await adminPool.query(`SELECT actor_id,permission,authority_class,valid_until IS NULL AS no_expiry FROM runtime_actor_grant WHERE tenant_id=$1 AND entity_id=$2 AND actor_id=ANY($3::text[]) ORDER BY actor_id,permission`,[ids.tenantId,ids.entityId,Object.values(actors)])).rows;
...
pgTest('Stage 1 provisioning creates only minimal read scope, replays exactly and grants the observed OIDC subject',async()=>{
  ...
  const environment={
    ...
    REFS_STAGE1_OIDC_SUBJECT:'auth0|observed-stage1-subject',REFS_STAGE1_GRANT_EXPECTED_VERSION:'0',REFS_STAGE1_GRANT_IDEMPOTENCY_KEY:'stage1-grant-pg-0001',REFS_STAGE1_GRANT_VALID_UNTIL:new Date(Date.now()+60*60*1000).toISOString(),
  };
...
pgTest('controlled test single-actor self-upgrade is retired with zero grant evidence',async()=>{
  const ids=await seed(),actor='auth0|controlled-test-operator',input={tenantId:ids.tenantId,entityId:ids.entityId,actorId:actor,expectedVersion:3,permissions:STAGE1_CONTROLLED_TEST_WORKFLOW_PERMISSIONS,idempotencyKey:'controlled-workflow-upgrade-0001'};
  const before=(await adminPool.query("SELECT (SELECT count(*) FROM runtime_actor_grant)::int grants,(SELECT count(*) FROM runtime_grant_sync_receipt)::int receipts,(SELECT count(*) FROM audit_event)::int audits,(SELECT count(*) FROM outbox_event)::int outbox")).rows[0];
  await assert.rejects(upgradeStage1ControlledTestWorkflowAccess(grantSyncPool,input),error=>error.code==='STAGE1_CONTROLLED_TEST_UPGRADE_RETIRED');
  const after=(await adminPool.query("SELECT (SELECT count(*) FROM runtime_actor_grant)::int grants,(SELECT count(*) FROM runtime_grant_sync_receipt)::int receipts,(SELECT count(*) FROM audit_event)::int audits,(SELECT count(*) FROM outbox_event)::int outbox")).rows[0];
  assert.deepEqual(after,before);
});
...
pgTest('period close requires current approved settings and statement evidence, then is audited and replayable',async()=>{
  ...
  const reopenArgs={...ids,expectedVersion:Number(first.version),expectedCloseAuditEventId:history.items[0].audit_event_id,expectedReadinessHash:readiness.readiness_hash,reason:'Independent controller approved reopening against the retained close receipt.',idempotencyKey:'period-reopen-key-0001'};
  const sameActor=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'closer',AUTHORITATIVE_WORKFLOW_ROLES.GL_PERIOD_REOPENER.permissions)});await assert.rejects(sameActor.reopenPeriod(reopenArgs),error=>error.code==='42501');
  const reopener=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'independent-reopener',AUTHORITATIVE_WORKFLOW_ROLES.GL_PERIOD_REOPENER.permissions)}),reopened=await reopener.reopenPeriod(reopenArgs),replayReopen=await reopener.reopenPeriod(reopenArgs);assert.equal(reopened.status,'OPEN');assert.equal(reopened.idempotent,false);assert.equal(replayReopen.idempotent,true);assert.equal(reopened.prior_close_audit_event_id,history.items[0].audit_event_id);assert.equal((await adminPool.query('SELECT status FROM accounting_period WHERE period_id=$1',[ids.periodId])).rows[0].status,'OPEN');assert.equal((await adminPool.query("SELECT count(*)::int AS n FROM audit_event WHERE event_type='PERIOD_REOPENED_V1'")).rows[0].n,1);assert.equal((await adminPool.query("SELECT count(*)::int AS n FROM outbox_event WHERE event_type='PERIOD_REOPENED_V1'")).rows[0].n,1);
  const accountingCountsBeforeHistory=(await adminPool.query('SELECT (SELECT count(*) FROM journal_entry)::int journals,(SELECT count(*) FROM journal_line)::int journal_lines,(SELECT count(*) FROM ledger_line)::int ledger_lines')).rows[0];
  const reopenHistory=await kernel.readPeriodReopenHistory({...ids,limit:25});assert.equal(reopenHistory.schema_version,'PERIOD_REOPEN_HISTORY_PAGE_V1');assert.equal(reopenHistory.total_count,1);assert.equal(reopenHistory.read_count,1);assert.equal(reopenHistory.items[0].period_id,ids.periodId);assert.equal(reopenHistory.items[0].prior_close_audit_event_id,history.items[0].audit_event_id);assert.equal(reopenHistory.items[0].prior_readiness_hash,readiness.readiness_hash);assert.equal(reopenHistory.items[0].prior_closed_by,'closer');assert.equal(reopenHistory.items[0].reopened_by,'independent-reopener');assert.match(reopenHistory.items[0].prior_closed_by_hash,/^sha256:[0-9a-f]{64}$/);assert.match(reopenHistory.items[0].reopened_by_hash,/^sha256:[0-9a-f]{64}$/);assert.notEqual(reopenHistory.items[0].prior_closed_by_hash,reopenHistory.items[0].reopened_by_hash);assert.equal(reopenHistory.items[0].separation_verified,true);assert.equal(reopenHistory.items[0].integrity_verified,true);assert.equal(Object.hasOwn(reopenHistory.items[0],'reason'),false);assert.equal(Object.hasOwn(reopenHistory.items[0],'idempotency_key'),false);assert.match(reopenHistory.items[0].reason_hash,/^sha256:[0-9a-f]{64}$/);assert.deepEqual(reopenHistory.action_flags,{can_post:false,can_review:false,can_approve:false,can_create_draft:false});
  assert.deepEqual((await adminPool.query('SELECT (SELECT count(*) FROM journal_entry)::int journals,(SELECT count(*) FROM journal_line)::int journal_lines,(SELECT count(*) FROM ledger_line)::int ledger_lines')).rows[0],accountingCountsBeforeHistory);
  const reopenHistoryDenied=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'reopen-history-denied',['GL.JE.POST'])});await assert.rejects(reopenHistoryDenied.readPeriodReopenHistory({...ids,limit:25}),error=>error.code==='42501');
  const secondReadiness=await kernel.readPeriodCloseReadiness(ids);assert.equal(secondReadiness.ready,true);const secondClose=await kernel.closePeriod({...ids,expectedVersion:Number(reopened.version),expectedReadinessHash:secondReadiness.readiness_hash,reason:'Controller verified the second close against unchanged retained evidence.',idempotencyKey:'close-key-0002'});const secondCloseHistory=await kernel.readPeriodCloseHistory({...ids,limit:25});const secondReopen=await reopener.reopenPeriod({...ids,expectedVersion:Number(secondClose.version),expectedCloseAuditEventId:secondCloseHistory.items[0].audit_event_id,expectedReadinessHash:secondReadiness.readiness_hash,reason:'Independent controller approved the second reopen against its close.',idempotencyKey:'period-reopen-key-0002'});assert.equal(secondReopen.status,'OPEN');
  const reopenPageOne=await kernel.readPeriodReopenHistory({...ids,limit:1});assert.equal(reopenPageOne.total_count,2);assert.equal(reopenPageOne.read_count,1);assert.equal(reopenPageOne.has_more,true);assert.ok(reopenPageOne.next_cursor);const reopenPageTwo=await kernel.readPeriodReopenHistory({...ids,limit:1,cursorAt:reopenPageOne.next_cursor.recorded_at,cursorId:reopenPageOne.next_cursor.audit_event_id});assert.equal(reopenPageTwo.read_count,1);assert.equal(reopenPageTwo.has_more,false);assert.notEqual(reopenPageOne.items[0].audit_event_id,reopenPageTwo.items[0].audit_event_id);
  ...
  await adminPool.query(`INSERT INTO audit_event(tenant_id,entity_id,event_type,object_type,object_id,action,actor_id,actor_type,permission_used,request_id,correlation_id,idempotency_key,before_hash,after_hash,reason,metadata)
    SELECT tenant_id,entity_id,event_type,object_type,object_id,action,actor_id,actor_type,permission_used,request_id||'-drift',correlation_id||'-drift',idempotency_key||'-drift',before_hash,after_hash,reason,metadata||jsonb_build_object('reason','drifted reopen reason') FROM audit_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='PERIOD_REOPENED_V1' ORDER BY occurred_at LIMIT 1`,[ids.tenantId,ids.entityId]);
  await assert.rejects(kernel.readPeriodReopenHistory({...ids,limit:25}),error=>error.code==='23514');
});
...
pgTest('attachment reserve and scanner finalization are entity-scoped, idempotent and immutable',async()=>{
  ...
  const selfScanner=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'attachment-self-scanner',['ATTACHMENT.CREATE','ATTACHMENT.FINALIZE'])});
...
pgTest('legacy AI decision without immutable execution evidence cannot create a standard Draft',async()=>{
  ...
  const before=(await adminPool.query('SELECT count(*)::int n FROM ai_accounting_decision_draft_evidence WHERE tenant_id=$1 AND entity_id=$2',[ids.tenantId,ids.entityId])).rows[0].n;
  await assert.rejects(maker.createAiAccountingDecisionDraft({tenantId:ids.tenantId,entityId:ids.entityId,decisionId:retained.ai_accounting_decision_id,expectedDecisionHash:retained.decision_hash,expectedAcceptanceHash:accepted.evidence_hash,reason:'Incomplete legacy evidence must not create a Draft.',idempotencyKey:'draft-ai-decision-pg-1'}),error=>error.code==='40001');
  assert.equal((await adminPool.query('SELECT count(*)::int n FROM ai_accounting_decision_draft_evidence WHERE tenant_id=$1 AND entity_id=$2',[ids.tenantId,ids.entityId])).rows[0].n,before);
});

pgTest('AI decision run rejects unsafe or duplicate packets before any durable write',async()=>{
  ...
  const population=(packets,tag)=>({populationCount:packets.length,populationHash:hash(`invalid-population-${tag}`)}),firstPacket=packet(first.documentId);
  await assert.rejects(kernel.retainAiAccountingDecisionBatch({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,packets:[firstPacket,firstPacket],...population([firstPacket,firstPacket],'duplicate-packet'),idempotencyKey:'ai-batch-duplicate-packet-001'}),error=>error.code==='23514');
  ...
  const duplicateSource=[firstPacket,packet(first.documentId,{reason:'A distinct packet still cannot decide the same retained source twice.'})];
  await assert.rejects(kernel.retainAiAccountingDecisionBatch({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,packets:duplicateSource,...population(duplicateSource,'duplicate-source'),idempotencyKey:'ai-batch-duplicate-source-001'}),error=>error.code==='23514');
  ...
  const unsafePackets=[packet(first.documentId),unsafe];
  await assert.rejects(kernel.retainAiAccountingDecisionBatch({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,packets:unsafePackets,...population(unsafePackets,'unsafe'),idempotencyKey:'ai-batch-rollback-001'}),error=>error.code==='23514');
  ...
  assert.deepEqual(await baseline(),before,'invalid complete-population requests must leave every durable surface unchanged');
});

pgTest('fatal retained Payable exceptions remain visible to the AI scan only as blocked source evidence',async()=>{
  const ids=await seed({status:'DRAFT'}),wbs=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'wbs-ai-fatal-importer',['WBS.SNAPSHOT.IMPORT'])});
  const retained=await retainFinal1PayableFixture({pool:adminPool,kernel:wbs,ids,invoiceNo:null,idempotencyKey:'wbs-ai-fatal-final1-001'}),aiKernel=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'ai-fatal-controller',['AI.ANALYSIS.EXPLAIN'])}),rows=await aiKernel.listAiAdmittedSourceBookingEvidence({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,limit:10});
  assert.equal(rows.length,1);assert.equal(rows[0].source_document_id,retained.sourceDocumentId);assert.equal(rows[0].retained_outcome,'EXCEPTION_REVIEW_REQUIRED');assert.equal(rows[0].source_status,'QUARANTINED');assert.deepEqual(rows[0].exception_codes,['WBS_PAYABLE_ATTACHMENT_REQUIRED','WBS_PAYABLE_MAPPING_REVIEW_REQUIRED','WBS_PAYABLE_INVOICE_NUMBER_MISSING']);
  const result=await createAiAdmittedSourceUnbookedAnalysisService({bookingEvidenceReader:scope=>aiKernel.listAiAdmittedSourceBookingEvidence(scope)}).analyze({tenantId:ids.tenantId,entityId:ids.entityId,currentAccountingPeriodId:ids.periodId,limit:10});
  assert.equal(result.finding_count,1);assert.equal(result.findings[0].finding_type,'BLOCKED_SOURCE_INCOMPLETE');assert.equal(result.findings[0].source_trace.source_status,'QUARANTINED');assert.equal(result.findings[0].suggested_journal,null);assert.deepEqual(result.findings[0].action_flags,{can_create_draft:false,can_review:false,can_approve:false,can_post:false});
});

pgTest('admitted Payable review findings are retained, assignable, resolved, and superseded when booking appears',async()=>{
  const ids=await seed({status:'DRAFT'}),wbs=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'wbs-admitted-review-importer',['WBS.SNAPSHOT.IMPORT'])});
  const retained=await retainFinal1PayableFixture({pool:adminPool,kernel:wbs,ids,invoiceNo:null,idempotencyKey:'wbs-admitted-review-final1-001'});
  const current=(await adminPool.query("SELECT ai_admitted_source_review_finding_id,finding_hash,finding_type,finding FROM ai_admitted_source_review_current_finding WHERE tenant_id=$1 AND entity_id=$2 AND source_document_id=$3",[ids.tenantId,ids.entityId,retained.sourceDocumentId])).rows;
  assert.equal(current.length,1);assert.equal(current[0].finding_type,'BLOCKED_SOURCE_INCOMPLETE');assert.deepEqual(current[0].finding.action_flags,{can_create_draft:false,can_review:false,can_approve:false,can_post:false});
  assert.equal((await adminPool.query("SELECT count(*)::int n FROM audit_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='AI_ADMITTED_SOURCE_REVIEW_FINDING_RETAINED'",[ids.tenantId,ids.entityId])).rows[0].n,1);
  assert.equal((await adminPool.query("SELECT count(*)::int n FROM outbox_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='AI_ADMITTED_SOURCE_REVIEW_FINDING_RETAINED'",[ids.tenantId,ids.entityId])).rows[0].n,1);
  const controller=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'admitted-review-controller',['AI.FINDING.ASSIGN'])}),candidates=await controller.listAiFindingAssignmentCandidates({tenantId:ids.tenantId,entityId:ids.entityId,limit:100}),candidate=candidates.find(row=>row.finding_kind==='ADMITTED_SOURCE_REVIEW');
  assert.ok(candidate);assert.equal(candidate.finding_id,current[0].ai_admitted_source_review_finding_id);assert.equal(candidate.finding_hash,current[0].finding_hash);assert.equal(candidate.can_post,false);
  const assigned=await controller.assignAiFindingAction({tenantId:ids.tenantId,entityId:ids.entityId,findingKind:'ADMITTED_SOURCE_REVIEW',findingId:candidate.finding_id,findingHash:candidate.finding_hash,owner:'CONTROLLER',dueDate:'2026-08-31',expectedRevision:0,idempotencyKey:'admitted-review-assign-001'});assert.equal(assigned.can_create_draft,false);
  const afterAssignment=await controller.listAiFindingAssignmentCandidates({tenantId:ids.tenantId,entityId:ids.entityId,limit:100});assert.equal(afterAssignment.some(row=>row.finding_kind==='ADMITTED_SOURCE_REVIEW'&&row.finding_id===candidate.finding_id),false);
  const resolver=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'admitted-review-resolver',['AI.FINDING.RESOLVE'])});
  const resolved=await resolver.resolveAiFindingAction({tenantId:ids.tenantId,entityId:ids.entityId,aiFindingActionId:assigned.ai_finding_action_id,findingHash:candidate.finding_hash,reason:'Controller documented the retained source exception follow-up.',expectedRevision:0,idempotencyKey:'admitted-review-resolve-001'});assert.equal(resolved.status,'RESOLVED');assert.equal(resolved.can_post,false);
  await adminPool.query("INSERT INTO business_document(tenant_id,entity_id,source_document_id,document_kind,document_number,counterparty_ref,counterparty_name,currency,accounting_date,gross_amount,open_balance,status,created_by) SELECT $1,$2,$3,'AP_BILL','ADMITTED-REVIEW-BOOKED-1','VENDOR-1','Vendor 1',currency,accounting_date,abs(gross_amount),abs(gross_amount),'DRAFT','AP_PREPARER' FROM source_document WHERE tenant_id=$1 AND entity_id=$2 AND source_document_id=$3",[ids.tenantId,ids.entityId,retained.sourceDocumentId]);
  assert.equal((await adminPool.query('SELECT count(*)::int n FROM ai_admitted_source_review_current_finding WHERE tenant_id=$1 AND entity_id=$2 AND source_document_id=$3',[ids.tenantId,ids.entityId,retained.sourceDocumentId])).rows[0].n,0);
  const lifecycle=(await adminPool.query('SELECT disposition,successor_finding_id,successor_finding_hash FROM ai_admitted_source_review_lifecycle WHERE tenant_id=$1 AND entity_id=$2 AND ai_admitted_source_review_finding_id=$3',[ids.tenantId,ids.entityId,candidate.finding_id])).rows[0];assert.deepEqual(lifecycle,{disposition:'SUPERSEDED_BY_NEW_EVIDENCE',successor_finding_id:null,successor_finding_hash:null});
  const autoSource=await retainFinal1PayableFixture({pool:adminPool,kernel:wbs,ids,invoiceNo:'WBS-AUTO-SUPERSEDE',idempotencyKey:'wbs-admitted-review-final1-002',ensureCompanyMapping:false}),autoFinding=(await adminPool.query('SELECT ai_admitted_source_review_finding_id,finding_hash FROM ai_admitted_source_review_current_finding WHERE tenant_id=$1 AND entity_id=$2 AND source_document_id=$3',[ids.tenantId,ids.entityId,autoSource.sourceDocumentId])).rows[0],autoAssigned=await controller.assignAiFindingAction({tenantId:ids.tenantId,entityId:ids.entityId,findingKind:'ADMITTED_SOURCE_REVIEW',findingId:autoFinding.ai_admitted_source_review_finding_id,findingHash:autoFinding.finding_hash,owner:'CONTROLLER',dueDate:'2026-08-31',expectedRevision:0,idempotencyKey:'admitted-review-assign-002'});
  await adminPool.query("INSERT INTO business_document(tenant_id,entity_id,source_document_id,document_kind,document_number,counterparty_ref,counterparty_name,currency,accounting_date,gross_amount,open_balance,status,created_by) SELECT $1,$2,$3,'AP_BILL','ADMITTED-REVIEW-BOOKED-2','VENDOR-1','Vendor 1',currency,accounting_date,abs(gross_amount),abs(gross_amount),'DRAFT','AP_PREPARER' FROM source_document WHERE tenant_id=$1 AND entity_id=$2 AND source_document_id=$3",[ids.tenantId,ids.entityId,autoSource.sourceDocumentId]);
  const autoAction=(await adminPool.query('SELECT status,resolution_reason,revision FROM ai_finding_action WHERE tenant_id=$1 AND entity_id=$2 AND ai_finding_action_id=$3',[ids.tenantId,ids.entityId,autoAssigned.ai_finding_action_id])).rows[0];assert.deepEqual(autoAction,{status:'RESOLVED',resolution_reason:'Superseded automatically by newer authoritative booking or source evidence.',revision:1});
  assert.equal((await adminPool.query("SELECT count(*)::int n FROM audit_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='AI_ADMITTED_SOURCE_ACTION_SUPERSEDED'",[ids.tenantId,ids.entityId])).rows[0].n,1);
  assert.equal((await adminPool.query("SELECT count(*)::int n FROM outbox_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='AI_ADMITTED_SOURCE_ACTION_SUPERSEDED'",[ids.tenantId,ids.entityId])).rows[0].n,1);
  assert.equal((await adminPool.query('SELECT count(*)::int n FROM journal_entry WHERE tenant_id=$1 AND entity_id=$2',[ids.tenantId,ids.entityId])).rows[0].n,1,'retained review lifecycle must not create a Journal');
});
...
pgTest('retained WBS Payable and approved settings drive the production AI decision through human Post reports and outcome review',async()=>{
  ...
  const aiKernel=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'ai-e2e-controller',['AI.ANALYSIS.EXPLAIN','AI.ACCOUNTING.SETTINGS.VIEW','AI.AMORTIZATION.VIEW'])}),settingsAdapter=createAiAccountingApprovedSettingsAdapter({settingsReader:({tenantId,entityId,periodId})=>aiKernel.readApprovedWbsAiEntityPeriodSettings({tenantId,entityId,periodId,readOnly:true}),accountMasterReader:scope=>aiKernel.readAiAccountMasterBindings(scope)}),classification=createAiInvoiceAccountingClassificationService({classificationInputReader:scope=>aiKernel.readAiInvoiceClassificationSource(scope),duplicateFindingReader:({tenantId,entityId,accountingPeriodId,limit})=>aiKernel.listAiDuplicatePayableFindingsForPeriod({tenantId,entityId,periodId:accountingPeriodId,limit}),capitalizationPolicyReader:scope=>settingsAdapter.readCapitalizationPolicy(scope)}),decisionService=createAiAccountingApprovedDecisionService({populationReader:scope=>aiKernel.readAiAccountingDecisionPopulation(scope),classificationService:classification,scheduleReader:scope=>aiKernel.listAiAmortizationSchedules(scope),settingsAdapter});
  const admittedRows=await aiKernel.listAiAdmittedSourceBookingEvidence({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,limit:10});assert.equal(admittedRows.length,1);assert.equal(admittedRows[0].retained_outcome,'STAGING_REVIEW_REQUIRED');assert.deepEqual(admittedRows[0].exception_codes,['WBS_PAYABLE_ATTACHMENT_REQUIRED','WBS_PAYABLE_MAPPING_REVIEW_REQUIRED']);assert.equal(admittedRows[0].source_status,'PENDING_REVIEW');const admittedReview=await createAiAdmittedSourceUnbookedAnalysisService({bookingEvidenceReader:scope=>aiKernel.listAiAdmittedSourceBookingEvidence(scope)}).analyze({tenantId:ids.tenantId,entityId:ids.entityId,currentAccountingPeriodId:ids.periodId,limit:10});assert.equal(admittedReview.scanned_source_count,1);assert.equal(admittedReview.finding_count,1);assert.equal(admittedReview.findings[0].finding_type,'BLOCKED_SOURCE_INCOMPLETE');assert.deepEqual(admittedReview.findings[0].source_trace.exception_codes,admittedRows[0].exception_codes);assert.equal(admittedReview.findings[0].suggested_journal,null);assert.deepEqual(admittedReview.findings[0].action_flags,{can_create_draft:false,can_review:false,can_approve:false,can_post:false});
  ...
  const accepted=await maker.humanDecideAiAccounting({tenantId:ids.tenantId,entityId:ids.entityId,decisionId:decision.ai_accounting_decision_id,expectedDecisionHash:decision.decision_hash,expectedRevision:0,outcome:'ACCEPTED',reason:'Human maker verified retained WBS source and approved settings.',idempotencyKey:'wbs-ai-human-accept-e2e-001'}),acceptedQueue=await maker.readAiAccountingDecisionQueue({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,limit:25,offset:0}),draftArtifacts=async()=>(await adminPool.query("SELECT (SELECT count(*)::int FROM ai_accounting_decision_draft_evidence WHERE ai_accounting_decision_id=$1) evidence,(SELECT count(*)::int FROM audit_event WHERE object_id=$1 AND event_type='AI_ACCOUNTING_DECISION_DRAFT_CREATED') audit,(SELECT count(*)::int FROM outbox_event WHERE aggregate_id=$1 AND event_type='AI_ACCOUNTING_DECISION_DRAFT_CREATED') outbox,(SELECT count(*)::int FROM idempotency_receipt WHERE operation_scope=$2) receipts",[decision.ai_accounting_decision_id,`AI_ACCOUNTING_DECISION_DRAFT:${ids.entityId}`])).rows[0],beforeDraft=await draftArtifacts();assert.equal(acceptedQueue.rows[0].workflow_state,'ACCEPTED_READY_FOR_DRAFT');assert.equal(acceptedQueue.rows[0].human_decision.evidence_hash,accepted.evidence_hash);assert.equal(acceptedQueue.rows[0].action_flags.can_accept_or_reject,false);assert.equal(acceptedQueue.rows[0].action_flags.can_create_draft,true);
  ...
  const draftQueue=await maker.readAiAccountingDecisionQueue({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,limit:25,offset:0});assert.equal(draftQueue.rows[0].workflow_state,'DRAFT_CREATED');assert.equal(draftQueue.rows[0].draft_evidence.journal_entry_id,draft.journal_entry_id);assert.equal(draftQueue.rows[0].action_flags.can_create_draft,false);assert.equal(draftQueue.rows[0].action_flags.can_submit,false);
...
pgTest('construction loan Draw reaches settings-bound Decision human ACCEPT and standard Draft with drift-safe zero later workflow',async()=>{
  const ids=await seed({status:'DRAFT'}),importBatchId=randomUUID(),rawEventId=randomUUID(),sourceDocumentId=randomUUID(),sourceLineId=randomUUID(),attachmentId=randomUUID(),recordId=`LOAN-DRAW-${randomUUID()}`,rawPayload={schema_version:'CONSTRUCTION_LOAN_TRANSACTION_V1',transaction_kind:'DRAW',direction:'INFLOW',amount:'125.0000',currency:'USD',business_date:'2026-07-16',accounting_date:'2026-07-16',lender_ref:'LENDER-1',loan_ref:'LOAN-1',bank_account_ref:'BANK-1',description:'Construction loan DRAW funding'},rawHash=(await adminPool.query('SELECT refs_jsonb_hash($1::jsonb) value',[JSON.stringify(rawPayload)])).rows[0].value,attachmentRow={storage_version:'loan-draw-attachment-v1'};
  await adminPool.query("UPDATE entity SET source_system='CONSTRUCTION_LOAN_LEDGER' WHERE tenant_id=$1 AND entity_id=$2",[ids.tenantId,ids.entityId]);
  await adminPool.query("INSERT INTO import_batch(import_batch_id,tenant_id,entity_id,connector_code,source_module,source_entity_id,idempotency_key,request_hash) VALUES($1,$2,$3,'LOAN_LEDGER_IMPORT','loan',$4,$5,$6)",[importBatchId,ids.tenantId,ids.entityId,ids.sourceEntityId,`loan-import-${recordId}`,hash(`loan-import-${recordId}`)]);
  await adminPool.query(`INSERT INTO raw_event(raw_event_id,tenant_id,entity_id,import_batch_id,source_system,source_module,source_entity_id,source_record_id,source_version,event_type,occurred_at,payload_hash,payload_ref,correlation_id) VALUES($1,$2,$3,$4,'CONSTRUCTION_LOAN_LEDGER','loan',$5,$6,'1','UPSERT',now(),$7,$8,$6)`,[rawEventId,ids.tenantId,ids.entityId,importBatchId,ids.sourceEntityId,recordId,rawHash,`object://loan-raw/${rawEventId}`]);
  await adminPool.query(`INSERT INTO source_document(source_document_id,tenant_id,entity_id,raw_event_id,source_system,source_module,source_entity_id,source_record_id,source_version,document_type,business_date,accounting_date,currency,gross_amount,source_ref,payload_hash,status) VALUES($1,$2,$3,$4,'CONSTRUCTION_LOAN_LEDGER','loan',$5,$6,'1','CONSTRUCTION_LOAN_TRANSACTION','2026-07-16','2026-07-16','USD',125,$7,$8,'READY_FOR_DRAFT')`,[sourceDocumentId,ids.tenantId,ids.entityId,rawEventId,ids.sourceEntityId,recordId,`CONSTRUCTION_LOAN_LEDGER:${recordId}`,rawHash]);
  await adminPool.query(`INSERT INTO source_document_line(source_document_line_id,tenant_id,entity_id,source_document_id,source_line_id,line_no,description,amount,direction,party_ref,loan_ref,bank_account_ref,external_dimension_refs) VALUES($1,$2,$3,$4,'1',1,'Construction loan DRAW funding',125,'INFLOW','LENDER-1','LOAN-1','BANK-1',$5::jsonb)`,[sourceLineId,ids.tenantId,ids.entityId,sourceDocumentId,JSON.stringify({transaction_kind:'DRAW'})]);
  await adminPool.query(`INSERT INTO attachment(attachment_id,tenant_id,entity_id,name,media_type,size_bytes,content_hash,storage_ref,storage_version,uploaded_by,uploaded_at,verified_at,scan_status,finalization_status,finalized_at) VALUES($1,$2,$3,'construction-loan-draw.pdf','application/pdf',100,$4,$5,$6,'loan-importer',now(),now(),'CLEAN','VERIFIED_CLEAN',now())`,[attachmentId,ids.tenantId,ids.entityId,hash(`loan-attachment-${recordId}`),`object://loan-attachments/${attachmentId}`,attachmentRow.storage_version]);
  await adminPool.query("INSERT INTO source_link(tenant_id,entity_id,link_type,source_document_id,attachment_id,created_by) VALUES($1,$2,'SOURCE_ATTACHMENT',$3,$4,'loan-importer')",[ids.tenantId,ids.entityId,sourceDocumentId,attachmentId]);
  await installApprovedAiSettingsFixture({pool:adminPool,ids,companyCode:ids.sourceEntityId});
  const aiKernel=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'loan-ai-controller',['AI.ANALYSIS.EXPLAIN','AI.ACCOUNTING.SETTINGS.VIEW','AI.AMORTIZATION.VIEW'])}),settingsAdapter=createAiAccountingApprovedSettingsAdapter({settingsReader:({tenantId,entityId,periodId})=>aiKernel.readApprovedWbsAiEntityPeriodSettings({tenantId,entityId,periodId,readOnly:true}),accountMasterReader:scope=>aiKernel.readAiAccountMasterBindings(scope)}),classification=createAiInvoiceAccountingClassificationService({classificationInputReader:scope=>aiKernel.readAiInvoiceClassificationSource(scope),duplicateFindingReader:({tenantId,entityId,accountingPeriodId,limit})=>aiKernel.listAiDuplicatePayableFindingsForPeriod({tenantId,entityId,periodId:accountingPeriodId,limit}),capitalizationPolicyReader:scope=>settingsAdapter.readCapitalizationPolicy(scope)}),decisionService=createAiAccountingApprovedDecisionService({populationReader:scope=>aiKernel.readAiAccountingDecisionPopulation(scope),classificationService:classification,scheduleReader:scope=>aiKernel.listAiAmortizationSchedules(scope),settingsAdapter});
  const loanPopulation=await aiKernel.readAiAccountingDecisionPopulation({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,pageSize:10}),loanIdentity=loanPopulation.loan_rows.map(row=>({tenant_id:row.tenant_id,entity_id:row.entity_id,accounting_period_id:row.accounting_period_id,source_kind:'LOAN_TRANSACTION',accounting_date:row.accounting_date,source_document_id:row.source_document_id,line_no:row.line_no,source_document_line_id:row.source_document_line_id,source_payload_hash:row.source_payload_hash,source_line_hash:row.source_line_hash,retained_outcome:null,retained_exception_codes:[],source_status:row.source_status})),databasePopulationHash=(await adminPool.query('SELECT refs_jsonb_hash($1::jsonb) value',[JSON.stringify(loanIdentity)])).rows[0].value;assert.equal(loanPopulation.total_count,1);assert.equal(loanPopulation.population_hash,databasePopulationHash);assert.equal(loanPopulation.population_validation_hash,canonicalRequestHash(loanIdentity));assert.equal(loanPopulation.loan_rows[0].line_no,1);const batch=await decisionService.analyze({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,limit:10}),packet=batch.packets[0];assert.equal(batch.row_count,1);assert.equal(packet.status,'READY_FOR_HUMAN_REVIEW',JSON.stringify(packet));assert.equal(packet.classification,'LOAN');assert.deepEqual(packet.proposed_journal.lines.map(line=>[line.side,line.account_code]),[['DEBIT','710005'],['CREDIT','710018']]);assert.deepEqual(packet.action_flags,{can_create_draft:false,can_review:false,can_approve:false,can_post:false});assert.equal(packet.source.source_detail.execution_evidence.attachments[0].evidence_type,'source_attachment');
  const retained=(await aiKernel.retainAiAccountingDecisionBatch({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,packets:[packet],populationCount:batch.population.total_count,populationHash:batch.population.population_hash,idempotencyKey:'loan-draw-retain-001'})).receipts[0],maker=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'AP_PREPARER',['GL.JE.CREATE'])}),accepted=await maker.humanDecideAiAccounting({tenantId:ids.tenantId,entityId:ids.entityId,decisionId:retained.ai_accounting_decision_id,expectedDecisionHash:retained.decision_hash,expectedRevision:0,outcome:'ACCEPTED',reason:'Human maker verified the retained construction loan Draw.',idempotencyKey:'loan-draw-accept-001'}),zero=async()=>(await adminPool.query("SELECT (SELECT count(*)::int FROM ai_accounting_decision_draft_evidence WHERE ai_accounting_decision_id=$1) drafts,(SELECT count(*)::int FROM audit_event WHERE tenant_id=$2 AND entity_id=$3 AND event_type='AI_ACCOUNTING_DECISION_DRAFT_CREATED') audits,(SELECT count(*)::int FROM outbox_event WHERE tenant_id=$2 AND entity_id=$3 AND event_type='AI_ACCOUNTING_DECISION_DRAFT_CREATED') outbox",[retained.ai_accounting_decision_id,ids.tenantId,ids.entityId])).rows[0];
  const before=await zero();await adminPool.query('ALTER TABLE attachment DISABLE TRIGGER USER');await adminPool.query("UPDATE attachment SET storage_version='drifted-version' WHERE attachment_id=$1",[attachmentId]);await adminPool.query('ALTER TABLE attachment ENABLE TRIGGER USER');await assert.rejects(maker.createAiAccountingDecisionDraft({tenantId:ids.tenantId,entityId:ids.entityId,decisionId:retained.ai_accounting_decision_id,expectedDecisionHash:retained.decision_hash,expectedAcceptanceHash:accepted.evidence_hash,reason:'Drift must fail.',idempotencyKey:'loan-draw-drift-001'}),error=>error.code==='40001');assert.deepEqual(await zero(),before);await adminPool.query('ALTER TABLE attachment DISABLE TRIGGER USER');await adminPool.query('UPDATE attachment SET storage_version=$2 WHERE attachment_id=$1',[attachmentId,attachmentRow.storage_version]);await adminPool.query('ALTER TABLE attachment ENABLE TRIGGER USER');
  const assertDraftRejectsZero=async idempotencyKey=>{const snapshot=await zero();await assert.rejects(maker.createAiAccountingDecisionDraft({tenantId:ids.tenantId,entityId:ids.entityId,decisionId:retained.ai_accounting_decision_id,expectedDecisionHash:retained.decision_hash,expectedAcceptanceHash:accepted.evidence_hash,reason:'Changed server evidence must fail closed.',idempotencyKey}),error=>error.code==='40001');assert.deepEqual(await zero(),snapshot);};
  await adminPool.query('ALTER TABLE source_document DISABLE TRIGGER USER');for(const [column,value,key] of [['payload_hash',hash('loan-payload-drift'),'loan-draw-payload-drift'],['business_date','2026-07-17','loan-draw-date-drift'],['currency','CAD','loan-draw-currency-drift']]){const original=(await adminPool.query(`SELECT ${column} value FROM source_document WHERE source_document_id=$1`,[sourceDocumentId])).rows[0].value;await adminPool.query(`UPDATE source_document SET ${column}=$2 WHERE source_document_id=$1`,[sourceDocumentId,value]);await assertDraftRejectsZero(key);await adminPool.query(`UPDATE source_document SET ${column}=$2 WHERE source_document_id=$1`,[sourceDocumentId,original]);}await adminPool.query('ALTER TABLE source_document ENABLE TRIGGER USER');
  await adminPool.query('ALTER TABLE source_document_line DISABLE TRIGGER USER');for(const [column,value,key] of [['amount',126,'loan-draw-amount-drift'],['direction','OUTFLOW','loan-draw-line-drift']]){const original=(await adminPool.query(`SELECT ${column} value FROM source_document_line WHERE source_document_line_id=$1`,[sourceLineId])).rows[0].value;await adminPool.query(`UPDATE source_document_line SET ${column}=$2 WHERE source_document_line_id=$1`,[sourceLineId,value]);await assertDraftRejectsZero(key);await adminPool.query(`UPDATE source_document_line SET ${column}=$2 WHERE source_document_line_id=$1`,[sourceLineId,original]);}await adminPool.query('ALTER TABLE source_document_line ENABLE TRIGGER USER');
  await adminPool.query("UPDATE account_master SET active=false WHERE tenant_id=$1 AND entity_id=$2 AND account_code='710005'",[ids.tenantId,ids.entityId]);await assertDraftRejectsZero('loan-draw-account-drift');await adminPool.query("UPDATE account_master SET active=true,requires_member=true,required_member_type='VENDOR' WHERE tenant_id=$1 AND entity_id=$2 AND account_code='710005'",[ids.tenantId,ids.entityId]);await assertDraftRejectsZero('loan-draw-account-member-requirement-drift');await adminPool.query("UPDATE account_master SET requires_member=false,required_member_type=NULL WHERE tenant_id=$1 AND entity_id=$2 AND account_code='710005'",[ids.tenantId,ids.entityId]);await adminPool.query("UPDATE member_master SET active=false WHERE tenant_id=$1 AND entity_id=$2 AND member_ref='BANK-1'",[ids.tenantId,ids.entityId]);await assertDraftRejectsZero('loan-draw-bank-member-drift');await adminPool.query("UPDATE member_master SET active=true WHERE tenant_id=$1 AND entity_id=$2 AND member_ref='BANK-1'",[ids.tenantId,ids.entityId]);
  await adminPool.query("INSERT INTO source_link(tenant_id,entity_id,link_type,source_document_id,journal_entry_id,created_by) VALUES($1,$2,'SOURCE_TO_JE',$3,$4,'parallel-booking')",[ids.tenantId,ids.entityId,sourceDocumentId,ids.journalId]);await assertDraftRejectsZero('loan-draw-existing-draft');await adminPool.query('ALTER TABLE journal_entry DISABLE TRIGGER USER');await adminPool.query("UPDATE journal_entry SET status='POSTED',reviewed_by='reviewer',approved_by='approver',posted_by='poster',posted_at=now() WHERE journal_entry_id=$1",[ids.journalId]);await adminPool.query('ALTER TABLE journal_entry ENABLE TRIGGER USER');await assertDraftRejectsZero('loan-draw-existing-posted');await adminPool.query('ALTER TABLE source_link DISABLE TRIGGER USER');await adminPool.query("DELETE FROM source_link WHERE source_document_id=$1 AND journal_entry_id=$2 AND created_by='parallel-booking'",[sourceDocumentId,ids.journalId]);await adminPool.query('ALTER TABLE source_link ENABLE TRIGGER USER');
  const duplicateBatch=randomUUID(),duplicateRaw=randomUUID(),duplicateDocument=randomUUID(),duplicateLine=randomUUID(),duplicateRecord=`LOAN-DUP-${randomUUID()}`;await adminPool.query("INSERT INTO import_batch(import_batch_id,tenant_id,entity_id,connector_code,source_module,source_entity_id,idempotency_key,request_hash) VALUES($1,$2,$3,'LOAN_LEDGER_IMPORT','loan',$4,$5,$6)",[duplicateBatch,ids.tenantId,ids.entityId,ids.sourceEntityId,`loan-dup-${duplicateRecord}`,hash(`loan-dup-${duplicateRecord}`)]);await adminPool.query(`INSERT INTO raw_event(raw_event_id,tenant_id,entity_id,import_batch_id,source_system,source_module,source_entity_id,source_record_id,source_version,event_type,occurred_at,payload_hash,payload_ref,correlation_id) VALUES($1,$2,$3,$4,'CONSTRUCTION_LOAN_LEDGER','loan',$5,$6,'1','UPSERT',now(),$7,$8,$6)`,[duplicateRaw,ids.tenantId,ids.entityId,duplicateBatch,ids.sourceEntityId,duplicateRecord,hash(`dup-${duplicateRecord}`),`object://loan-raw/${duplicateRaw}`]);await adminPool.query(`INSERT INTO source_document(source_document_id,tenant_id,entity_id,raw_event_id,source_system,source_module,source_entity_id,source_record_id,source_version,document_type,business_date,accounting_date,currency,gross_amount,source_ref,payload_hash,status) VALUES($1,$2,$3,$4,'CONSTRUCTION_LOAN_LEDGER','loan',$5,$6,'1','CONSTRUCTION_LOAN_TRANSACTION','2026-07-16','2026-07-16','USD',125,$7,$8,'READY_FOR_DRAFT')`,[duplicateDocument,ids.tenantId,ids.entityId,duplicateRaw,ids.sourceEntityId,duplicateRecord,`CONSTRUCTION_LOAN_LEDGER:${duplicateRecord}`,hash(`dup-${duplicateRecord}`)]);await adminPool.query(`INSERT INTO source_document_line(source_document_line_id,tenant_id,entity_id,source_document_id,source_line_id,line_no,description,amount,direction,party_ref,loan_ref,bank_account_ref,external_dimension_refs) VALUES($1,$2,$3,$4,'1',1,'Construction loan DRAW funding',125,'INFLOW','LENDER-1','LOAN-1','BANK-1',$5::jsonb)`,[duplicateLine,ids.tenantId,ids.entityId,duplicateDocument,JSON.stringify({transaction_kind:'DRAW'})]);await assertDraftRejectsZero('loan-draw-duplicate-drift');await adminPool.query('ALTER TABLE source_document_line DISABLE TRIGGER USER');await adminPool.query('ALTER TABLE source_document DISABLE TRIGGER USER');await adminPool.query('ALTER TABLE raw_event DISABLE TRIGGER USER');await adminPool.query('DELETE FROM source_document_line WHERE source_document_line_id=$1',[duplicateLine]);await adminPool.query('DELETE FROM source_document WHERE source_document_id=$1',[duplicateDocument]);await adminPool.query('DELETE FROM raw_event WHERE raw_event_id=$1',[duplicateRaw]);await adminPool.query('DELETE FROM import_batch WHERE import_batch_id=$1',[duplicateBatch]);await adminPool.query('ALTER TABLE raw_event ENABLE TRIGGER USER');await adminPool.query('ALTER TABLE source_document ENABLE TRIGGER USER');await adminPool.query('ALTER TABLE source_document_line ENABLE TRIGGER USER');
  const actualVersion=(await adminPool.query('SELECT storage_version FROM attachment WHERE attachment_id=$1',[attachmentId])).rows[0].storage_version;assert.equal(actualVersion,attachmentRow.storage_version);const refreshed=(await decisionService.analyze({tenantId:ids.tenantId,entityId:ids.entityId,accountingPeriodId:ids.periodId,limit:10})).packets[0];assert.equal(refreshed.source.source_line_hash,packet.source.source_line_hash);
  await adminPool.query(`CREATE OR REPLACE FUNCTION refs_test_pause_loan_draft() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN IF NEW.journal_number LIKE 'AI-DEC-%' THEN PERFORM pg_advisory_xact_lock(259259); END IF; RETURN NEW; END $$`);await adminPool.query('CREATE TRIGGER refs_test_pause_loan_draft BEFORE INSERT ON journal_entry FOR EACH ROW EXECUTE FUNCTION refs_test_pause_loan_draft()');const pause=await adminPool.connect();await pause.query('SELECT pg_advisory_lock(259259)');const draftArgs={tenantId:ids.tenantId,entityId:ids.entityId,decisionId:retained.ai_accounting_decision_id,expectedDecisionHash:retained.decision_hash,expectedAcceptanceHash:accepted.evidence_hash,reason:'Create the human-controlled standard Draft.',idempotencyKey:'loan-draw-draft-001'},draftPromise=maker.createAiAccountingDecisionDraft(draftArgs);let waiting=false;for(let attempt=0;attempt<100&&!waiting;attempt++){waiting=(await adminPool.query("SELECT EXISTS(SELECT 1 FROM pg_stat_activity WHERE datname=current_database() AND wait_event_type='Lock' AND query LIKE '%refs_create_ai_accounting_decision_draft%') waiting")).rows[0].waiting;if(!waiting)await new Promise(resolve=>setTimeout(resolve,25));}assert.equal(waiting,true);const mustBlock=async(sql,args)=>{const client=await adminPool.connect();try{await client.query('BEGIN');await client.query("SET LOCAL statement_timeout='250ms'");await assert.rejects(client.query(sql,args),error=>error.code==='57014');await client.query('ROLLBACK');}finally{try{await client.query('ROLLBACK');}catch{}client.release();}};try{await mustBlock('DELETE FROM source_link WHERE source_document_id=$1 AND attachment_id=$2',[sourceDocumentId,attachmentId]);await mustBlock('UPDATE attachment SET storage_version=storage_version WHERE attachment_id=$1',[attachmentId]);await mustBlock(`INSERT INTO source_document_line(source_document_line_id,tenant_id,entity_id,source_document_id,source_line_id,line_no,description,amount,direction) VALUES($1,$2,$3,$4,'parallel',99,'parallel duplicate',125,'INFLOW')`,[randomUUID(),ids.tenantId,ids.entityId,sourceDocumentId]);}finally{await pause.query('SELECT pg_advisory_unlock(259259)');pause.release();}const draft=await draftPromise;await adminPool.query('DROP TRIGGER refs_test_pause_loan_draft ON journal_entry');await adminPool.query('DROP FUNCTION refs_test_pause_loan_draft()');assert.equal(draft.status,'DRAFT');assert.deepEqual(await zero(),{drafts:1,audits:1,outbox:1});const replay=await maker.createAiAccountingDecisionDraft(draftArgs);assert.equal(replay.journal_entry_id,draft.journal_entry_id);assert.equal(replay.idempotent,true);assert.deepEqual(await zero(),{drafts:1,audits:1,outbox:1});const evidence=(await adminPool.query('SELECT ai_accounting_decision_draft_evidence_id,ai_accounting_human_decision_id,journal_entry_id FROM ai_accounting_decision_draft_evidence WHERE ai_accounting_decision_id=$1',[retained.ai_accounting_decision_id])).rows[0],audit=(await adminPool.query("SELECT object_id,metadata FROM audit_event WHERE event_type='AI_ACCOUNTING_DECISION_DRAFT_CREATED' AND object_id=$1",[evidence.journal_entry_id])).rows[0],outbox=(await adminPool.query("SELECT aggregate_id,payload FROM outbox_event WHERE event_type='AI_ACCOUNTING_DECISION_DRAFT_CREATED' AND aggregate_id=$1",[evidence.ai_accounting_decision_draft_evidence_id])).rows[0];for(const payload of [audit.metadata,outbox.payload]){assert.equal(payload.ai_accounting_decision_id,retained.ai_accounting_decision_id);assert.equal(payload.ai_accounting_human_decision_id,evidence.ai_accounting_human_decision_id);assert.equal(payload.journal_entry_id,draft.journal_entry_id);}const draftLines=(await adminPool.query('SELECT line_no,account_code,debit_amount::text,credit_amount::text,member_ref,dimensions FROM journal_line WHERE journal_entry_id=$1 ORDER BY line_no',[draft.journal_entry_id])).rows;assert.deepEqual(draftLines.map(row=>[row.account_code,row.debit_amount,row.credit_amount,row.member_ref,row.dimensions]),[['710005','125.0000','0.0000',null,{}],['710018','0.0000','125.0000',null,{}]]);assert.equal((await adminPool.query('SELECT currency FROM journal_entry WHERE journal_entry_id=$1',[draft.journal_entry_id])).rows[0].currency,'USD');const draftLinks=(await adminPool.query('SELECT link_type,source_document_id,attachment_id,journal_entry_id FROM source_link WHERE tenant_id=$1 AND entity_id=$2 AND (source_document_id=$3 OR journal_entry_id=$4) ORDER BY link_type,attachment_id NULLS LAST',[ids.tenantId,ids.entityId,sourceDocumentId,draft.journal_entry_id])).rows;assert.equal(draftLinks.some(row=>row.link_type==='SOURCE_ATTACHMENT'&&row.attachment_id===attachmentId&&row.source_document_id===sourceDocumentId),true);assert.equal(draftLinks.some(row=>row.source_document_id===sourceDocumentId&&row.journal_entry_id===draft.journal_entry_id),true);assert.equal((await adminPool.query('SELECT count(*)::int n FROM ledger_line WHERE journal_entry_id=$1',[draft.journal_entry_id])).rows[0].n,0);assert.equal((await adminPool.query("SELECT count(*)::int n FROM audit_event WHERE object_id=$1 AND event_type IN ('JOURNAL_SUBMIT','JOURNAL_REVIEW','JOURNAL_APPROVE','JOURNAL_POSTED')",[draft.journal_entry_id])).rows[0].n,0);
});
...
pgTest('server-derived AI Posted outcome review binds decision human workflow ledger trial balance and report atomically',async()=>{
  const ids=await seed({status:'DRAFT',extraAccounts:[{accountCode:'610000',accountName:'Operating expense'}]}),wbs=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'ai-outcome-importer',['WBS.SNAPSHOT.IMPORT'])});
  ...
  await adminPool.query('UPDATE financial_statement_snapshot_row SET period_debit=period_debit+1,ending_debit=ending_debit+1,display_balance=display_balance+1 WHERE financial_statement_snapshot_id=$1 AND account_code=$2',[snapshotId,packet.proposed_journal.lines[0].account_code]);
...
pgTest('production HTTP listener verifies an RS256 access token before DB context issuance and immutable posting',async()=>{
  ...
  const permissions={maker:['GL.JE.CREATE'],submitter:['GL.JE.SUBMIT'],reviewer:['GL.JE.REVIEW'],approver:['GL.JE.APPROVE'],poster:['GL.JE.POST']};
  for(const [actor,grants] of Object.entries(permissions))await trustedSession(ids,actor,grants);
...
pgTest('signed Property Rent charge reaches independent review AR Draft four-role Post and exact reports',async()=>{
  ...
  const aiReader=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'rent-ai-reader',['AI.ANALYSIS.EXPLAIN'])});
  const beforePosting=await aiReader.listAiPropertyRentRevenueReviews({...ids,periodId:ids.periodId,limit:10});assert.equal(beforePosting.length,1);assert.deepEqual({status:beforePosting[0].review_status,posted:beforePosting[0].posted_revenue_amount,variance:beforePosting[0].variance_amount},{status:'UNPOSTED_RENT_CUTOFF_REVIEW',posted:'0.0000',variance:'100.0000'});
  ...
  assert.deepEqual(await aiReader.listAiPropertyRentRevenueReviews({...ids,periodId:ids.periodId,limit:10}),[],'only complete exact Posted ledger lineage may clear the Property Rent review');
...
pgTest('construction-loan rollforward admits a credit-normal liability only through one exact immutable mapping snapshot and retains posted ledger evidence',async()=>{
  ...
  const aiReader=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'construction-loan-ai-reader',['AI.ANALYSIS.EXPLAIN'])}),aiRows=await aiReader.getAiConstructionLoanGlBalances({tenantId:ids.tenantId,entityId:ids.entityId,periodId:ids.periodId});
  assert.equal(aiRows.length,1);assert.deepEqual({entity:aiRows[0].entity_id,currency:aiRows[0].currency,period:aiRows[0].period_id,code:aiRows[0].period_code,start:aiRows[0].period_start,end:aiRows[0].period_end,account:aiRows[0].account_code,closing:aiRows[0].closing_balance},{entity:ids.entityId,currency:'USD',period:ids.periodId,code:'2026-07',start:'2026-07-01',end:'2026-07-31',account:'291001',closing:'100.0000'});assert.deepEqual(aiRows[0].source_document_ids,[trace.documentId]);assert.deepEqual(await aiReader.getAiConstructionLoanLenderBalances({tenantId:ids.tenantId,entityId:ids.entityId,periodId:ids.periodId}),[]);
...
pgTest('signed insurance source reaches independent monthly review, AUTO Draft, standard Post, GL and prepaid rollforward',async()=>{
  ...
  const permissionOnlyEntity=await seed({tenantId:ids.tenantId,status:'DRAFT',attachmentStatus:null}),crossReviewActor='insurance-cross-entity-reviewer',crossDraftActor='insurance-cross-entity-drafter';
  await adminPool.query("INSERT INTO runtime_actor_grant(tenant_id,actor_id,entity_id,permission,authority_class,valid_until) VALUES($1,$2,$3,'PREPAID.AMORTIZATION.REVIEW','REVIEW',now()+interval '1 hour'),($1,$4,$3,'PREPAID.AMORTIZATION.DRAFT','DRAFT',now()+interval '1 hour'),($1,$4,$3,'GL.JE.AUTO.CREATE','DRAFT',now()+interval '1 hour')",[ids.tenantId,crossReviewActor,permissionOnlyEntity.entityId,crossDraftActor]);
  const crossReviewReader=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,crossReviewActor,['GL.REPORT.VIEW'])}),crossReviewRead=await crossReviewReader.inSession(async client=>(await client.query('SELECT * FROM refs_read_insurance_prepaid_amortization($1,$2,$3,50)',[ids.tenantId,ids.entityId,ids.periodId])).rows[0].refs_read_insurance_prepaid_amortization);
  const crossDraftReader=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,crossDraftActor,['GL.REPORT.VIEW'])}),crossDraftRead=await crossDraftReader.inSession(async client=>(await client.query('SELECT * FROM refs_read_insurance_prepaid_amortization($1,$2,$3,50)',[ids.tenantId,ids.entityId,ids.periodId])).rows[0].refs_read_insurance_prepaid_amortization);
  assert.equal(crossReviewRead.readiness_status,'READY_FOR_INDEPENDENT_REVIEW');assert.equal(crossReviewRead.can_independently_review,false);assert.equal(crossDraftRead.can_create_draft,false);
...
pgTest('cross-entity payment review finds one exact signed counterparty invoice only after a standard Posted AP payment and never writes accounting data',async()=>{
  const current=await seed({status:'DRAFT'}),counterparty=await seed({status:'DRAFT',tenantId:current.tenantId}),businessId=`CROSS-ENTITY-${randomUUID()}`,invoiceNo=`INV-XE-${randomUUID()}`;
  const currentImporter=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(current,'cross-entity-current-importer',['WBS.SNAPSHOT.IMPORT'])}),counterpartyImporter=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(counterparty,'cross-entity-counterparty-importer',['WBS.SNAPSHOT.IMPORT'])});
  const currentSource=await retainFinal1PayableFixture({pool:adminPool,kernel:currentImporter,ids:current,companyCode:current.sourceEntityId,amount:'125.0000',businessId,invoiceNo,idempotencyKey:'cross-entity-current-source-001'});
  const counterpartySource=await retainFinal1PayableFixture({pool:adminPool,kernel:counterpartyImporter,ids:counterparty,companyCode:counterparty.sourceEntityId,amount:'125.0000',businessId,invoiceNo,idempotencyKey:'cross-entity-counterparty-source-001'});
  const billId=randomUUID();await adminPool.query(`INSERT INTO business_document(business_document_id,tenant_id,entity_id,source_document_id,document_kind,document_number,counterparty_ref,counterparty_name,currency,accounting_date,due_date,gross_amount,open_balance,status,created_by) VALUES($1,$2,$3,$4,'AP_BILL',$5,'VENDOR-1','Vendor One','USD','2026-07-16','2026-08-15',125,125,'APPROVED','cross-entity-fixture')`,[billId,current.tenantId,current.entityId,currentSource.sourceDocumentId,invoiceNo]);
  const maker=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(current,'cross-entity-payment-maker',['AP.PAYMENT.CREATE'])}),submitter=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(current,'cross-entity-payment-submitter',['GL.JE.SUBMIT'])}),reviewer=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(current,'cross-entity-payment-reviewer',['GL.JE.REVIEW'])}),approver=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(current,'cross-entity-payment-approver',['GL.JE.APPROVE'])}),poster=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(current,'cross-entity-payment-poster',['GL.JE.POST'])});
  const payment=await maker.createApPayment({...current,businessDocumentId:billId,paymentNumber:'PAY-CROSS-ENTITY-1',paymentDate:'2026-07-20',cashAccountCode:'111000',bankMemberRef:'BANK-1',amount:125,reason:'Cross-entity detector PostgreSQL fixture.',idempotencyKey:'cross-entity-payment-create-001'});
  ...
  const denied=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(current,'cross-entity-ai-denied',['AI.ANALYSIS.EXPLAIN'])});await assert.rejects(denied.readAiCrossEntityPaymentInvoices({tenantId:current.tenantId,entityId:current.entityId,periodId:current.periodId,counterpartyEntityId:counterparty.entityId,counterpartyPeriodId:counterparty.periodId,limit:10}),error=>error.code==='42501');
});
...
pgTest('controlled test unsigned WBS Bank rows create isolated source evidence and one ordinary DRAFT reconciliation',async()=>{
  ...
  const oneItem=await reader.getReconciliationWorksheetItem({tenantId:ids.tenantId,entityId:ids.entityId,reconciliationId:created.reconciliation_id,bankSourceId:receipt.bank_source_ids[1]});
  assert.equal(oneItem.bank_source_id,receipt.bank_source_ids[1]);assert.equal(oneItem.transaction_date,'2026-07-12');
...
pgTest('WBS TEST Bank monthly identity admits legacy July hashes, isolates months, and rejects changed same-month payloads',async()=>{
  ...
  const retained=(await adminPool.query(`SELECT receipt.bank_account_ref,ir.source_record_hash,re.source_record_id,re.payload_hash,re.payload_ref,d.source_ref
    FROM wbs_test_bank_import_stage_row ir JOIN wbs_test_bank_import_receipt receipt ON receipt.tenant_id=ir.tenant_id AND receipt.entity_id=ir.entity_id AND receipt.wbs_test_bank_import_stage_id=ir.wbs_test_bank_import_stage_id
    JOIN raw_event re ON re.tenant_id=ir.tenant_id AND re.raw_event_id=ir.raw_event_id
    ...
    WHERE ir.tenant_id=$1 AND ir.entity_id=$2 ORDER BY receipt.bank_account_ref`,[ids.tenantId,ids.entityId])).rows;
...
pgTest('WBS TEST Bank stages and set-publishes 1888 January rows with one reconciliation and exact replay',async()=>{
  ...
  assert.deepEqual(await counts(),{batches:1,raw:1888,documents:1888,bank:1888,reconciliations:0,imports:1,stages:1,chunks:19,staged_rows:1888,finals:0});
...
pgTest('WBS H1 Controller approves one exact Settings proposal atomically without creating accounting entries',async()=>{
  const ids=await seed({status:'DRAFT',attachmentStatus:null,extraAccounts:[{accountCode:'164100',accountName:'CWIP - Land'}]}),periodId=randomUUID();
  await adminPool.query("INSERT INTO accounting_period(period_id,tenant_id,entity_id,ledger_code,period_code,starts_on,ends_on,status) VALUES($1,$2,$3,'PRIMARY','2026-01','2026-01-01','2026-01-31','OPEN')",[periodId,ids.tenantId,ids.entityId]);
  for(const row of [[1,'0LD067','164100','CWIP - Land','Project',hash('wbs-h1-decision-ready')],[2,'','','','',hash('wbs-h1-decision-blocked-default')]])await adminPool.query(`INSERT INTO wbs_h1_accounting_setting_stage(tenant_id,company_code,setting_id,setting_type,category,business_type,detail,project_codes,journal_code,account_name,supplementary,effective_from,effective_to,setting_hash) VALUES($1,$2,$3,'Debit','Payable',4,$4,'',$5,$6,$7,'2026-01-01','2026-12-31',$8)`,[ids.tenantId,ids.sourceEntityId,...row]);
  const controller=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'wbs-h1-settings-controller',['WBS.AUTOREC.VIEW','WBS.H1.SETTINGS.DECIDE'])});
  const proposal=await controller.readWbsH1AccountingSettingsProposal({tenantId:ids.tenantId,entityId:ids.entityId,periodId});assert.equal(proposal.status,'READY_FOR_HUMAN_REVIEW');assert.equal(proposal.ready_rule_count,1);assert.equal(proposal.blocked_rule_count,1);
  const before=(await adminPool.query("SELECT (SELECT count(*)::int FROM journal_entry WHERE tenant_id=$1 AND entity_id=$2) journals,(SELECT count(*)::int FROM ledger_line WHERE tenant_id=$1 AND entity_id=$2) ledger,(SELECT count(*)::int FROM audit_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='WBS_H1_ACCOUNTING_SETTINGS_DECIDED') audit,(SELECT count(*)::int FROM outbox_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='WBS_H1_ACCOUNTING_SETTINGS_DECIDED') outbox",[ids.tenantId,ids.entityId])).rows[0];
  const args={tenantId:ids.tenantId,entityId:ids.entityId,periodId,expectedProposalHash:proposal.proposal_hash,outcome:'APPROVED',reason:'Controller verified the exact WBS rule and blocked default.',idempotencyKey:'wbs-h1-settings-controller-0001'};
  const receipt=await controller.decideWbsH1AccountingSettings(args),replay=await controller.decideWbsH1AccountingSettings(args);assert.equal(receipt.outcome,'APPROVED');assert.equal(receipt.approved_rule_count,1);assert.equal(receipt.idempotent,false);assert.equal(replay.idempotent,true);assert.equal(replay.decision_id,receipt.decision_id);assert.deepEqual({create:receipt.can_create_draft,review:receipt.can_review,approve:receipt.can_approve,post:receipt.can_post},{create:false,review:false,approve:false,post:false});
  const read=await controller.readWbsH1AccountingSettingsDecision({tenantId:ids.tenantId,entityId:ids.entityId,periodId,proposalHash:proposal.proposal_hash});assert.equal(read.decision_hash,receipt.decision_hash);assert.equal(read.outcome,'APPROVED');
  const after=(await adminPool.query("SELECT (SELECT count(*)::int FROM journal_entry WHERE tenant_id=$1 AND entity_id=$2) journals,(SELECT count(*)::int FROM ledger_line WHERE tenant_id=$1 AND entity_id=$2) ledger,(SELECT count(*)::int FROM audit_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='WBS_H1_ACCOUNTING_SETTINGS_DECIDED') audit,(SELECT count(*)::int FROM outbox_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='WBS_H1_ACCOUNTING_SETTINGS_DECIDED') outbox",[ids.tenantId,ids.entityId])).rows[0];assert.deepEqual(after,{journals:before.journals,ledger:before.ledger,audit:before.audit+1,outbox:before.outbox+1});
  const denied=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'wbs-h1-settings-reader-only',['WBS.AUTOREC.VIEW'])});await assert.rejects(denied.decideWbsH1AccountingSettings({...args,idempotencyKey:'wbs-h1-settings-denied-001'}),error=>error.code==='42501');
});

pgTest('WBS H1 Payable accounting proposal matches approved debit and credit Settings without creating a Draft',async()=>{
  const ids=await seed({status:'DRAFT',attachmentStatus:null,extraAccounts:[{accountCode:'164100',accountName:'CWIP - Land'}]}),periodId=randomUUID(),sourceHash=hash('wbs-h1-payable-proposal-source');
  await adminPool.query("INSERT INTO accounting_period(period_id,tenant_id,entity_id,ledger_code,period_code,starts_on,ends_on,status) VALUES($1,$2,$3,'PRIMARY','2026-01','2026-01-01','2026-01-31','OPEN')",[periodId,ids.tenantId,ids.entityId]);
  await adminPool.query("INSERT INTO account_master(tenant_id,entity_id,account_code,account_name,requires_member,required_member_type,active) VALUES($1,$2,'291001','Due to Vendor',true,'VENDOR',true) ON CONFLICT(tenant_id,entity_id,account_code) DO UPDATE SET active=true,requires_member=true,required_member_type='VENDOR'",[ids.tenantId,ids.entityId]);
  await adminPool.query("INSERT INTO member_master(tenant_id,entity_id,member_ref,member_type,display_name,active) VALUES($1,$2,'V-1','VENDOR','Test vendor',true)",[ids.tenantId,ids.entityId]);
  await adminPool.query(`INSERT INTO wbs_h1_payable_mapping_source_stage(tenant_id,entity_id,company_code,period_code,wbs_uuid,source_record_hash,accounting_date,amount,project_code,cost_code,vendor_no,source_fact_hash,provider_content_hash,captured_at) VALUES($1,$2,$3,'2026-01','WBS-PAY-1',$4,'2026-01-15','125.0000','P-100','0LD067','V-1',$5,$6,'2026-08-22T12:00:00Z')`,[ids.tenantId,ids.entityId,ids.sourceEntityId,sourceHash,hash('wbs-h1-payable-proposal-fact'),hash('wbs-h1-payable-proposal-provider')]);
  for(const row of [[1,'Debit','0LD067','P-100','164100','CWIP - Land','Project',hash('wbs-h1-payable-proposal-debit')],[2,'Credit','','','291001','Due to Vendor','Vendor',hash('wbs-h1-payable-proposal-credit')]])await adminPool.query(`INSERT INTO wbs_h1_accounting_setting_stage(tenant_id,company_code,setting_id,setting_type,category,business_type,detail,project_codes,journal_code,account_name,supplementary,effective_from,effective_to,setting_hash) VALUES($1,$2,$3,$4,'Payable',4,$5,$6,$7,$8,$9,'2026-01-01','2026-12-31',$10)`,[ids.tenantId,ids.sourceEntityId,...row]);
  const controller=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'wbs-h1-payable-proposal-controller',['WBS.AUTOREC.VIEW','WBS.H1.SETTINGS.DECIDE'])});
  let result=await controller.readWbsH1PayableAccountingProposal({tenantId:ids.tenantId,entityId:ids.entityId,periodId,limit:50,offset:0});assert.equal(result.ready_count,0);assert.equal(result.exception_count,1);assert.deepEqual(result.rows[0].exception_codes,['SETTINGS_NOT_APPROVED']);
  const settings=await controller.readWbsH1AccountingSettingsProposal({tenantId:ids.tenantId,entityId:ids.entityId,periodId});await controller.decideWbsH1AccountingSettings({tenantId:ids.tenantId,entityId:ids.entityId,periodId,expectedProposalHash:settings.proposal_hash,outcome:'APPROVED',reason:'Controller approved the exact effective WBS debit Settings.',idempotencyKey:'wbs-h1-payable-proposal-settings-001'});
  const before=(await adminPool.query('SELECT count(*)::int count FROM journal_entry WHERE tenant_id=$1 AND entity_id=$2',[ids.tenantId,ids.entityId])).rows[0].count;
  result=await controller.readWbsH1PayableAccountingProposal({tenantId:ids.tenantId,entityId:ids.entityId,periodId,limit:50,offset:0});assert.equal(result.schema_version,'WBS_H1_PAYABLE_ACCOUNTING_PROPOSAL_V1');assert.equal(result.settings_outcome,'APPROVED');assert.equal(result.ready_count,1);assert.equal(result.exception_count,0);assert.equal(result.rows[0].status,'READY_FOR_CONTROLLER_REVIEW');assert.deepEqual(result.rows[0].proposed_lines.map(line=>[line.side,line.account_code,line.amount,line.member_ref,line.project_ref]),[['DEBIT','164100','125.0000',null,'P-100'],['CREDIT','291001','125.0000','V-1',null]]);assert.deepEqual({create:result.can_create_draft,review:result.can_review,approve:result.can_approve,post:result.can_post},{create:false,review:false,approve:false,post:false});
  const after=(await adminPool.query('SELECT count(*)::int count FROM journal_entry WHERE tenant_id=$1 AND entity_id=$2',[ids.tenantId,ids.entityId])).rows[0].count;assert.equal(after,before);
...
pgTest('separate humans take one approved WBS H1 Payable mapping from Draft through POSTED GL and financial statements',async()=>{
  ...
  // Migration 269 remains a compatibility path for historical combined-import
  // rows.  Migration 275 no longer lets the service actor create this trace,
  // so retain one explicit pre-275 fixture before exercising the old reclass.
  await adminPool.query('ALTER TABLE source_document_line DISABLE TRIGGER USER');
  try{await adminPool.query(`UPDATE source_document_line l SET external_dimension_refs=jsonb_set(l.external_dimension_refs,'{schema_version}','"WBS_TEST_IMPORT_LINE_V1"'::jsonb)
    FROM wbs_test_payable_source_receipt r WHERE r.tenant_id=$1 AND r.entity_id=$2 AND r.source_record_hash=$3 AND l.source_document_line_id=r.source_document_line_id`,[ids.tenantId,ids.entityId,sourceHash]);}finally{await adminPool.query('ALTER TABLE source_document_line ENABLE TRIGGER USER');}
  await adminPool.query(`INSERT INTO wbs_test_import_draft(tenant_id,entity_id,period_id,observation_hash,source_record_hash,provider_content_sha256,row_index,import_batch_id,raw_event_id,source_document_id,source_document_line_id,attachment_id,business_document_id,journal_entry_id,request_hash,created_by)
    SELECT r.tenant_id,r.entity_id,r.period_id,r.observation_hash,r.source_record_hash,r.provider_content_sha256,r.row_index,r.import_batch_id,r.raw_event_id,r.source_document_id,r.source_document_line_id,r.attachment_id,e.business_document_id,e.journal_entry_id,refs_jsonb_hash(jsonb_build_object('fixture','pre-275-reclass','source_record_hash',r.source_record_hash)),'pre-275-historical-fixture'
    FROM wbs_test_payable_source_receipt r JOIN wbs_test_payable_draft_evidence e USING(tenant_id,entity_id,wbs_test_payable_source_receipt_id)
    WHERE r.tenant_id=$1 AND r.entity_id=$2 AND r.source_record_hash=$3`,[ids.tenantId,ids.entityId,sourceHash]);
  ...
  const before=(await adminPool.query("SELECT (SELECT count(*)::int FROM journal_entry WHERE tenant_id=$1 AND entity_id=$2) journals,(SELECT count(*)::int FROM ledger_line WHERE tenant_id=$1 AND entity_id=$2) ledger",[ids.tenantId,ids.entityId])).rows[0],args={tenantId:ids.tenantId,entityId:ids.entityId,periodId,sourceRecordHash:sourceHash,proposalHash:proposal.rows[0].proposal_hash,reason:'Maker creates the approved WBS mapping Draft only.',idempotencyKey:'h1-human-reclass-draft-001'};
  const draftArtifacts=async()=>(await adminPool.query("SELECT (SELECT count(*)::int FROM journal_entry WHERE tenant_id=$1 AND entity_id=$2) journals,(SELECT count(*)::int FROM wbs_h1_payable_reclass_draft_evidence WHERE tenant_id=$1 AND entity_id=$2) evidence,(SELECT count(*)::int FROM audit_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='WBS_H1_PAYABLE_RECLASS_DRAFT_CREATED') audit,(SELECT count(*)::int FROM outbox_event WHERE tenant_id=$1 AND entity_id=$2 AND event_type='WBS_H1_PAYABLE_RECLASS_DRAFT_CREATED') outbox",[ids.tenantId,ids.entityId])).rows[0],failureBaseline=await draftArtifacts();
  const observedDrift=[{tenant_id:ids.tenantId,entity_id:ids.entityId,company_code:ids.sourceEntityId,period_code:'2026-01',wbs_uuid:'WBS-HUMAN-DRAFT-1',source_record_hash:sourceHash,accounting_date:'2026-01-15',amount:'126.0000',project_code:'P-100',cost_code:'0LD067',vendor_no:'REAL-WBS-VENDOR',source_fact_hash:hash('h1-human-draft-observed-drift'),provider_content_hash:hash('h1-human-draft-observed-provider'),captured_at:'2026-08-23T12:00:00.000Z'}];
  const driftReceipt=(await adminPool.query('SELECT refs_retain_wbs_h1_payable_mapping_source_rows($1::jsonb) receipt',[JSON.stringify(observedDrift)])).rows[0].receipt;assert.deepEqual({expected:driftReceipt.expected_count,exact:driftReceipt.exact_count,conflicts:driftReceipt.conflict_count,inserted:driftReceipt.conflict_inserted_count},{expected:1,exact:0,conflicts:1,inserted:1});
  const conflict=(await adminPool.query('SELECT retained_amount::text,observed_amount::text,retained_source_fact_hash,observed_source_fact_hash,retained_provider_content_hash,observed_provider_content_hash FROM wbs_h1_payable_mapping_source_conflict WHERE tenant_id=$1 AND entity_id=$2 AND source_record_hash=$3',[ids.tenantId,ids.entityId,sourceHash])).rows[0];assert.deepEqual(conflict,{retained_amount:'125.0000',observed_amount:'126.0000',retained_source_fact_hash:hash('h1-human-draft-fact'),observed_source_fact_hash:hash('h1-human-draft-observed-drift'),retained_provider_content_hash:hash('h1-human-draft-provider'),observed_provider_content_hash:hash('h1-human-draft-observed-provider')});
  const blockedProposal=await maker.readWbsH1PayableAccountingProposal({tenantId:ids.tenantId,entityId:ids.entityId,periodId,limit:50,offset:0});assert.equal(blockedProposal.ready_count,0);assert.equal(blockedProposal.exception_count,1);assert.deepEqual(blockedProposal.rows[0].exception_codes,['SOURCE_FACT_DRIFT_UNRESOLVED']);
  await assert.rejects(maker.createWbsH1PayableReclassDraft({...args,idempotencyKey:'h1-human-reclass-unresolved-source-drift'}),error=>error.code==='40001');assert.deepEqual(await draftArtifacts(),failureBaseline);
  await adminPool.query('ALTER TABLE wbs_h1_payable_mapping_source_conflict DISABLE TRIGGER USER');
  try{await adminPool.query('DELETE FROM wbs_h1_payable_mapping_source_conflict WHERE tenant_id=$1 AND entity_id=$2 AND source_record_hash=$3',[ids.tenantId,ids.entityId,sourceHash]);}finally{await adminPool.query('ALTER TABLE wbs_h1_payable_mapping_source_conflict ENABLE TRIGGER USER');}
  assert.equal((await maker.readWbsH1PayableAccountingProposal({tenantId:ids.tenantId,entityId:ids.entityId,periodId,limit:50,offset:0})).ready_count,1);
  const trace=(await adminPool.query('SELECT source_document_line_id,journal_entry_id FROM wbs_test_import_draft WHERE tenant_id=$1 AND entity_id=$2 AND source_record_hash=$3',[ids.tenantId,ids.entityId,sourceHash])).rows[0];
  await adminPool.query('ALTER TABLE journal_line DISABLE TRIGGER USER');await adminPool.query("UPDATE journal_line SET member_ref='WRONG-TEST-VENDOR' WHERE tenant_id=$1 AND entity_id=$2 AND journal_entry_id=$3 AND account_code='291001'",[ids.tenantId,ids.entityId,trace.journal_entry_id]);await adminPool.query('ALTER TABLE journal_line ENABLE TRIGGER USER');
  try{await assert.rejects(maker.createWbsH1PayableReclassDraft({...args,idempotencyKey:'h1-human-reclass-wrong-original-leg'}),error=>error.code==='40001');assert.deepEqual(await draftArtifacts(),failureBaseline);}finally{await adminPool.query('ALTER TABLE journal_line DISABLE TRIGGER USER');await adminPool.query("UPDATE journal_line SET member_ref='WBS_TEST_VENDOR' WHERE tenant_id=$1 AND entity_id=$2 AND journal_entry_id=$3 AND account_code='291001'",[ids.tenantId,ids.entityId,trace.journal_entry_id]);await adminPool.query('ALTER TABLE journal_line ENABLE TRIGGER USER');}
  ...
  const receipt=await maker.createWbsH1PayableReclassDraft(args),replay=await maker.createWbsH1PayableReclassDraft(args);assert.equal(receipt.status,'DRAFT');assert.equal(receipt.idempotent,false);assert.equal(replay.idempotent,true);assert.equal(replay.journal_entry_id,receipt.journal_entry_id);assert.deepEqual({submit:receipt.can_submit,review:receipt.can_review,approve:receipt.can_approve,post:receipt.can_post},{submit:false,review:false,approve:false,post:false});
  ...
  const after=(await adminPool.query("SELECT (SELECT count(*)::int FROM journal_entry WHERE tenant_id=$1 AND entity_id=$2) journals,(SELECT count(*)::int FROM ledger_line WHERE tenant_id=$1 AND entity_id=$2) ledger",[ids.tenantId,ids.entityId])).rows[0];assert.deepEqual(after,{journals:before.journals+1,ledger:before.ledger});
  const controllerWithMakerPermissions=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'h1-settings-controller',['WBS.AUTOREC.VIEW','WBS.H1.SETTINGS.DECIDE','WBS.H1.PAYABLE.DRAFT','GL.JE.CREATE'])});await assert.rejects(controllerWithMakerPermissions.createWbsH1PayableReclassDraft({...args,idempotencyKey:'h1-controller-cannot-make'}),error=>error.code==='42501');
  const submitter=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'h1-formal-submitter',['GL.JE.SUBMIT'])}),reviewer=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'h1-formal-reviewer',['GL.JE.REVIEW'])}),approver=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'h1-formal-approver',['GL.JE.APPROVE'])}),poster=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'h1-formal-poster',['GL.JE.POST'])});
  ...
  const reportReader=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'h1-formal-report-reader',['GL.REPORT.VIEW','WBS.AUTOREC.VIEW'])}),statements=await reportReader.getFinancialStatements({tenantId:ids.tenantId,entityId:ids.entityId,periodId});
  const cwip=statements.find(row=>row.statement_type==='TRIAL_BALANCE'&&row.account_code==='164100'),expense=statements.find(row=>row.statement_type==='TRIAL_BALANCE'&&row.account_code==='610000'),payable=statements.find(row=>row.statement_type==='TRIAL_BALANCE'&&row.account_code==='291001');assert.ok(cwip);assert.ok(expense);assert.ok(payable);assert.deepEqual({debit:cwip.period_debit,credit:cwip.period_credit,balance:cwip.display_balance},{debit:'125.0000',credit:'0.0000',balance:'125.0000'});assert.deepEqual({debit:expense.period_debit,credit:expense.period_credit,balance:expense.display_balance},{debit:'125.0000',credit:'125.0000',balance:'0.0000'});assert.deepEqual({debit:payable.period_debit,credit:payable.period_credit,balance:payable.display_balance},{debit:'125.0000',credit:'250.0000',balance:'-125.0000'});for(const row of [cwip,expense])assert.ok(row.journal_entry_ids.includes(receipt.journal_entry_id));for(const row of [cwip,expense,payable])assert.ok(row.source_document_ids.includes(receipt.source_document_id));
  const inventory=await reportReader.readWbsH1ImportInventory({tenantId:ids.tenantId,entityId:ids.entityId,limit:50,offset:0});assert.equal(inventory.totals.formal_mapping_posted_count,1);assert.equal(inventory.rows.find(row=>row.source_record_hash===sourceHash).mapping_state,'FORMAL_MAPPING_POSTED');
  await adminPool.query('SELECT refs_retain_wbs_h1_payable_mapping_source_rows($1::jsonb)',[JSON.stringify(observedDrift)]);
  const down271=await readFile(new URL('../db/migrations/down/271_wbs_h1_payable_mapping_source_conflict.sql',import.meta.url),'utf8');await assert.rejects(adminPool.query(down271),error=>error.code==='55000');
