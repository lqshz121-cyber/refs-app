...
pgTest('cross-entity payment review finds one exact signed counterparty invoice only after a standard Posted AP payment and never writes accounting data',async()=>{
  ...
  const actor='cross-entity-ai-reader';await adminPool.query(`INSERT INTO runtime_actor_grant(tenant_id,actor_id,entity_id,permission) VALUES($1,$2,$3,'AI.ANALYSIS.EXPLAIN'),($1,$2,$4,'AI.ANALYSIS.EXPLAIN')`,[current.tenantId,actor,current.entityId,counterparty.entityId]);
  const reader=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(current,actor,['AI.ANALYSIS.EXPLAIN'])}),before=(await adminPool.query(`SELECT (SELECT count(*)::int FROM journal_entry WHERE tenant_id=$1) journals,(SELECT count(*)::int FROM ledger_line WHERE tenant_id=$1) ledger,(SELECT count(*)::int FROM audit_event WHERE tenant_id=$1) audit,(SELECT count(*)::int FROM outbox_event WHERE tenant_id=$1) outbox`,[current.tenantId])).rows[0];
  ...
  const after=(await adminPool.query(`SELECT (SELECT count(*)::int FROM journal_entry WHERE tenant_id=$1) journals,(SELECT count(*)::int FROM ledger_line WHERE tenant_id=$1) ledger,(SELECT count(*)::int FROM audit_event WHERE tenant_id=$1) audit,(SELECT count(*)::int FROM outbox_event WHERE tenant_id=$1) outbox`,[current.tenantId])).rows[0];assert.deepEqual(after,before);
