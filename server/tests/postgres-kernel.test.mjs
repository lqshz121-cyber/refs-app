...
pgTest('admitted signed Payable composition persists receipt Raw Normalized Staging and replays with zero journal writes',async()=>{
  ...
  assert.ok(candidate);assert.deepEqual({version:candidate.source_version,receipt:candidate.receipt_hash,evidence:candidate.evidence_hash,revision:candidate.revision,period:candidate.period_id,number:candidate.document_number,invoice:candidate.invoice_date,due:candidate.due_date,date:candidate.accounting_date,currency:candidate.currency,amount:candidate.gross_amount,vendor:candidate.vendor_name,offset:candidate.offset_account_code,setting:candidate.setting_snapshot_id,mapping:candidate.mapping_snapshot_id,readiness:candidate.review_readiness,canReview:candidate.can_review},{version:stored.source_version,receipt:stored.receipt_hash,evidence:stored.evidence_hash,revision:'0',period:ids.periodId,number:'WBS-INV-PG-001',invoice:'2026-07-01',due:'2026-07-05',date:'2026-07-11',currency:'USD',amount:'89.1250',vendor:'Signed WBS vendor',offset:'610000',setting:settingId,mapping:mappingId,readiness:'READY_FOR_REVIEW',canReview:true});
  assert.equal(candidate.attachment_choices.length,1);assert.deepEqual(Object.keys(candidate.attachment_choices[0]).sort(),['attachment_id','media_type','name','verified_at']);
