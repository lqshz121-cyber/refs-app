...
pgTest('Insurance PC company mapping Controller164 records proposes approves resumes and preserves zero accounting writes',async()=>{
  ...
  await assert.rejects(migrateDown(adminPool),error=>error.code==='55000');assert.deepEqual(await businessCounts(),zeroAccounting);
});
...
pgTest('WBS Final-1 retained evidence persists exact Payables and Insurance source rows with prepaid findings but zero accounting action',async()=>{
  ...
  const buildPayablePopulation=size=>{
    const delivery={...payableDelivery,admission_id:randomUUID(),nonce:`nonce-payables-population-${size}`,receipt_hash:hash(`payables-population-${size}-receipt`),request_raw_hash:hash(`payables-population-${size}-request`),response_raw_hash:hash(`payables-population-${size}-response`),package_raw_hash:hash(`payables-population-${size}-package-raw`),package_hash:hash(`payables-population-${size}-package`),snapshot_id:randomUUID(),row_count:size};
