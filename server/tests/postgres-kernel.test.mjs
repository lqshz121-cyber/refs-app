...
pgTest('dimension profitability reads only exact POSTED ledger dimensions and never fills a missing property, project, unit, or lot',async()=>{
  const ids=await seed({status:'APPROVED',journalType:'AUTO',attachmentStatus:null,
    ...
    journalLines:[
      {lineNo:1,accountCode:'111000',debit:75,credit:0,memberRef:'BANK-1',dimensions:{property_ref:'PROPERTY-01',project_ref:'PROJECT-01',unit_ref:'UNIT-01',lot_ref:'LOT-18'}},
      {lineNo:2,accountCode:'400000',debit:0,credit:100,dimensions:{property_ref:'PROPERTY-01',project_ref:'PROJECT-01',unit_ref:'UNIT-01',lot_ref:'LOT-18'}},
      {lineNo:3,accountCode:'610000',debit:25,credit:0,dimensions:{property_ref:'PROPERTY-01',project_ref:'PROJECT-01',unit_ref:'UNIT-01',lot_ref:'LOT-18'}}
    ]});
  const trace=await attachAutoSource(ids);
  ...
  const lotRows=await reader.getLotProfitability({tenantId:ids.tenantId,entityId:ids.entityId,periodId:ids.periodId,lotRef:'LOT-18'});
  ...
  assert.ok(lotRows.every(row=>row.lot_ref==='LOT-18'&&row.classification_basis==='POSTED_LEDGER_DIMENSION_EXACT'&&row.journal_entry_ids.includes(ids.journalId)&&row.source_document_ids.includes(trace.documentId)));
  assert.deepEqual(await reader.getLotProfitability({tenantId:ids.tenantId,entityId:ids.entityId,periodId:ids.periodId,lotRef:'LOT-MISSING'}),[]);
  ...
  const lotResponse=await api({method:'GET',url:`/api/v1/entities/${ids.entityId}/reports/lot-profitability?periodId=${ids.periodId}&lotRef=LOT-18`,body:null,headers:{}});
  assert.equal(lotResponse.status,200);assert.equal(lotResponse.headers['cache-control'],'no-store');assert.equal(lotResponse.body.data.length,2);
});
