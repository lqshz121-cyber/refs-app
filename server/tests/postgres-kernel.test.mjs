...
pgTest('WBS Final-1 Controller167 persists five-domain signed controls and exact business evidence with zero accounting action',async()=>{
  ...
  const controls=amounts=>{const total=amounts.reduce((sum,value)=>sum+BigInt(value.replace('.','')),0n),per_currency_totals=[{currency:'USD',gross_amount:`${total/10000n}.${String(total%10000n).padStart(4,'0')}`}],row_count=amounts.length;return {row_count,per_currency_totals,control_totals_hash:canonicalRequestHash({row_count,per_currency_totals})};};
  ...
  const insuranceControls=controls(['999999999999999999.9900','100.0000']),insuranceDelivery={...deliveryBase('INSURANCE'),...insuranceControls,company_mapping_hash:mappingHash,plan_hash:hash('insurance-plan')},candidateHash=hash('insurance-candidate'),gapHash=hash('insurance-gap');
  ...
  assert.equal(insuranceTrace.gross_amount,'999999999999999999.9900');assert.equal(insuranceTrace.external_dimension_refs.insurance_pc_mapping_match_count,1);assert.equal(insuranceTrace.external_dimension_refs.insurance_pc_mapping_approved,true);assert.equal(insuranceTrace.external_dimension_refs.insurance_pc_mapping_resolved_entity_id,ids.entityId);assert.equal(insuranceTrace.external_dimension_refs.insurance_pc_mapping_resolved_company_code,'WBPA');
