...
pgTest('AI insurance proposal requires a different controller to accept one exact standard Draft before the normal four-role post and report trace',async()=>{
  ...
  const controller=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'insurance-controller',['AI.AMORTIZATION.ACCEPT'])});
  const acceptance=await controller.acceptAiAmortizationSchedule({tenantId:ids.tenantId,entityId:ids.entityId,aiAmortizationScheduleId:proposal.ai_amortization_schedule_id,aiAmortizationScheduleLineId:scheduleLine.ai_amortization_schedule_line_id,journalEntryId:draft.journal_entry_id,reason:'Controller accepted exact source coverage, January amount, accounts, and maker Draft.',idempotencyKey:'insurance-amortization-accept-001'});
  ...
  const replay=await controller.acceptAiAmortizationSchedule({tenantId:ids.tenantId,entityId:ids.entityId,aiAmortizationScheduleId:proposal.ai_amortization_schedule_id,aiAmortizationScheduleLineId:scheduleLine.ai_amortization_schedule_line_id,journalEntryId:draft.journal_entry_id,reason:'Controller accepted exact source coverage, January amount, accounts, and maker Draft.',idempotencyKey:'insurance-amortization-accept-001'});assert.equal(replay.idempotent,true);
