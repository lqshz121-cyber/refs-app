...
pgTest('server-derived AI Posted outcome review binds decision human workflow ledger trial balance and report atomically',async()=>{
  ...
  await adminPool.query('ALTER TABLE outbox_event DISABLE TRIGGER USER');await adminPool.query(`DO $$ DECLARE c text; BEGIN SELECT conname INTO c FROM pg_constraint WHERE conrelid='outbox_event'::regclass AND contype='u' AND pg_get_constraintdef(oid) LIKE '%aggregate_type%event_type%payload_hash%'; EXECUTE format('ALTER TABLE outbox_event DROP CONSTRAINT %I',c); END $$`);await adminPool.query("DELETE FROM outbox_event WHERE outbox_event_id=(SELECT outbox_event_id FROM outbox_event WHERE aggregate_id=$1 AND event_type='JOURNAL_REVIEW' LIMIT 1)",[draft.journal_entry_id]);await adminPool.query("INSERT INTO outbox_event(tenant_id,aggregate_type,aggregate_id,event_type,payload,payload_hash,status,attempt_count,available_at,published_at,last_error,created_at) SELECT tenant_id,aggregate_type,aggregate_id,event_type,payload,payload_hash,status,attempt_count,available_at,published_at,last_error,created_at FROM outbox_event WHERE aggregate_id=$1 AND event_type='JOURNAL_SUBMIT' LIMIT 1",[draft.journal_entry_id]);await adminPool.query('ALTER TABLE outbox_event ENABLE TRIGGER USER');
  assert.equal((await adminPool.query("SELECT count(*)::int n FROM outbox_event WHERE aggregate_id=$1 AND event_type IN ('JOURNAL_SUBMIT','JOURNAL_REVIEW','JOURNAL_APPROVE','JOURNAL_POSTED')",[draft.journal_entry_id])).rows[0].n,4);
  ...
  const receiptDrift=await outcomeReviewer.retainAiAccountingPostedOutcomeReview({tenantId:ids.tenantId,entityId:ids.entityId,decisionId:retained.ai_accounting_decision_id,expectedDecisionHash:driftHash,expectedReviewRevision:4,idempotencyKey:'retain-ai-posted-outcome-pg-receipt-drift'});assert.ok(receiptDrift.reason_codes.includes('ACCEPTANCE_EVIDENCE_MISMATCH'));assert.ok(receiptDrift.reason_codes.includes('DRAFT_EVIDENCE_MISMATCH'));
});
