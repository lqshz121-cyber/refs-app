...
pgTest('provider-signed Bank source survives exact Match Unmatch adjustment Post reconciliation and same-source reports',async()=>{
  ...
  const statementMatchArgs={...ids,bankSourceId:admittedSource.bank_source_id,paymentOccurrenceId:receipt.payment_occurrence_id,expectedBankVersion:0,expectedOccurrenceVersion:1,reason:'Exact provider-signed bank source matched to posted AR receipt',idempotencyKey:'wbs-statement-bank-match-001'};
  const statementMatch=await statementMatcher.createBankPaymentMatch(statementMatchArgs),statementMatchReplay=await statementMatcher.createBankPaymentMatch(statementMatchArgs);
  assert.equal(statementMatch.status,'ACTIVE');assert.equal(statementMatch.idempotent,false);assert.equal(statementMatchReplay.idempotent,true);assert.equal(statementMatchReplay.bank_match_id,statementMatch.bank_match_id);
  ...
  const statementUnmatchArgs={...ids,bankSourceId:admittedSource.bank_source_id,bankMatchId:statementMatch.bank_match_id,expectedMatchVersion:0,reason:'Independent controller returns the exact signed source to adjustment review',idempotencyKey:'wbs-statement-bank-unmatch-001'};
  const unmatched=await statementUnmatcher.unmatchBankPayment(statementUnmatchArgs),unmatchReplay=await statementUnmatcher.unmatchBankPayment(statementUnmatchArgs);
  assert.equal(unmatched.status,'UNMATCHED');assert.equal(unmatched.revision,1);assert.equal(unmatchReplay.idempotent,true);assert.equal(unmatchReplay.bank_match_id,statementMatch.bank_match_id);
  ...
  const adjustmentWriteCounts=async()=>{
    const {rows:[row]}=await adminPool.query("SELECT (SELECT count(*)::int FROM journal_entry WHERE tenant_id=$1) journal_entries,(SELECT count(*)::int FROM journal_line WHERE tenant_id=$1) journal_lines,(SELECT count(*)::int FROM source_link WHERE tenant_id=$1) source_links,(SELECT count(*)::int FROM reconciliation_adjustment_draft WHERE tenant_id=$1) adjustment_drafts,(SELECT count(*)::int FROM outbox_event WHERE tenant_id=$1) outbox",[ids.tenantId]);
    return row;
  };
  const reconciliationWriteCounts=async()=>{
    const {rows:[row]}=await adminPool.query("SELECT (SELECT count(*)::int FROM reconciliation WHERE tenant_id=$1) reconciliations,(SELECT count(*)::int FROM reconciliation_item WHERE tenant_id=$1) reconciliation_items,(SELECT count(*)::int FROM reconciliation_snapshot WHERE tenant_id=$1) snapshots,(SELECT count(*)::int FROM outbox_event WHERE tenant_id=$1) outbox,(SELECT count(*)::int FROM idempotency_receipt WHERE tenant_id=$1) idempotency_receipts",[ids.tenantId]);
    return row;
  };
  ...
  assert.deepEqual(adjustmentStatements.filter(row=>row.statement_type==='INCOME_STATEMENT'&&row.journal_entry_ids.includes(adjustment.journal_entry_id)),[], 'Dr Cash / Cr Liability adjustment must not create an Income Statement effect');
