...
pgTest('dimension profitability reads only exact POSTED ledger dimensions and never fills a missing property, project, unit, or lot',async()=>{
  ...
  assert.equal(lotRows.length,2);assert.ok(lotRows.every(row=>row.statement_type==='LOT_PROFITABILITY'&&row.classification_basis==='POSTED_LEDGER_DIMENSION_EXACT'));
  assert.ok(lotRows.every(row=>row.journal_entry_ids.includes(ids.journalId)&&row.source_document_ids.includes(trace.documentId)));
...
pgTest('AI insurance proposal requires a different controller to accept one exact standard Draft before the normal four-role post and report trace',async()=>{
  ...
  const attachmentId=(await adminPool.query('SELECT attachment_id FROM source_link WHERE tenant_id=$1 AND entity_id=$2 AND journal_entry_id=$3 AND attachment_id IS NOT NULL',[ids.tenantId,ids.entityId,ids.journalId])).rows[0].attachment_id;
  const trace=await attachAutoSource(ids,{linkJournal:false,sourceModule:'payable',sourceRecordPrefix:'INSURANCE'});
  await adminPool.query("UPDATE source_document SET status='READY_FOR_DRAFT' WHERE tenant_id=$1 AND entity_id=$2 AND source_document_id=$3",[ids.tenantId,ids.entityId,trace.documentId]);
  await adminPool.query("INSERT INTO source_document_line(tenant_id,entity_id,source_document_id,source_line_id,line_no,amount,direction,project_ref,property_ref) VALUES($1,$2,$3,'insurance-accepted-line',1,100,'DEBIT','PROJECT-1','PROPERTY-1')",[ids.tenantId,ids.entityId,trace.documentId]);
  const proposalArgs=[ids.tenantId,ids.entityId,trace.documentId,hash('auto-doc'),'2026-07-01','2027-06-30','141500','610100',JSON.stringify({project_ref:'PROJECT-1',property_ref:'PROPERTY-1',allocation_basis:'SOURCE_DIMENSIONED'}),'0.9500','Insurance policy coverage supports a controller-reviewed prepaid amortization schedule'];
  const proposer=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'insurance-ai-proposer',['AI.AMORTIZATION.PROPOSE'])});
  const proposal=await proposer.inSession(async client=>{const requestHash=(await client.query('SELECT refs_propose_ai_amortization_schedule_hash($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11) AS request_hash',proposalArgs)).rows[0].request_hash;return (await client.query('SELECT refs_propose_ai_amortization_schedule($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11,$12,$13) AS result',[...proposalArgs,'insurance-proposal-full-close-001',requestHash])).rows[0].result;});
  const scheduleLine=(await adminPool.query('SELECT ai_amortization_schedule_line_id,amount FROM ai_amortization_schedule_line WHERE tenant_id=$1 AND entity_id=$2 AND ai_amortization_schedule_id=$3 ORDER BY line_no LIMIT 1',[ids.tenantId,ids.entityId,proposal.ai_amortization_schedule_id])).rows[0];
  const aiHttpPermissions={'insurance-ai-http-reader':['AI.AMORTIZATION.VIEW'],'insurance-controller':['AI.AMORTIZATION.ACCEPT']};
  const aiApi=createAccountingApi({
    authenticate:async({headers})=>({trusted:true,tenantId:ids.tenantId,actorId:headers['x-test-actor']}),
    kernelFactory:async principal=>new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,principal.actorId,aiHttpPermissions[principal.actorId]||[])})
  });
  const scheduleRead=await aiApi({method:'GET',url:`/api/v1/entities/${ids.entityId}/ai/amortization/schedules?limit=10`,body:null,headers:{'x-test-actor':'insurance-ai-http-reader'}});
  assert.equal(scheduleRead.status,200);assert.equal(scheduleRead.headers['cache-control'],'no-store');
  const visibleSchedule=scheduleRead.body.data.find(item=>item.ai_amortization_schedule_id===proposal.ai_amortization_schedule_id);
  assert.deepEqual({lines:visibleSchedule.schedule_lines.length,source:visibleSchedule.source_document_id,draft:visibleSchedule.can_create_draft,post:visibleSchedule.can_post},{lines:12,source:trace.documentId,draft:false,post:false});
  const maker=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'insurance-draft-maker',['GL.JE.CREATE','GL.JE.SUBMIT'])});
  const draft=await maker.createManualJournal({tenantId:ids.tenantId,entityId:ids.entityId,periodId:ids.periodId,journalNumber:'JE-INS-AMORT-2026-07',journalDate:'2026-07-01',currency:'USD',description:'July insurance amortization from accepted schedule',attachmentIds:[attachmentId],idempotencyKey:'insurance-amortization-draft-001',lines:[{line_no:1,account_code:'610100',debit_amount:scheduleLine.amount,credit_amount:0,description:'July insurance expense',dimensions:{}},{line_no:2,account_code:'141500',debit_amount:0,credit_amount:scheduleLine.amount,description:'Reduce prepaid insurance',dimensions:{}}]});
  await assert.rejects(maker.acceptAiAmortizationSchedule({tenantId:ids.tenantId,entityId:ids.entityId,aiAmortizationScheduleId:proposal.ai_amortization_schedule_id,aiAmortizationScheduleLineId:scheduleLine.ai_amortization_schedule_line_id,journalEntryId:draft.journal_entry_id,reason:'Maker may not self-accept an AI linked insurance amortization Draft.',idempotencyKey:'insurance-amortization-self-accept-001'}),error=>error.code==='42501');
  const acceptancePath=`/api/v1/entities/${ids.entityId}/ai/amortization/schedules/${proposal.ai_amortization_schedule_id}/lines/${scheduleLine.ai_amortization_schedule_line_id}/acceptances`,acceptanceBody={journalEntryId:draft.journal_entry_id,reason:'Controller accepted exact source coverage, January amount, accounts, and maker Draft.'};
  const accepted=await aiApi({method:'POST',url:acceptancePath,headers:{'x-test-actor':'insurance-controller','idempotency-key':'insurance-amortization-accept-001'},body:acceptanceBody});
  assert.equal(accepted.status,201);assert.equal(accepted.headers['cache-control'],'no-store');const acceptance=accepted.body.data;
  assert.deepEqual({status:acceptance.status,draft:acceptance.can_create_draft,post:acceptance.can_post},{status:'ACCEPTED_FOR_STANDARD_JE_WORKFLOW',draft:false,post:false});
  const replay=await aiApi({method:'POST',url:acceptancePath,headers:{'x-test-actor':'insurance-controller','idempotency-key':'insurance-amortization-accept-001'},body:acceptanceBody});assert.equal(replay.status,200);assert.equal(replay.body.data.idempotent,true);
  assert.equal((await adminPool.query("SELECT count(*)::int n FROM source_link WHERE tenant_id=$1 AND entity_id=$2 AND link_type='AI_AMORTIZATION_ACCEPTANCE_SOURCE' AND source_document_id=$3 AND journal_entry_id=$4",[ids.tenantId,ids.entityId,trace.documentId,draft.journal_entry_id])).rows[0].n,1);
  await maker.transitionJournal({tenantId:ids.tenantId,entityId:ids.entityId,journalEntryId:draft.journal_entry_id,action:'SUBMIT',expectedRevision:0,idempotencyKey:'insurance-amortization-submit-001'});
  const reviewer=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'insurance-reviewer',['GL.JE.REVIEW'])});await reviewer.transitionJournal({tenantId:ids.tenantId,entityId:ids.entityId,journalEntryId:draft.journal_entry_id,action:'REVIEW',expectedRevision:1,idempotencyKey:'insurance-amortization-review-001'});
  const approver=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'insurance-approver',['GL.JE.APPROVE'])});await approver.transitionJournal({tenantId:ids.tenantId,entityId:ids.entityId,journalEntryId:draft.journal_entry_id,action:'APPROVE',expectedRevision:2,idempotencyKey:'insurance-amortization-approve-001'});
  const poster=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'insurance-poster',['GL.JE.POST'])});await poster.postJournal({...ids,journalEntryId:draft.journal_entry_id,expectedRevision:3,idempotencyKey:'insurance-amortization-post-001'});
  const ledger=(await adminPool.query('SELECT account_code,debit_amount,credit_amount FROM ledger_line WHERE tenant_id=$1 AND journal_entry_id=$2 ORDER BY account_code',[ids.tenantId,draft.journal_entry_id])).rows;assert.deepEqual(ledger,[{account_code:'141500',debit_amount:'0.0000',credit_amount:scheduleLine.amount},{account_code:'610100',debit_amount:scheduleLine.amount,credit_amount:'0.0000'}]);
  assert.equal((await adminPool.query("SELECT count(*)::int n FROM audit_event WHERE tenant_id=$1 AND event_type IN ('AI_AMORTIZATION_ACCEPTED','JOURNAL_CREATED','JOURNAL_SUBMIT','JOURNAL_REVIEW','JOURNAL_APPROVE','JOURNAL_POSTED')",[ids.tenantId])).rows[0].n,6);
  const inputKeys={account_code:'141500'},snapshotHash=(await adminPool.query("SELECT refs_jsonb_hash(jsonb_build_object('input_keys',$1::jsonb,'output_rules',jsonb_build_object('classification','PREPAID'))) AS snapshot_hash",[JSON.stringify(inputKeys)])).rows[0].snapshot_hash,mappingId=randomUUID();
  await adminPool.query(`INSERT INTO mapping_snapshot(mapping_snapshot_id,tenant_id,entity_id,family,scope_type,scope_key,input_key_hash,version,priority,effective_from,status,input_keys,output_rules,snapshot_hash,created_by,approved_by,approved_at)
    VALUES($1,$2,$3::uuid,'PREPAID_ACCOUNT_CLASSIFICATION','ENTITY',$3::text,$4,1,0,'2026-01-01','APPROVED',$5::jsonb,jsonb_build_object('classification','PREPAID'),$6,'insurance-map-maker','insurance-map-approver',now())`,[mappingId,ids.tenantId,ids.entityId,hash('insurance-prepaid-map'),JSON.stringify(inputKeys),snapshotHash]);
  const reportReader=new PostgresAccountingKernel(runtimePool,{sessionProvider:sessionProvider(ids,'insurance-report-reader',['GL.REPORT.VIEW'])});const rows=await reportReader.getPrepaidRollforward({tenantId:ids.tenantId,entityId:ids.entityId,periodId:ids.periodId});assert.equal(rows.length,1);assert.equal(rows[0].account_code,'141500');assert.deepEqual(rows[0].source_document_ids,[trace.documentId]);
});
...
pgTest('AI exact duplicate payable finding retains paired source evidence without changing either source or creating a journal',async()=>{
  ...
  const auditPath=`/api/v1/entities/${ids.entityId}/ai/findings/duplicate-payables?limit=10`,summaryPath=`/api/v1/entities/${ids.entityId}/ai/analysis-summary`,beforeAuditRead=await auditReadCounts();
  ...
  const summaryDenied=await auditApi({method:'GET',url:summaryPath,body:null,headers:{'x-test-actor':'ai-audit-no-grant'}});assert.equal(summaryDenied.status,403);
  const summaryRead=await auditApi({method:'GET',url:summaryPath,body:null,headers:{'x-test-actor':'ai-audit-http-reader'}});
  assert.equal(summaryRead.status,200,JSON.stringify(summaryRead.body));assert.equal(summaryRead.headers['cache-control'],'no-store');assert.deepEqual(summaryRead.body.data.map(row=>({category:row.category,total:row.total_findings,high:row.high_findings,draft:row.can_create_draft,post:row.can_post})),[{category:'DUPLICATE_PAYABLE',total:'1',high:'1',draft:false,post:false}]);
