...
pgTest('server-derived AI Posted outcome review binds decision human workflow ledger trial balance and report atomically',async()=>{
  ...
  const driftHash=(await adminPool.query('SELECT decision_hash FROM ai_accounting_decision WHERE ai_accounting_decision_id=$1',[retained.ai_accounting_decision_id])).rows[0].decision_hash,wrongReport=await outcomeReviewer.retainAiAccountingPostedOutcomeReview({tenantId:ids.tenantId,entityId:ids.entityId,decisionId:retained.ai_accounting_decision_id,expectedDecisionHash:driftHash,expectedReviewRevision:0,idempotencyKey:'retain-ai-posted-outcome-pg-report-drift'});assert.equal(wrongReport.status,'MISMATCH');assert.ok(wrongReport.reason_codes.includes('REPORT_SNAPSHOT_MISMATCH'));

  await adminPool.query('ALTER TABLE outbox_event DISABLE TRIGGER USER');await adminPool.query("DELETE FROM outbox_event WHERE outbox_event_id=(SELECT outbox_event_id FROM outbox_event WHERE aggregate_id=$1 AND event_type='JOURNAL_REVIEW' LIMIT 1)",[draft.journal_entry_id]);await adminPool.query('ALTER TABLE outbox_event ENABLE TRIGGER USER');
  const missingOutbox=await outcomeReviewer.retainAiAccountingPostedOutcomeReview({tenantId:ids.tenantId,entityId:ids.entityId,decisionId:retained.ai_accounting_decision_id,expectedDecisionHash:driftHash,expectedReviewRevision:1,idempotencyKey:'retain-ai-posted-outcome-pg-outbox-missing'});assert.ok(missingOutbox.reason_codes.includes('WORKFLOW_EVIDENCE_MISSING_OR_MISMATCHED'));
  ...
  const receiptDrift=await outcomeReviewer.retainAiAccountingPostedOutcomeReview({tenantId:ids.tenantId,entityId:ids.entityId,decisionId:retained.ai_accounting_decision_id,expectedDecisionHash:driftHash,expectedReviewRevision:3,idempotencyKey:'retain-ai-posted-outcome-pg-receipt-drift'});assert.ok(receiptDrift.reason_codes.includes('ACCEPTANCE_EVIDENCE_MISMATCH'));assert.ok(receiptDrift.reason_codes.includes('DRAFT_EVIDENCE_MISMATCH'));
});
