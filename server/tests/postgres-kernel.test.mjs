...
pgTest('expired attachment cleanup is claimed exclusively, retries failures and leaves an immutable audit trail',async()=>{
  ...
  assert.equal((await competing.completeAttachmentCleanup({tenantId:ids.tenantId,entityId:ids.entityId,attachmentId,claimToken:recovered.claim_token,deleted:false,error:'minio unavailable'})).status,'CLEANUP_FAILED');
