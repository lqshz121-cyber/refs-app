...
pgTest('admitted signed Payable composition persists receipt Raw Normalized Staging and replays with zero journal writes',async()=>{
  const ids=await seed({status:'DRAFT'}),{privateKey,publicKey}=generateKeyPairSync('ed25519'),keyId='wbs-payable-composition-pg',capturedAt='2026-07-11T03:00:00.000Z',snapshotToken=`pg-payable-${randomUUID()}`,sourceId=randomUUID();
  const rows=[{ap_guid:sourceId,ap_type:'AUTOC',company_code:ids.sourceEntityId,currency:'USD',amount:'89.1250',incurred_date:'2026-07-10',posting_date:'2026-07-11',vendor_no:'VENDOR-PG'}];
  ...
  const stored=(await adminPool.query("SELECT i.wbs_inbound_row_id,r.receipt_hash,i.source_record_id,i.source_version,i.raw,i.normalized,i.outcome,i.outcome_kind,refs_wbs_payable_review_evidence_hash(i.wbs_inbound_row_id,i.source_record_id,i.source_version,r.receipt_hash,i.raw,i.normalized,i.outcome,i.outcome_kind) evidence_hash FROM wbs_inbound_receipt r JOIN wbs_inbound_row i USING(receipt_id) WHERE r.tenant_id=$1 AND r.entity_id=$2 AND i.source_record_id=$3",[ids.tenantId,ids.entityId,sourceId])).rows[0];
  ...
  const settingId=randomUUID(),mappingId=randomUUID(),mappingInput={company_key:ids.sourceEntityId,currency:'USD',vendor_ref:'VENDOR-PG',cost_code_ref:null},mappingOutput={vendor_ref:'VENDOR-PG',offset_account_code:'610000',amount_multiplier:'-1',source_direction:'DEBIT',rule_code:'WBS_PAYABLE_AP_MAP',rule_version:'1'};
