...
pgTest('separate humans take one approved WBS H1 Payable mapping from Draft through POSTED GL and financial statements',async()=>{
  ...
  await adminPool.query(`INSERT INTO wbs_h1_payable_mapping_source_stage(tenant_id,entity_id,company_code,period_code,wbs_uuid,source_record_hash,accounting_date,amount,project_code,cost_code,vendor_no,source_fact_hash,provider_content_hash,captured_at) VALUES($1,$2,$3,'2026-01','WBS-HUMAN-DRAFT-1',$4,'2026-01-15','125.0000','P-100','0LD067','WBS_TEST_VENDOR',$5,$6,'2026-08-22T12:00:00Z')`,[ids.tenantId,ids.entityId,ids.sourceEntityId,sourceHash,hash('h1-human-draft-fact'),hash('h1-human-draft-provider')]);
