...
pgTest('authorized WBS snapshot import persists immutable observations without creating source documents or journals',async()=>{
  ...
  const productionViews=snapshot.views.map(view=>({...view,row_count:view.rows.length,first_primary_key:view.rows[0].apGuId,last_primary_key:view.rows.at(-1).apGuId}));const {privateKey,publicKey}=generateKeyPairSync('ed25519');const production={...snapshot,schema_version:'WBS_READONLY_SNAPSHOT_V2',snapshot_id:randomUUID(),environment:'PRODUCTION',views:productionViews,delivery:{mode:'READONLY_VIEW_EXPORT',extract_started_at:capturedAt,extract_completed_at:capturedAt,consistency:'COMPLETE',read_consistency:'SNAPSHOT_ISOLATION',pagination:'PRIMARY_KEY_SEEK'},detached_signature:{key_id:'wbs-prod-test',algorithm:'Ed25519',value:''}};delete production.package_hash;const {detached_signature,...productionManifest}=production;production.package_hash=canonicalRequestHash(productionManifest);production.detached_signature.value=sign(null,Buffer.from(production.package_hash),privateKey).toString('base64');
