...
pgTest('AI amortization creates a human Draft then standard Posted JE with immutable source and ledger lineage',async()=>{
  ...
  const inputKeys={account_code:'141500'},mappingId=randomUUID(),snapshotHash=(await adminPool.query("SELECT refs_jsonb_hash(jsonb_build_object('input_keys',$1::jsonb,'output_rules',jsonb_build_object('classification','PREPAID'))) AS snapshot_hash",[JSON.stringify(inputKeys)])).rows[0].snapshot_hash;
  await adminPool.query(`INSERT INTO mapping_snapshot(mapping_snapshot_id,tenant_id,entity_id,family,scope_type,scope_key,input_key_hash,version,priority,effective_from,status,input_keys,output_rules,snapshot_hash,created_by,approved_by,approved_at)
    VALUES($1,$2,$3,'PREPAID_ACCOUNT_CLASSIFICATION','ENTITY',$7,$4,1,0,'2026-01-01','APPROVED',$5::jsonb,jsonb_build_object('classification','PREPAID'),$6,'ai-prepaid-maker','ai-prepaid-approver',now())`,[mappingId,ids.tenantId,ids.entityId,hash('ai-prepaid-account-mapping-key'),JSON.stringify(inputKeys),snapshotHash,ids.entityId]);
  const prepaid=await reader.getPrepaidRollforward({tenantId:ids.tenantId,entityId:ids.entityId,periodId:ids.periodId});assert.equal(prepaid.length,1);assert.deepEqual({account:prepaid[0].account_code,status:prepaid[0].mapping_status,amortization:prepaid[0].period_amortization,closing:prepaid[0].closing_balance,mapping:prepaid[0].mapping_snapshot_id,hash:prepaid[0].mapping_snapshot_hash},{account:'141500',status:'MAPPED_PREPAID_ACCOUNT',amortization:'8.3333',closing:'-8.3333',mapping:mappingId,hash:snapshotHash});assert.ok(prepaid[0].journal_entry_ids.includes(drafted.journal_entry_id));assert.ok(prepaid[0].source_document_ids.includes(trace.documentId));
});
