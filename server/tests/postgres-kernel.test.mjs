...
pgTest('WBS TEST Bank retained checkpoint rejects changed chunk replay and resumes without partial core visibility',async()=>{
  ...
  assert.equal((await stagedCall()).idempotent,false);assert.equal((await stagedCall()).idempotent,true);await assert.rejects(stagedCall({changed:true}),error=>error.code==='23505');
