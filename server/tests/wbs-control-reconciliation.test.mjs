...
const receipt=(id,scope)=>({hash:'sha256:'+id.repeat(64).slice(0,64),ref:`object://receipt/${id}`,version:'v1',scope});
...
const costArgs={sourceType:'COST_GENERAL_LEDGER',scope:costScope,sourceReceipt:receipt('a',costScope),targetReceipt:receipt('b',costScope),approvedMapping:costMapping,sourceMetrics:costMetrics,targetMetrics:costMetrics};
...
test('control receipts cannot be replayed across company, period, or currency scope',()=>{
  assert.throws(()=>reconcileWbsControlEvidence({...costArgs,sourceReceipt:receipt('a',{...costScope,period:'2026-07'})}),error=>error.code==='WBS_CONTROL_RECEIPT_SCOPE_MISMATCH');
  assert.throws(()=>reconcileWbsControlEvidence({...costArgs,targetReceipt:receipt('b',{...costScope,company_key:'COMPANY-B'})}),error=>error.code==='WBS_CONTROL_RECEIPT_SCOPE_MISMATCH');
  assert.throws(()=>reconcileWbsControlEvidence({...costArgs,sourceReceipt:receipt('a',{})}),error=>error.code==='WBS_CONTROL_RECEIPT_SCOPE_REQUIRED');
