...
const request=(query=`periodId=${periodId}`,extra={})=>({method:'GET',url:`/api/v1/entities/${entityId}/report-mappings?${query}`,headers:{authorization:'Bearer signed',...(extra.headers||{})},body:extra.body??null});

test('serves complete approved report mappings through an exact no-store read boundary',async()=>{
  ...
  assert.equal(response.body.data.population.population_complete,true);assert.equal(response.body.data.mappings.length,24);
