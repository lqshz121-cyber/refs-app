...
test('decision run delegates the complete validated population to one atomic batch command',async()=>{
  ...
  assert.equal(response.status,201);assert.equal(response.headers['cache-control'],'no-store');assert.deepEqual(response.body.data.population,batch.population);assert.equal(calls.length,1);assert.deepEqual(calls[0],{tenantId,entityId,accountingPeriodId:periodId,packets:[packet],populationCount:1,populationHash:hash('e'),idempotencyKey:'decision-batch-root-001'});
});

test('decision run rejects a missing population receipt before the durable writer',async()=>{
  const incomplete={schema_version:'AI_ACCOUNTING_DECISION_PACKET_FULL_BATCH_V1',scope:{tenant_id:tenantId,entity_id:entityId,accounting_period_id:periodId},row_count:1,decision_counts:{ready_for_human_review:1,exception:0},packets:[packet],action_flags:{can_create_draft:false,can_review:false,can_approve:false,can_post:false}};let writes=0;
  const api=createAccountingApi({authenticate:async()=>({trusted:true,tenantId,actorId:'ai-controller'}),kernelFactory:async()=>({retainAiAccountingDecisionBatch:async()=>{writes+=1;throw new Error('writer must not run');}}),aiAccountingDecisionPacketServiceFactory:async()=>({analyze:async()=>incomplete})});
  const response=await api({method:'POST',url:`/api/v1/entities/${entityId}/ai/accounting-decision-runs`,headers:{'idempotency-key':'decision-batch-missing-population-001'},body:{accounting_period_id:periodId,limit:100}});
  assert.equal(response.status,502);assert.equal(response.headers['cache-control'],'no-store');assert.equal(writes,0);
});
...
test('OpenAPI exposes the production full packet batch without granting accounting authority',async()=>{
  ...
  const run=contract.paths['/entities/{entityId}/ai/accounting-decision-runs'].post;for(const status of ['200','201'])assert.equal(run.responses[status].content['application/json'].schema.$ref,'#/components/schemas/AiAccountingDecisionRunReceiptEnvelope');const runReceipt=contract.components.schemas.AiAccountingDecisionRunReceipt;assert.equal(runReceipt.additionalProperties,false);assert.equal(runReceipt.properties.population.properties.population_complete.const,true);for(const action of ['can_create_draft','can_review','can_approve','can_post'])assert.equal(runReceipt.properties[action].const,false);
  ...
  const details=contract.components.schemas.AiAccountingDecisionFullSourceDetail.oneOf,invoice=details.find(schema=>schema.properties?.invoice_number),loan=details.find(schema=>schema.properties?.loan_ref);assert.equal(details.every(schema=>schema.type==='object'&&schema.additionalProperties===false&&Array.isArray(schema.required)&&Object.keys(schema.properties).length>0),true);assert.equal(invoice.required.includes('execution_evidence'),true);assert.equal(loan.required.includes('execution_evidence'),true);assert.equal(invoice.properties.execution_evidence.$ref,'#/components/schemas/AiAccountingInvoiceExecutionEvidence');assert.equal(loan.properties.execution_evidence.$ref,'#/components/schemas/AiAccountingLoanExecutionEvidence');
  for(const name of ['AiAccountingExecutionAccountMaster','AiAccountingInvoiceExecutionAttachment','AiAccountingLoanExecutionAttachment','AiAccountingImportLineage','AiAccountingInvoiceExecutionEvidence','AiAccountingLoanExecutionEvidence']){const schema=contract.components.schemas[name];assert.equal(schema.type,'object');assert.equal(schema.additionalProperties,false);assert.ok(schema.required.length>0);assert.ok(Object.keys(schema.properties).length>0);}assert.equal(contract.components.schemas.AiAccountingLoanExecutionAttachment.properties.evidence_type.const,'source_attachment');
});
...
test('full batch validator rejects missing lineage and duplicate retained source lines before persistence',()=>{const safe={schema_version:'AI_ACCOUNTING_DECISION_PACKET_FULL_BATCH_V1',scope:{tenant_id:tenantId,entity_id:entityId,accounting_period_id:periodId},row_count:1,decision_counts:{ready_for_human_review:1,exception:0},packets:[packet],action_flags:{can_create_draft:false,can_review:false,can_approve:false,can_post:false}},missing={...packet,source:{...packet.source}};delete missing.source.source_line_hash;for(const unsafe of [{...safe,packets:[missing]},{...safe,row_count:2,decision_counts:{ready_for_human_review:2,exception:0},packets:[packet,structuredClone(packet)]}])assert.throws(()=>assertAiAccountingDecisionPacketFullBatch(unsafe,{tenantId,entityId,accountingPeriodId:periodId}),error=>error.code==='AI_ACCOUNTING_DECISION_RESPONSE_INVALID');});
