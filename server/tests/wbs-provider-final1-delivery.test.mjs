...
const controls=rows=>{const total=rows.reduce((sum,row)=>sum+units(row.amount),0n),per_currency_totals=[{currency:'USD',gross_amount:`${total/10000n}.${String(total%10000n).padStart(4,'0')}`}],row_count=rows.length;return {row_count,per_currency_totals,control_totals_hash:canonicalRequestHash({row_count,per_currency_totals})};};
...
async function fixture({credentials=false,requestText=null,currency='USD',receiptExtra={},packageExtra={},rows=null}={}){
  ...
  const signedControls=controls(signedRows),unsigned={schema_version:'WBS_READONLY_SNAPSHOT_V2',snapshot_id:'22222222-2222-4222-8222-222222222222',captured_at:'2026-08-15T00:00:00Z',environment:'PRODUCTION',source_system:'WBS',domain:'PAYABLES',company_key:'WBPA',date_from:'2026-01-01',date_to:'2026-06-30',views:{list_payables:view},control_totals:signedControls,...packageExtra};
  ...
  const unsignedReceipt={issuer:'refs-mcp.wbm3.com',kid,algorithm:'Ed25519',request_sha256:hash(requestRaw),response_sha256:hash(responseRaw),package_hash:hash(packageRaw),nonce:'final1-nonce',signed_at:'2026-08-15T00:01:00Z',expires_at:'2026-08-15T00:16:00Z',tenant_id:'33333333-3333-4333-8333-333333333333',entity_id:'44444444-4444-4444-8444-444444444444',company_code:'WBPA',immutable_version:pkg.snapshot_id,nonempty:true,control_totals:signedControls,...receiptExtra};
