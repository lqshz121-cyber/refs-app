...
test('paged WBS test range import is closed, bounded, and aggregate-only',()=>{
  ...
  const body=operation.requestBody.content['application/json'].schema;assert.equal(body.additionalProperties,false);assert.deepEqual(body.required,['companyCode','dateFrom','dateTo','pageSize','maxPages']);assert.equal(body.properties.dateFrom.const,'2026-01-01');assert.equal(body.properties.dateTo.const,'2026-06-30');assert.equal(body.properties.pageSize.const,10);assert.equal(body.properties.maxPages.maximum,50);
