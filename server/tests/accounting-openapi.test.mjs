...
test('controlled test Bank import is a separate closed staging-only reconciliation command',()=>{
  ...
  const body=operation.requestBody.content['application/json'].schema;assert.equal(body.additionalProperties,false);assert.deepEqual(body.required,['periodId','companyCode','dateFrom','dateTo','limit']);
  const result=contract.components.schemas.WbsControlledTestBankResult;assert.equal(result.additionalProperties,false);assert.equal(result.properties.test_only.const,true);assert.equal(result.properties.provenance_mode.const,'CONTROLLED_TEST_UNSIGNED');assert.equal(result.properties.bank_account_ref.const,'WBS_TEST_BANK');assert.equal(result.properties.status.const,'DRAFT');
});

test('paged WBS test range import is closed, bounded, and aggregate-only',()=>{
  ...
  const body=operation.requestBody.content['application/json'].schema;assert.equal(body.additionalProperties,false);assert.deepEqual(body.required,['companyCode','dateFrom','dateTo','pageSize','maxPages']);assert.equal(body.properties.dateFrom.const,'2026-01-01');assert.equal(body.properties.dateTo.const,'2026-06-30');assert.equal(body.properties.pageSize.maximum,10);assert.equal(body.properties.maxPages.maximum,50);
