...
test('accounting OpenAPI is 3.1, authenticated and operation ids match the runtime kernel surface',()=>{
  ...
  assert.ok(operationIds.length>=135);assert.ok(operationIds.includes('bindFixedAssetDisposalSource'));assert.ok(operationIds.includes('createSalesReceiptBankMatch'));assert.ok(operationIds.includes('createFixedAssetDepreciation'));assert.ok(operationIds.includes('createFixedAssetDisposal'));assert.ok(operationIds.includes('reviewFixedAssetPostImpairmentPolicy'));for(const id of ['createUnitTransfer','transitionUnitTransfer','cancelUnitTransfer','postUnitTransfer','createUnitTransferReversal','transitionUnitTransferReversal','cancelUnitTransferReversal','postUnitTransferReversal'])assert.ok(operationIds.includes(id));for(const id of ['createIntercompanyElimination','transitionIntercompanyElimination','cancelIntercompanyElimination','postIntercompanyElimination','createAccountingSettingsWorkflow','transitionAccountingSettingsWorkflow'])assert.ok(operationIds.includes(id));for(const id of ['createCashTransfer','transitionCashTransfer','cancelCashTransfer','postCashTransfer','createCashTransferBankAccountControl','approveCashTransferBankAccountControl','retireCashTransferBankAccountControl','linkCashTransferBankLeg','createConsolidationConfiguration','transitionConsolidationConfiguration','createReportSavedView','updateReportSavedView'])assert.ok(operationIds.includes(id));
  for(const id of ['proposeCounterpartyChange','reviewCounterpartyChange'])assert.ok(operationIds.includes(id));
  ...
  for(const id of ['createApPayment','createArReceipt','createArRefund'])assert.ok(!operationIds.includes(id));
  ...
  assert.ok(operationIds.includes('reopenAccountingPeriod'));
  assert.ok(operationIds.includes('resolveAiBankDuplicatePayment'));
  assert.ok(operationIds.includes('closeAccountingPeriod'));
...
test('Unit Transfer reversal OpenAPI exposes closed Draft and lifecycle commands',()=>{
 const base='/entities/{entityId}/unit-transfers/{pairId}/reversals',create=contract.paths[base].post;
 assert.equal(create.operationId,'createUnitTransferReversal');assert.deepEqual(create.requestBody.content['application/json'].schema.required,['expectedPairRevision','expectedSourceRevision','expectedTargetRevision','reversalDate','sourceJournalNumber','targetJournalNumber','reason']);assert.equal(create.requestBody.content['application/json'].schema.additionalProperties,false);
 for(const status of ['200','201'])assert.equal(create.responses[status].content['application/json'].schema.$ref,'#/components/schemas/UnitTransferReversalDraftEnvelope');
 for(const [suffix,operationId,schema] of [['transitions','transitionUnitTransferReversal','UnitTransferReversalTransitionEnvelope'],['cancel','cancelUnitTransferReversal','UnitTransferReversalCancelEnvelope'],['post','postUnitTransferReversal','UnitTransferReversalPostEnvelope']]){const operation=contract.paths[`${base}/{reversalPairId}/${suffix}`].post;assert.equal(operation.operationId,operationId);assert.equal(operation.requestBody.content['application/json'].schema.additionalProperties,false);assert.equal(operation.responses['200'].content['application/json'].schema.$ref,`#/components/schemas/${schema}`);assert.equal(operation.responses['201'],undefined);assert.equal(operation.responses['412'].$ref,'#/components/responses/PreconditionFailed');}
 for(const name of ['UnitTransferReversalDraftReceipt','UnitTransferReversalTransitionReceipt','UnitTransferReversalCancelReceipt','UnitTransferReversalPostReceipt']){const schema=contract.components.schemas[name];assert.equal(schema.additionalProperties,false);assert.deepEqual(new Set(Object.keys(schema.properties)),new Set(schema.required));}
 assert.ok(contract.components.schemas.UnitTransferReversalPostReceipt.required.includes('elimination_reversal_basis_id'));assert.ok(contract.components.schemas.UnitTransferReversalPostReceipt.required.includes('evidence_hash'));
 assert.match(contract.paths['/entities/{entityId}/unit-transfers'].get.description,/ordered reversal_history, nullable active_reversal, and server-computed can_reverse/);
 assert.match(contract.paths['/entities/{entityId}/unit-transfers/{pairId}'].get.responses['200'].description,/ordered reversal_history, nullable active_reversal, and server-computed can_reverse/);
});

test('fixed asset depreciation OpenAPI matches the evidence-bound Draft runtime',()=>{
  const options=contract.paths['/entities/{entityId}/fixed-assets/register/{assetId}/depreciation-options'].get,draft=contract.paths['/entities/{entityId}/fixed-assets/register/{assetId}/depreciations'].post;
  assert.equal(options.operationId,'readFixedAssetDepreciationOptions');assert.match(options.description,/FIXED_ASSET\.DEPRECIATION\.DRAFT/);assert.equal(options.responses['200'].headers['Cache-Control'].schema.const,'no-store');assert.equal(options.responses['200'].content['application/json'].schema.$ref,'#/components/schemas/FixedAssetDepreciationOptionsEnvelope');
  assert.deepEqual(options.parameters.map(parameter=>parameter.$ref||`${parameter.in}:${parameter.name}`),['#/components/parameters/EntityId','path:assetId','query:periodId']);
  assert.equal(draft.operationId,'createFixedAssetDepreciation');assert.match(draft.description,/never approves or posts/i);assert.deepEqual(draft.parameters.map(parameter=>parameter.$ref||`${parameter.in}:${parameter.name}`),['#/components/parameters/EntityId','#/components/parameters/IdempotencyKey','path:assetId']);
  ...
  const optionData=contract.components.schemas.FixedAssetDepreciationOptions;assert.equal(optionData.additionalProperties,false);assert.equal(optionData.properties.schema_version.const,'FIXED_ASSET_DEPRECIATION_OPTIONS_V2');assert.equal(optionData.properties.readiness_status.enum.length,11);assert.equal(optionData.properties.member_trace.additionalProperties,false);assert.equal(optionData.properties.source.anyOf[0].additionalProperties,false);assert.equal(optionData.properties.pending_journals.maxItems,20);
  assert.ok(optionData.required.includes('post_impairment_policy'));assert.equal(optionData.properties.post_impairment_policy.anyOf[0].$ref,'#/components/schemas/FixedAssetPostImpairmentDepreciationPolicyReadEvidence');assert.deepEqual(optionData.properties.schedule.properties.schedule_basis.enum,['ORIGINAL_REGISTER','POST_IMPAIRMENT_REVISED']);assert.deepEqual(optionData.properties.schedule.properties.revised_period_number.type,['integer','null']);
  const revised=contract.components.schemas.FixedAssetPostImpairmentDepreciationPolicyReadEvidence;assert.equal(revised.additionalProperties,false);assert.equal(revised.properties.status.const,'INDEPENDENTLY_REVIEWED');assert.equal(revised.properties.reviewed_at.format,'date-time');for(const field of ['revised_carrying_value','revised_depreciable_basis','regular_period_amount','final_period_amount']){assert.equal(new RegExp(revised.properties[field].pattern).test('0.0000'),false);assert.equal(new RegExp(revised.properties[field].pattern).test('0.0001'),true);}
...
test('fixed asset disposal OpenAPI exposes server-derived amounts and Draft-only state',()=>{
  const options=contract.paths['/entities/{entityId}/fixed-assets/register/{assetId}/disposal-options'].get,draft=contract.paths['/entities/{entityId}/fixed-assets/register/{assetId}/disposals'].post;
  assert.equal(options.operationId,'readFixedAssetDisposalOptions');assert.match(options.description,/FIXED_ASSET\.DISPOSAL\.DRAFT/);assert.equal(options.responses['200'].headers['Cache-Control'].schema.const,'no-store');assert.deepEqual(options.parameters.map(parameter=>parameter.$ref||`${parameter.in}:${parameter.name}`),['#/components/parameters/EntityId','path:assetId','query:periodId','query:disposalDate']);
  assert.equal(draft.operationId,'createFixedAssetDisposal');assert.match(draft.description,/derives proceeds/i);const body=draft.requestBody.content['application/json'].schema;assert.equal(body.additionalProperties,false);assert.equal(body.required.includes('proceeds'),false);assert.deepEqual(body.required,['periodId','journalNumber','disposalDate','sourceDocumentId','expectedSourceVersion','expectedSourceHash','proceedsAccountCode','proceedsMemberRef','gainLossAccountCode','expectedOptionsHash','reason']);
  for(const status of ['200','201']){assert.equal(draft.responses[status].headers['Cache-Control'].schema.const,'no-store');assert.equal(draft.responses[status].headers.ETag.schema.const,'"1"');assert.equal(draft.responses[status].content['application/json'].schema.$ref,'#/components/schemas/FixedAssetDisposalEnvelope');}
  const receipt=contract.components.schemas.FixedAssetDisposalEnvelope.properties.data;assert.equal(receipt.additionalProperties,false);assert.equal(receipt.properties.status.const,'DRAFT');assert.equal(receipt.properties.revision.const,1);
});
...
test('controlled test Bank import is a separate closed immutable-receipt command',()=>{
...
test('paged WBS test month import is closed, bounded, and aggregate-only',()=>{
  ...
  assert.equal(contract.components.schemas.WbsTestRangePayablesResult.properties.record_count.maximum,10000);assert.equal(contract.components.schemas.WbsTestRangePayablesResult.properties.h1_record_count.maximum,10000);assert.equal(contract.components.schemas.WbsTestRangePayablesResult.properties.posted_count.const,0);assert.equal(contract.components.schemas.WbsTestRangeBankResult.properties.bank_source_count.maximum,10000);assert.equal(contract.components.schemas.WbsTestRangeBankResult.properties.provider_page_count.maximum,1000);assert.ok(contract.components.schemas.WbsTestRangeBankResult.properties.receipt.oneOf);
  assert.ok(contract.components.schemas.WbsTestRangeBankReceipt.required.includes('period_id'));assert.ok(contract.components.schemas.WbsTestRangeBankReceipt.required.includes('receipt_hash'));assert.equal(contract.components.schemas.WbsTestRangeBankReceipt.properties.period_id.$ref,'#/components/schemas/Uuid');
...
test('all responses are no-store and use a structured success or problem envelope',()=>{
  ...
  for(const operation of accountingCommands.filter(item=>item.deprecated!==true)){assert.ok(operation.responses['200']);assert.ok(operation.responses['503']);assert.ok(operation.responses.default);if(!['verifyWbsAutoRecTransitionContract','closeAccountingPeriod','reopenAccountingPeriod','reviewCounterpartyChange','transitionUnitTransfer','cancelUnitTransfer','postUnitTransfer','transitionUnitTransferReversal','cancelUnitTransferReversal','postUnitTransferReversal','transitionIntercompanyElimination','cancelIntercompanyElimination','postIntercompanyElimination','transitionAccountingSettingsWorkflow','transitionForecastWorkflow','transitionWebhookSubscription','updateReportSavedView','transitionRecurringSchedule','runDueRecurringSchedules'].includes(operation.operationId))assert.ok(operation.responses['201']);}
  ...
  const explanation=contract.paths['/entities/{entityId}/ai/analysis-explanation'].post;assert.ok(explanation.responses['200']);assert.ok(explanation.responses['502']);assert.ok(explanation.responses['503']);assert.equal(explanation.responses['201'],undefined);assert.match(explanation.description,/cannot create a Draft.*approve.*post/i);assert.equal(explanation.responses['200'].content['application/json'].schema.properties.data.$ref,'#/components/schemas/AiAccountingAnalysisExplanationReceipt');const explanationResult=contract.components.schemas.AiAccountingAnalysisExplanationResult,explanationAction=contract.components.schemas.AiAccountingAnalysisControllerAction,explanationReceipt=contract.components.schemas.AiAccountingAnalysisExplanationReceipt;assert.equal(explanationResult.additionalProperties,false);assert.deepEqual(explanationResult.required,['headline','risk_level','narrative','controller_actions','can_create_draft','can_review','can_approve','can_post']);assert.deepEqual(explanationAction.required,['category','finding_ids','action']);assert.equal(explanationAction.properties.finding_ids.minItems,1);assert.equal(explanationAction.properties.finding_ids.uniqueItems,true);assert.equal(explanationReceipt.additionalProperties,false);assert.ok(explanationReceipt.properties.providerRequestId.oneOf.some(schema=>schema.type==='null'));
...
test('import/export history, recurring schedules, and custom reports are closed OpenAPI contracts',()=>{
  const history=contract.paths['/entities/{entityId}/import-export-history'].get;
  assert.equal(history.operationId,'listImportExportHistory');
  assert.equal(history.responses['200'].$ref,'#/components/responses/ImportExportHistoryPageEnvelope');
  assert.deepEqual(history.parameters.slice(1).map(parameter=>parameter.name),['jobType','limit','afterCompletedAt','afterJobId']);
  assert.equal(contract.components.schemas.ImportExportHistoryPage.properties.action_flags.properties.can_post.const,false);
  const exportHistory=contract.paths['/entities/{entityId}/import-export-history/posted-ledger-exports'].post;
  assert.equal(exportHistory.operationId,'recordPostedLedgerExportHistory');
  assert.deepEqual(exportHistory.parameters.map(parameter=>parameter.$ref),['#/components/parameters/EntityId','#/components/parameters/IdempotencyKey']);
  assert.equal(exportHistory.requestBody.content['application/json'].schema.$ref,'#/components/schemas/PostedLedgerExportHistoryCreate');
  ...
  assert.equal(report.operationId,'readCustomReport');
