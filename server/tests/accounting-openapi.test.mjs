...
test('accounting OpenAPI is 3.1, authenticated and operation ids match the runtime kernel surface',()=>{
  ...
  assert.equal(operationIds.length,111);assert.ok(operationIds.includes('bindFixedAssetDisposalSource'));assert.ok(operationIds.includes('createSalesReceiptBankMatch'));assert.ok(operationIds.includes('createFixedAssetDepreciation'));assert.ok(operationIds.includes('reviewFixedAssetPostImpairmentPolicy'));
