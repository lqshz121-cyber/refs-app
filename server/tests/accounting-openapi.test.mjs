...
test('accounting OpenAPI is 3.1, authenticated and operation ids match the runtime kernel surface',()=>{
  ...
  assert.deepEqual(operations.map(operation=>operation.operationId).sort(),['applyApVendorCredit','createApBillVoid','createApPayment','createApVendorCredit','createArReceipt','createAutoJournal','createJournalAdjustment','createManualJournal','finalizeAttachment','postJournal','reserveAttachment','transitionJournal']);
});
