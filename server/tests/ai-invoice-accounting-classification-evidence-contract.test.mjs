...
test('repository reads only the approved capitalization policy projection',async()=>{
  const calls=[],kernel=new PostgresAccountingKernel({},{sessionProvider:async()=>({})});kernel.inSession=async work=>work({query:async(sql,args)=>(calls.push({sql,args}),{rows:[{result:{schema_version:'AI_CAPITALIZATION_POLICY_EVIDENCE_V1'}}]})});
  assert.deepEqual(await kernel.getAiCapitalizationPolicyEvidence({tenantId:'tenant',entityId:'entity',accountingPeriodId:'period'}),{schema_version:'AI_CAPITALIZATION_POLICY_EVIDENCE_V1'});
  assert.match(calls[0].sql,/refs_read_ai_capitalization_policy_evidence/);assert.deepEqual(calls[0].args,['tenant','entity','period']);
});
