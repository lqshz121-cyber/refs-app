...
function harness({duplicate=false,loop=false,identityMismatch=false,identityMismatchTool=null,outOfOrder=false,reverseRows=false,repartition=null,payableCount=2,payableDates=null,bankCount=2,bankPartial=false,tokenMode=false,pipelineDelayMs=0,failWorkflowIndexes=[],serializeWorkflowIndexes=[]}={}){
  ...
  const kernelForActor=actor=>({
    ...
    async createWbsControlledTestBankScope(args){calls.push(['bank',actor,args]);if(bankPartial)return {status:'WBS_TEST_BANK_IMPORT_PARTIAL',stage_id:uuid(999),next_chunk_index:20,chunk_count:Math.ceil(args.observation.rows.length/100),transaction_count:args.observation.rows.length,test_only:true,provenance_mode:'CONTROLLED_TEST_UNSIGNED',idempotent:false};if(bankReceipts.has(args.idempotencyKey))return {...bankReceipts.get(args.idempotencyKey),idempotent:true};const n=++bank,ids=args.observation.rows.map((_,index)=>uuid(120+n+index)),result={wbs_controlled_test_bank_import_id:uuid(100+n),reconciliation_id:uuid(110+n),bank_source_ids:ids,bank_account_ref:args.bankAccountRef,statement_ending_date:args.observation.rows.at(-1).accounting_date,transaction_count:ids.length,status:'DRAFT',provenance_mode:'CONTROLLED_TEST_UNSIGNED',test_only:true,idempotent:false};bankReceipts.set(args.idempotencyKey,result);return result;}
  });
...
test('reads every real cursor-only page despite changing capture times before any monthly accounting write',async()=>{
  ...
  assert.deepEqual(result,{status:'WBS_TEST_MONTH_IMPORT_COMPLETE',period_code:'2026-01',date_from:'2026-01-01',date_to:'2026-01-31',page_size:10,payables:{provider_page_count:2,h1_record_count:2,record_count:2,imported_count:2,replayed_count:0,posted_count:2},bank:{provider_page_count:2,record_count:2,reconciliation:{bank_account_ref:'WBS_TEST_BANK_2026_01',period_code:'2026-01',period_id:uuid(201),reconciliation_id:uuid(111),transaction_count:2},bank_source_ids:[uuid(121),uuid(122)]},test_only:true});
...
test('a population above twenty chunks returns a closed resumable checkpoint without publishing Bank IDs',async()=>{
  ...
  assert.equal(result.status,'WBS_TEST_MONTH_IMPORT_PARTIAL');assert.equal(result.bank.record_count,2001);assert.deepEqual(result.bank.bank_source_ids,[]);assert.equal(result.bank.reconciliation,null);
