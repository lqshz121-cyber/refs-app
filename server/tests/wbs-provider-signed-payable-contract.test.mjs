...
test('migration 106 freezes append-only nonce, TTL, raw hashes, server scope and one atomic inbound command',async()=>{
  const sql=await readFile(new URL('../db/migrations/106_wbs_provider_signed_payable_admission.sql',import.meta.url),'utf8');
  const down=await readFile(new URL('../db/migrations/down/106_wbs_provider_signed_payable_admission.sql',import.meta.url),'utf8');
