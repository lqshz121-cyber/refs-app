...
test('fails a section closed for action authority, wrong scope, or secret-shaped evidence',async()=>{
  for(const unsafe of [
    ...
    batch('X',[{entity_id:entity,accounting_period_id:period,rule_id:'AI_HIDDEN_AUTHORITY_V1',risk_level:'HIGH',reason:'This finding carries contradictory hidden accounting authority.',suggested_action:'Reject finding-level accounting authority before producing the Controller scan.',can_post:true}]),
    batch('X',[{entity_id:entity,accounting_period_id:period,rule_id:'AI_NESTED_AUTHORITY_V1',risk_level:'HIGH',reason:'This finding carries contradictory nested accounting authority.',suggested_action:'Reject nested accounting authority before producing the Controller scan.',action_flags:{...actions,can_approve:true}}]),
    batch(undefined),
    batch('X',[{entity_id:entity,accounting_period_id:period,rule_id:'AI_DUPLICATE_V1',risk_level:'HIGH',reason:'This exact finding appears twice in one Controller section.',suggested_action:'Reject duplicate findings before producing the Controller risk summary.'},{accounting_period_id:period,entity_id:entity,reason:'This exact finding appears twice in one Controller section.',risk_level:'HIGH',rule_id:'AI_DUPLICATE_V1',suggested_action:'Reject duplicate findings before producing the Controller risk summary.'}]),
...
test('production wiring includes vendor, bank, source, cutoff, accrual, rent, loan, reporting, and manual-JE controls',()=>{
  ...
  for(const token of ['listAiPropertyRentRevenueReviews','getAiFinancialStatementVarianceComparison','getAiFinancialVariancePolicy','getAiBudgetVsActualSource','getAiBudgetVariancePolicy','getAiConstructionLoanCwipPopulationAttestation','getAiConstructionLoanGlBalances','getAiConstructionLoanLenderBalances','getAiConstructionLoanBalancePolicy','listInsurancePrepaidAmortization','getAiFixedAssetDisposalGapSource','detectFixedAssetDisposalGaps','getAiApAgingRiskSource','getAiApAgingRiskPolicy','listAiIntercompanyCounterpartyPeriods','getIntercompanyReconciliation','readAiCrossEntityPaymentInvoices','detectCrossEntityPaymentInvoiceReviews','listAiUnmatchedBankPaymentFindingsForPeriod','period.period_end','AI_ACCRUAL_CONTROLLER_SCAN_BATCH_V1','projectPrepaidAmortizationControllerReviews'])assert.match(source,new RegExp(token.replace('.','\\.')));
  assert.match(source,/createAiIntercompanyControllerScanService/);
  assert.match(source,/assertCompleteAiPopulation/);
  for(const code of ['AI_PROPERTY_RENT_REVENUE_POPULATION','AI_PREPAID_AMORTIZATION_POPULATION','AI_BANK_RECONCILIATION_POPULATION','AI_PREPAID_COVERAGE_POPULATION','AI_DUPLICATE_PAYABLE_POPULATION','AI_COST_DIMENSION_POPULATION','AI_LOAN_REFERENCE_POPULATION'])assert.match(source,new RegExp(code));
