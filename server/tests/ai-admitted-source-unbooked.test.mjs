...
const lookup=(overrides={})=>{const explicitHash=overrides.lookup_receipt_hash,{lookup_receipt_hash:ignored,...fields}=overrides,payload={schema_version:'ACCOUNTING_BOOKING_LOOKUP_V1',tenant_id:scope.tenantId,entity_id:scope.entityId,company_code:'WBPA',accounting_period_id:scope.accountingPeriodId,source_document_id:scope.sourceDocumentId,source_document_line_id:id(5),source_line_hash:hash(2),lookup_status:'COMPLETE',lookup_receipt_id:id(7),queried_at:'2026-08-20T12:00:00Z',ap_match_count:0,posted_journal_match_count:0,ledger_line_match_count:0,ap_document_ids:[],journal_entry_ids:[],ledger_line_ids:[],...fields};return {...payload,lookup_receipt_hash:explicitHash??hashAiAdmittedSourceBookingEvidence(payload)};};
...
test('fails closed for caller-shaped source or lookup evidence, scope drift, and count drift',async()=>{
  ...
  await assert.rejects(()=>service(source,lookup({lookup_receipt_hash:hash(9)})).analyze(scope),error=>error.code==='AI_ADMITTED_SOURCE_UNBOOKED_LOOKUP_INVALID');
