...
const productionEnvelope=rows=>({contract_version:'WBS-REFS-MCP-V1',tool:'list_payables',environment:'production',captured_at:'2026-08-09T12:00:00.000Z',source:{system:'WBS'},scope:{company:'COMPANY-A',currency:'USD',snapshot_token:'wbs-prod-snapshot-1'},record_count:rows.length,content_sha256:canonicalRequestHash(rows).slice(7),cursor_next:null,etl_notice:'Snapshot comparison required',rows});
...
test('a real Ed25519 production snapshot must pass both pipeline and adapter keyring verification before persistence',async()=>{
  ...
  const rows=[{ap_guid:'11111111-1111-4111-8111-111111111111',ap_type:'AUTOC',company_code:'COMPANY-A',currency:'USD',amount:'100.0000',incurred_date:'2026-08-09',posting_date:'2026-08-09'}],envelope=productionEnvelope(rows);
