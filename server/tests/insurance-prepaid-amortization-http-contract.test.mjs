...
const here=resolve(fileURLToPath(new URL('.',import.meta.url))),repository=await readFile(resolve(here,'../runtime/kernel-repository.mjs'),'utf8'),cas=await readFile(resolve(here,'../db/migrations/146_insurance_prepaid_amortization_http_cas.sql'),'utf8'),down=await readFile(resolve(here,'../db/migrations/down/146_insurance_prepaid_amortization_http_cas.sql'),'utf8'),openapi=JSON.parse(await readFile(resolve(here,'../api/openapi-accounting.json'),'utf8'));
