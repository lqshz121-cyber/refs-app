...
const sample=index=>({sample_id:`WBS-SAMPLE-${String(index).padStart(2,'0')}`,company_code:'WBPA',package_hash:hash(index.toString(16).padStart(1,'a').slice(-1)),snapshot_id:randomUUID(),bank_source_record_id:`bank-source-${index}`,business_source_record_id:`business-source-${index}`,bank_staging_item_id:`bank-stage-${index}`,business_staging_item_id:`business-stage-${index}`,bank_review_event_id:`bank-review-${index}`,business_review_event_id:`business-review-${index}`,bank_source_document_id:`bank-document-${index}`,business_source_document_id:`business-document-${index}`,bank_raw_event_id:`bank-raw-${index}`,business_raw_event_id:`business-raw-${index}`,bank_journal_entry_id:`bank-je-${index}`,business_journal_entry_id:`business-je-${index}`,bank_audit_event_id:`bank-audit-${index}`,business_audit_event_id:`business-audit-${index}`,report_id:`report-${index}`,control_total_hash:hash((15-index).toString(16).padStart(1,'b').slice(-1)),bank_reviewed_at:'2026-09-13T00:00:00.000Z',business_reviewed_at:'2026-09-13T00:00:00.000Z',bank_posted_at:'2026-09-13T00:01:00.000Z',business_posted_at:'2026-09-13T00:01:00.000Z',manual_review_completed:true,signed_package_verified:true,g11_posted_trace_verified:true,gl_report_control_total_matched:true,authoritative_api_readback_verified:true,signed_package_evidence:{reference:'object://wbs/signed/package-'+index,content_hash:hash(index.toString(16).padStart(1,'a').slice(-1)),verification_id:'verify-'+index,key_id:'wbs-production-key',algorithm:'Ed25519',verified_at:'2026-09-13T00:00:00.000Z'},authoritative_api_readback_evidence:{endpoint:'https://refs-accounting-api-staging.onrender.com/v1/wbs/samples/'+index,response_hash:hash((15-index).toString(16).padStart(1,'b').slice(-1)),authenticated_subject:'user|reviewer-'+index,read_at:'2026-09-13T00:00:00.000Z',http_status:200}});
const manifest=()=>({schema_version:'WBS_TWELVE_SAMPLE_ACCEPTANCE_V1',release_sha:'a'.repeat(40),verified_at:'2026-09-13T00:00:00.000Z',samples:Array.from({length:12},(_,index)=>sample(index+1))});
...
test('twelve sample acceptance rejects incomplete counts, duplicate evidence, and missing online readback',()=>{
  ...
  const shortRelease=manifest();shortRelease.release_sha='abcdef0123456789';assert.throws(()=>verifyWbsTwelveSampleAcceptance(shortRelease),{code:'WBS_TWELVE_SAMPLE_MANIFEST_INVALID'});
  const uppercaseRelease=manifest();uppercaseRelease.release_sha='A'.repeat(40);assert.throws(()=>verifyWbsTwelveSampleAcceptance(uppercaseRelease),{code:'WBS_TWELVE_SAMPLE_MANIFEST_INVALID'});
  ...
  const missingEvidence=manifest();delete missingEvidence.samples[0].signed_package_evidence;assert.throws(()=>verifyWbsTwelveSampleAcceptance(missingEvidence),{code:'WBS_TWELVE_SAMPLE_SIGNED_PACKAGE_EVIDENCE_INVALID'});
  const mismatchedPackage=manifest();mismatchedPackage.samples[0].signed_package_evidence.content_hash=hash('f');assert.throws(()=>verifyWbsTwelveSampleAcceptance(mismatchedPackage),{code:'WBS_TWELVE_SAMPLE_SIGNED_PACKAGE_EVIDENCE_INVALID'});
  const weakReadback=manifest();weakReadback.samples[0].authoritative_api_readback_evidence.endpoint='http://example.invalid/readback';assert.throws(()=>verifyWbsTwelveSampleAcceptance(weakReadback),{code:'WBS_TWELVE_SAMPLE_API_READBACK_EVIDENCE_INVALID'});
