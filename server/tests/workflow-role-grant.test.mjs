...
test('workflow roles are complete mutually exclusive grant sets with no provider/import permission',()=>{
  ...
  assert.deepEqual(AUTHORITATIVE_WORKFLOW_ROLES.AI_CONTROLLER_REVIEWER,[
    'AP.VIEW','AR.VIEW','BANK.VIEW','GL.JE.VIEW','GL.REPORT.VIEW','WBS.AUTOREC.VIEW','AI.ANALYSIS.EXPLAIN','AI.AMORTIZATION.VIEW','AI.ACCOUNTING.SETTINGS.VIEW',
  ]);
  ...
  assert.deepEqual(AUTHORITATIVE_WORKFLOW_ROLES.AI_AMORTIZATION_PREPARER,[...AUTHORITATIVE_WORKFLOW_ROLES.AI_CONTROLLER_REVIEWER.slice(0,6),'AI.AMORTIZATION.VIEW','AI.AMORTIZATION.PROPOSE','AI.ACCOUNTING.SETTINGS.VIEW']);
  assert.deepEqual(AUTHORITATIVE_WORKFLOW_ROLES.AI_AMORTIZATION_DRAFT_MAKER,[...AUTHORITATIVE_WORKFLOW_ROLES.AI_CONTROLLER_REVIEWER.slice(0,6),'AI.AMORTIZATION.VIEW','AI.AMORTIZATION.DRAFT','GL.JE.CREATE']);
  assert.deepEqual(AUTHORITATIVE_WORKFLOW_ROLES.JE_SUBMITTER,[...AUTHORITATIVE_WORKFLOW_ROLES.AI_CONTROLLER_REVIEWER.slice(0,6),'GL.JE.SUBMIT']);
  assert.equal(AUTHORITATIVE_WORKFLOW_ROLES.AI_AMORTIZATION_PREPARER.includes('AI.AMORTIZATION.DRAFT'),false);
  assert.equal(AUTHORITATIVE_WORKFLOW_ROLES.AI_AMORTIZATION_DRAFT_MAKER.includes('AI.AMORTIZATION.PROPOSE'),false);
  for(const role of ['AI_AMORTIZATION_PREPARER','AI_AMORTIZATION_DRAFT_MAKER','JE_SUBMITTER'])for(const denied of ['GL.JE.REVIEW','GL.JE.APPROVE','GL.JE.POST'])assert.equal(AUTHORITATIVE_WORKFLOW_ROLES[role].includes(denied),false);
