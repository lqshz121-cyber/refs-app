...
test('repository and HTTP expose one authenticated entity-period no-store read',async()=>{
  ...
  assert.equal((await api({method:'GET',url:`/api/v1/entities/${entityId}/reports/financial-statements?periodId=${periodId}`,headers:{'if-match':'"0"'},body:null})).body.code,'IF_MATCH_NOT_ALLOWED');
...
test('dimension profitability is a bounded POSTED-ledger read and never infers a missing dimension',async()=>{
  ...
  assert.equal((await api({method:'GET',url:base,headers:{'idempotency-key':'forbidden'},body:null})).body.code,'IDEMPOTENCY_KEY_NOT_ALLOWED');
  assert.equal((await api({method:'GET',url:base,headers:{'if-match':'"0"'},body:null})).body.code,'IF_MATCH_NOT_ALLOWED');
  assert.equal((await api({method:'GET',url:base,headers:{},body:{}})).body.code,'READ_BODY_FORBIDDEN');
});

test('Lot profitability is a distinct POSTED-ledger read and cannot fall back to another dimension',async()=>{
  const up=await readFile(new URL('../db/migrations/134_lot_profitability_read.sql',import.meta.url),'utf8');
  const down=await readFile(new URL('../db/migrations/down/134_lot_profitability_read.sql',import.meta.url),'utf8');
...
test('budget versus actual reads only approved immutable budgets against same-scope POSTED ledger evidence',async()=>{
  ...
  const response=await api({method:'GET',url:base,headers:{},body:null});assert.equal(response.status,200);assert.equal(response.headers['cache-control'],'no-store');assert.deepEqual(httpCalls,[{tenantId,entityId,periodId}]);assert.equal((await api({method:'GET',url:`${base}&extra=1`,headers:{},body:null})).body.code,'UNEXPECTED_QUERY_PARAMETER');assert.equal((await api({method:'GET',url:base,headers:{'idempotency-key':'forbidden'},body:null})).body.code,'IDEMPOTENCY_KEY_NOT_ALLOWED');assert.equal((await api({method:'GET',url:base,headers:{'if-match':'"0"'},body:null})).body.code,'IF_MATCH_NOT_ALLOWED');assert.equal((await api({method:'GET',url:base,headers:{},body:{}})).body.code,'READ_BODY_FORBIDDEN');
});

test('consolidation is a read-only immutable member, mapping, elimination, and posted-ledger report',async()=>{
  const up=await readFile(new URL('../db/migrations/082_consolidation_read.sql',import.meta.url),'utf8');
  const down=await readFile(new URL('../db/migrations/down/082_consolidation_read.sql',import.meta.url),'utf8');
  for(const token of ['CREATE TABLE consolidation_snapshot','CREATE TABLE consolidation_member','CREATE TABLE consolidation_account_map','CREATE TABLE consolidation_elimination_evidence','refs_get_consolidation',"'GL.REPORT.VIEW'",'refs_assert_scope','BLOCKED_MEMBER_SCOPE_REQUIRED','BLOCKED_MEMBER_PERIOD_OR_CURRENCY_REQUIRED','BLOCKED_MEMBER_POSTED_EVIDENCE_REQUIRED','BLOCKED_ELIMINATION_EVIDENCE_REQUIRED','APPROVED_CONSOLIDATION_SNAPSHOT_AND_POSTED_LEDGER_EXACT','consolidation_snapshot_append_only','REVOKE ALL','GRANT EXECUTE'])assert.match(up,new RegExp(token));
  assert.doesNotMatch(up,/INSERT INTO journal_entry|UPDATE journal_entry|DELETE FROM journal_entry|INSERT INTO ledger_line|UPDATE ledger_line|DELETE FROM ledger_line|refs_post_journal|INSERT INTO reconciliation/i);
  for(const token of ['DROP FUNCTION refs_get_consolidation','DROP TRIGGER consolidation_snapshot_append_only','DROP TABLE consolidation_elimination_evidence','DROP TABLE consolidation_account_map','DROP TABLE consolidation_member','DROP TABLE consolidation_snapshot'])assert.match(down,new RegExp(token));
  const calls=[],kernel=Object.create(PostgresAccountingKernel.prototype);kernel.inSession=async work=>work({query:async(sql,args)=>{calls.push({sql,args});return {rows:[{report_status:'APPROVED_CONSOLIDATION_SNAPSHOT_AND_POSTED_LEDGER_EXACT'}]};}});
  assert.deepEqual(await kernel.getConsolidation({tenantId:'tenant',entityId:'entity',periodId:'period',groupRef:'GROUP-01'}),[{report_status:'APPROVED_CONSOLIDATION_SNAPSHOT_AND_POSTED_LEDGER_EXACT'}]);assert.deepEqual(calls,[{sql:'SELECT * FROM refs_get_consolidation($1,$2,$3,$4)',args:['tenant','entity','period','GROUP-01']}]);
  const tenantId=randomUUID(),entityId=randomUUID(),periodId=randomUUID(),httpCalls=[];const api=createAccountingApi({authenticate:async()=>({trusted:true,tenantId,actorId:'reader'}),kernelFactory:async()=>({getConsolidation:async scope=>{httpCalls.push(scope);return [];}})});const base=`/api/v1/entities/${entityId}/reports/consolidation?periodId=${periodId}&groupRef=GROUP-01`;
  const response=await api({method:'GET',url:base,headers:{},body:null});assert.equal(response.status,200);assert.equal(response.headers['cache-control'],'no-store');assert.deepEqual(httpCalls,[{tenantId,entityId,periodId,groupRef:'GROUP-01'}]);assert.equal((await api({method:'GET',url:`${base}&extra=1`,headers:{},body:null})).body.code,'UNEXPECTED_QUERY_PARAMETER');assert.equal((await api({method:'GET',url:base,headers:{'idempotency-key':'forbidden'},body:null})).body.code,'IDEMPOTENCY_KEY_NOT_ALLOWED');assert.equal((await api({method:'GET',url:base,headers:{'if-match':'"0"'},body:null})).body.code,'IF_MATCH_NOT_ALLOWED');assert.equal((await api({method:'GET',url:base,headers:{},body:{}})).body.code,'READ_BODY_FORBIDDEN');
});
