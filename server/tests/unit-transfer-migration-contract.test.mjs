...
test('Unit Transfer paired reversal closes every tagged DO body with its matching tag',()=>{
 for(const tag of ['unit_transfer_post_patch','unit_transfer_journal_guard_patch','unit_transfer_unit_protect_patch','unit_transfer_read_patch']){
  const start=reversalUp.indexOf('DO  const index=MIGRATION_MANIFEST.findIndex(entry=>entry.name==='371_unit_transfer_paired_reversal.sql');
...
+tag+' const index=MIGRATION_MANIFEST.findIndex(entry=>entry.name==='371_unit_transfer_paired_reversal.sql');
 assert.ok(index>=0);assert.equal(MIGRATION_MANIFEST[index-1]?.name,'370_unit_transfer_authoritative.sql');
 const entry=MIGRATION_MANIFEST[index];assert.equal(entry.up,createHash('sha256').update(reversalUp).digest('hex'));assert.equal(entry.down,createHash('sha256').update(reversalDown).digest('hex'));
});
...
),end=reversalUp.indexOf('END; const index=MIGRATION_MANIFEST.findIndex(entry=>entry.name==='371_unit_transfer_paired_reversal.sql');
 assert.ok(index>=0);assert.equal(MIGRATION_MANIFEST[index-1]?.name,'370_unit_transfer_authoritative.sql');
 const entry=MIGRATION_MANIFEST[index];assert.equal(entry.up,createHash('sha256').update(reversalUp).digest('hex'));assert.equal(entry.down,createHash('sha256').update(reversalDown).digest('hex'));
});
