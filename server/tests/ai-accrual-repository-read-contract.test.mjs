...
const makeKernel=(rows=[])=>{const calls=[],kernel=Object.create(PostgresAccountingKernel.prototype);kernel.inSession=async work=>work({query:async(sql,params)=>{calls.push({sql,params});return {rows};}});return {kernel,calls};};
...
test('AI accrual repository reads only current Final-1 retained evidence under the explain capability',async()=>{
  ...
  assert.equal(history.length,1);assert.deepEqual(current,['00000000-0000-4000-8000-000000000001']);assert.deepEqual(posted,['00000000-0000-4000-8000-000000000001']);
  const sql=calls.map(call=>call.sql).join('\n');
  assert.match(sql,/refs_assert_scope\(\$1,\$2,'AI\.ANALYSIS\.EXPLAIN'\)/);assert.match(sql,/wbs_final1_retained_source_row/);assert.match(sql,/e\.is_current/);assert.match(sql,/j\.status='POSTED'/);
  assert.doesNotMatch(sql,/INSERT|UPDATE|DELETE|CREATE\s+JOURNAL/i);
});
