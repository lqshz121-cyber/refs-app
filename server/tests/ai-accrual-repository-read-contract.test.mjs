...
const makeKernel=(rows=[])=>{const calls=[],kernel=Object.create(PostgresAccountingKernel.prototype);kernel.inSession=async work=>work({query:async(sql,params)=>{calls.push({sql,params});const resultRows=/FROM accounting_period/.test(sql)?[{period_id:periodId,period_code:'2026-08',period_end:'2026-08-31',period_ordinal:24320}]:rows;return {rows:resultRows,rowCount:resultRows.length};}});return {kernel,calls};};
...
test('AI accrual repository reads only current Final-1 retained evidence under the explain capability',async()=>{
  ...
  assert.equal(history.length,1);assert.deepEqual(current,['00000000-0000-4000-8000-000000000001']);assert.deepEqual(posted,['00000000-0000-4000-8000-000000000001']);assert.equal(period.period_id,periodId);assert.equal(period.period_end,'2026-08-31');assert.equal(period.company_code,undefined);
  const sql=calls.map(call=>call.sql).join('\n');
  assert.equal(calls.find(call=>/ORDER BY p\.starts_on DESC/.test(call.sql)).params[3],13);
  ...
  assert.match(sql,/to_char\(p\.ends_on,'YYYY-MM-DD'\) AS period_end/);assert.match(sql,/FROM accounting_period/);assert.doesNotMatch(sql,/INSERT|UPDATE|DELETE|CREATE\s+JOURNAL/i);
});
