...
test('factory injects only scoped read capabilities and returns a non-dispatchable projection',async()=>{
  ...
  assert.deepEqual(calls.map(([name])=>name).sort(),['control','inbound','mapping','state']);assert.ok(calls.every(([,input])=>input.read_only===true&&input.tenantId==='t1'&&input.entityId==='e1'&&input.companyKey==='COMPANY-A'));
});
...
test('missing capability, repository failure, and cross-scope rows return zero candidates',async()=>{
  ...
  const failure={readPersistedWbsInboundRows:async()=>{throw Error('masked');},readPersistedWbsControlRows:async()=>({companyRows:[],detailRows:[],persistedRows:[]}),readApprovedWbsAutoRecMappings:async()=>[],readWbsAutoRecObservedStateEvidence:async()=>[]};
  assert.equal((await createPostgresWbsInboundAutoRecReader({kernel:failure}).read(scope)).code,'WBS_AUTOREC_READ_FAILED');
  const leaked={readPersistedWbsInboundRows:async()=>[{...bank,entity_id:'e2'}],readPersistedWbsControlRows:async()=>({companyRows:[control],detailRows:[],persistedRows:[control]}),readApprovedWbsAutoRecMappings:async()=>[mapping],readWbsAutoRecObservedStateEvidence:async()=>[]};
