...
const mapping={mapping_id:'map-bank',version:'1',status:'APPROVED',source_type:'BANK_TRANSACTION',entity_id:'e1',company_key:'COMPANY-A',currency:'USD',bank_account_ref:'BANK-1'};

test('factory injects only the three scoped read capabilities and returns a non-dispatchable projection',async()=>{
  const calls=[];
  const kernel={readPersistedWbsInboundRows:async input=>(calls.push(['inbound',input]),[bank]),readPersistedWbsControlRows:async input=>(calls.push(['control',input]),{companyRows:[control],detailRows:[],persistedRows:[control]}),readApprovedWbsAutoRecMappings:async input=>(calls.push(['mapping',input]),[mapping])};
  const result=await createPostgresWbsInboundAutoRecReader({kernel}).read(scope);
  assert.equal(result.status,'READ_ONLY_PROJECTED');assert.equal(result.candidates.length,1);assert.equal(result.can_dispatch,false);assert.equal(result.can_create_draft,false);assert.equal(result.can_post,false);
  assert.deepEqual(calls.map(([name])=>name).sort(),['control','inbound','mapping']);assert.ok(calls.every(([,input])=>input.read_only===true&&input.tenantId==='t1'&&input.entityId==='e1'&&input.companyKey==='COMPANY-A'));
});
...
test('missing capability, repository failure, and cross-scope rows return zero candidates',async()=>{
  ...
  const failure={readPersistedWbsInboundRows:async()=>{throw Error('masked');},readPersistedWbsControlRows:async()=>({companyRows:[],detailRows:[],persistedRows:[]}),readApprovedWbsAutoRecMappings:async()=>[]};
  assert.equal((await createPostgresWbsInboundAutoRecReader({kernel:failure}).read(scope)).code,'WBS_AUTOREC_READ_FAILED');
  const leaked={readPersistedWbsInboundRows:async()=>[{...bank,entity_id:'e2'}],readPersistedWbsControlRows:async()=>({companyRows:[control],detailRows:[],persistedRows:[control]}),readApprovedWbsAutoRecMappings:async()=>[mapping]};
