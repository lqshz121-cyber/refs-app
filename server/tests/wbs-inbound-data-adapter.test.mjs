...
test('Draft and AutoRec requests are review-only seams with immutable source trace',async()=>{
  ...
  const matchingPolicy={policy_id:'policy-1',version:'1',mapping_id:'matching-map-1',mapping_version:'1',policy_snapshot_hash:'sha256:'+'3'.repeat(64),effective_from:'2026-01-01T00:00:00.000Z',effective_to:null,rule_id:'rule-1',rule_version:'1',bank_mapping_id:'bank-map-1',bank_mapping_version:'1',bank_mapping_snapshot_hash:'sha256:'+'4'.repeat(64),business_mapping_id:'pay-map-1',business_mapping_version:'1',business_mapping_snapshot_hash:'sha256:'+'5'.repeat(64),status:'APPROVED',company_key:'COMPANY-A',currency:'USD',bank_account_ref:'BANK-OP',amount_tolerance:'0.0000',date_window_days:3,date_match_basis:'BUSINESS_AND_ACCOUNTING',receipt_id:'policy-receipt-1',receipt_ref:'object://receipt/policy-1',receipt_hash:'sha256:'+'f'.repeat(64)};
  const policyAutoRec=buildAutoReconciliationReviewRequest({bankStaging:{...reviewedBank,mapping_id:'bank-map-1',mapping_version:'1',snapshot_hash:matchingPolicy.bank_mapping_snapshot_hash,effective_from:'2026-01-01T00:00:00.000Z',effective_to:null},businessStaging:{...reviewedPayable,mapping_id:'pay-map-1',mapping_version:'1',snapshot_hash:matchingPolicy.business_mapping_snapshot_hash,effective_from:'2026-01-01T00:00:00.000Z',effective_to:null},matchingPolicy,dateWindowDays:0,dateMatchBasis:'ACCOUNTING_ONLY',tolerance:999});
  ...
  const bankRelation={bank_record_relation:'trace-only'},payableRelation={payable_source_detail:{relation:'trace-only'}};
  const relationAutoRec=buildAutoReconciliationReviewRequest({bankStaging:{...reviewedBank,external_trace:bankRelation,external_trace_hash:canonicalRequestHash(bankRelation)},businessStaging:{...reviewedPayable,external_trace:payableRelation,external_trace_hash:canonicalRequestHash(payableRelation)}});
  assert.equal(relationAutoRec.trace.bank_external_relation_trace_hash,canonicalRequestHash(bankRelation));assert.equal(relationAutoRec.trace.business_external_relation_trace_hash,canonicalRequestHash(payableRelation));
  ...
  assert.deepEqual({receipt:trace.trace.receipt_hash,businessDate:trace.trace.business_date,direction:trace.trace.direction},{receipt:reviewedPayable.receipt_hash,businessDate:'2026-08-01',direction:'DEBIT'});
  ...
  assert.throws(()=>validatePostedJournalTrace({draftRequest:draft,postedEvidence:{...postedEvidence,source_trace:{...draft.trace,receipt_hash:'sha256:'+'9'.repeat(64)}}}),error=>error.code==='WBS_POSTED_SOURCE_TRACE_MISMATCH');
  assert.throws(()=>validatePostedJournalTrace({draftRequest:draft,postedEvidence:{...postedEvidence,source_trace:{...draft.trace,mapping_snapshot_hash:'sha256:'+'9'.repeat(64)}}}),error=>error.code==='WBS_POSTED_SOURCE_TRACE_MISMATCH');
  const payableDraftRelation={payable_source_detail:{relation:'trace-only'}};
  const relatedDraft=buildStandardDraftRequest({stagingItem:{...reviewedPayable,external_trace:payableDraftRelation,external_trace_hash:canonicalRequestHash(payableDraftRelation)},mapping:draftMapping,journal:{period_id:'period-1',journal_number:'AUTO-RELATION-1',company_key:'COMPANY-A',currency:'USD',accounting_date:'2026-08-01',lines:[{debit_amount:100,credit_amount:0},{debit_amount:0,credit_amount:100}]}});
  assert.equal(relatedDraft.trace.external_relation_trace_hash,canonicalRequestHash(payableDraftRelation));
  assert.equal(validatePostedJournalTrace({draftRequest:relatedDraft,postedEvidence:{...postedEvidence,source_trace:{...relatedDraft.trace}}}).ok,true);
  assert.throws(()=>validatePostedJournalTrace({draftRequest:relatedDraft,postedEvidence:{...postedEvidence,source_trace:{...relatedDraft.trace,external_relation_trace_hash:'sha256:'+'9'.repeat(64)}}}),error=>error.code==='WBS_POSTED_SOURCE_TRACE_MISMATCH');
  assert.throws(()=>buildStandardDraftRequest({stagingItem:{...reviewedPayable,external_trace:payableDraftRelation,external_trace_hash:'sha256:forged'},mapping:draftMapping,journal:{period_id:'period-1',journal_number:'AUTO-RELATION-2',company_key:'COMPANY-A',currency:'USD',accounting_date:'2026-08-01',lines:[{debit_amount:100,credit_amount:0},{debit_amount:0,credit_amount:100}]}}),error=>error.code==='WBS_STAGING_TRACE_REQUIRED');
  assert.throws(()=>buildStandardDraftRequest({stagingItem:reviewedPayable,mapping:{...draftMapping,source_type:'BANK_TRANSACTION'},journal:{period_id:'period-1',journal_number:'AUTO-1',company_key:'COMPANY-A',currency:'USD',accounting_date:'2026-08-01',lines:[{debit_amount:100,credit_amount:0},{debit_amount:0,credit_amount:100}]}}),error=>error.code==='WBS_MAPPING_APPROVED_REQUIRED');
  const bankJournal={period_id:'period-1',journal_number:'AUTO-BANK-1',company_key:'COMPANY-A',currency:'USD',accounting_date:'2026-08-02',lines:[{debit_amount:100,credit_amount:0},{debit_amount:0,credit_amount:100}]};
  const bankDraft=buildStandardDraftRequest({stagingItem:reviewedBank,mapping:{mapping_id:'bank-map-1',version:'4',snapshot_hash:'sha256:'+'2'.repeat(64),status:'APPROVED',source_type:'BANK_TRANSACTION',company_key:'COMPANY-A',currency:'USD',bank_account_ref:'BANK-OP',effective_from:'2026-01-01T00:00:00.000Z',effective_to:null},journal:bankJournal});
  assert.equal(bankDraft.mapping.bank_account_ref,'BANK-OP');
...
test('G11 accepts only both posted AutoRec legs with exact source trace and per-member 291001 net zero',()=>{
  ...
  assert.throws(()=>validateWbsAutoRecG11PostedTrace({reviewRequest:{...review,trace:{...review.trace,bank_external_relation_trace_hash:'sha256:forged',business_external_relation_trace_hash:'sha256:'+'2'.repeat(64)}},postedJournals:[payable,autoc]}),error=>error.code==='WBS_AUTOREC_G11_RELATION_TRACE_REQUIRED');
  const controlTrace={...review.trace,company_control_snapshot_hash:'sha256:'+'4'.repeat(64)},controlReview={...review,trace:controlTrace},controlPayable={...payable,source_trace:controlTrace},controlAutoc={...autoc,source_trace:controlTrace};
  assert.equal(validateWbsAutoRecG11PostedTrace({reviewRequest:controlReview,postedJournals:[controlPayable,controlAutoc]}).status,'POSTED_TRACE_VERIFIED');
  assert.throws(()=>validateWbsAutoRecG11PostedTrace({reviewRequest:controlReview,postedJournals:[controlPayable,{...controlAutoc,source_trace:{...controlTrace,company_control_snapshot_hash:'sha256:'+'5'.repeat(64)}}]}),error=>error.code==='WBS_AUTOREC_G11_SOURCE_TRACE_MISMATCH');
  assert.throws(()=>validateWbsAutoRecG11PostedTrace({reviewRequest:{...controlReview,trace:{...controlTrace,company_control_snapshot_hash:'sha256:forged'}},postedJournals:[controlPayable,controlAutoc]}),error=>error.code==='WBS_AUTOREC_G11_CONTROL_TRACE_REQUIRED');
  ...
  const policyBoundTrace={...review.trace,review_plan_id:'sha256:'+'c'.repeat(64),allocation_edge_id:'sha256:'+'d'.repeat(64),proposal_allocation_edge_id:'sha256:'+'e'.repeat(64),matching_policy:{policy_id:'policy-1',version:'1',status:'APPROVED',mapping_id:'mapping-1',mapping_version:'1',policy_snapshot_hash:'sha256:'+'1'.repeat(64),effective_from:'2026-01-01T00:00:00.000Z',effective_to:null,rule_id:'rule-1',rule_version:'1',bank_mapping_id:'bank-map-1',bank_mapping_version:'1',bank_mapping_snapshot_hash:'sha256:'+'2'.repeat(64),bank_mapping_effective_from:'2026-01-01T00:00:00.000Z',bank_mapping_effective_to:null,business_mapping_id:'pay-map-1',business_mapping_version:'1',business_mapping_snapshot_hash:'sha256:'+'3'.repeat(64),business_mapping_effective_from:'2026-01-01T00:00:00.000Z',business_mapping_effective_to:null,date_match_basis:'BUSINESS_AND_ACCOUNTING',receipt_id:'policy-receipt-1',receipt_ref:'object://receipt/policy-1',receipt_hash:'sha256:'+'f'.repeat(64)}};
...
test('persistence plan binds receipt/raw/normalized/staging trace and idempotency to the actual snapshot command seam',async()=>{
  ...
  const plan=buildWbsInboundPersistencePlan({snapshot:value,prepared,tenantId:'55555555-5555-4555-8555-555555555555',entityId:'66666666-6666-4666-8666-666666666666',idempotencyKey:'wbs-inbound-20260805-company-a-0001'});
  assert.equal(plan.receipt_persistence.kernel_method,'recordWbsSnapshot');assert.equal(plan.receipt_persistence.supported,true);assert.equal(plan.raw_normalized_staging_persistence.supported,false);assert.equal(plan.raw_normalized_staging_persistence.code,'WBS_INBOUND_MULTI_RECEIPT_ATOMICITY_REQUIRED');assert.equal(plan.raw_normalized_staging_persistence.kernel_method,'persistWbsInboundSnapshotRows');assert.equal(plan.raw_normalized_staging_persistence.receipt_groups.length,3);assert.equal(plan.ingress.import_batch_id_from_snapshot_record,true);assert.equal(plan.ingress.trace_rows.length,3);assert.equal(plan.can_dispatch,false);
  const oneSnapshot=singleReceiptSnapshot(),onePrepared=await createWbsInboundDataAdapter({snapshotReader:reader(oneSnapshot)}).pull(),onePlan=buildWbsInboundPersistencePlan({snapshot:oneSnapshot,prepared:onePrepared,tenantId:'55555555-5555-4555-8555-555555555555',entityId:'66666666-6666-4666-8666-666666666666',idempotencyKey:'wbs-inbound-20260805-company-a-0002'});assert.equal(onePlan.raw_normalized_staging_persistence.supported,true);assert.equal(onePlan.raw_normalized_staging_persistence.receipt_groups.length,1);
  assert.throws(()=>buildWbsInboundPersistencePlan({snapshot:value,prepared,tenantId:'bad',entityId:'66666666-6666-4666-8666-666666666666',idempotencyKey:'short'}),error=>error.code==='WBS_INBOUND_SCOPE_INVALID');
  assert.throws(()=>buildWbsInboundPersistencePlan({snapshot:value,prepared:{...prepared,package_hash:'sha256:forged'},tenantId:'55555555-5555-4555-8555-555555555555',entityId:'66666666-6666-4666-8666-666666666666',idempotencyKey:'wbs-inbound-20260805-company-a-0001'}),error=>error.code==='WBS_INBOUND_PREPARED_TRACE_INVALID');
});

test('orchestrator persists receipt before typed rows, blocks Draft/AutoRec dispatch, and replays one stable result',async()=>{
  const value=singleReceiptSnapshot(),adapter=createWbsInboundDataAdapter({snapshotReader:reader(value)}),calls=[];
  let receiptSequence=0;const kernel={recordWbsSnapshot:async request=>(calls.push(['snapshot',request]),{ok:true,import_batch_id:'77777777-7777-4777-8777-777777777777'}),persistWbsInboundRows:async request=>(calls.push(['rows',request]),{ok:true,receipt_id:`99999999-9999-4999-8999-${String(++receiptSequence).padStart(12,'0')}`,row_count:request.rows.length})};
  const service=createWbsInboundOrchestrator({adapter,kernel});
  const input={snapshot:value,tenantId:'55555555-5555-4555-8555-555555555555',entityId:'66666666-6666-4666-8666-666666666666',idempotencyKey:'wbs-inbound-20260805-company-a-0001'};
  const first=await service.persist(input),replay=await service.persist(input);
  assert.strictEqual(first,replay);assert.deepEqual(calls.map(([kind])=>kind),['snapshot','rows']);assert(calls.slice(1).every(([,request])=>request.importBatchId==='77777777-7777-4777-8777-777777777777'&&request.rows.length===1&&request.receipt.payload_hash&&request.requestHash));assert.equal(first.row_persistence.length,1);assert(first.trace.trace_rows.every(item=>/^99999999-9999-4999-8999-\d{12}$/.test(item.receipt_id)));assert.equal(first.trace.import_batch_id,'77777777-7777-4777-8777-777777777777');assert.deepEqual({draft:first.can_dispatch_draft,autorec:first.can_dispatch_autorec,post:first.can_post},{draft:false,autorec:false,post:false});
  await assert.rejects(()=>service.persist({...input,tenantId:'88888888-8888-4888-8888-888888888888'}),error=>error.code==='WBS_INBOUND_IDEMPOTENCY_CONFLICT');assert.equal(calls.length,2);
  const multiService=createWbsInboundOrchestrator({adapter:createWbsInboundDataAdapter({snapshotReader:reader(snapshot())}),kernel});await assert.rejects(()=>multiService.persist({...input,snapshot:snapshot(),idempotencyKey:'wbs-inbound-20260805-company-a-0009'}),error=>error.code==='WBS_INBOUND_MULTI_RECEIPT_ATOMICITY_REQUIRED');assert.equal(calls.filter(([kind])=>kind==='rows').length,1);
  const atomicCalls=[],atomicKernel={recordWbsSnapshot:async request=>(atomicCalls.push(['snapshot',request]),{ok:true,import_batch_id:'77777777-7777-4777-8777-777777777777'}),persistWbsInboundSnapshotRows:async request=>(atomicCalls.push(['atomic',request]),{ok:true,import_batch_id:request.importBatchId,row_count:request.groups.reduce((sum,group)=>sum+group.rows.length,0),groups:request.groups.map((group,index)=>({receipt_id:`aaaaaaaa-aaaa-4aaa-8aaa-${String(index+1).padStart(12,'0')}`,receipt_hash:group.receipt.payload_hash,payload_ref:group.receipt.payload_ref,row_count:group.rows.length}))})};
  const atomicService=createWbsInboundOrchestrator({adapter:createWbsInboundDataAdapter({snapshotReader:reader(snapshot())}),kernel:atomicKernel}),atomic=await atomicService.persist({...input,snapshot:snapshot(),idempotencyKey:'wbs-inbound-20260805-company-a-0010'});
  assert.deepEqual(atomicCalls.map(([kind])=>kind),['snapshot','atomic']);assert.equal(atomicCalls[1][1].groups.length,3);assert.equal(atomic.row_persistence.length,3);assert(atomic.trace.trace_rows.every(item=>/^aaaaaaaa-aaaa-4aaa-8aaa-\d{12}$/.test(item.receipt_id)));
...
test('orchestrator fails closed without capability, after receipt failure, and after row persistence failure',async()=>{
  const value=singleReceiptSnapshot(),adapter=createWbsInboundDataAdapter({snapshotReader:reader(value)}),input={snapshot:value,tenantId:'55555555-5555-4555-8555-555555555555',entityId:'66666666-6666-4666-8666-666666666666',idempotencyKey:'wbs-inbound-20260805-company-a-0001'};
  const noCapability=createWbsInboundOrchestrator({adapter,kernel:{recordWbsSnapshot:async()=>({ok:true,import_batch_id:'77777777-7777-4777-8777-777777777777'})}});await assert.rejects(()=>noCapability.persist(input),error=>error.code==='WBS_INBOUND_KERNEL_PERSISTENCE_UNAVAILABLE');
  ...
  const missingImportBatch=createWbsInboundOrchestrator({adapter,kernel:{recordWbsSnapshot:async()=>({ok:true}),persistWbsInboundRows:async()=>{rows++;return {ok:true};}}});await assert.rejects(()=>missingImportBatch.persist({...input,idempotencyKey:'wbs-inbound-20260805-company-a-0003'}),error=>error.code==='WBS_INBOUND_RECEIPT_PERSISTENCE_FAILED');assert.equal(rows,0);
  const receiptMismatch=createWbsInboundOrchestrator({adapter,kernel:{recordWbsSnapshot:async()=>({ok:true,import_batch_id:'77777777-7777-4777-8777-777777777777'}),persistWbsInboundRows:async()=>({ok:true,receipt_id:'99999999-9999-4999-8999-000000000001',row_count:99})}});await assert.rejects(()=>receiptMismatch.persist({...input,idempotencyKey:'wbs-inbound-20260805-company-a-0004'}),error=>error.code==='WBS_INBOUND_ROW_PERSISTENCE_FAILED');
  let receiptCalls=0,rowCalls=0;const rowFailure=createWbsInboundOrchestrator({adapter,kernel:{recordWbsSnapshot:async()=>{receiptCalls++;return {ok:true,import_batch_id:'77777777-7777-4777-8777-777777777777'};},persistWbsInboundRows:async()=>{rowCalls++;throw new Error('masked');}}});await assert.rejects(()=>rowFailure.persist(input),error=>error.code==='WBS_INBOUND_ROW_PERSISTENCE_FAILED');await assert.rejects(()=>rowFailure.persist(input),error=>error.code==='WBS_INBOUND_ROW_PERSISTENCE_FAILED');assert.deepEqual({receiptCalls,rowCalls},{receiptCalls:1,rowCalls:1});
});
...
test('adapter rejects a mutable reader, cross-currency AutoRec pair, and unbalanced Draft request',async()=>{
  ...
  const staged={receipt_id:'receipt',receipt_ref:'object://wbs/receipt',receipt_hash:'sha256:'+'a'.repeat(64),stage:'STAGING_REVIEWED',source_record_id:'one',source_version:'v',currency:'USD',amount:100,company_key:'C',raw_event_id:'raw',source_document_id:'doc',staging_item_id:'stg',business_date:'2026-08-01',accounting_date:'2026-08-01',direction:'DEBIT',bank_account_ref:'BANK',account_before:'291000',account_after:'291001',review_event_id:'review'};
  ...
  assert.throws(()=>buildStandardDraftRequest({stagingItem:scopedStaging,mapping:scopedMapping,journal:{...scopedJournal,journal_number:'j-both-sides',lines:[{debit_amount:50,credit_amount:50},{debit_amount:50,credit_amount:50}]}}),error=>error.code==='WBS_DRAFT_REQUEST_UNBALANCED');
  assert.throws(()=>buildStandardDraftRequest({stagingItem:scopedStaging,mapping:scopedMapping,journal:{...scopedJournal,journal_number:'j-zero-line',lines:[{debit_amount:100,credit_amount:0},{debit_amount:0,credit_amount:100},{debit_amount:0,credit_amount:0}]}}),error=>error.code==='WBS_DRAFT_REQUEST_UNBALANCED');
