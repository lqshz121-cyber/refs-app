...
test('Draft and AutoRec requests are review-only seams with immutable source trace',async()=>{
  ...
  const policyAutoRec=buildAutoReconciliationReviewRequest({bankStaging:{...reviewedBank,mapping_id:'bank-map-1',mapping_version:'1'},businessStaging:{...reviewedPayable,mapping_id:'pay-map-1',mapping_version:'1'},matchingPolicy});
  assert.match(policyAutoRec.allocation_edge_id,/^sha256:[0-9a-f]{64}$/);assert.match(policyAutoRec.proposal_allocation_edge_id,/^sha256:[0-9a-f]{64}$/);assert.equal(policyAutoRec.trace.matching_policy.rule_id,'rule-1');
...
test('G11 accepts only both posted AutoRec legs with exact source trace and per-member 291001 net zero',()=>{
  ...
  const policyBoundReview={...review,trace:policyBoundTrace};
