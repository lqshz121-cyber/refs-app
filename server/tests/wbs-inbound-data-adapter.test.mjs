...
test('Draft and AutoRec requests are review-only seams with immutable source trace',async()=>{
  ...
  const draftMapping={mapping_id:'map-1',version:'4',status:'APPROVED',source_type:'PAYABLE',company_key:'COMPANY-A',currency:'USD'};
  ...
  const bankDraft=buildStandardDraftRequest({stagingItem:reviewedBank,mapping:{mapping_id:'bank-map-1',version:'4',status:'APPROVED',source_type:'BANK_TRANSACTION',company_key:'COMPANY-A',currency:'USD',bank_account_ref:'BANK-OP'},journal:bankJournal});
...
test('adapter rejects a mutable reader, cross-currency AutoRec pair, and unbalanced Draft request',async()=>{
  ...
  const scopedMapping={mapping_id:'m',version:'1',status:'APPROVED',source_type:'PAYABLE',company_key:'C',currency:'USD'};
