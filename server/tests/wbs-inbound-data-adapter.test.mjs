...
test('Draft and AutoRec requests are review-only seams with immutable source trace',async()=>{
  ...
  assert.match(policyAutoRec.allocation_edge_id,/^sha256:[0-9a-f]{64}$/);assert.match(policyAutoRec.proposal_allocation_edge_id,/^sha256:[0-9a-f]{64}$/);assert.equal(policyAutoRec.trace.matching_policy.rule_id,'rule-1');assert.deepEqual({window:policyAutoRec.date_window_days,basis:policyAutoRec.date_match_basis,difference:policyAutoRec.amount_difference},{window:3,basis:'BUSINESS_AND_ACCOUNTING',difference:0});
...
test('G11 accepts only both posted AutoRec legs with exact source trace and per-member 291001 net zero',()=>{
  ...
  const policyBoundTrace={...review.trace,allocation_edge_id:'sha256:'+'c'.repeat(64),proposal_allocation_edge_id:'sha256:'+'d'.repeat(64),matching_policy:{policy_id:'policy-1',version:'1',mapping_id:'mapping-1',mapping_version:'1',rule_id:'rule-1',rule_version:'1',bank_mapping_id:'bank-map-1',bank_mapping_version:'1',business_mapping_id:'pay-map-1',business_mapping_version:'1',date_match_basis:'BUSINESS_AND_ACCOUNTING',receipt_id:'policy-receipt-1',receipt_ref:'object://receipt/policy-1',receipt_hash:'sha256:'+'e'.repeat(64)}};
  const policyBoundReview={...review,allocation_edge_id:policyBoundTrace.allocation_edge_id,proposal_allocation_edge_id:policyBoundTrace.proposal_allocation_edge_id,matching_policy:policyBoundTrace.matching_policy,trace:policyBoundTrace};
