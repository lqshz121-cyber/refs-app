...
test('orchestrator persists receipt before typed rows, blocks Draft/AutoRec dispatch, and replays one stable result',async()=>{
  ...
  const kernel={recordWbsSnapshot:async request=>(calls.push(['receipt',request]),{ok:true,receipt_id:'receipt-1'}),persistWbsInboundRows:async request=>(calls.push(['rows',request]),{ok:true,receipt_id:request.receiptId,raw_event_ids:['raw-1'],staging_item_ids:['stg-1']})};
  ...
  assert.strictEqual(first,replay);assert.deepEqual(calls.map(([kind])=>kind),['receipt','rows']);assert.equal(calls[1][1].receiptId,'receipt-1');assert(calls[1][1].receiptTrace.every(item=>item.receipt_id==='receipt-1'));assert.equal(first.trace.receipt_id,'receipt-1');assert.deepEqual({draft:first.can_dispatch_draft,autorec:first.can_dispatch_autorec,post:first.can_post},{draft:false,autorec:false,post:false});
...
test('orchestrator fails closed without capability, after receipt failure, and after row persistence failure',async()=>{
  ...
  const missingReceiptId=createWbsInboundOrchestrator({adapter,kernel:{recordWbsSnapshot:async()=>({ok:true}),persistWbsInboundRows:async()=>{rows++;return {ok:true};}}});await assert.rejects(()=>missingReceiptId.persist({...input,idempotencyKey:'wbs-inbound-20260805-company-a-0003'}),error=>error.code==='WBS_INBOUND_RECEIPT_PERSISTENCE_FAILED');assert.equal(rows,0);
  const receiptMismatch=createWbsInboundOrchestrator({adapter,kernel:{recordWbsSnapshot:async()=>({ok:true,receipt_id:'receipt-good'}),persistWbsInboundRows:async()=>({ok:true,receipt_id:'receipt-other'})}});await assert.rejects(()=>receiptMismatch.persist({...input,idempotencyKey:'wbs-inbound-20260805-company-a-0004'}),error=>error.code==='WBS_INBOUND_ROW_PERSISTENCE_FAILED');
  let receiptCalls=0,rowCalls=0;const rowFailure=createWbsInboundOrchestrator({adapter,kernel:{recordWbsSnapshot:async()=>{receiptCalls++;return {ok:true,receipt_id:'receipt-row-failure'};},persistWbsInboundRows:async()=>{rowCalls++;throw new Error('masked');}}});await assert.rejects(()=>rowFailure.persist(input),error=>error.code==='WBS_INBOUND_ROW_PERSISTENCE_FAILED');await assert.rejects(()=>rowFailure.persist(input),error=>error.code==='WBS_INBOUND_ROW_PERSISTENCE_FAILED');assert.deepEqual({receiptCalls,rowCalls},{receiptCalls:1,rowCalls:1});
});
