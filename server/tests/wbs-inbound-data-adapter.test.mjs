...
test('Draft and AutoRec requests are review-only seams with immutable source trace',async()=>{
  ...
  const policyAutoRec=buildAutoReconciliationReviewRequest({bankStaging:{...reviewedBank,mapping_id:'bank-map-1',mapping_version:'1',snapshot_hash:matchingPolicy.bank_mapping_snapshot_hash},businessStaging:{...reviewedPayable,mapping_id:'pay-map-1',mapping_version:'1',snapshot_hash:matchingPolicy.business_mapping_snapshot_hash},matchingPolicy,dateWindowDays:0,dateMatchBasis:'ACCOUNTING_ONLY',tolerance:999});
  ...
  const draftMapping={mapping_id:'map-1',version:'4',snapshot_hash:'sha256:'+'1'.repeat(64),status:'APPROVED',source_type:'PAYABLE',company_key:'COMPANY-A',currency:'USD'};
  ...
  const bankDraft=buildStandardDraftRequest({stagingItem:reviewedBank,mapping:{mapping_id:'bank-map-1',version:'4',snapshot_hash:'sha256:'+'2'.repeat(64),status:'APPROVED',source_type:'BANK_TRANSACTION',company_key:'COMPANY-A',currency:'USD',bank_account_ref:'BANK-OP'},journal:bankJournal});
...
test('adapter rejects a mutable reader, cross-currency AutoRec pair, and unbalanced Draft request',async()=>{
  ...
  const scopedMapping={mapping_id:'m',version:'1',snapshot_hash:'sha256:'+'3'.repeat(64),status:'APPROVED',source_type:'PAYABLE',company_key:'C',currency:'USD'};
