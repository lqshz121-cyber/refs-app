...
const result={status:'WBS_TEST_RANGE_IMPORT_COMPLETE',date_from:'2026-01-01',date_to:'2026-06-30',page_size:10,payables:{page_count:1,record_count:1,imported_count:1,replayed_count:0,posted_count:1},bank:{provider_page_count:1,record_count:1,reconciliations:[{bank_account_ref:'WBS_TEST_BANK_2026_01',period_code:'2026-01',reconciliation_id:'00000000-0000-4000-8000-000000000004',transaction_count:1}],bank_source_ids:['00000000-0000-4000-8000-000000000005']},test_only:true};
...
test('rejects malformed paging and identity surfaces before calling the range service',async()=>{
  ...
  for(const candidate of [request({headers:{}}),request({body:{...body,pageSize:11}}),request({body:{...body,maxPages:51}}),request({body:{...body,actorId:'forbidden'}}),request({url:`${url}?cursor=forbidden`})])assert.equal((await api(candidate)).status,400);
