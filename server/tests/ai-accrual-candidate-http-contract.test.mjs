...
const nullableCandidate={...candidate,recurring_obligation_id:null,service_frequency:null,prior_source_trace:candidate.prior_source_trace.map(item=>({...item,recurring_obligation_id:null,service_frequency:null,obligation_status:null}))};
...
test('AI accrual candidates are authenticated, period-bound no-store analysis reads with no accounting authority',async()=>{
  ...
  const nullableApi=createAccountingApi({authenticate:async()=>({trusted:true,tenantId,actorId:'controller-a'}),kernelFactory:async()=>({}),aiAccrualCandidateAnalysisServiceFactory:async()=>({analyze:async()=>({...safe,candidates:[nullableCandidate]})})});
  const nullableResponse=await nullableApi({method:'GET',url:path,headers:{},body:null});assert.equal(nullableResponse.status,200);assert.deepEqual(nullableResponse.body.data.candidates,[nullableCandidate]);
