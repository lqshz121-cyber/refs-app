...
test('reconciliation OpenAPI mirrors the scoped optimistic lifecycle and controlled adjustment Draft boundary',()=>{
  ...
  const admittedDetail=contract.paths['/entities/{entityId}/bank/reconciliations/admitted-statements/{statementReceiptId}'].get;
  ...
  assert.equal(admittedDetail.operationId,'getAdmittedWbsBankStatementReceipt');assert.match(admittedDetail.description,/returns 403 before receipt lookup/i);assert.match(admittedDetail.description,/non-disclosing 404/i);assert.equal(admittedDetail.responses['200'].$ref,'#/components/responses/AdmittedWbsBankStatementDetailOk');assert.equal(admittedDetail.responses['403'].$ref,'#/components/responses/Problem');
