...
test('Bank transactions are a bounded paginated authenticated entity and account scoped read',async()=>{
  ...
  assert.equal((await api({method:'GET',url:`/api/v1/entities/${entityId}/bank/transactions?bankAccountRef=BANK-1&offset=-1`,body:null,headers:{}})).status,400);
  assert.equal((await api({method:'GET',url:`/api/v1/entities/${entityId}/bank/transactions?bankAccountRef=BANK-1&offset=10001`,body:null,headers:{}})).status,400);
...
test('Bank transaction detail is an authenticated exact evidence read with no query, body, or idempotency authority',async()=>{
  calls.length=0;const bankSourceId=randomUUID();
  const response=await api({method:'GET',url:`/api/v1/entities/${entityId}/bank/transactions/${bankSourceId}`,body:null,headers:{}});
  assert.equal(response.status,200);assert.equal(response.headers['cache-control'],'no-store');
  assert.deepEqual(calls[0],['getBankTransactionDetail',{tenantId,entityId,bankSourceId}]);
  assert.equal((await api({method:'GET',url:`/api/v1/entities/${entityId}/bank/transactions/${bankSourceId}?unexpected=x`,body:null,headers:{}})).body.code,'UNEXPECTED_QUERY_PARAMETER');
  assert.equal((await api({method:'GET',url:`/api/v1/entities/${entityId}/bank/transactions/${bankSourceId}`,body:{},headers:{}})).body.code,'READ_BODY_FORBIDDEN');
  assert.equal((await api({method:'GET',url:`/api/v1/entities/${entityId}/bank/transactions/${bankSourceId}`,body:null,headers:{'Idempotency-Key':'read-not-allowed'}})).body.code,'IDEMPOTENCY_KEY_NOT_ALLOWED');
  assert.equal((await api({method:'GET',url:`/api/v1/entities/${entityId}/bank/transactions/not-a-uuid`,body:null,headers:{}})).body.code,'INVALID_PATH_PARAMETER');
});
