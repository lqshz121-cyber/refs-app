...
Object.assign(kernel,{
  prepareFinancialStatementSnapshot:async args=>(calls.push(['prepareFinancialStatementSnapshot',args]),{financial_statement_snapshot_proposal_id:statementProposalId,status:'PENDING_APPROVAL',snapshot_hash:statementHash,ledger_evidence_hash:statementEvidenceHash,prepared_by:'preparer',idempotent:false}),
  approveFinancialStatementSnapshot:async args=>(calls.push(['approveFinancialStatementSnapshot',args]),{financial_statement_snapshot_id:statementSnapshotId,financial_statement_snapshot_proposal_id:args.proposalId,version:'1',status:'APPROVED',prepared_by:'preparer',approved_by:'approver',snapshot_hash:statementHash,ledger_evidence_hash:statementEvidenceHash,idempotent:false}),
  readFinancialStatementSnapshotProposalQueue:async args=>(calls.push(['readFinancialStatementSnapshotProposalQueue',args]),{schema_version:'FINANCIAL_STATEMENT_SNAPSHOT_PROPOSAL_QUEUE_V1',scope:{tenant_id:tenantId,entity_id:entityId,period_id:periodId},total_count:1,read_count:1,limit:args.limit,offset:args.offset,population_complete:true,rows:[statementQueueItem]}),
  readFinancialStatementSnapshotProposal:async args=>(calls.push(['readFinancialStatementSnapshotProposal',args]),{...statementQueueItem,schema_version:'FINANCIAL_STATEMENT_SNAPSHOT_PROPOSAL_V1',rows:[{statement_type:'TRIAL_BALANCE',statement_section:'Assets',classification_basis:'ACCOUNT_CODE_PREFIX_AND_BANK_MEMBER',account_code:'1000',account_name:'Cash',opening_debit:'0.0000',opening_credit:'0.0000',period_debit:'10.0000',period_credit:'0.0000',ending_debit:'10.0000',ending_credit:'0.0000',display_balance:'10.0000',journal_entry_ids:[journalEntryId],journal_line_ids:[statementJournalLineId],ledger_line_ids:[statementLedgerLineId],source_document_ids:[],row_hash:statementRowHash}]})
});
...
test('statement snapshot proposal queue and detail are recoverable no-store reads with closed authoritative DTOs',async()=>{
  calls.length=0;
  let response=await api({method:'GET',url:`/api/v1/entities/${entityId}/reports/financial-statement-snapshot-proposals?periodId=${periodId}&limit=20&offset=0`,body:null,headers:{}});
  assert.equal(response.status,200);assert.equal(response.headers['cache-control'],'no-store');assert.equal(response.body.data.rows[0].can_approve,true);
  assert.deepEqual(calls.at(-1),['readFinancialStatementSnapshotProposalQueue',{tenantId,entityId,periodId,limit:20,offset:0}]);
  response=await api({method:'GET',url:`/api/v1/entities/${entityId}/reports/financial-statement-snapshot-proposals/${statementProposalId}?periodId=${periodId}`,body:null,headers:{}});
  assert.equal(response.status,200);assert.equal(response.body.data.rows[0].display_balance,'10.0000');
  assert.deepEqual(calls.at(-1),['readFinancialStatementSnapshotProposal',{tenantId,entityId,periodId,proposalId:statementProposalId}]);
  for(const request of [
    {method:'GET',url:`/api/v1/entities/${entityId}/reports/financial-statement-snapshot-proposals?periodId=${periodId}`,body:{},headers:{}},
    {method:'GET',url:`/api/v1/entities/${entityId}/reports/financial-statement-snapshot-proposals?periodId=${periodId}`,body:null,headers:{'Idempotency-Key':'forbidden'}},
    {method:'GET',url:`/api/v1/entities/${entityId}/reports/financial-statement-snapshot-proposals?periodId=${periodId}&limit=101`,body:null,headers:{}},
    {method:'GET',url:`/api/v1/entities/${entityId}/reports/financial-statement-snapshot-proposals/${statementProposalId}?periodId=${periodId}`,body:null,headers:{'If-Match':'"0"'}}
  ])assert.equal((await api(request)).status,400);
  const malformed=createAccountingApi({authenticate:async()=>({trusted:true,tenantId,actorId:'approver'}),kernelFactory:async()=>({...kernel,readFinancialStatementSnapshotProposalQueue:async()=>({...await kernel.readFinancialStatementSnapshotProposalQueue({limit:20,offset:0}),unexpected:true})})});
  response=await malformed({method:'GET',url:`/api/v1/entities/${entityId}/reports/financial-statement-snapshot-proposals?periodId=${periodId}&limit=20&offset=0`,body:null,headers:{}});
  assert.equal(response.status,502);assert.equal(response.body.code,'STATEMENT_SNAPSHOT_PROPOSAL_QUEUE_INVALID');
});
