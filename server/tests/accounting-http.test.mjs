...
test('Lot profitability is an exact, GET-only, no-store evidence read',async()=>{
  calls.length=0;const path=`/api/v1/entities/${entityId}/reports/lot-profitability?periodId=${periodId}&lotRef=LOT-18`;
  ...
  assert.deepEqual(calls,[['getLotProfitability',{tenantId,entityId,periodId,lotRef:'LOT-18'}]]);
  assert.equal((await api({method:'GET',url:path,body:{},headers:{}})).body.code,'READ_BODY_FORBIDDEN');
  assert.equal((await api({method:'GET',url:`${path}&extra=true`,body:null,headers:{}})).body.code,'UNEXPECTED_QUERY_PARAMETER');
  assert.equal((await api({method:'GET',url:`/api/v1/entities/${entityId}/reports/lot-profitability?periodId=${periodId}&lotRef=LOT-18%20`,body:null,headers:{}})).body.code,'INVALID_QUERY_PARAMETER');
  assert.equal((await api({method:'GET',url:path,body:null,headers:{'Idempotency-Key':'not-for-read'}})).body.code,'IDEMPOTENCY_KEY_NOT_ALLOWED');
});
...
test('Reconciled snapshot and posted lineage are bodyless retained-evidence reads',async()=>{
  calls.length=0;const reconciliationId=randomUUID(),bankSourceId=randomUUID(),linkedJournalId=randomUUID();const readApi=createAccountingApi({authenticate:async()=>({trusted:true,tenantId,actorId:'reader'}),kernelFactory:async()=>({...kernel,getReconciledSnapshot:async args=>{calls.push(['snapshot',args]);return [{reconciliation_snapshot_id:randomUUID()}]},getReconciliationPostedLineage:async args=>{calls.push(['lineage',args]);return [{reconciliation_id:reconciliationId,bank_source_id:bankSourceId,journal_entry_id:linkedJournalId,link_type:'RECONCILIATION_ADJUSTMENT_DRAFT'}]}})});
  const snapshot=await readApi({method:'GET',url:`/api/v1/entities/${entityId}/bank/reconciliations/${reconciliationId}/snapshot`,body:null,headers:{}});assert.equal(snapshot.status,200);assert.equal(snapshot.headers['cache-control'],'no-store');
  const lineage=await readApi({method:'GET',url:`/api/v1/entities/${entityId}/bank/reconciliations/${reconciliationId}/posted-lineage?bankSourceId=${bankSourceId}&journalEntryId=${linkedJournalId}`,body:null,headers:{}});assert.equal(lineage.status,200);assert.equal(lineage.headers['cache-control'],'no-store');assert.equal(calls.length,2);
  const rejected=await readApi({method:'GET',url:`/api/v1/entities/${entityId}/bank/reconciliations/${reconciliationId}/snapshot`,body:{},headers:{}});assert.equal(rejected.body.code,'READ_BODY_FORBIDDEN');
});
