...
test('invoice accrual proposal requires exact retained ACCRUAL_REVIEW evidence',async()=>{
  const up=await read('../db/migrations/188_ai_invoice_accrual_proposal.sql');
...
test('read is scoped and rollback preserves retained proposals',async()=>{
  const up=await read('../db/migrations/188_ai_invoice_accrual_proposal.sql'),down=await read('../db/migrations/down/188_ai_invoice_accrual_proposal.sql');
