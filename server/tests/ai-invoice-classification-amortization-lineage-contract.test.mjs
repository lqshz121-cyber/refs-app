...
test('amortization proposal requires one exact retained prepaid classification',async()=>{
  const up=await read('../db/migrations/187_ai_invoice_classification_amortization_lineage.sql');
  for(const token of [
...
test('rollback cannot erase retained amortization classification lineage',async()=>{
  const down=await read('../db/migrations/down/187_ai_invoice_classification_amortization_lineage.sql');
