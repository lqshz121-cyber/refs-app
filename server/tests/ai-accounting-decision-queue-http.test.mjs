...
const queue={schema_version:'AI_ACCOUNTING_DECISION_QUEUE_SNAPSHOT_V1',scope:{tenant_id:tenantId,entity_id:entityId,accounting_period_id:periodId},total_count:1,read_count:1,limit:50,offset:0,population_complete:true,population_hash:hash,snapshot_token:hash,rows:[row]};
...
test('retained decision queue is a no-store scoped read with no command authority',async()=>{
  let seen;const api=createAccountingApi({authenticate:async()=>principal,kernelFactory:async()=>({readAiAccountingDecisionQueueSnapshot:async input=>(seen=input,queue)})});
  ...
  assert.equal(response.status,200);assert.equal(response.headers['cache-control'],'no-store');assert.deepEqual(seen,{tenantId,entityId,accountingPeriodId:periodId,limit:50,offset:0,snapshotToken:null});assert.equal(response.body.data.rows[0].action_flags.can_post,false);assert.equal(response.body.data.snapshot_token,hash);
});
...
test('retained decision queue rejects partial, duplicate, open, and post-capable evidence',async()=>{
  for(const unsafe of [
    ...
    const api=createAccountingApi({authenticate:async()=>principal,kernelFactory:async()=>({readAiAccountingDecisionQueueSnapshot:async()=>unsafe})});
...
test('later decision queue pages require the exact first-page snapshot token',async()=>{
  const seen=[],api=createAccountingApi({authenticate:async()=>principal,kernelFactory:async()=>({readAiAccountingDecisionQueueSnapshot:async input=>(seen.push(input),{...queue,offset:1,read_count:0,rows:[],population_complete:true})})});
  const missing=await api({method:'GET',url:`/api/v1/entities/${entityId}/ai/accounting-decision-queue?periodId=${periodId}&offset=1`,headers:{},body:null});assert.equal(missing.status,400);assert.equal(missing.body.code,'AI_ACCOUNTING_DECISION_QUEUE_SNAPSHOT_REQUIRED');assert.equal(seen.length,0);
  const accepted=await api({method:'GET',url:`/api/v1/entities/${entityId}/ai/accounting-decision-queue?periodId=${periodId}&offset=1&snapshotToken=${encodeURIComponent(hash)}`,headers:{},body:null});assert.equal(accepted.status,200);assert.equal(seen[0].snapshotToken,hash);
});
...
test('OpenAPI exposes only the closed recoverable decision queue GET',async()=>{
  ...
  assert.equal(spec.components.schemas.AiAccountingDecisionQueue.properties.snapshot_token.$ref,'#/components/schemas/Sha256');
