...
test('offline-verifies, admits with the M2M token, and reads the closed review queue with a separate token',async()=>{
  ...
  const fetchImpl=async(url,options)=>{calls.push({url,options});if(options.method==='POST')return jsonResponse(201,{status:'PERSISTED_PAYABLE_STAGING_REVIEW_REQUIRED',signature_verified:true,wbs_provider_signed_payable_admission_id:admissionId,snapshot_id:input.package.snapshot_id,company_code:input.companyCode,row_count:1,idempotent:false,can_create_draft:false,can_approve:false,can_post:false,...rawHashes});return jsonResponse(200,[{wbs_inbound_row_id:randomUUID(),source_record_id:input.recordId,source_version:'snapshot:provider-v1',receipt_hash:`sha256:${'a'.repeat(64)}`,evidence_hash:`sha256:${'b'.repeat(64)}`,review_readiness:'VERIFIED_ATTACHMENT_REQUIRED',can_review:false}]);};
  ...
  assert.equal(result.status,'ADMITTED_SIGNED_PAYABLE_EVIDENCE');assert.equal(result.admission_http_status,201);assert.equal(result.readback.record_count,1);assert.equal(result.can_post,false);
