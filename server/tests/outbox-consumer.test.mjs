...
test('closed event binds header, server scope, canonical timestamp and rejects secret material',()=>{
  ...
  for(const delta of [{extra:true},{tenant_id:'cccccccc-cccc-4ccc-8ccc-cccccccccccc'},{attempt_count:0},{created_at:'2026-02-30T00:00:00.000Z'},{payload:{authorization:'private'}},{payload:{memo:'Bearer secret-secret-token'}},{payload:{count:1e30}}]) assert.throws(()=>validateEvent({...e,...delta},config,headers(e)));
...
test('repository returns only durable exact receipt and maps SQL conflicts/hash errors safely',async()=>{
  ...
  assert.deepEqual(await repo.accept(e),receipt(e));assert.match(query[0],/refs_outbox_consumer.accept/);assert.equal(query[1][0],JSON.stringify(e));
  for(const [code,status] of [['P0409',409],['P0400',400],['XX000',503]]) {
    ...
    await assert.rejects(bad.accept(e),err=>err.status===status&&!err.message.includes('password'));
  }
  await assert.rejects(new ConsumerRepository({query:async()=>({rows:[{receipt:{...receipt(e),extra:true}}]})},config).accept(e));
});
...
test('existing dispatcher consumes exact durable HTTP receipt; retry keeps one retained event',async()=>{
  ...
  await withServer({ready:async()=>true,accept:async e=>{if(!records.has(e.outbox_event_id)){records.set(e.outbox_event_id,e);writes++;}return receipt(e);}},async base=>{
    ...
    assert.deepEqual(await publisher.publish(fixture()),receipt(fixture()));
    await publisher.publish({...fixture(),attempt_count:2});assert.equal(writes,1);
