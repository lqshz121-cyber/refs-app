...
test('closing settlement review requires source, property, period and balanced-line evidence without accounting authority',()=>{const item=AI_ACCOUNTING_SKILLS.find(skill=>skill.id==='CLOSING_SETTLEMENT_ACCOUNTING_REVIEW');assert.ok(item);assert.equal(item.status,'IMPLEMENTED_REVIEW_CANDIDATE');for(const field of ['source_document_id','source_document_line_id','source_payload_hash','source_line_hash','closing_date','settlement_type','property_ref','side','amount'])assert.ok(item.required_evidence.includes(field));assert.deepEqual(item.allowed_outputs,['closing_settlement_treatment_review_candidate','closing_statement_imbalance_candidate']);assert.deepEqual(item.prohibited_actions,{can_create_draft:false,can_review:false,can_approve:false,can_post:false});});
