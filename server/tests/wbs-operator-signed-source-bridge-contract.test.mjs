...
const up=await readFile(new URL('../db/migrations/112_wbs_operator_signed_source_bridge.sql',import.meta.url),'utf8');
const down=await readFile(new URL('../db/migrations/down/112_wbs_operator_signed_source_bridge.sql',import.meta.url),'utf8');
...
test('112 links exception evidence only to an independently signed exact Payable source',()=>{
  for(const token of [
