...
test('rollback restores the previous seven-category report and drops only the added readers',()=>{assert.match(down,/jsonb_array_length\(p_summary\)\s*>\s*7/);for(const category of categories)assert.match(down,new RegExp(category));assert.match(down,/DROP FUNCTION refs_read_ai_manual_journal_risk_findings/);});
