...
test('AI accounting summary HTTP boundary rejects malformed counts, duplicate categories, unsafe actions, and impossible timestamps',async()=>{
  for(const unsafe of [[{...row,total_findings:'4'}],[row,{...row}],[{...row,can_post:true}],[{...row,latest_materialized_at:'2026-02-30T00:00:00Z'}],[{...row,secret:'sk-live-EXAMPLE'}]]){const api=createAccountingApi({authenticate:async()=>({trusted:true,tenantId,actorId:'controller-a'}),kernelFactory:async()=>({readAiAccountingAnalysisSummary:async()=>unsafe})});const response=await api({method:'GET',url:path,headers:{},body:null});assert.equal(response.status,502);}
});
