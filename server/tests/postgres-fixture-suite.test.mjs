...
test('PostgreSQL fixture suite names each isolated accounting closure explicitly',()=>{
  assert.deepEqual(FIXTURES.map(item=>item.id),['controlled-ap-close','ar-rent-pickup-close','signed-wbs-payable-post','signed-bank-same-source-close','bank-reconcile-close','wbs-autorec-match-review','ai-exception-lineage','real-estate-reports']);
  ...
  assert.deepEqual(selectFixtures(['--fixture','wbs-autorec-match-review']).map(item=>item.id),['wbs-autorec-match-review']);
});
