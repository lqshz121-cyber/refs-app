...
const control=(row_count,per_currency_totals)=>({row_count,per_currency_totals,control_totals_hash:canonicalRequestHash({row_count,per_currency_totals})});
...
function fixture(domain,{mutateReceipt,mutatePackage,requestText='POST /mcp HTTP/1.1\r\nX-Request-Id: safe\r\n\r\n{}'}={}){
  ...
  const config=domain==='BANK'?{tool:'list_bank_transactions',rows:[bankRow()],scope:{company_codes:['WBPA'],currency:'USD',date_range:['2026-08-01','2026-08-31'],snapshot_token:'snapshot-bank-1'},totals:[{currency:'USD',gross_amount:'10.0000'}]}:domain==='COST'?{tool:'list_control_totals',rows:[{amount:'25.5000',metric_key:'COST_METRIC_01'}],scope:{company_codes:['WBPA'],currency:'USD',date_range:['2026-08-01','2026-08-31'],report_type:'COST_GENERAL_LEDGER',period:'2026-08',snapshot_token:'snapshot-cost-1'},totals:[{currency:'USD',gross_amount:'25.5000'}]}:{tool:'list_control_totals',rows:[{amount:'100.0000',metric_key:'PROPERTY_VALUE'}],scope:{bank_account_ref:'BANK-1',company_codes:['WBPA'],currency:'USD',date_range:['2026-08-01','2026-08-31'],period_end:'2026-08-31',period_start:'2026-08-01',property_ref:'PROPERTY-A',report_type:'PROPERTY_COMPARISON',snapshot_token:'snapshot-property-1'},totals:[{currency:'USD',gross_amount:'100.0000'}]};
  const controls=control(config.rows.length,config.totals),view={scope:config.scope,row_count:config.rows.length,content_hash:sha256(canonical(config.rows)),rows:config.rows};
  let unsigned={schema_version:'WBS_READONLY_SNAPSHOT_V2',snapshot_id:'22222222-2222-4222-8222-222222222222',captured_at:'2026-08-15T00:00:00Z',environment:'PRODUCTION',source_system:'WBS',domain,source_tool:config.tool,company_key:'WBPA',date_from:'2026-08-01',date_to:'2026-08-31',views:{[config.tool]:view},control_totals:controls};
  ...
  let unsignedReceipt={issuer:'refs-mcp.wbm3.com',kid,algorithm:'Ed25519',request_sha256:sha256(requestRaw),response_sha256:sha256(responseRaw),package_hash:sha256(packageRaw),nonce:`final1-${domain.toLowerCase()}-nonce`,signed_at:'2026-08-15T00:01:00Z',expires_at:'2026-08-15T00:16:00Z',tenant_id:'33333333-3333-4333-8333-333333333333',entity_id:'44444444-4444-4444-8444-444444444444',company_code:'WBPA',immutable_version:pkg.snapshot_id,nonempty:true,control_totals:controls};
...
test('Final-1 Bank, Cost control, and Property control evidence require exact signed controls and normalize without accounting authority',()=>{
  for(const domain of ['BANK','COST','PROPERTY']){
    ...
    assert.equal(verified.domain,domain);assert.equal(verified.signature_verified,true);assert.equal(verified.control_totals_hash,verified.package.control_totals.control_totals_hash);
...
test('Final-1 business verification rejects missing, changed, and noncanonical signed controls',()=>{
  assert.throws(()=>verifyWbsProviderFinal1BusinessDelivery(fixture('BANK',{mutateReceipt:value=>{delete value.control_totals;return value;}})),error=>error.code==='WBS_FINAL1_BUSINESS_RECEIPT_INVALID');
  assert.throws(()=>verifyWbsProviderFinal1BusinessDelivery(fixture('BANK',{mutateReceipt:value=>{value.control_totals.per_currency_totals[0].gross_amount='11.0000';value.control_totals.control_totals_hash=canonicalRequestHash({row_count:1,per_currency_totals:value.control_totals.per_currency_totals});return value;}})),error=>error.code==='WBS_FINAL1_CONTROL_DRIFT');
  assert.throws(()=>verifyWbsProviderFinal1BusinessDelivery(fixture('COST',{mutatePackage:value=>{value.control_totals.per_currency_totals=[{currency:'USD',gross_amount:'25.5'}];value.control_totals.control_totals_hash=canonicalRequestHash({row_count:1,per_currency_totals:value.control_totals.per_currency_totals});return value;}})),error=>error.code==='WBS_FINAL1_CONTROL_INVALID');
});
...
test('Final-1 business verification rejects extra fields, wrong tools, wrong scope, and unsigned or old contracts',()=>{
  ...
  const old8=fixture('BANK');delete old8.receipt.control_totals;
