...
test('fails closed on unsafe reader evidence and action-enabled persistence receipts',async()=>{const credential=['sk','-bank-payment-secret'].join(''),unsafeRead=createAiBankDuplicatePaymentService({sourceReader:async()=>rows.map((row,index)=>index?row:{...row,external_bank_line_id:credential})});await assert.rejects(()=>unsafeRead.analyze({tenantId:id(80),entityId:id(1),currentAccountingPeriodId:id(8)}),error=>error.code==='AI_BANK_DUPLICATE_PAYMENT_POPULATION_INCOMPLETE');const unsafeWrite=createAiBankDuplicatePaymentService({sourceReader:async()=>rows,materializeWriter:async()=>({can_create_draft:false,can_review:false,can_approve:false,can_post:true})});await assert.rejects(()=>unsafeWrite.analyzeAndMaterialize({tenantId:id(80),entityId:id(1),currentAccountingPeriodId:id(8),idempotencyKey:'bank-duplicate-unsafe'}),error=>error.code==='AI_BANK_DUPLICATE_PAYMENT_PERSISTENCE_INVALID');});
