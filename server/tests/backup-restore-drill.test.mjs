...
test('backup restore drill restores migrations and persisted accounting data before cleanup',()=>{
  assert.match(source,/waitForPostgresReadiness\(\{probe:\(\)=>probePostgres\(migrationUrl\)\}\)/);assert.match(source,/pg_dump -U/);assert.match(source,/pg_restore -U/);assert.match(source,/count\(\*\) FROM refs_schema_migration/);assert.match(source,/tenant_code='BKDRILL'/);assert.match(source,/dropdb -U/);
});
