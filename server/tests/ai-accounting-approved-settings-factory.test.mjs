...
test('production AI settings factory binds the adapter to the authenticated kernel reader only',async()=>{
  ...
  const factory=createProductionAiAccountingSettingsAdapterFactory({kernelFor:principal=>({
    readApprovedWbsAiEntityPeriodSettings:async input=>{calls.push({principal,input});throw Object.assign(new Error('approved settings unavailable'),{code:'WBS_AI_APPROVED_SETTINGS_INVALID'});},readAiAccountMasterBindings:async()=>[]
  })});
...
test('production AI settings factory rejects untrusted, cross-tenant, mutable, or non-kernel access',async()=>{
  ...
  const calls=[],factory=createProductionAiAccountingSettingsAdapterFactory({kernelFor:()=>({readApprovedWbsAiEntityPeriodSettings:async input=>(calls.push(input),{}),readAiAccountMasterBindings:async()=>[]})}),adapter=factory({trusted:true,tenantId,actorId:'human-controller'});
...
test('production decision service builder receives only a principal-scoped kernel and approved-settings adapter',async()=>{
  const kernel={readApprovedWbsAiEntityPeriodSettings:async()=>{throw new Error('not called by this proof');},readAiAccountMasterBindings:async()=>[]},principal=Object.freeze({trusted:true,tenantId,actorId:'human-controller'}),seen=[];
