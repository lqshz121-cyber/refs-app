...
const roles=['READER','MAKER','EXPENSE_MAKER','PAYMENT_MAKER','RECEIPT_MAKER','SALES_RECEIPT_MAKER','REVERSAL_MAKER','ADJUSTMENT_MAKER','REFUND_MAKER','ALLOCATOR','SUBMITTER','REVIEWER','APPROVER','POSTER','RECONCILIATION_STARTER','CLEARER','UNMATCHER','REOPENER','PERIOD_CLOSER','PERIOD_REOPENER','CASH_TRANSFER_RECONCILER','RECURRING_RUNNER','WBS_TEST_IMPORTER'];
...
test('internal full-test router retains distinct finite identities by workflow stage',()=>{
  ...
  assert.equal(route('POST',`/api/v1/entities/${entity}/wbs/test-import/payables`).actorId,actors.wbsTestImporter);
  assert.equal(route('POST',`/api/v1/entities/${entity}/wbs/test-import/bank-transactions`).actorId,actors.wbsTestImporter);
