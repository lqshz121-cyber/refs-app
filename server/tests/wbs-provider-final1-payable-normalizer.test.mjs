...
test('normalizes complete typed tax-statement identity without creating accounting authority',()=>{
  const typed={document_evidence_schema_version:'WBS_FINAL1_PAYABLE_DOCUMENT_EVIDENCE_V1',document_kind:'TAX_STATEMENT',tax_year:2026,taxing_jurisdiction:'Cook County',tax_statement_identifier:'PIN-2026-42',tax_coverage_period_start:'2026-01-01',tax_coverage_period_end:'2026-12-31',tax_obligation_basis:'MILLAGE_RATE',controlled_property_ref:'PROPERTY-1',parcel_identifier:'17-09-123-045',document_revision_schema_version:'WBS_FINAL1_PAYABLE_DOCUMENT_REVISION_V1',document_revision_kind:'ORIGINAL',document_revision:1,predecessor_document_evidence_hash:null,predecessor_document_revision_hash:null,predecessor_document_revision:null,predecessor_source_record_id:null},output=normalizeVerifiedWbsProviderFinal1Payables({verified:fixture({rowExtra:typed}).verified,expectedCurrency:'USD'}).staging_rows[0];
  assert.equal(output.normalized.documentEvidenceStatus,'COMPLETE');assert.equal(output.normalized.documentKind,'TAX_STATEMENT');assert.equal(output.normalized.taxingJurisdiction,'Cook County');assert.equal(output.normalized.taxStatementIdentifier,'PIN-2026-42');assert.equal(output.normalized.controlledPropertyRef,'PROPERTY-1');assert.equal(output.normalized.parcelIdentifier,'17-09-123-045');assert.equal(output.normalized.documentRevisionKind,'ORIGINAL');assert.equal(output.normalized.documentRevision,1);assert.equal(output.can_create_draft,false);assert.equal(output.can_post,false);
});
