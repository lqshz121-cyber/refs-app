...
test('G11 read verifier accepts only scoped kernel-read posted evidence and keeps all accounting actions disabled',async()=>{
  ...
  const journal=(type,lines)=>({tenant_id:'t1',entity_id:'e1',review_candidate_id:'candidate-1',...g11Scope,accounting_type:type,source_system:'REFS_STANDARD_JE',status:'POSTED',journal_entry_id:`je-${type}`,audit_event_id:`audit-${type}`,audit_event_type:'AUTO_JOURNAL_CREATED',source_trace:sourceTrace,ledger_lines:lines});
  ...
  const wrongCandidate=createWbsAutoRecG11ReadVerifier({repository:{readReviewedWbsAutoRecRequest:async()=>review,readPostedWbsAutoRecJournalEvidence:async()=>[{...posted[0],review_candidate_id:'candidate-other'},posted[1]]}});
  assert.equal((await wrongCandidate.verify(selection)).code,'WBS_AUTOREC_G11_READ_SCOPE_INVALID');
