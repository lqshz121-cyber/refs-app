...
const map=row=>({mapping_id:`map-${row.source_record_id}`,version:'1',snapshot_hash:'sha256:'+(row.source_type==='BANK_TRANSACTION'?'b':'a').repeat(64),status:'APPROVED',source_type:row.source_type,entity_id:'e1',company_key:'COMPANY-A',currency:'USD',...(row.source_type==='BANK_TRANSACTION'?{bank_account_ref:'BANK-1'}:{})});
...
function repository({badScope=false,fail=false}={}){return {readPersistedWbsInboundRows:async()=>{if(fail)throw Error('unavailable');return badScope?[{...bank,company_key:'COMPANY-B'}]:[bank,payable]},readPersistedWbsControlRows:async()=>({companyRows:[control],detailRows:[],persistedRows:[control]}),readApprovedWbsAutoRecMappings:async()=>[map(bank),map(payable)],readWbsAutoRecObservedStateEvidence:async()=>[observedState]};}

test('composes only scoped persisted receipt-backed rows and returns stable replay',async()=>{
  const reader=createWbsInboundAutoRecReadComposition({repository:repository()});const first=await reader.read(scope);assert.equal(first.status,'READ_ONLY_PROJECTED');assert.equal(first.candidates.length,2);assert.deepEqual({count:first.observed_state_evidence.length,state:first.observed_state_evidence[0].observed_state,release:first.observed_state_evidence[0].can_release},{count:1,state:'RELEASED',release:false});assert.equal(first.can_post,false);
