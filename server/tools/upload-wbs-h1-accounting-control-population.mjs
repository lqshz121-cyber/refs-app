...
const MAX_PAGE_BYTES=7*1024*1024,MAX_ATTEMPTS=5;
...
export async function uploadWbsH1AccountingControlPopulation({env=process.env,fetcher=request}={}){
  ...
  const sourceVersion=canonicalRequestHash({schema_version:'WBS_H1_ACCOUNTING_CONTROL_SOURCE_V1',manifest:sourceManifest}),streamArgs={filePath,sourceManifest,tenantId,entityId,currency,sourceVersion},summary=await summarizeWbsH1AccountingStream(streamArgs),runId=env.REFS_WBS_H1_ACCOUNTING_RUN_ID||randomUUID(),root=`/api/v1/entities/${entityId}/wbs/h1-accounting-control-runs`;
  await fetcher({baseUrl,token,path:root,idempotencyKey:env.REFS_WBS_H1_ACCOUNTING_IDEMPOTENCY_KEY||`wbs-h1-accounting:${sourceVersion.slice(7,55)}`,body:{runId,companyCode,currency,sourceVersion,snapshotTokenHash:summary.snapshot_token_hash,providerContentHash:summary.provider_content_hash,sourceManifest,capturedAt:summary.captured_at,expectedRowCount:summary.expected_row_count,includedH1RowCount:summary.included_h1_row_count,excludedRowCount:summary.excluded_row_count,expectedDebitAmount:summary.expected_debit_amount,expectedCreditAmount:summary.expected_credit_amount,populationHash:summary.population_hash}});
  let page=[],pageIndex=0;
  const flush=async()=>{if(!page.length)return;await fetcher({baseUrl,token,path:`${root}/${runId}/lines`,body:{lines:page}});page=[];pageIndex++;};
  for await(const normalized of streamNormalizedWbsH1AccountingPages(streamArgs))for(const line of normalized){const candidate=[...page,line];if(candidate.length>1000||Buffer.byteLength(JSON.stringify({lines:candidate}),'utf8')>MAX_PAGE_BYTES){await flush();if(Buffer.byteLength(JSON.stringify({lines:[line]}),'utf8')>MAX_PAGE_BYTES)throw new Error('One normalized accounting row exceeds the 7 MiB page boundary');}page.push(line);}
  await flush();
  const receipt=await fetcher({baseUrl,token,path:`${root}/${runId}/finalize`,body:{}});
  return {runId,pageCount:pageIndex,receipt};
}
if(import.meta.url===pathToFileURL(process.argv[1]).href)uploadWbsH1AccountingControlPopulation().then(result=>process.stdout.write(`${JSON.stringify(result)}\n`)).catch(error=>{process.stderr.write(`${JSON.stringify({status:'WBS_H1_ACCOUNTING_CONTROL_UPLOAD_FAILED',message:error.message})}\n`);process.exitCode=1;});
