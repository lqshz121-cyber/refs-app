...
export function verifyStage1PayableLiveAcceptance({providerTrust,receipt,raw,chain,now=Date.now()}={}){
  ...
  if(!payable||!UUID.test(text(payable.wbs_inbound_row_id))||!text(payable.source_record_id)||!text(payable.source_version)||!HASH.test(text(payable.receipt_hash))||!HASH.test(text(payable.provider_receipt_hash))||!HASH.test(text(payable.evidence_hash)))fail('STAGE1_PAYABLE_SOURCE_INVALID');
  const signedPayable=(scope.source_receipts||[]).find(source=>source.source_module==='BGDATA.payable'&&source.source_record_id===payable.source_record_id&&source.source_version===payable.source_version);
  if(!signedPayable||signedPayable.ingestion_kind!=='TRANSACTION_CANDIDATE'||signedPayable.payload_hash!==payable.receipt_hash||signedPayable.provider_receipt_hash!==payable.provider_receipt_hash)fail('STAGE1_PAYABLE_SOURCE_INVALID');
  if(!attachment||attachment.wbs_inbound_row_id!==payable.wbs_inbound_row_id||attachment.source_version!==payable.source_version||attachment.receipt_hash!==payable.receipt_hash||attachment.provider_receipt_hash!==payable.provider_receipt_hash||attachment.status!=='VERIFIED_CLEAN'||!text(attachment.object_version)||!HASH.test(text(attachment.content_hash)))fail('STAGE1_PAYABLE_ATTACHMENT_INVALID');
  if(!review||review.wbs_inbound_row_id!==payable.wbs_inbound_row_id||review.expected_evidence_hash!==payable.evidence_hash||review.expected_provider_receipt_hash!==payable.provider_receipt_hash||review.attachment_id!==attachment.attachment_id||review.status!=='READY_FOR_DRAFT'||!text(review.reviewer_actor_id))fail('STAGE1_PAYABLE_REVIEW_INVALID');
