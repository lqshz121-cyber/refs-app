...
const AI_INVOICE_CLASSIFICATION_KEYS=['action_flags','classification','confidence','reason','required_human_fields','schema_version','source_document_id','source_document_line_id','source_line_hash','source_payload_hash'].sort();
...
const safeAiInvoiceClassification=value=>exactKeys(value,AI_INVOICE_CLASSIFICATION_KEYS)&&value.schema_version==='AI_INVOICE_ACCOUNTING_CLASSIFICATION_V1'&&UUID.test(value.source_document_id||'')&&UUID.test(value.source_document_line_id||'')&&AI_ACCRUAL_HASH.test(value.source_payload_hash||'')&&AI_ACCRUAL_HASH.test(value.source_line_hash||'')&&AI_INVOICE_CLASSIFICATIONS.has(value.classification)&&boundedText(value.reason,2000)&&typeof value.confidence==='number'&&Number.isFinite(value.confidence)&&value.confidence>=0&&value.confidence<=1&&Array.isArray(value.required_human_fields)&&value.required_human_fields.length<=20&&value.required_human_fields.every(field=>boundedText(field,64))&&safeNoAccountingActions(value.action_flags);
