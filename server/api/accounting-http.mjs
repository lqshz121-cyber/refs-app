...
function statusFor(error){
  ...
  const problemFor=error=>{const status=statusFor(error);const code=isRevisionPrecondition(error)?'PRECONDITION_FAILED':error?.code==='WBS_SNAPSHOT_SIGNATURE_REQUIRED'?'WBS_SNAPSHOT_SIGNATURE_REQUIRED':error?.code==='WBS_BANK_ADMISSION_SIGNATURE_REQUIRED'?'WBS_BANK_ADMISSION_SIGNATURE_REQUIRED':error?.code==='WBS_READ_SERVICE_UNAVAILABLE'?'WBS_READ_SERVICE_UNAVAILABLE':error?.code==='WBS_PAYABLE_ADMISSION_UNAVAILABLE'?'WBS_PAYABLE_ADMISSION_UNAVAILABLE':error?.code==='WBS_FINAL1_ADMISSION_UNAVAILABLE'?'WBS_FINAL1_ADMISSION_UNAVAILABLE':error?.code==='WBS_OPERATOR_ATTEST_UNAVAILABLE'?'WBS_OPERATOR_ATTEST_UNAVAILABLE':error?.code==='AI_FINDING_READ_UNAVAILABLE'?'AI_FINDING_READ_UNAVAILABLE':error?.code==='AI_PREPAID_COVERAGE_FINDING_READ_UNAVAILABLE'?'AI_PREPAID_COVERAGE_FINDING_READ_UNAVAILABLE':error?.code==='AI_DUPLICATE_PAYABLE_FINDING_READ_UNAVAILABLE'?'AI_DUPLICATE_PAYABLE_FINDING_READ_UNAVAILABLE':error?.code==='AI_UNMATCHED_BANK_PAYMENT_FINDING_READ_UNAVAILABLE'?'AI_UNMATCHED_BANK_PAYMENT_FINDING_READ_UNAVAILABLE':error?.code==='AI_COST_DIMENSION_FINDING_READ_UNAVAILABLE'?'AI_COST_DIMENSION_FINDING_READ_UNAVAILABLE':error?.code==='AI_LOAN_REFERENCE_FINDING_READ_UNAVAILABLE'?'AI_LOAN_REFERENCE_FINDING_READ_UNAVAILABLE':error?.code==='AI_ACCOUNTING_ANALYSIS_SUMMARY_UNAVAILABLE'?'AI_ACCOUNTING_ANALYSIS_SUMMARY_UNAVAILABLE':error?.code==='AI_ANALYSIS_EXPLANATION_UNAVAILABLE'?'AI_ANALYSIS_EXPLANATION_UNAVAILABLE':error?.code==='AI_AMORTIZATION_PROPOSAL_UNAVAILABLE'?'AI_AMORTIZATION_PROPOSAL_UNAVAILABLE':error?.code==='AI_AMORTIZATION_DRAFT_UNAVAILABLE'?'AI_AMORTIZATION_DRAFT_UNAVAILABLE':error instanceof WbsCompanyCatalogControllerError?error.code:error instanceof WbsProviderFinal1RetainedEvidenceError?error.code:error instanceof WbsProviderSignedPayableAdmissionError?error.code:error instanceof WbsOperatorAttestedPayableError?error.code:error instanceof WbsSignedBankAdmissionError?error.code:error instanceof WbsAdmittedPayableIngestionError?error.code:error instanceof WbsLivePilotError?error.code:error instanceof AiAnalysisExplanationError?error.code:error instanceof WbsReadContractError?error.code:status===503?'SERIALIZATION_RETRY_EXHAUSTED':error.code||'INTERNAL_ERROR';const message=status>=500?'Internal server error':status===403?'Forbidden':error.message;const headers={'content-type':'application/problem+json','cache-control':'no-store'};if(status===503)headers['retry-after']='1';return {status,headers,body:{ok:false,code,message}};};
