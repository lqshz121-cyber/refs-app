...
export function createAccountingApi({authenticate,kernelFactory,attachmentServiceFactory,wbsReadServiceFactory,wbsLivePilotServiceFactory,wbsAdmittedPayableServiceFactory,wbsProviderSignedPayableServiceFactory,wbsOperatorAttestedPayableServiceFactory,aiAnalysisExplanationServiceFactory,stage1SelfGrantServiceFactory,stage1SelfWbsReadUpgradeServiceFactory,stage1SelfWbsOperatorUpgradeServiceFactory}={}){
  ...
  return async function dispatch({method,url,headers={},body=null}){
    try{
      ...
      if(method==='GET'&&parts.length===8&&parts[4]==='wbs'&&parts[5]==='inbound'&&parts[6]==='cost-cwip'&&parts[7]==='reviews'){
        if(header(headers,'idempotency-key')!=null||header(headers,'if-match')!=null)throw new AccountingApiError(400,'READ_COMMAND_HEADERS_FORBIDDEN','WBS Cost-to-CWIP evidence reads do not accept command headers');
        if(body!==null)throw new AccountingApiError(400,'READ_BODY_FORBIDDEN','Read operations do not accept a request body');
        requireExactQuery(parsedUrl.searchParams,['limit']);
        const kernel=await kernelFactory(principal);if(!kernel||typeof kernel.listWbsCostCwipReviewEvidence!=='function')throw new AccountingApiError(503,'WBS_COST_CWIP_EVIDENCE_READ_UNAVAILABLE','WBS Cost-to-CWIP evidence read is unavailable');
        result=await kernel.listWbsCostCwipReviewEvidence({tenantId:principal.tenantId,entityId,limit:optionalAdmittedStatementLimit(parsedUrl.searchParams.get('limit'))});
        return {status:200,headers:{'content-type':'application/json','cache-control':'no-store'},body:{ok:true,data:result}};
      }
      ...
      }else if(parts.length===11&&parts[4]==='ai'&&parts[5]==='amortization'&&parts[6]==='schedules'&&parts[8]==='lines'&&parts[10]==='acceptances'){
        requireExactQuery(parsedUrl.searchParams,[]);if(header(headers,'if-match')!=null)throw new AccountingApiError(400,'IF_MATCH_NOT_ALLOWED','AI amortization acceptance binds immutable schedule and Draft evidence, not If-Match');
        allowOnly(payload,['journalEntryId','reason']);
        const kernel=await kernelFactory(principal);if(!kernel||typeof kernel.acceptAiAmortizationSchedule!=='function')throw new AccountingApiError(503,'AI_AMORTIZATION_ACCEPTANCE_UNAVAILABLE','AI amortization acceptance persistence is unavailable');
        result=await kernel.acceptAiAmortizationSchedule({tenantId:principal.tenantId,entityId,aiAmortizationScheduleId:requireUuid(parts[7],'aiAmortizationScheduleId'),aiAmortizationScheduleLineId:requireUuid(parts[9],'aiAmortizationScheduleLineId'),journalEntryId:requireUuid(payload.journalEntryId,'journalEntryId'),reason:requireReviewReason(payload.reason),idempotencyKey});
        if(!result||result.status!=='ACCEPTED_FOR_STANDARD_JE_WORKFLOW'||!UUID.test(result.ai_amortization_schedule_acceptance_id||'')||result.can_create_draft!==false||result.can_review!==false||result.can_approve!==false||result.can_post!==false)throw new AccountingApiError(500,'AI_AMORTIZATION_ACCEPTANCE_RESULT_INVALID','AI amortization acceptance must bind only an existing standard Draft JE');
