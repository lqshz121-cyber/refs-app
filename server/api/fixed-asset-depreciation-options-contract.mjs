...
export function validFixedAssetDepreciationOptions(value,{tenantId,entityId,assetId,periodId}){
 ...
 if(value.source!==null&&(!keys(value.source,'source_document_id source_document_version source_payload_hash source_document_line_id source_line_snapshot_hash original_evidence_id original_evidence_hash')||!UUID.test(value.source.source_document_id||'')||!Number.isSafeInteger(value.source.source_document_version)||value.source.source_document_version<0||!HASH.test(value.source.source_payload_hash||'')||!UUID.test(value.source.source_document_line_id||'')||!HASH.test(value.source.source_line_snapshot_hash||'')||!UUID.test(value.source.original_evidence_id||'')||!HASH.test(value.source.original_evidence_hash||'')))return false;
