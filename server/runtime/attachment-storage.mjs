...
export class HttpVirusScanner{
  constructor({endpoint,bearerToken,fetcher=globalThis.fetch,timeoutMs=30000,maxAttempts=3,retryBaseMs=100,sleeper=ms=>new Promise(resolve=>setTimeout(resolve,ms))}={}){let url;try{url=new URL(endpoint);}catch{throw new Error('Scanner endpoint must be a valid URL');}if(url.protocol!=='https:'||!bearerToken||typeof fetcher!=='function'||!Number.isInteger(maxAttempts)||maxAttempts<1||maxAttempts>5)throw new Error('Scanner requires HTTPS endpoint, credential, and safe retry policy');this.endpoint=url;this.bearerToken=bearerToken;this.fetcher=fetcher;this.timeoutMs=timeoutMs;this.maxAttempts=maxAttempts;this.retryBaseMs=retryBaseMs;this.sleeper=sleeper;}
  async scan(evidence){if(!evidence?.storageRef||!evidence?.storageVersion||String(evidence.storageVersion).startsWith('pending:'))throw new Error('Scanner requires an exact finalized object version');let last;for(let attempt=1;attempt<=this.maxAttempts;attempt++){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(new Error('Scanner request timeout')),this.timeoutMs);try{const response=await this.fetcher(this.endpoint,{method:'POST',redirect:'error',signal:controller.signal,headers:{authorization:`Bearer ${this.bearerToken}`,'content-type':'application/json','accept':'application/json'},body:JSON.stringify({storage_ref:evidence.storageRef,storage_version:evidence.storageVersion})});if(!response.ok){const error=new Error(`Scanner failed with ${response.status}`);error.retryable=response.status>=500||response.status===429;throw error;}const result=await response.json();if(typeof result.clean!=='boolean'||typeof result.scan_ref!=='string'||!result.scan_ref)throw new Error('Scanner response is invalid');return {clean:result.clean,scanRef:result.scan_ref};}catch(error){last=error;if(attempt>=this.maxAttempts||error.retryable===false||(/with 4\d\d/.test(error.message)&&!error.message.includes('429')))throw error;await this.sleeper(this.retryBaseMs*2**(attempt-1));}finally{clearTimeout(timer);}}throw last;}
}
