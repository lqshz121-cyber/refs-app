...
const shell=['set -eu',`cleanup(){ dropdb -U \"$POSTGRES_USER\" \"${restoredDatabase}\" 2>/dev/null || true; rm -f /tmp/refs-backup.dump; }`,'trap cleanup EXIT',"pg_dump -U \"$POSTGRES_USER\" -Fc \"$POSTGRES_DB\" -f /tmp/refs-backup.dump",`createdb -U \"$POSTGRES_USER\" \"${restoredDatabase}\"`,`pg_restore -U \"$POSTGRES_USER\" --exit-on-error -d \"${restoredDatabase}\" /tmp/refs-backup.dump`,`test \"$(psql -U \"$POSTGRES_USER\" -d \"${restoredDatabase}\" -Atqc 'SELECT count(*) FROM refs_schema_migration')\" = '${MIGRATION_MANIFEST.length}'`,`test \"$(psql -U \"$POSTGRES_USER\" -d \"${restoredDatabase}\" -Atqc \"SELECT count(*) FROM tenant WHERE tenant_code='BKDRILL'\")\" = '1'`].join('; ');
