...
export async function verifyStage2AuthoritativeE2e({config,fetcher=globalThis.fetch}={}){
  ...
  const [bank,reconciliation,signedSnapshot,journal,ledger,statements,source]=await Promise.all([
    ...
    getJson(fetcher,`${base}/source-documents/${scenario.sourceDocumentId}`,accessToken,'source document'),
  ]);
  ...
  expect(source?.source_document_id===scenario.sourceDocumentId||contains(source,scenario.sourceDocumentId),'source-document detail does not retain the exact bank reconciliation source identity');
  ...
  return Object.freeze({ok:true,mode:'READ_ONLY_SIGNED_RECONCILIATION_EVIDENCE',release:stamps,checks:['same-release-stamps','bank-source-and-match','immutable-signed-snapshot','posted-journal','ledger','trial-balance','balance-sheet','cash-flow','source-document','cross-source-identifiers'],reconciliationId:scenario.reconciliationId});
}
