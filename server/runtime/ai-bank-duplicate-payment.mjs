...
const validRow=row=>row&&typeof row==='object'&&!Array.isArray(row)&&UUID.test(row.bank_source_id||'')&&UUID.test(row.source_document_id||'')&&SHA256.test(row.source_payload_hash||'')&&UUID.test(row.entity_id||'')&&UUID.test(row.accounting_period_id||'')&&text(row.bank_account_ref,128)&&text(row.external_bank_line_id,256)&&validDate(row.transaction_date)&&/^[A-Z]{3}$/.test(row.currency||'')&&MONEY4.test(row.amount||'')&&row.source_admission_status==='ADMITTED'&&row.signature_verified===true;
const key=row=>[row.entity_id,row.bank_account_ref.trim().toUpperCase(),row.transaction_date,row.currency,row.amount].join('|');

export function detectDuplicateBankPayments(rows,{currentAccountingPeriodId}={}){
  if(!Array.isArray(rows)||rows.length>500||!UUID.test(currentAccountingPeriodId||''))throw Object.assign(new Error('Duplicate bank-payment analysis requires one accounting period and at most 500 retained bank rows.'),{code:'AI_BANK_DUPLICATE_PAYMENT_SCOPE_INVALID'});
  ...
  for(const row of rows.filter(item=>item.accounting_period_id===currentAccountingPeriodId)){const identity=key(row);if(!groups.has(identity))groups.set(identity,[]);groups.get(identity).push(row);}
  const findings=[];
  ...
  return Object.freeze({schema_version:'AI_BANK_DUPLICATE_PAYMENT_BATCH_V1',current_accounting_period_id:currentAccountingPeriodId,scanned_payment_count:rows.filter(row=>row.accounting_period_id===currentAccountingPeriodId).length,finding_count:findings.length,findings:Object.freeze(findings),action_flags:ACTIONS});
}
