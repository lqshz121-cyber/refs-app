...
export function assertWbsTestRangeImportResult(value){
  ...
  if(!exactObject(value,top)||value.status!=='WBS_TEST_RANGE_IMPORT_COMPLETE'||value.test_only!==true||!date(value.date_from)||!date(value.date_to)||value.date_from>value.date_to||!Number.isSafeInteger(value.page_size)||value.page_size<1||value.page_size>10||!exactObject(value.payables,payableKeys)||!exactObject(value.bank,bankKeys))fail('WBS_TEST_RANGE_RESULT_INVALID','Test range-import result is incomplete or unsafe.');
...
export function createWbsTestImportService({pilotService,kernelForActor,authorizeBank,scope}={}){
  ...
  return Object.freeze({
    ...
    async importRange({tenantId,entityId,companyCode,dateFrom,dateTo,pageSize=10,maxPages=50,idempotencyKey}={}){
      if(tenantId!==scope.tenantId||entityId!==scope.entityId)fail('WBS_TEST_IMPORT_SCOPE_DENIED','Test import is restricted to its configured tenant and entity.');
      if(companyCode!==scope.companyCode||dateFrom!=='2026-01-01'||dateTo!=='2026-06-30'||!Number.isSafeInteger(pageSize)||pageSize<1||pageSize>10)fail('WBS_TEST_IMPORT_SELECTION_INVALID','The range import is restricted to the exact 2026 H1 test window and page sizes from 1 to 10.');
      ...
      const readPages=async(tool,rowValidator)=>{
        const pages=[],sourceHashes=new Set(),cursors=new Set();let cursor=null;
        for(let pageIndex=0;pageIndex<maxPages;pageIndex++){
          const page=await pilotService.readObservationPage({tenantId,entityId,tool,limit:pageSize,company_code:companyCode,date_from:dateFrom,date_to:dateTo,cursor});
          if(!exactObject(page,['cursor_next','observation']))fail('WBS_TEST_IMPORT_ROW_INVALID','Provider page envelope is invalid.');
      ...
      for(const [periodCode,periodId] of periodByCode){
        ...
        reconciliations.push({bank_account_ref:bankAccountRef,period_code:periodCode,reconciliation_id:result.reconciliation_id,transaction_count:result.transaction_count});bankSourceIds.push(...result.bank_source_ids);
      }
