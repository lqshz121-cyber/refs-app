...
export function assertWbsControlledTestBankResult(value){
  ...
  if(value?.status==='WBS_TEST_BANK_IMPORT_PARTIAL'){
    if(!exactObject(value,partialKeys)||!UUID.test(value.stage_id||'')||value.test_only!==true||value.provenance_mode!=='CONTROLLED_TEST_UNSIGNED'||typeof value.idempotent!=='boolean'||!Number.isSafeInteger(value.transaction_count)||value.transaction_count<2001||value.transaction_count>WBS_TEST_MONTH_MAX_ROWS||!Number.isSafeInteger(value.chunk_count)||value.chunk_count!==Math.ceil(value.transaction_count/100)||!Number.isSafeInteger(value.next_chunk_index)||value.next_chunk_index<1||value.next_chunk_index>=value.chunk_count)fail('WBS_TEST_BANK_RESULT_INVALID','Controlled test Bank checkpoint is incomplete or unsafe.');
    return value;
  }
