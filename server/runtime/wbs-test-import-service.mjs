...
export function assertWbsControlledTestBankResult(value){
  const partialKeys=['chunk_count','idempotent','next_chunk_index','provenance_mode','stage_id','status','test_only','transaction_count'];
  if(value?.status==='WBS_TEST_BANK_IMPORT_PARTIAL'){
    if(!exactObject(value,partialKeys)||!UUID.test(value.stage_id||'')||value.test_only!==true||value.provenance_mode!=='CONTROLLED_TEST_UNSIGNED'||typeof value.idempotent!=='boolean'||!Number.isSafeInteger(value.transaction_count)||value.transaction_count<1||value.transaction_count>WBS_TEST_MONTH_MAX_ROWS||!Number.isSafeInteger(value.chunk_count)||value.chunk_count!==Math.ceil(value.transaction_count/100)||!Number.isSafeInteger(value.next_chunk_index)||value.next_chunk_index<1||value.next_chunk_index>=value.chunk_count)fail('WBS_TEST_BANK_RESULT_INVALID','Controlled test Bank checkpoint is incomplete or unsafe.');
    return value;
  }
...
export function assertWbsTestRangeImportResult(value){
  ...
  if(value.status==='WBS_TEST_MONTH_IMPORT_PARTIAL'){
    ...
    if(!exactObject(bank,[...bankKeys,'checkpoint'])||!Number.isSafeInteger(bank.provider_page_count)||bank.provider_page_count<0||bank.provider_page_count>WBS_TEST_MONTH_MAX_PAGES||!Number.isSafeInteger(bank.record_count)||bank.record_count<1||bank.record_count>WBS_TEST_MONTH_MAX_ROWS||reconciliation!==null||!Array.isArray(bank.bank_source_ids)||bank.bank_source_ids.length!==0||!exactObject(checkpoint,['chunk_count','next_chunk_index','stage_id','transaction_count'])||!UUID.test(checkpoint.stage_id||'')||checkpoint.transaction_count!==bank.record_count||checkpoint.chunk_count!==Math.ceil(bank.record_count/100)||!Number.isSafeInteger(checkpoint.next_chunk_index)||checkpoint.next_chunk_index<1||checkpoint.next_chunk_index>=checkpoint.chunk_count)fail('WBS_TEST_RANGE_RESULT_INVALID','Test month-import Bank checkpoint is invalid.');
    return value;
  }
