...
import {containsAiSecret,redactAiSecretText} from './ai-secret-safety.mjs';
...
export function redactAiFacts(value,{depth=0}={}){
  ...
  if(typeof value==='string'){const secret=containsAiSecret(value),redacted=redactAiSecretText(value);if(secret&&value.length<=12000)return '[REDACTED]';return redacted.length>12000?`${redacted.slice(0,12000)} [TRUNCATED]`:redacted;}
...
export class LiteLlmGateway {
  ...
  async analyzeJson({traceId=`refs-ai-${randomUUID()}`,traceName='refs-accounting-analysis',actorId,facts,systemInstruction,jsonSchema}={}){
    ...
    if([traceId,traceName,actorId,systemInstruction].some(value=>typeof value!=='string'||containsAiSecret(value)))throw new LiteLlmGatewayError('AI_ANALYSIS_METADATA_INVALID','AI model metadata and instructions must not contain credential-like values');
