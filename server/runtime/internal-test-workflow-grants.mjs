...
const AUTHORITY=Object.freeze({reader:'READ',maker:'DRAFT',paymentMaker:'PAYMENT',reversalMaker:'REVERSAL',allocator:'ALLOCATION',submitter:'SUBMIT',reviewer:'REVIEW',approver:'APPROVE',poster:'POST',reconciliationStarter:'DRAFT',clearer:'DRAFT',reopener:'REOPEN',periodCloser:'CLOSE',periodReopener:'REOPEN',cashTransferReconciler:'JE_REVIEW'});
...
export async function reconcileInternalTestWorkflowActorGrants({grantSync,scope}={}){
  ...
  for(const role of ROLES){
    ...
    for(let attempt=0;attempt<2;attempt++){
      ...
      try{
        await grantSync.reconcile({tenantId:scope.tenantId,entityId:scope.entityId,actorId,permissions:INTERNAL_TEST_WORKFLOW_GRANT_BUNDLES[role],authorityClass:AUTHORITY[role],validUntil,expectedVersion,idempotencyKey:`internal-test-${role}-grant-v3-${expectedVersion}`});
        break;
