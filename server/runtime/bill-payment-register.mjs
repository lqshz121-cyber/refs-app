...
export function validBillPaymentRegister(value,{entityId,periodId,afterId=null,limit=50}){
  if(!validBillPaymentRegisterSelection({periodId,afterId,limit})||!exact(value,pageKeys)||value.schema_version!=='BILL_PAYMENT_REGISTER_V2'||value.entity_id!==entityId||value.period_id!==periodId||typeof value.period_code!=='string'||!value.period_code.trim()||value.after_id!==afterId||value.limit!==limit||!Array.isArray(value.rows)||value.rows.length>limit||!exact(value.action_flags,flagKeys)||flagKeys.some(key=>value.action_flags[key]!==false))return false;
  ...
  for(const row of value.rows){
    ...
    if(![row.source_document_id,row.journal_entry_id,row.journal_line_id,row.ledger_line_id,row.bank_match_id,row.draft_audit_event_id,row.posted_audit_event_id].every(optionalUuid)||!optionalText(row.journal_number)||!optionalText(row.journal_status)||!optionalRevision(row.journal_revision)||!optionalText(row.cash_account_code)||!optionalText(row.bank_member_ref)||!optionalText(row.bank_match_status)||!optionalRevision(row.bank_match_revision))return false;
    const journalLinked=row.journal_entry_id!==null;
    if(journalLinked!==[row.journal_number,row.journal_status,row.journal_revision,row.cash_account_code,row.bank_member_ref].every(v=>v!==null)||row.journal_status!==null&&!['DRAFT','PENDING_REVIEW','PENDING_APPROVAL','APPROVED','POSTED'].includes(row.journal_status))return false;
