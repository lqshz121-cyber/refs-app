...
const LOOKUP_KEYS=Object.freeze(['accounting_period_id','ap_document_ids','ap_match_count','company_code','entity_id','journal_entry_ids','ledger_line_ids','ledger_line_match_count','lookup_receipt_hash','lookup_receipt_id','lookup_status','posted_journal_match_count','queried_at','schema_version','source_document_id','source_document_line_id','source_line_hash','tenant_id']);
...
function validLookup(lookup,source){
  const receiptPayload=lookup&&Object.fromEntries(Object.entries(lookup).filter(([key])=>key!=='lookup_receipt_hash'));
  if(!exact(lookup,LOOKUP_KEYS)||lookup.schema_version!=='ACCOUNTING_BOOKING_LOOKUP_V1'||lookup.tenant_id!==source.tenant_id||lookup.entity_id!==source.entity_id||lookup.company_code!==source.company_code||lookup.accounting_period_id!==source.accounting_period_id||lookup.source_document_id!==source.source_document_id||lookup.source_document_line_id!==source.source_document_line_id||lookup.source_line_hash!==source.source_line_hash||!UUID.test(lookup.lookup_receipt_id||'')||lookup.lookup_receipt_hash!==digest(receiptPayload)||!text(lookup.queried_at,80)||Number.isNaN(Date.parse(lookup.queried_at))||!['COMPLETE','UNAVAILABLE','AMBIGUOUS'].includes(lookup.lookup_status))return false;
  if(lookup.lookup_status!=='COMPLETE')return lookup.ap_match_count===null&&lookup.posted_journal_match_count===null&&lookup.ledger_line_match_count===null&&Array.isArray(lookup.ap_document_ids)&&lookup.ap_document_ids.length===0&&Array.isArray(lookup.journal_entry_ids)&&lookup.journal_entry_ids.length===0&&Array.isArray(lookup.ledger_line_ids)&&lookup.ledger_line_ids.length===0;
  return [lookup.ap_match_count,lookup.posted_journal_match_count,lookup.ledger_line_match_count].every(value=>Number.isSafeInteger(value)&&value>=0)&&uniqueIds(lookup.ap_document_ids)&&uniqueIds(lookup.journal_entry_ids)&&uniqueIds(lookup.ledger_line_ids)&&lookup.ap_document_ids.length===lookup.ap_match_count&&lookup.journal_entry_ids.length===lookup.posted_journal_match_count&&lookup.ledger_line_ids.length===lookup.ledger_line_match_count;
}

export const hashAiAdmittedSourceBookingEvidence=digest;

const blocked=(source,lookup)=>freeze({schema_version:'AI_ADMITTED_SOURCE_BOOKING_REVIEW_V1',status:'BLOCKED',finding:null,source_trace:{admission_id:source.admission_id,admission_hash:source.admission_hash,source_document_id:source.source_document_id,source_document_line_id:source.source_document_line_id,source_payload_hash:source.source_payload_hash,source_line_hash:source.source_line_hash},lookup_trace:{lookup_receipt_id:lookup.lookup_receipt_id,lookup_receipt_hash:lookup.lookup_receipt_hash,lookup_status:lookup.lookup_status},suggested_journal:null,action_flags:ACTIONS});
...
export function createAiAdmittedSourceUnbookedService({admittedSourceReader,accountingLookupReader}={}){
  ...
  return Object.freeze({analyze:async scope=>{
    ...
    const absent=lookup.ap_match_count===0&&lookup.posted_journal_match_count===0&&lookup.ledger_line_match_count===0;
    const finding=absent?freeze({schema_version:'AI_ADMITTED_SOURCE_UNBOOKED_FINDING_V1',finding_type:'ADMITTED_SOURCE_UNBOOKED',risk_level:'HIGH',rule_id:'ADMITTED_PAYABLE_WITH_ZERO_ACCOUNTING_MATCH_V1',tenant_id:source.tenant_id,entity_id:source.entity_id,company_code:source.company_code,accounting_period_id:source.accounting_period_id,vendor_ref:source.vendor_ref,business_date:source.business_date,accounting_date:source.accounting_date,currency:source.currency,amount:source.amount,reason:'An admitted Payable has no AP document, Posted Journal, or ledger line in the exact accounting scope.',suggested_action:'A human accountant must compare the admitted source with AP and GL evidence before deciding whether a Draft Journal is appropriate.',required_human_fields:['booking_determination','accounting_treatment','source_support_review','owner','due_date'],source_trace:{admission_id:source.admission_id,admission_hash:source.admission_hash,source_document_id:source.source_document_id,source_document_line_id:source.source_document_line_id,source_payload_hash:source.source_payload_hash,source_line_hash:source.source_line_hash},lookup_trace:{lookup_receipt_id:lookup.lookup_receipt_id,lookup_receipt_hash:lookup.lookup_receipt_hash,lookup_status:'COMPLETE',ap_match_count:0,posted_journal_match_count:0,ledger_line_match_count:0},suggested_journal:null,action_flags:ACTIONS}):null;
    return freeze({schema_version:'AI_ADMITTED_SOURCE_BOOKING_REVIEW_V1',status:absent?'FINDING':'BOOKING_PRESENT',finding,source_trace:{admission_id:source.admission_id,admission_hash:source.admission_hash,source_document_id:source.source_document_id,source_document_line_id:source.source_document_line_id,source_payload_hash:source.source_payload_hash,source_line_hash:source.source_line_hash},lookup_trace:{lookup_receipt_id:lookup.lookup_receipt_id,lookup_receipt_hash:lookup.lookup_receipt_hash,lookup_status:'COMPLETE'},suggested_journal:null,action_flags:ACTIONS});
  }});
}
