...
export function validSalesReceiptOptions(v,{entityId,optionKind,query='',afterRef=null,limit=50}){
 ...
 for(const row of v.rows){if(!exact(row,['ref','label','kind'])||!text(row.ref,optionKind.endsWith('_ACCOUNT')?64:128)||!text(row.label,Infinity)||!kinds.includes(row.kind)||previous!==null&&Buffer.compare(Buffer.from(previous,'utf8'),Buffer.from(row.ref,'utf8'))>=0)return false;previous=row.ref;}
