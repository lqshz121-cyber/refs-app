...
const validPolicy=policy=>policy&&JSON.stringify(Object.keys(policy).sort())===JSON.stringify(POLICY_FIELDS)&&policy.schema_version==='AI_VENDOR_INVOICE_FREQUENCY_ANOMALY_POLICY_V1'&&UUID.test(policy.setting_snapshot_id||'')&&SHA256.test(policy.setting_snapshot_hash||'')&&Number.isInteger(policy.policy_version)&&policy.policy_version>=1&&Number.isInteger(policy.minimum_history_periods)&&policy.minimum_history_periods>=3&&policy.minimum_history_periods<=24&&Number.isInteger(policy.frequency_ratio_threshold_basis_points)&&policy.frequency_ratio_threshold_basis_points>=20000&&policy.frequency_ratio_threshold_basis_points<=100000&&Number.isInteger(policy.minimum_excess_invoice_count)&&policy.minimum_excess_invoice_count>=2&&policy.minimum_excess_invoice_count<=100;
