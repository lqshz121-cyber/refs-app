...
const release = value => {
  ...
  assert.ok(SHA.test(text), 'REFS_RELEASE_SHA must be a 40-character Git SHA');
  return text;
};

export async function verifyRenderStagingRelease({releaseSha, apiBaseUrl, webOrigin, fetchImpl = globalThis.fetch} = {}) {
  const expected = release(releaseSha);
  const api = origin(apiBaseUrl);
  const web = origin(webOrigin);
  assert.equal(typeof fetchImpl, 'function', 'fetch implementation is required');
  const readJson = async url => {
    const response = await fetchImpl(url, { method:'GET', cache:'no-store', headers:{accept:'application/json'} });
  ...
  const [live, ready, buildResponse, anonymous] = await Promise.all([
    ...
    fetchImpl(`${api}/api/v1/accounting-scopes`, { method:'GET', cache:'no-store', headers:{accept:'application/json'} }),
  ]);
  ...
  return {ok:true,release:expected,api:{live:live.release,ready:ready.release},web:build.sha,anonymousStatus:anonymous.status};
}

if (process.argv[1] && import.meta.url === new URL(process.argv[1], 'file:').href) {
  const result = await verifyRenderStagingRelease({
    releaseSha:process.env.REFS_RELEASE_SHA,
    apiBaseUrl:process.env.REFS_STAGING_API_BASE_URL,
    webOrigin:process.env.REFS_STAGING_WEB_ORIGIN,
  });
