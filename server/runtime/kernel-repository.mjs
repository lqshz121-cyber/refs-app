...
export class PostgresAccountingKernel{
  ...
  async ensureInternalTestCashTransferEvidence({tenantId,entityId,idempotencyKey}){
    return this.inSession(async client=>requireRow(await client.query(
      'SELECT refs_ensure_internal_test_cash_transfer_evidence($1,$2,$3) AS result',[tenantId,entityId,idempotencyKey]
    ),'INTERNAL_TEST_CASH_TRANSFER_EVIDENCE_UNAVAILABLE','Internal test cash-transfer evidence could not be prepared').result);
  }
  ...
  async readAiAccountingDecisionPopulation({tenantId,entityId,accountingPeriodId,pageSize=250,maxRows=10000}){
    ...
    return this.inSession(async client=>{
      ...
      const invoiceRows=await scan({kind:'invoice',functionName:'refs_read_ai_invoice_decision_population_page',dateFields:['accounting_date','invoice_date','service_period_start','service_period_end','tax_coverage_period_start','tax_coverage_period_end']});
  ...
  async readCreditAllocationHistory({tenantId,entityId,subjectId,subjectKind,afterId=null,limit=50}){
    return this.inSession(async client=>requireRow(await client.query(
      ...
      [tenantId,entityId,subjectId,subjectKind,afterId,limit]
    ),'CREDIT_HISTORY_UNAVAILABLE','Credit allocation history is unavailable').result);
  }

  async readCreditAllocationTargets({tenantId,entityId,action,businessAdjustmentId,periodId,query='',afterId=null,limit=50}){
    return this.inSession(async client=>requireRow(await client.query(
      'SELECT refs_read_credit_allocation_targets($1,$2,$3,$4,$5,$6,$7,$8) AS result',
      [tenantId,entityId,action,businessAdjustmentId,periodId,query,afterId,limit]
    ),'CREDIT_TARGETS_UNAVAILABLE','Credit allocation targets are unavailable').result);
  }
  ...
  async readPeriodCloseHistory({tenantId,entityId,periodId,limit=25,cursorAt=null,cursorId=null}){
    return this.inSession(async client=>requireRow(await client.query(
      'SELECT refs_read_period_close_history($1,$2,$3,$4,$5::timestamptz,$6::uuid) AS result',
