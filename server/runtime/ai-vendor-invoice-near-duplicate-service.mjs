...
export function createAiVendorInvoiceNearDuplicateService({sourceReader,detailReader,evidenceReader,policyReader,materializeWriter=null}={}){
  ...
  const analyze=async({tenantId,entityId,currentAccountingPeriodId,limit=500})=>{
    ...
    if(!Array.isArray(documents)||!safeAiEvidenceTree(documents,{maxArrayLength:500})||!safeAiEvidenceTree(policy,{maxArrayLength:20}))throw Object.assign(new Error('Vendor near-duplicate source or policy evidence is unsafe.'),{code:'AI_VENDOR_NEAR_DUPLICATE_SOURCE_INVALID'});
    const rows=[],wbsDocuments=documents.filter(row=>row.source_system==='WBS');
    if(wbsDocuments.length>=limit)throw Object.assign(new Error('The bounded vendor near-duplicate source read cannot prove population completeness.'),{code:'AI_VENDOR_NEAR_DUPLICATE_POPULATION_INCOMPLETE'});
    for(const document of wbsDocuments){
      ...
      if(!safeAiEvidenceTree(details,{maxArrayLength:500})||!safeAiEvidenceTree(evidence,{maxArrayLength:100}))throw Object.assign(new Error('Vendor near-duplicate retained evidence is unsafe.'),{code:'AI_VENDOR_NEAR_DUPLICATE_SOURCE_INVALID'});
