...
export function assertControlledTestBankWorkflowResult(value){
  ...
  if(!exactObject(value,keys)||value.status!=='CONTROLLED_TEST_BANK_WORKFLOW_REOPENED'||value.test_only!==true||value.provenance_mode!=='CONTROLLED_TEST_UNSIGNED'
    ...
    ||value.processed_count>10000||value.matched_count>10000||value.adjusted_count>10000||value.cleared_count>10000
...
export function assertControlledTestBankWorkflowPartialResult(value){
  const keys=['adjusted_count','cleared_count','idempotent','matched_count','processed_count','provenance_mode','reconciliation_id','remaining_count','revision','status','test_only','total_count'];
  if(!exactObject(value,keys)||value.status!=='CONTROLLED_TEST_BANK_WORKFLOW_PARTIAL'||value.test_only!==true||value.provenance_mode!=='CONTROLLED_TEST_UNSIGNED'||value.idempotent!==false
    ||!UUID.test(value.reconciliation_id||'')||!['adjusted_count','cleared_count','matched_count','processed_count','remaining_count','revision','total_count'].every(key=>Number.isSafeInteger(value[key])&&value[key]>=0)
    ||value.total_count<1||value.total_count>10000||value.remaining_count<1||value.remaining_count!==value.total_count-value.cleared_count
    ||value.processed_count!==value.adjusted_count+value.matched_count||value.processed_count!==value.cleared_count){
    fail('CONTROLLED_TEST_BANK_RESULT_INVALID','Controlled-test Bank partial workflow result is incomplete or unsafe.');
  }
  return value;
}
...
export function assertControlledTestBankRangeWorkflowResult(value){
  ...
  if(!exactObject(value,keys)||value.status!=='CONTROLLED_TEST_BANK_RANGE_WORKFLOW_REOPENED'||value.test_only!==true||value.provenance_mode!=='CONTROLLED_TEST_UNSIGNED'||typeof value.idempotent!=='boolean'
    ...
    ||value.processed_count>60000||value.matched_count>60000||value.adjusted_count>60000||value.cleared_count>60000
...
export function createControlledTestBankWorkflowService({kernelForActor,authorize,scope}={}){
  ...
  const run=async({tenantId,entityId,periodId,reconciliationId,reason:reviewReason,idempotencyKey,maxItems=25}={})=>{
      assertSelection({tenantId,entityId,periodId,reconciliationId,reason:reviewReason,idempotencyKey},scope);
      if(!Number.isSafeInteger(maxItems)||maxItems<1||maxItems>100)fail('CONTROLLED_TEST_BANK_SELECTION_INVALID','Controlled-test Bank workflow maxItems must be an integer from 1 to 100.');
      ...
      const partialResult=rows=>{
        const counts=workflowCounts(rows),total=rows.length,remaining=total-counts.cleared_count;
        const revision=rows.length?Number(rows[0].reconciliation_version):Number(reconciliation.version);
        return Object.freeze(assertControlledTestBankWorkflowPartialResult({status:'CONTROLLED_TEST_BANK_WORKFLOW_PARTIAL',test_only:true,provenance_mode:'CONTROLLED_TEST_UNSIGNED',idempotent:false,
          reconciliation_id:reconciliationId,total_count:total,processed_count:counts.adjusted_count+counts.matched_count,matched_count:counts.matched_count,
          adjusted_count:counts.adjusted_count,cleared_count:counts.cleared_count,remaining_count:remaining,revision}));
      };
