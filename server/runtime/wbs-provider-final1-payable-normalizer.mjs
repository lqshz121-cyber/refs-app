...
function typedDocumentEvidence(row){
  ...
  if(row.document_kind==='INVOICE'){
    if(Object.hasOwn(row,'tax_year')&&row.tax_year!==null||taxValues.some(value=>value!==null))fail('WBS_FINAL1_PAYABLE_DOCUMENT_EVIDENCE_CONTRADICTORY','An INVOICE must carry explicit nulls for every tax-statement field.');
    ...
    if(!Object.hasOwn(row,'tax_year')||!Number.isSafeInteger(row.tax_year)||row.tax_year<1900||row.tax_year>9999||!taxValues.every(value=>typeof value==='string'&&value.trim().length>0)||!exactDate(row.tax_coverage_period_start)||!exactDate(row.tax_coverage_period_end)||row.tax_coverage_period_start>row.tax_coverage_period_end||Number(row.tax_coverage_period_start.slice(0,4))!==row.tax_year||Number(row.tax_coverage_period_end.slice(0,4))!==row.tax_year||!TAX_OBLIGATION_BASES.includes(row.tax_obligation_basis))fail('WBS_FINAL1_PAYABLE_DOCUMENT_EVIDENCE_INVALID','A TAX_STATEMENT requires an explicit signed tax year consistent with exact statutory coverage.');
...
function normalizedRow(row,{verified,expectedCurrency,currencyAuthority,sourceRowOrdinal}){
  ...
  return freeze({
    ...
    normalized:freeze({
      ...
      documentEvidenceStatus:documentEvidence.status,documentEvidenceSchemaVersion:documentEvidence.schemaVersion,documentKind:documentEvidence.documentKind,taxYear:documentEvidence.taxYear,
