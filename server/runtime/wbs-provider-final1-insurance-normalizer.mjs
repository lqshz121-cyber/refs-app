...
function normalizeRow(row,verified){
  ...
  return deepFreeze({source_system:'WBS',source_module:'payable',source_domain:'insurance',source_record_id:row.policy_id,source_primary_key:String(row.id),source_surface:{database:'wb_insurance',table:'insurance_data',stable_keys:['id','policy_id']},source_version:`final1:${verified.snapshot_id}:${rawRowHash.slice(7,23)}`,raw_row:rawRow,raw_row_hash:rawRowHash,provider_package_hash:verified.package_hash,provider_raw_package_hash:verified.raw_package_hash,provider_snapshot_id:verified.snapshot_id,provider_company_code:verified.company_code,company_mapping_hash:verified.company_mapping_hash,currency:'USD',currency_authority:'REFS_BUSINESS_OWNER_CONFIRMED_CURRENCY',normalized:{policyId:row.policy_id,sourceId:String(row.id),pcCode:row.pc_code||null,propertyCode:row.property_code||null,unitCode:row.unit_code||null,policyNumber:row.policy_number||null,carrier:row.carrier||null,insuranceType:row.insurance_type||null,finalPremium:row.final_premium,startDate:row.start_date||null,expireDate:row.expire_date||null,attachmentCount:row.attachment_count??null,policyAttachmentId:row.policy_attachment_id||null},outcome:candidate?'AMORTIZATION_COVERAGE_EVIDENCE_CANDIDATE':'EXCEPTION_REVIEW_REQUIRED',exception_codes:exceptions,can_retain_evidence:true,can_propose_amortization:false,can_create_draft:false,can_review:false,can_approve:false,can_post:false});
}
...
export function normalizeVerifiedWbsProviderFinal1Insurance({verified,expectedCurrency='USD'}={}){
  ...
  const rows=verified.package.views.list_insurance.rows.map(row=>normalizeRow(row,verified));
  if(rows.length===0)fail('WBS_FINAL1_INSURANCE_NORMALIZATION_EMPTY','Insurance evidence must contain at least one signed row.');
  const provenance=deepFreeze({tenant_id:verified.tenant_id,entity_id:verified.entity_id,company_code:verified.company_code,company_mapping_hash:verified.company_mapping_hash,snapshot_id:verified.snapshot_id,source_surface:{database:'wb_insurance',table:'insurance_data',stable_keys:['id','policy_id']},source_row_count:rows.length,currency:'USD',currency_authority:'REFS_BUSINESS_OWNER_CONFIRMED_CURRENCY'});
