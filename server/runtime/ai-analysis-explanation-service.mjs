...
const evidenceRow=(category,value)=>{
  ...
  const sourceRefs=['source_document_id','candidate_source_document_id','source_document_line_id','source_record_id','source_evidence_row_id','external_bank_line_id','journal_entry_id'].map(key=>bounded(value[key],128)).filter(Boolean);
  const evidenceHashes=['source_payload_hash','candidate_payload_hash','source_line_hash','source_row_hash','provider_content_hash','observation_hash','match_key_hash','finding_hash'].map(key=>text(value[key])).filter(value=>HASH.test(value));
  ...
  if(!ruleId||!reason||!suggestedAction||!createdAt||sourceRefs.length===0||evidenceHashes.length===0)return null;
...
const validAction=(action,{traceById,requireTrace=false}={})=>{
  if(!action||typeof action!=='object'||!CATEGORIES.has(action.category)||text(action.action).length<1||text(action.action).length>1000)return false;
  const keys=Object.keys(action).sort().join('|');if(requireTrace&&keys!=='action|category|finding_ids')return false;if(!requireTrace&&keys!=='action|category'&&keys!=='action|category|finding_ids')return false;
  if(!Array.isArray(action.finding_ids))return !requireTrace;
  if(action.finding_ids.length<1||action.finding_ids.length>10||new Set(action.finding_ids).size!==action.finding_ids.length||!action.finding_ids.every(id=>UUID.test(id)))return false;
  return !traceById||action.finding_ids.every(id=>traceById.get(id)===action.category);
};
...
export function createAiAnalysisExplanationService({gateway,summaryReader,evidenceReader,auditRepository}={}){
  ...
  if(typeof summaryReader!=='function')throw new AiAnalysisExplanationError('AI_SUMMARY_READER_REQUIRED','AI analysis requires an authoritative summary reader');
  if(typeof evidenceReader!=='function')throw new AiAnalysisExplanationError('AI_EVIDENCE_READER_REQUIRED','AI analysis requires an authoritative retained-evidence reader');
  if(!auditRepository||typeof auditRepository.beginAiAccountingAnalysisExplanation!=='function'||typeof auditRepository.completeAiAccountingAnalysisExplanation!=='function'||typeof auditRepository.abandonAiAccountingAnalysisExplanation!=='function')throw new AiAnalysisExplanationError('AI_ANALYSIS_AUDIT_REQUIRED','AI analysis requires a durable audit receipt repository');
  return Object.freeze({async explain({tenantId,entityId,actorId,traceId}={}){
    ...
    try{
      const output=await gateway.analyzeJson({traceId,traceName:'refs-ai-accounting-analysis-summary',actorId,facts:{schema_version:'REFS_AI_ACCOUNTING_ANALYSIS_EVIDENCE_V1',tenant_scope:'AUTHORIZED',entity_id:entityId,findings:summary,retained_evidence:evidence},systemInstruction:'You are REFS Accounting Brain. Explain only the provided retained finding counts and redacted, source-traceable finding evidence. Do not infer unprovided facts, accounting entries, accounts, balances, entities, sources, attachments, or approvals. Return a concise controller memo with review actions only. Never claim an item was posted, approved, reconciled, or complete.',jsonSchema:responseSchema});
