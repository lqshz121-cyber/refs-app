...
import {analyzeAttestedConstructionLoanDrawCwip} from './ai-construction-loan-cwip-population-attestation.mjs';
...
import {createAiIntercompanyControllerScanService} from './ai-intercompany-controller-scan-service.mjs';
import {detectCrossEntityPaymentInvoiceReviews} from './ai-cross-entity-payment-invoice-review.mjs';
import {assertCompleteAiPopulation} from './ai-bounded-population.mjs';
...
const WBS_TEST_IMPORT_READINESS="SELECT to_regprocedure('refs_create_wbs_test_payable_draft_hash(uuid,uuid,uuid,text)') IS NOT NULL AND to_regprocedure('refs_create_wbs_test_payable_draft(uuid,uuid,uuid,text,text,text)') IS NOT NULL AND to_regprocedure('refs_create_wbs_controlled_test_bank_scope_hash(uuid,uuid,uuid,text,jsonb,text)') IS NOT NULL AND to_regprocedure('refs_begin_wbs_test_bank_staged_import(uuid,uuid,uuid,text,jsonb,text,text,text)') IS NOT NULL AND to_regprocedure('refs_append_wbs_test_bank_staged_chunk(uuid,uuid,uuid,integer,jsonb,text)') IS NOT NULL AND to_regprocedure('refs_finalize_wbs_test_bank_import_receipt(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('refs_start_wbs_test_bank_reconciliation(uuid,uuid,uuid,text,text)') IS NOT NULL AND to_regprocedure('refs_list_reconciliation_adjustment_evidence(uuid,uuid,integer)') IS NOT NULL AND to_regprocedure('refs_wbs_test_bank_adjustment_draft_batch(uuid,uuid,uuid,uuid,uuid[],uuid[],text,text)') IS NOT NULL AND to_regprocedure('refs_wbs_test_bank_adjustment_submit_batch(uuid,uuid,uuid,uuid[],text)') IS NOT NULL AND to_regprocedure('refs_wbs_test_bank_adjustment_review_batch(uuid,uuid,uuid,uuid[],text)') IS NOT NULL AND to_regprocedure('refs_wbs_test_bank_adjustment_approve_batch(uuid,uuid,uuid,uuid[],text)') IS NOT NULL AND to_regprocedure('refs_wbs_test_bank_adjustment_post_batch(uuid,uuid,uuid,uuid,uuid[],text,text)') IS NOT NULL AND to_regprocedure('refs_wbs_test_bank_adjustment_clear_batch(uuid,uuid,uuid,uuid[],text,text)') IS NOT NULL AND to_regprocedure('refs_resolve_wbs_test_bank_match_fixture(uuid,uuid)') IS NOT NULL AS ready";
...
const AI_FULL_CONTROLLER_MODEL_READINESS="SELECT to_regprocedure('refs_prepare_ai_full_controller_model_run(uuid,uuid,uuid,text,text,jsonb,text)') IS NOT NULL AND to_regprocedure('refs_begin_ai_full_controller_model_run(uuid,uuid,uuid,text,text,jsonb,text)') IS NOT NULL AND to_regprocedure('refs_begin_ai_full_controller_model_chunk(uuid,text,text,text,integer,text)') IS NOT NULL AND to_regprocedure('refs_complete_ai_full_controller_model_chunk(uuid,text,text,text,integer,text,jsonb)') IS NOT NULL AND to_regprocedure('refs_begin_ai_full_controller_model_memo(uuid,text,text,text,jsonb,jsonb)') IS NOT NULL AND to_regprocedure('refs_complete_ai_full_controller_model_run(uuid,text,text,text,jsonb)') IS NOT NULL AND to_regprocedure('refs_abandon_ai_full_controller_model_stage(uuid,text,text,text,text)') IS NOT NULL AS ready";
const ACCOUNTING_SETTINGS_WORKFLOW_READINESS="SELECT to_regclass('public.accounting_settings_workflow') IS NOT NULL AND to_regclass('public.accounting_settings_workflow_history') IS NOT NULL AND to_regprocedure('public.refs_read_accounting_settings_workflow(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('public.refs_read_accounting_settings_workflow_evidence(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('public.refs_read_accounting_settings_workflow_options(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('public.refs_read_accounting_settings_workflow_register(uuid,uuid,integer)') IS NOT NULL AND to_regprocedure('public.refs_accounting_settings_create_hash(uuid,uuid,uuid,jsonb,text)') IS NOT NULL AND to_regprocedure('public.refs_accounting_settings_transition_hash(uuid,uuid,uuid,text,bigint,text)') IS NOT NULL AND to_regprocedure('public.refs_create_accounting_settings_workflow(uuid,uuid,uuid,jsonb,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_transition_accounting_settings_workflow(uuid,uuid,uuid,text,bigint,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_read_wbs_ai_approved_entity_period_settings(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('public.refs_read_authoritative_setting_history(uuid,uuid,text,integer,bigint,uuid)') IS NOT NULL AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_accounting_settings_workflow(uuid,uuid,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_accounting_settings_workflow_evidence(uuid,uuid,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_accounting_settings_workflow_options(uuid,uuid,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_accounting_settings_workflow_register(uuid,uuid,integer)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_accounting_settings_create_hash(uuid,uuid,uuid,jsonb,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_accounting_settings_transition_hash(uuid,uuid,uuid,text,bigint,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_create_accounting_settings_workflow(uuid,uuid,uuid,jsonb,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_transition_accounting_settings_workflow(uuid,uuid,uuid,text,bigint,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_wbs_ai_approved_entity_period_settings(uuid,uuid,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_authoritative_setting_history(uuid,uuid,text,integer,bigint,uuid)'),'EXECUTE') AS ready";
const RECENT_ACCOUNTING_WORKFLOW_READINESS="SELECT to_regclass('public.report_saved_view') IS NOT NULL AND to_regclass('public.webhook_subscription') IS NOT NULL AND to_regclass('public.forecast_workflow') IS NOT NULL AND to_regclass('public.import_export_history_job') IS NOT NULL AND to_regclass('public.recurring_schedule') IS NOT NULL AND to_regclass('public.recurring_schedule_run') IS NOT NULL AND to_regprocedure('public.refs_read_report_saved_views(uuid,uuid,integer,timestamptz,uuid)') IS NOT NULL AND to_regprocedure('public.refs_create_report_saved_view(uuid,uuid,text,text,uuid,jsonb,text,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_update_report_saved_view(uuid,uuid,uuid,bigint,text,text,uuid,jsonb,text,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_read_webhook_subscription(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('public.refs_create_webhook_subscription(uuid,uuid,text,text,text,text[],text,text,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_transition_webhook_subscription(uuid,uuid,uuid,text,bigint,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_read_webhook_delivery_history(uuid,uuid,uuid,integer,timestamptz,uuid)') IS NOT NULL AND to_regprocedure('public.refs_read_forecast_workflow(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('public.refs_create_forecast_workflow(uuid,uuid,uuid,text,text,jsonb,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_transition_forecast_workflow(uuid,uuid,uuid,text,bigint,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_read_import_export_history(uuid,uuid,text,integer,timestamptz,uuid)') IS NOT NULL AND to_regprocedure('public.refs_record_posted_ledger_export_history(uuid,uuid,uuid,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_read_recurring_schedule(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('public.refs_create_recurring_schedule(uuid,uuid,text,text,text,text,date,date,jsonb,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_transition_recurring_schedule(uuid,uuid,uuid,text,bigint,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_run_due_recurring_schedules(uuid,uuid,date,integer,text,text)') IS NOT NULL AND to_regprocedure('public.refs_parse_source_document(uuid,uuid,uuid,uuid,text,text,text,text,text)') IS NOT NULL AND to_regprocedure('public.refs_read_custom_report(uuid,uuid,uuid,text,text,text,integer,text)') IS NOT NULL AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_report_saved_views(uuid,uuid,integer,timestamptz,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_create_report_saved_view(uuid,uuid,text,text,uuid,jsonb,text,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_update_report_saved_view(uuid,uuid,uuid,bigint,text,text,uuid,jsonb,text,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_webhook_subscription(uuid,uuid,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_create_webhook_subscription(uuid,uuid,text,text,text,text[],text,text,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_transition_webhook_subscription(uuid,uuid,uuid,text,bigint,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_webhook_delivery_history(uuid,uuid,uuid,integer,timestamptz,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_forecast_workflow(uuid,uuid,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_create_forecast_workflow(uuid,uuid,uuid,text,text,jsonb,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_transition_forecast_workflow(uuid,uuid,uuid,text,bigint,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_import_export_history(uuid,uuid,text,integer,timestamptz,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_record_posted_ledger_export_history(uuid,uuid,uuid,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_recurring_schedule(uuid,uuid,uuid)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_create_recurring_schedule(uuid,uuid,text,text,text,text,date,date,jsonb,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_transition_recurring_schedule(uuid,uuid,uuid,text,bigint,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_run_due_recurring_schedules(uuid,uuid,date,integer,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_parse_source_document(uuid,uuid,uuid,uuid,text,text,text,text,text)'),'EXECUTE') AND has_function_privilege('refs_app',to_regprocedure('public.refs_read_custom_report(uuid,uuid,uuid,text,text,text,integer,text)'),'EXECUTE') AS ready";
...
export function createProductionAccountingServer({runtimePool,issuerPool,grantSyncPool,stage1SelfGrant,stage1SelfWbsReadUpgrade,stage1SelfWbsOperatorUpgrade,stage1SelfControlledTestWorkflowUpgrade,authenticator,attachmentStorage,wbsImmutableEvidenceStorage,virusScanner,scannerServiceActorId,wbsSnapshotVerifier,wbsSignedBankAdmissionVerifier,wbsAutoRecTransitionContractVerifier,wbsLivePilotClient,wbsTestImport,controlledTestAiWorkflow,wbsProviderSignedTrust,wbsProviderSignedServiceActorId,aiGateway,runtimeLoginAllowlist=['refs_runtime'],maxBodyBytes,releaseSha,allowedOrigins=[],internalTest=null,internalTestPrincipalRouter=null}={}){
  ...
  const aiAdmittedSourceUnbookedServiceFactory=principal=>{const kernel=kernelFor(principal);return createAiAdmittedSourceUnbookedAnalysisService({
    bookingEvidenceReader:scope=>kernel.listAiAdmittedSourceBookingEvidence(scope)
  });};
  const aiFullControllerScanServiceFactory=principal=>{
    ...
    const admittedSourceUnbooked={analyze:input=>aiAdmittedSourceUnbookedServiceFactory(principal).analyze({...input,limit:Math.min(input.limit,500)})};
    ...
    const constructionLoan={analyze:async input=>{const scope={tenantId:input.tenantId,entityId:input.entityId,periodId:input.currentAccountingPeriodId},[glRows,statementRows,policy]=await Promise.all([kernel.getAiConstructionLoanGlBalances(scope),kernel.getAiConstructionLoanLenderBalances(scope),kernel.getAiConstructionLoanBalancePolicy(scope)]);if(!policy)throw Object.assign(new Error('Approved construction-loan balance policy is required.'),{code:'AI_CONSTRUCTION_LOAN_BALANCE_POLICY_REQUIRED'});return Object.freeze({...detectConstructionLoanBalanceReviews(glRows,statementRows,{entityId:input.entityId,accountingPeriodId:input.currentAccountingPeriodId,policy,limit:input.limit}),current_accounting_period_id:input.currentAccountingPeriodId});}};
    const constructionLoanDrawCwip={analyze:async input=>{const scope={tenantId:input.tenantId,entityId:input.entityId,accountingPeriodId:input.currentAccountingPeriodId},[attestation,policy]=await Promise.all([kernel.getAiConstructionLoanCwipPopulationAttestation(scope),kernel.getAiConstructionLoanDrawCwipPolicy(scope)]);if(!policy)throw Object.assign(new Error('Approved construction-loan draw to CWIP policy is required.'),{code:'AI_LOAN_DRAW_CWIP_POLICY_REQUIRED'});return analyzeAttestedConstructionLoanDrawCwip(attestation,{...scope,policy});}};
    ...
    const prepaidAmortization={analyze:async input=>{const populationLimit=Math.min(input.limit,100),rows=assertCompleteAiPopulation(await kernel.listInsurancePrepaidAmortization({tenantId:input.tenantId,entityId:input.entityId,periodId:input.currentAccountingPeriodId,limit:populationLimit}),{limit:populationLimit,code:'AI_PREPAID_AMORTIZATION_POPULATION'});return projectPrepaidAmortizationControllerReviews(rows,{entityId:input.entityId,currentAccountingPeriodId:input.currentAccountingPeriodId});}};
    ...
    const intercompany=createAiIntercompanyControllerScanService({pairReader:scope=>kernel.listAiIntercompanyCounterpartyPeriods(scope),reconciliationReader:scope=>kernel.getIntercompanyReconciliation(scope),detector:detectIntercompanyCloseReviews,paymentInvoiceReader:scope=>kernel.readAiCrossEntityPaymentInvoices(scope),paymentInvoiceDetector:detectCrossEntityPaymentInvoiceReviews});
    const bankReconciliation={analyze:async input=>{const populationLimit=Math.min(input.limit,100),findings=assertCompleteAiPopulation(await kernel.listAiUnmatchedBankPaymentFindingsForPeriod({tenantId:input.tenantId,entityId:input.entityId,periodId:input.currentAccountingPeriodId,limit:populationLimit}),{limit:populationLimit,code:'AI_BANK_RECONCILIATION_POPULATION'});return Object.freeze({schema_version:'AI_BANK_RECONCILIATION_EXCEPTION_SCAN_BATCH_V1',current_accounting_period_id:input.currentAccountingPeriodId,finding_count:findings.length,findings:Object.freeze(findings),action_flags:actions});}};
    const retainedFinding=(schemaVersion,reader,populationCode)=>({analyze:async input=>{const populationLimit=Math.min(input.limit,100),rows=assertCompleteAiPopulation(await reader({tenantId:input.tenantId,entityId:input.entityId,periodId:input.currentAccountingPeriodId,limit:populationLimit}),{limit:populationLimit,code:populationCode}),findings=rows.map(row=>Object.freeze({...row,entity_id:input.entityId,accounting_period_id:input.currentAccountingPeriodId,action_flags:actions}));return Object.freeze({schema_version:schemaVersion,current_accounting_period_id:input.currentAccountingPeriodId,finding_count:findings.length,findings:Object.freeze(findings),action_flags:actions});}});
    const prepaidCoverage=retainedFinding('AI_PREPAID_COVERAGE_SCAN_BATCH_V1',scope=>kernel.listAiPrepaidCoverageFindingsForPeriod(scope),'AI_PREPAID_COVERAGE_POPULATION');
    const duplicatePayable=retainedFinding('AI_DUPLICATE_PAYABLE_SCAN_BATCH_V1',scope=>kernel.listAiDuplicatePayableFindingsForPeriod(scope),'AI_DUPLICATE_PAYABLE_POPULATION');
    const costDimension=retainedFinding('AI_COST_DIMENSION_SCAN_BATCH_V1',scope=>kernel.listAiCostDimensionFindingsForPeriod(scope),'AI_COST_DIMENSION_POPULATION');
    const loanReference=retainedFinding('AI_LOAN_REFERENCE_SCAN_BATCH_V1',scope=>kernel.listAiLoanReferenceFindingsForPeriod(scope),'AI_LOAN_REFERENCE_POPULATION');
    ...
    return createAiFullControllerScanService({analyzers:{
      ...
      ADMITTED_SOURCE_UNBOOKED:admittedSourceUnbooked,
