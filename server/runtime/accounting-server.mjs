...
const INSURANCE_PC_MAPPING_READINESS="SELECT to_regprocedure('refs_record_wbs_insurance_pc_mapping_pre_admission(uuid,uuid,jsonb,jsonb)') IS NOT NULL AND to_regprocedure('refs_propose_wbs_insurance_pc_mapping_hash(uuid,uuid,uuid,text,text)') IS NOT NULL AND to_regprocedure('refs_create_wbs_insurance_pc_mapping_proposal(uuid,uuid,uuid,text,text,text,text)') IS NOT NULL AND to_regprocedure('refs_approve_wbs_insurance_pc_mapping_hash(uuid,uuid,uuid,bigint,text,text,uuid,text,date,date,text)') IS NOT NULL AND to_regprocedure('refs_approve_wbs_insurance_pc_mapping_proposal(uuid,uuid,uuid,bigint,text,text,uuid,text,date,date,text,text,text)') IS NOT NULL AND to_regprocedure('refs_read_wbs_insurance_pc_mapping_proposal(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('refs_read_wbs_insurance_pc_mapping_trace(uuid,uuid,text,date)') IS NOT NULL AND to_regprocedure('refs_read_wbs_insurance_pc_mapping_admission_resume(uuid,uuid,uuid,text,uuid,text,text)') IS NOT NULL AND to_regprocedure('refs_ap_control_total(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('refs_ar_control_total(uuid,uuid,uuid)') IS NOT NULL AND to_regprocedure('refs_record_wbs_final1_signed_control_total(uuid,uuid,uuid,integer,jsonb,text)') IS NOT NULL AND to_regprocedure('refs_retain_wbs_final1_business_evidence(uuid,uuid,jsonb,jsonb,jsonb,text,text)') IS NOT NULL AND to_regprocedure('refs_retain_wbs_final1_source_evidence_with_signed_controls(uuid,uuid,jsonb,jsonb,jsonb,text,text)') IS NOT NULL AS ready";
