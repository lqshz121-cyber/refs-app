...
import {S3AttachmentStorage,S3ImmutableEvidenceStorage,HttpVirusScanner} from './attachment-storage.mjs';
...
import {stage1SelfControlledTestWorkflowUpgradeConfig,stage1SelfGrantConfig,stage1SelfWbsOperatorUpgradeConfig,stage1SelfWbsReadUpgradeConfig} from './stage1-bootstrap.mjs';
...
export function accountingServerConfig(env=process.env){
  ...
  const attachmentKeys=[...s3Keys,'VIRUS_SCANNER_ENDPOINT','VIRUS_SCANNER_TOKEN','VIRUS_SCANNER_SERVER_NAME','ATTACHMENT_SCANNER_ACTOR_ID'];
  if(attachmentMode==='REQUIRED')requireAll(env,attachmentKeys);
  if(wbsIngestMode==='REQUIRED')requireAll(env,[...s3Keys,'REFS_WBS_EVIDENCE_RETENTION_DAYS']);
  ...
  if(internalTest&&origins.length!==1)throw new Error('REFS_INTERNAL_TEST_MODE requires exactly one HTTPS REFS_HTTP_ALLOWED_ORIGINS value');
