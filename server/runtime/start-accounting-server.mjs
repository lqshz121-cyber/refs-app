...
import {stage1SelfGrantConfig} from './stage1-bootstrap.mjs';
...
export async function startAccountingServer({env=process.env,fetcher=globalThis.fetch,logger=console}={}){
  ...
  const grantSyncPool=config.stage1SelfGrant?await createPool({databaseUrl:config.database.grantSyncDatabaseUrl,applicationName:'refs-accounting-http-stage1-grant',max:1}):null;
  ...
  const server=createProductionAccountingServer({runtimePool,issuerPool,grantSyncPool,stage1SelfGrant:config.stage1SelfGrant,authenticator,attachmentStorage,virusScanner,scannerServiceActorId:config.scanner?.actorId,wbsSnapshotVerifier,wbsAutoRecTransitionContractVerifier,maxBodyBytes:config.maxBodyBytes,allowedOrigins:config.allowedOrigins});
