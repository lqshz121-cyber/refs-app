...
const validPolicy=policy=>policy&&safeAiEvidenceTree(policy,{maxArrayLength:20})&&JSON.stringify(Object.keys(policy).sort())===JSON.stringify(POLICY_FIELDS)&&policy.schema_version==='AI_VENDOR_INVOICE_AMOUNT_DROP_POLICY_V1'&&UUID.test(policy.setting_snapshot_id||'')&&SHA256.test(policy.setting_snapshot_hash||'')&&Number.isSafeInteger(policy.policy_version)&&policy.policy_version>=1&&Number.isSafeInteger(policy.minimum_history_periods)&&policy.minimum_history_periods>=3&&policy.minimum_history_periods<=24&&Number.isSafeInteger(policy.drop_ratio_threshold_basis_points)&&policy.drop_ratio_threshold_basis_points>=500&&policy.drop_ratio_threshold_basis_points<=7500&&MONEY4.test(policy.minimum_absolute_drop||'');
