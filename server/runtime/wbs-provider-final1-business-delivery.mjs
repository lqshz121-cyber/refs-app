...
export function verifyWbsProviderFinal1BusinessDelivery({providerTrust,receipt,requestRaw,responseRaw,packageRaw,expectedScope,domain,now=Date.now()}={}){
  ...
  const receiptKeys=['issuer','kid','algorithm','request_sha256','response_sha256','package_hash','nonce','signed_at','expires_at','tenant_id','entity_id','company_code','immutable_version','nonempty','detached_signature'];
  ...
  const packageKeys=['schema_version','snapshot_id','captured_at','environment','source_system','domain','source_tool','company_key','date_from','date_to','views','package_hash','detached_signature'];
  ...
  const view=pkg.views[config.tool],viewKeys=['scope','row_count','content_hash','control_totals','control_totals_hash','rows'];
  try{validateWbsFinal1SignedControlTotals({control_totals:view?.control_totals,control_totals_hash:view?.control_totals_hash},{label:'Package view'});}catch{fail('WBS_FINAL1_CONTROL_INVALID','Signed business control totals are malformed or hash-mismatched.');}
  ...
  try{validateWbsFinal1SignedControlTotals({control_totals:view.control_totals,control_totals_hash:view.control_totals_hash},{label:'Package view',expected:computed});}catch{fail('WBS_FINAL1_CONTROL_INVALID','Signed business control totals differ from the exact signed row population.');}
  ...
  const result=deepFreeze({status:'VERIFIED_FINAL1_BUSINESS_EVIDENCE_ONLY',format:'WBS_PROVIDER_FINAL1_BUSINESS_V1',signature_verified:true,tenant_id:expectedScope.tenant_id,entity_id:expectedScope.entity_id,company_code:expectedScope.company_code,domain,source_tool:config.tool,snapshot_id:pkg.snapshot_id,date_from:pkg.date_from,date_to:pkg.date_to,row_count:rows.length,control_totals:computed.control_totals,control_totals_hash:computed.control_totals_hash,package_hash:pkg.package_hash,raw_package_hash:receipt.package_hash,raw_contains_credentials:rawContainsCredentials,admission_blockers:rawContainsCredentials?['RAW_ARTIFACT_CREDENTIAL_REDACTION_REQUIRED']:[],package:pkg,can_persist:false,can_create_transaction:false,can_create_draft:false,can_review:false,can_approve:false,can_post:false});
