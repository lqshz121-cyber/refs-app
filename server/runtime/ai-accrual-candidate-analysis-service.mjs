...
import {adaptRetainedWbsPayableForAiAccrual} from './ai-accrual-retained-source-adapter.mjs';

const text=value=>typeof value==='string'?value.trim():'';
const UUID=/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const PERIOD=/^\d{4}-\d{2}$/;
const scope=value=>value&&typeof value==='object'&&!Array.isArray(value)&&UUID.test(text(value.tenantId))&&UUID.test(text(value.entityId))&&UUID.test(text(value.currentPeriodId))&&PERIOD.test(text(value.currentPeriodKey))&&Number.isSafeInteger(value.currentPeriodOrdinal)&&value.currentPeriodOrdinal>=0;
...
export function createAiAccrualCandidateAnalysisService({retainedHistoryReader,currentSourceReader,postedSourceReader}={}){
  ...
  return Object.freeze({async analyze(input={}){
    ...
    const request=Object.freeze({tenantId:text(input.tenantId),entityId:text(input.entityId),currentPeriodId:text(input.currentPeriodId),currentPeriodKey:text(input.currentPeriodKey),currentPeriodOrdinal:input.currentPeriodOrdinal});
    ...
    const history=rawHistory.map(adaptRetainedWbsPayableForAiAccrual);
    ...
    return Object.freeze({status:'AI_ACCRUAL_ANALYSIS_COMPLETE',entity_id:request.entityId,accounting_period_id:request.currentPeriodId,candidates:Object.freeze(candidates),can_create_draft:false,can_review:false,can_approve:false,can_post:false});
  }});
}
