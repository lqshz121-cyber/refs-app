...
export function assertAiAccountingDecisionPacketBatch(value,{tenantId,entityId,accountingPeriodId}){
  const batchKeys=['action_flags','decision_counts','packets','row_count','schema_version','scope'];
  const packetKeys=['accounting_period_id','action_flags','classification','confidence','entity_id','proposed_journal','reason','report_impact','required_human_fields','risk_level','rule_id','schema_version','settings_trace','source_trace','status','trace'];
  const falseActions=item=>exact(item,Object.keys(ACTIONS))&&Object.keys(ACTIONS).every(key=>item[key]===false);
  const validPacket=packet=>exact(packet,packetKeys)&&packet.schema_version==='AI_ACCOUNTING_DECISION_PACKET_V1'&&packet.entity_id===entityId&&packet.accounting_period_id===accountingPeriodId&&
    ['READY_FOR_HUMAN_REVIEW','EXCEPTION'].includes(packet.status)&&['EXPENSE','PREPAID_AMORTIZATION','ACCRUAL_REVIEW','CAPITALIZATION_REVIEW','BLOCKED'].includes(packet.classification)&&
    text(packet.rule_id,128)&&text(packet.reason,2000)&&typeof packet.confidence==='number'&&packet.confidence>=0&&packet.confidence<=1&&['LOW','MEDIUM','HIGH'].includes(packet.risk_level)&&Array.isArray(packet.required_human_fields)&&
    exact(packet.source_trace,['source_document_id','source_document_line_id','source_line_hash','source_payload_hash','vendor_ref'])&&UUID.test(packet.source_trace.source_document_id||'')&&UUID.test(packet.source_trace.source_document_line_id||'')&&SHA.test(packet.source_trace.source_payload_hash||'')&&SHA.test(packet.source_trace.source_line_hash||'')&&text(packet.source_trace.vendor_ref,200)&&
    exact(packet.settings_trace,['snapshot_hash','snapshot_id','status','version'])&&UUID.test(packet.settings_trace.snapshot_id||'')&&SHA.test(packet.settings_trace.snapshot_hash||'')&&packet.settings_trace.status==='APPROVED'&&Number.isSafeInteger(packet.settings_trace.version)&&packet.settings_trace.version>0&&
    exact(packet.proposed_journal,['balanced','lines','status'])&&packet.proposed_journal.status==='SUGGESTED_ONLY'&&typeof packet.proposed_journal.balanced==='boolean'&&Array.isArray(packet.proposed_journal.lines)&&
    exact(packet.report_impact,['balance_sheet','cash_flow','income_statement'])&&['balance_sheet','cash_flow','income_statement'].every(key=>Array.isArray(packet.report_impact[key]))&&
    exact(packet.trace,['decision_to_draft','decision_to_posted_ledger','decision_to_report','settings_to_decision','source_to_decision'])&&packet.trace.source_to_decision===true&&packet.trace.settings_to_decision===true&&packet.trace.decision_to_draft===false&&packet.trace.decision_to_posted_ledger===false&&packet.trace.decision_to_report===false&&falseActions(packet.action_flags)&&
    (packet.status==='EXCEPTION'?packet.classification==='BLOCKED'&&packet.proposed_journal.lines.length===0&&packet.proposed_journal.balanced===false:packet.classification!=='BLOCKED'&&packet.proposed_journal.lines.length===2&&packet.proposed_journal.balanced===true);
  if(!exact(value,batchKeys)||value.schema_version!=='AI_ACCOUNTING_DECISION_PACKET_BATCH_V1'||!exact(value.scope,['accounting_period_id','entity_id','tenant_id'])||value.scope.tenant_id!==tenantId||value.scope.entity_id!==entityId||value.scope.accounting_period_id!==accountingPeriodId||!Number.isSafeInteger(value.row_count)||value.row_count<0||value.row_count>500||!Array.isArray(value.packets)||value.packets.length!==value.row_count||!safeAiEvidenceTree(value)||value.packets.some(packet=>!validPacket(packet))||!exact(value.decision_counts,['exception','ready_for_human_review'])||value.decision_counts.exception+value.decision_counts.ready_for_human_review!==value.row_count||!falseActions(value.action_flags))throw error('AI_ACCOUNTING_DECISION_RESPONSE_INVALID','AI accounting decision response is unsafe or incomplete.');
  return value;
}
