...
const validRow=row=>row&&Object.keys(row).sort().join('|')==='aging_date|business_document_id|currency|document_number|entity_id|movement_kind|open_amount|posted_journal_entry_id|source_document_id|source_payload_hash|tenant_id|vendor_name|vendor_ref'&&[row.tenant_id,row.entity_id,row.business_document_id,row.posted_journal_entry_id].every(value=>UUID.test(value||''))&&((row.source_document_id===null&&row.source_payload_hash===null)||(UUID.test(row.source_document_id||'')&&SHA.test(row.source_payload_hash||'')))&&['AP_BILL','AP_VENDOR_CREDIT'].includes(row.movement_kind)&&text(row.document_number,128)&&text(row.vendor_ref,128)&&text(row.vendor_name,255)&&/^[A-Z]{3}$/.test(row.currency||'')&&DATE.test(row.aging_date||'')&&MONEY.test(row.open_amount||'')&&((row.movement_kind==='AP_BILL'&&units(row.open_amount)>0n)||(row.movement_kind==='AP_VENDOR_CREDIT'&&units(row.open_amount)<0n));
const daysBetween=(asOf,value)=>Math.floor((Date.parse(`${asOf}T00:00:00Z`)-Date.parse(`${value}T00:00:00Z`))/86400000);

export function detectApAgingRisks(rows,{asOfDate,policy,limit=100}={}){
  if(!Array.isArray(rows)||rows.length>5000||!DATE.test(asOfDate||'')||!validPolicy(policy)||!Number.isSafeInteger(limit)||limit<1||limit>500)throw Object.assign(new Error('AP aging review requires one date, an approved policy, and bounded authoritative evidence.'),{code:'AI_AP_AGING_SCOPE_INVALID'});
  if(rows.some(row=>!validRow(row)||row.aging_date>asOfDate))throw Object.assign(new Error('AP aging review accepts only complete posted AP evidence in scope.'),{code:'AI_AP_AGING_EVIDENCE_INVALID'});
  const findings=[];
