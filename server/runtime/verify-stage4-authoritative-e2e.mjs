...
export async function verifyStage4AuthoritativeE2e({config,fetcher=globalThis.fetch}={}){
  ...
  const rowMatches=row=>row?.statement_type===scenario.statementType&&row?.account_code===scenario.accountCode&&contains(row,scenario.journalEntryId)&&contains(row,scenario.journalLineId)&&contains(row,scenario.ledgerLineId)&&contains(row,scenario.sourceDocumentId);
  expect(Array.isArray(snapshot)&&snapshot.some(row=>row?.financial_statement_snapshot_id===scenario.financialStatementSnapshotId&&rowMatches(row)&&/^sha256:[0-9a-f]{64}$/.test(String(row.snapshot_hash||''))&&/^sha256:[0-9a-f]{64}$/.test(String(row.ledger_evidence_hash||''))&&/^sha256:[0-9a-f]{64}$/.test(String(row.row_hash||''))),'immutable statement snapshot does not retain the exact lineage row');
  expect(Array.isArray(statements)&&statements.some(rowMatches),'live financial statements do not retain the exact lineage row');
