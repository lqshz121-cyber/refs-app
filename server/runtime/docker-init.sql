...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='refs_runtime') THEN
    CREATE ROLE refs_runtime LOGIN PASSWORD 'refs_runtime_local_only' NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname='refs_context_issuer') THEN
    CREATE ROLE refs_context_issuer LOGIN PASSWORD 'refs_context_issuer_local_only' NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT;
  END IF;
END;
$$;
