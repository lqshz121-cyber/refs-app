...
function mappingFor(row,mappings){
  ...
  const matches=(mappings||[]).filter(mapping=>text(mapping?.status)==='APPROVED'&&text(mapping?.mapping_id)&&text(mapping?.version)&&text(mapping?.bank_account_ref)&&/^sha256:[0-9a-f]{64}$/.test(text(mapping?.snapshot_hash))&&text(mapping?.source_type)===text(row.source_type)&&text(mapping?.entity_id)===text(row.entity_id)&&text(mapping?.company_key)===text(row.company_key)&&text(mapping?.currency)===text(row.currency)&&(row.source_type!=='BANK_TRANSACTION'||text(mapping?.bank_account_ref)===text(row.bank_account_ref)));
...
export function projectPersistedWbsInboundAutoRec({rows,mappings=[],companyControlRows=null,detailControlRows=[],persistedControlRows=null,scope=null}={}){
  ...
  for(const row of rows){
    ...
    const trace=freeze({receipt_id:row.receipt_id,receipt_ref:row.receipt_ref,receipt_hash:row.receipt_hash,raw_event_id:row.raw_event_id,source_document_id:row.source_document_id,staging_item_id:row.staging_item_id,source_record_id:row.source_record_id,source_version:row.source_version,mapping_id:resolution.mapping.mapping_id,mapping_version:resolution.mapping.version,mapping_snapshot_hash:resolution.mapping.snapshot_hash,mapping_bank_account_ref:text(resolution.mapping.bank_account_ref),external_relation_evidence:relationTrace,company_control_trace:controlTrace});
    candidates.push(freeze({review_candidate_id:canonicalRequestHash({side,trace}),stage:'STAGING_REVIEWED',side,source_type:row.source_type,entity_id:row.entity_id,company_key:row.company_key,currency:row.currency,amount:decimal(row.amount),business_date:row.business_date,accounting_date:row.accounting_date,bank_account_ref:bankAccountRef,source_record_id:row.source_record_id,source_version:row.source_version,raw_event_id:row.raw_event_id,source_document_id:row.source_document_id,staging_item_id:row.staging_item_id,mapping:freeze({mapping_id:resolution.mapping.mapping_id,version:resolution.mapping.version,snapshot_hash:resolution.mapping.snapshot_hash,bank_account_ref:text(resolution.mapping.bank_account_ref)}),external_relation_evidence:relationTrace,company_control_trace:controlTrace,trace,can_dispatch:false,can_allocate:false,can_release:false,can_create_draft:false,can_post:false}));
  }
