...
const sensitiveInput=value=>{
  ...
  if(typeof value==='object')return Object.entries(value).some(([key,item])=>(/(token|authorization|cookie|password|secret)/i.test(key)&&key!=='provider_snapshot_token_hash')||sensitiveInput(item));
  return false;
};
...
const autoRecDetailCaseEvidence=(row,trace,mappingBankAccountRef)=>{
  if(text(row?.source_type)!=='AUTOREC_PAYMENT_DETAIL')return {binding:null};
  const binding=trace?.auto_rec_case_binding;
  const required=['relation_id','relation_type','pd_guid','pb_guid','bank_account_ref','relation_receipt_hash','detail_content_hash','case_control_content_hash','policy_id','policy_version','policy_snapshot_hash','provider_snapshot_token_hash'];
  if(!binding||typeof binding!=='object'||required.some(key=>text(binding[key])==='')||text(binding.pd_guid)!==text(row.source_record_id)||text(binding.bank_account_ref)!==text(mappingBankAccountRef)||!/^sha256:[0-9a-f]{64}$/.test(text(binding.relation_receipt_hash))||!/^sha256:[0-9a-f]{64}$/.test(text(binding.detail_content_hash))||!/^sha256:[0-9a-f]{64}$/.test(text(binding.case_control_content_hash))||!/^sha256:[0-9a-f]{64}$/.test(text(binding.policy_snapshot_hash))||!/^sha256:[0-9a-f]{64}$/.test(text(binding.provider_snapshot_token_hash)))return {error:exception(row,'WBS_AUTOREC_DETAIL_CASE_BINDING_REQUIRED','AutoRec Detail requires an exact receipt-backed pd_guid-to-pb_guid case binding and mapped bank account before review projection.')};
  return {binding:freeze({relation_id:text(binding.relation_id),relation_type:text(binding.relation_type),pd_guid:text(binding.pd_guid),pb_guid:text(binding.pb_guid),bank_account_ref:text(binding.bank_account_ref),relation_receipt_hash:text(binding.relation_receipt_hash),detail_content_hash:text(binding.detail_content_hash),case_control_content_hash:text(binding.case_control_content_hash),policy_id:text(binding.policy_id),policy_version:text(binding.policy_version),policy_snapshot_hash:text(binding.policy_snapshot_hash),provider_snapshot_token_hash:text(binding.provider_snapshot_token_hash),can_match:false,can_transition:false,can_post:false})};
};
...
export function buildWbsObservedAutoRecStateHistory({observations,persistedRows,scope=null}={}){
  ...
  // This is an evidence index, not a declared WBS transition graph. It lets
  // a later provider contract be compared with every receipt-attested edge
  // (including reopen/cancel observations) without granting any REFS action.
  const transitionEvidence=new Map();
  for(const history of histories)for(const event of history.observations){
    if(!event.observed_change||!event.previous_observed_state)continue;
    const key=[event.previous_observed_state,event.observed_state].join('\u0000'),current=transitionEvidence.get(key)??{from_state:event.previous_observed_state,to_state:event.observed_state,sources:new Set(),receipt_hashes:new Set(),count:0};
    current.sources.add([history.tenant_id,history.entity_id,history.company_key,history.source_record_id].join('\u0000'));current.receipt_hashes.add(event.receipt.receipt_hash);current.count+=1;transitionEvidence.set(key,current);
  }
  const observed_transitions=freeze([...transitionEvidence.values()].map(item=>freeze({from_state:item.from_state,to_state:item.to_state,observed_transition_count:item.count,observed_source_count:item.sources.size,receipt_hashes:freeze([...item.receipt_hashes].sort()),semantics:'OBSERVED_UNVERIFIED',canonical_wbs_transition_graph:'UNKNOWN',can_transition_state:false,can_release:false,can_incur:false,can_reverse:false,can_post:false})).sort((left,right)=>`${left.from_state}\u0000${left.to_state}`.localeCompare(`${right.from_state}\u0000${right.to_state}`)));
  return freeze({evidence_type:'WBS_AUTOREC_OBSERVED_STATE_HISTORY_V1',canonical_wbs_transition_graph:'UNKNOWN',histories:freeze(histories),observed_transitions,exceptions:freeze(exceptions),can_transition_state:false,can_release:false,can_incur:false,can_reverse:false,can_post:false});
}
...
export function projectPersistedWbsInboundAutoRec({rows,mappings=[],companyControlRows=null,detailControlRows=[],persistedControlRows=null,scope=null}={}){
  ...
  for(const row of rows){
    ...
    const detailCase=autoRecDetailCaseEvidence(row,relation.trace,bankAccountRef);if(detailCase.error){exceptions.push(detailCase.error);continue;}
    ...
    const scopedControlTrace=companyControls?.size===1?[...companyControls.values()][0]:null;
    // Company Screening is a dated balance/control snapshot. It cannot be
    // silently reused for a source from another accounting month: retaining a
    // future or prior balance as if it controlled that source would corrupt
    // period control totals and reverse trace. Missing same-period evidence is
