...
function mappingFor(row,mappings){
  const matches=(mappings||[]).filter(mapping=>text(mapping?.status)==='APPROVED'&&text(mapping?.mapping_id)&&text(mapping?.version)&&/^sha256:[0-9a-f]{64}$/.test(text(mapping?.snapshot_hash))&&text(mapping?.source_type)===text(row.source_type)&&text(mapping?.entity_id)===text(row.entity_id)&&text(mapping?.company_key)===text(row.company_key)&&text(mapping?.currency)===text(row.currency)&&(row.source_type!=='BANK_TRANSACTION'||text(mapping?.bank_account_ref)===text(row.bank_account_ref)));
