...
export function assertAiFullControllerModelInputManifest(inputManifest){
  if(!exact(inputManifest,MANIFEST_KEYS)||inputManifest.schema_version!=='AI_FULL_CONTROLLER_MODEL_INPUT_MANIFEST_V1'||!UUID.test(inputManifest.snapshot_id||'')||!HASH.test(inputManifest.snapshot_hash||'')||!Number.isSafeInteger(inputManifest.chunk_count)||inputManifest.chunk_count<1||!Array.isArray(inputManifest.chunks)||inputManifest.chunks.length!==inputManifest.chunk_count||!Array.isArray(inputManifest.chunk_hashes)||inputManifest.chunk_hashes.length!==inputManifest.chunk_count||new Set(inputManifest.chunk_hashes).size!==inputManifest.chunk_hashes.length||inputManifest.chunk_hashes.some(hash=>!HASH.test(hash))||!actions(inputManifest.action_flags)||!safeAiEvidenceTree(inputManifest))fail('AI_FULL_SCAN_MODEL_OUTPUT_INPUT_INVALID','Model output requires one safe, complete retained input manifest.');
  ...
  for(const [index,chunk] of inputManifest.chunks.entries()){
    ...
    if(index>0){const first=inputManifest.chunks[0];for(const key of ['tenant_id','entity_id','accounting_period_id','release_sha'])if(chunk[key]!==first[key])fail('AI_FULL_SCAN_MODEL_OUTPUT_INPUT_INVALID','Every model chunk must use one exact accounting scope and release.');if(JSON.stringify(chunk.section_categories)!==JSON.stringify(first.section_categories))fail('AI_FULL_SCAN_MODEL_OUTPUT_INPUT_INVALID','Every model chunk must bind the same registered section set.');}
    for(const finding of chunk.findings)if(!exact(finding,FINDING_KEYS)||!UUID.test(finding.finding_id||'')||!HASH.test(finding.finding_hash||'')||!CODE.test(finding.section_category||'')||!chunk.section_categories.includes(finding.section_category)||!Number.isSafeInteger(finding.finding_index)||finding.finding_index<0||!finding.evidence||typeof finding.evidence!=='object'||Array.isArray(finding.evidence)||!CODE.test(finding.evidence.rule_id||'')||!['HIGH','MEDIUM','LOW'].includes(finding.evidence.risk_level)||finding.evidence.entity_id!==chunk.entity_id||finding.evidence.accounting_period_id!==chunk.accounting_period_id||!text(finding.evidence.reason,2000)||!text(finding.evidence.suggested_action,2000)||!safeAiEvidenceTree(finding.evidence)||digest(finding.evidence)!==finding.finding_hash)fail('AI_FULL_SCAN_MODEL_OUTPUT_INPUT_INVALID','Every model finding must be one closed retained identity and canonical scoped evidence hash.');
