...
export function createWbsOperatorAttestedPayableService({client,kernel}={}){
  ...
  return freeze({
    ...
    async attest({tenantId,entityId,expectedObservationHash,expectedProviderContentSha256,expectedCompanyCode=null,dateFrom=null,dateTo=null,reason,limit,idempotencyKey}={}){
      ...
      let observed;
      const providerArgs={limit};if(company!==null)providerArgs.company_code=company;if(hasDates){providerArgs.incurred_date_from=dateFrom;providerArgs.incurred_date_to=dateTo;providerArgs.posting_date_from=dateFrom;providerArgs.posting_date_to=dateTo;}
