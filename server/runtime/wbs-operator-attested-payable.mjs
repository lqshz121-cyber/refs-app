...
export function createWbsOperatorAttestedPayableService({client,kernel}={}){
  ...
  return freeze({
    ...
    async attest({tenantId,entityId,expectedObservationHash,expectedProviderContentSha256,reason,limit,idempotencyKey}={}){
      if(!UUID.test(text(tenantId))||!UUID.test(text(entityId))||!HASH.test(text(expectedObservationHash))||!BARE_HASH.test(text(expectedProviderContentSha256))||!Number.isSafeInteger(limit)||limit<1||limit>10||text(reason).length<8||text(reason).length>2000)fail('WBS_OPERATOR_ATTEST_INPUT_INVALID','Exact observation hashes, reason, scope, and limit are required.');
      await kernel.assertWbsOperatorPayableAttest({tenantId,entityId});
      let observed;
      try{await prepare();observed=await client.readView({toolName:'list_payables',args:{limit}});}catch{fail('WBS_OPERATOR_ATTEST_PROVIDER_UNAVAILABLE','The WBS Payable provider response was unavailable or unsafe.');}
      const publicObservation=buildWbsLivePilotObservation({observed,entityId,tool:'list_payables'});
      ...
      const rowCompanyCodes=[...new Set(observed.rows.map(row=>text(row?.company_code)).filter(Boolean))];
      const companyCodes=publicObservation.scope.company_codes.length?publicObservation.scope.company_codes:rowCompanyCodes;
      if(companyCodes.length!==1||rowCompanyCodes.length&&rowCompanyCodes.some(value=>value!==companyCodes[0]))fail('WBS_OPERATOR_ATTEST_SCOPE_INVALID','Operator attestation requires exactly one WBS company scope shared by every row.');
      let persisted;
      ...
      if(!persisted||persisted.status!=='EXCEPTION_REVIEW_REQUIRED'||persisted.signature_verified!==false||persisted.can_create_draft!==false||persisted.can_post!==false)fail('WBS_OPERATOR_ATTEST_RESULT_INVALID','Operator attestation returned an unsafe persistence result.');
