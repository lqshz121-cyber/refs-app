...
export async function startOutboxDispatchWorker({env=process.env,fetcher=globalThis.fetch,logger=console}={}){
  ...
  let healthServer=null;try{
    ...
    await Promise.all([runtimePool.query('SELECT 1'),issuerPool.query('SELECT 1')]);const readiness=await worker.checkReadiness();healthServer=createServer((req,res)=>{if(req.method!=='GET'||req.url!=='/health/ready'){res.writeHead(404,{'cache-control':'no-store','content-type':'application/json'});res.end(JSON.stringify({schema_version:'OUTBOX_DISPATCH_PUBLIC_READINESS_V1',ready:false}));return;}const response=outboxDispatchHealthResponse(worker);res.writeHead(response.status,response.headers);res.end(JSON.stringify(response.body));});await new Promise((resolve,reject)=>{healthServer.once('error',reject);healthServer.listen(config.healthPort,'127.0.0.1',resolve);});logger.info?.(JSON.stringify({event:'outbox_dispatch_ready',readiness}));worker.start();let stopping=false;const stop=async signal=>{if(stopping)return;stopping=true;logger.info?.(JSON.stringify({event:'outbox_dispatch_stopping',signal,metrics:worker.metrics}));await worker.stop();await new Promise(resolve=>healthServer.close(resolve));await Promise.allSettled([runtimePool.end(),issuerPool.end()]);};for(const signal of ['SIGTERM','SIGINT'])process.once(signal,()=>stop(signal).catch(error=>logger.error?.(JSON.stringify({event:'outbox_dispatch_stop_failed',code:error?.code||'OUTBOX_DISPATCH_INTERNAL'}))));return {worker,healthServer,stop,config,readiness};
