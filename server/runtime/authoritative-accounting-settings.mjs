...
export function projectAuthoritativeAccountingSettings(value,{tenantId,entityId,periodId}={}){
  ...
  return freeze({
    ...
    period_close_policy:{approved_period_status:close.period_status,allow_post:close.allow_post,effective_allow_post:settings.period_status==='OPEN'&&close.period_status==='OPEN'&&close.allow_post===true&&close.posting_lock===false&&close.hard_lock===false&&close.soft_lock===false,posting_lock:close.posting_lock,soft_lock:close.soft_lock,hard_lock:close.hard_lock,cutoff_date:close.cutoff_date,accrual_cutoff_date:close.accrual_cutoff_date,prepaid_boundary_date:close.prepaid_boundary_date,business_calendar:close.business_calendar,non_business_dates:[...close.non_business_dates],reversal_policy:close.reversal_policy,prior_period_adjustment_policy:close.prior_period_adjustment_policy,override_policy:close.override_policy},
