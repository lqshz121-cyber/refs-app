...
export function createAiVendorInvoiceAnomalyService({sourceReader,detailReader,evidenceReader,policyReader}={}){
  ...
  return Object.freeze({async analyze({tenantId,entityId,currentAccountingPeriodId,limit=500}){
    ...
    for(const document of (Array.isArray(documents)?documents:[]).filter(row=>row.source_system==='WBS').slice(0,limit)){
      const [details,evidence]=await Promise.all([detailReader({tenantId,entityId,sourceDocumentId:document.source_document_id}),evidenceReader({tenantId,entityId,sourceDocumentId:document.source_document_id})]);
