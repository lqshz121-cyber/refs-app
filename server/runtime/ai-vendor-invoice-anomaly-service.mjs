import {detectVendorInvoiceAmountAnomalies} from './ai-vendor-invoice-amount-anomaly.mjs';
...
export function createAiVendorInvoiceAnomalyService({sourceReader,detailReader,evidenceReader,policyReader,materializeWriter=null}={}){
  if(typeof sourceReader!=='function'||typeof detailReader!=='function'||typeof evidenceReader!=='function'||typeof policyReader!=='function')throw new Error('Vendor invoice anomaly service requires authoritative source, detail, signed evidence, and approved policy readers');
  const analyze=async({tenantId,entityId,currentAccountingPeriodId,limit=500})=>{
    if(!UUID.test(tenantId||'')||!UUID.test(entityId||'')||!UUID.test(currentAccountingPeriodId||'')||!Number.isInteger(limit)||limit<1||limit>500)throw Object.assign(new Error('Vendor invoice anomaly service scope is invalid'),{code:'AI_VENDOR_ANOMALY_SCOPE_INVALID'});
    ...
    if(!Array.isArray(documents)||!safeAiEvidenceTree(documents,{maxArrayLength:500})||!safeAiEvidenceTree(policy,{maxArrayLength:20}))throw Object.assign(new Error('Vendor invoice anomaly source or policy evidence is unsafe.'),{code:'AI_VENDOR_ANOMALY_SOURCE_INVALID'});
    const rows=[],wbsDocuments=documents.filter(row=>row.source_system==='WBS');
    if(wbsDocuments.length>=limit)throw Object.assign(new Error('The bounded vendor invoice source read cannot prove population completeness.'),{code:'AI_VENDOR_ANOMALY_POPULATION_INCOMPLETE'});
  ...
  return Object.freeze({analyze,async analyzeAndMaterialize({tenantId,entityId,currentAccountingPeriodId,limit=500,idempotencyKey}){
    if(typeof materializeWriter!=='function')throw Object.assign(new Error('Vendor anomaly persistence is unavailable'),{code:'AI_VENDOR_ANOMALY_PERSISTENCE_UNAVAILABLE'});
    if(typeof idempotencyKey!=='string'||idempotencyKey.length<8||idempotencyKey.length>200)throw Object.assign(new Error('Vendor anomaly materialization requires a stable idempotency key'),{code:'AI_VENDOR_ANOMALY_IDEMPOTENCY_INVALID'});
