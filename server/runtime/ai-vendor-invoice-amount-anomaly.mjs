...
function validPolicy(policy){return policy&&policy.schema_version==='AI_VENDOR_INVOICE_AMOUNT_ANOMALY_POLICY_V1'&&UUID.test(policy.setting_snapshot_id||'')&&SHA256.test(policy.setting_snapshot_hash||'')&&Number.isInteger(policy.policy_version)&&policy.policy_version>=1&&Number.isInteger(policy.minimum_history_periods)&&policy.minimum_history_periods>=3&&policy.minimum_history_periods<=24&&Number.isInteger(policy.ratio_threshold_basis_points)&&policy.ratio_threshold_basis_points>=15000&&policy.ratio_threshold_basis_points<=100000&&MONEY4.test(policy.minimum_absolute_delta||'');}
