...
const containsPayloadHash=value=>Array.isArray(value)?value.some(containsPayloadHash):value&&typeof value==='object'?Object.values(value).some(containsPayloadHash):typeof value==='string'&&/^sha256:[0-9a-f]{64}$/i.test(value);
...
export async function verifyStage2AuthoritativeE2e({config,fetcher=globalThis.fetch}={}){
  ...
  const [receipt,transaction,snapshot,lineage,journal,ledger,statements]=await Promise.all([
    get(fetcher,`${base}/bank/reconciliations/admitted-statements/${s.statementReceiptId}`,accessToken,'admitted bank statement'),
    get(fetcher,`${base}/bank/transactions/${s.bankSourceId}`,accessToken,'bank transaction detail'),
  ...
  ]),proof={receipt,transaction,snapshot,lineage,journal,ledger,statements};
  ...
  for(const [label,value,owner] of [
    ...
    ['bank source detail',s.bankSourceId,transaction],
  ...
  expect(contains(transaction,'bankFeed'),`bank transaction detail does not retain its source module`);
  expect(containsPayloadHash(transaction),`bank transaction detail does not retain its immutable source payload hash`);
