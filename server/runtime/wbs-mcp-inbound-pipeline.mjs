...
export function createWbsMcpInboundPipeline({pullService,signatureVerifier,inboundAdapter,inboundOrchestrator}={}){
  if(!pullService||typeof pullService.pullTransactionSnapshot!=='function'||typeof signatureVerifier!=='function'||!inboundAdapter||typeof inboundAdapter.prepare!=='function'||!inboundOrchestrator||typeof inboundOrchestrator.persist!=='function')fail('WBS_MCP_PIPELINE_DEPENDENCY_REQUIRED','Read-only pull, signature verification, inbound adapter, and inbound orchestrator are required.');
  return Object.freeze({
    ...
    async ingest({pull,tenantId,entityId,importBatchId,idempotencyKey}={}){
      ...
      let prepared;try{prepared=inboundAdapter.prepare(pulled.snapshot);}catch{fail('WBS_MCP_ADMISSION_FAILED','WBS snapshot did not meet Raw/Normalized/Staging admission rules.');}
