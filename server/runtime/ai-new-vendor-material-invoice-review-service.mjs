...
export function createAiNewVendorMaterialInvoiceReviewService({sourceReader,detailReader,evidenceReader,policyReader}={}){
  ...
  return Object.freeze({async analyze({tenantId,entityId,accountingPeriodId,limit=2000}={}){
    ...
    const [documents,policy]=await Promise.all([sourceReader({tenantId,entityId,limit}),policyReader({tenantId,entityId,accountingPeriodId})]),rows=[],wbsDocuments=(Array.isArray(documents)?documents:[]).filter(row=>row.source_system==='WBS');
    if(wbsDocuments.length>=limit)throw Object.assign(new Error('The bounded new-vendor source read cannot prove population completeness.'),{code:'AI_NEW_VENDOR_MATERIAL_POPULATION_INCOMPLETE'});
    ...
    return detectNewVendorMaterialInvoices(rows,{policy,entityId,currentAccountingPeriodId:accountingPeriodId});
  }});
}
