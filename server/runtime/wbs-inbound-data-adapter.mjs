...
export function validateWbsAutoRecG11PostedTrace({reviewRequest,postedJournals}={}){
  ...
  const traceFields=['company_key','currency','bank_account_ref','bank_business_date','bank_accounting_date','business_business_date','business_accounting_date','bank_receipt_id','business_receipt_id','bank_raw_event_id','business_raw_event_id','bank_source_record_id','bank_source_version','business_source_record_id','business_source_version','bank_staging_item_id','business_staging_item_id'];
  if(!expected||traceFields.some(field=>!text(expected[field]))||text(reviewRequest.company_key)!==text(expected.company_key)||text(reviewRequest.currency)!==text(expected.currency)||text(reviewRequest.bank_account_ref)!==text(expected.bank_account_ref)||!validIsoDate(expected.bank_business_date)||!validIsoDate(expected.bank_accounting_date)||!validIsoDate(expected.business_business_date)||!validIsoDate(expected.business_accounting_date))fail('WBS_AUTOREC_G11_TRACE_REQUIRED','AutoRec review trace is incomplete or outside its exact company, currency, bank, and accounting-date scope.');
