...
export function buildStandardDraftRequest({stagingItem,mapping,journal}={}){
  ...
  if(text(mapping?.status)!=='APPROVED'||!text(mapping?.mapping_id)||!text(mapping?.version)||!/^sha256:[0-9a-f]{64}$/.test(text(mapping?.snapshot_hash))||text(mapping?.source_type)!==text(stagingItem.source_type)||text(mapping?.company_key)!==text(stagingItem.company_key)||text(mapping?.currency)!==text(stagingItem.currency)||(text(stagingItem.source_type)==='BANK_TRANSACTION'&&text(mapping?.bank_account_ref)!==text(stagingItem.bank_account_ref)))fail('WBS_MAPPING_APPROVED_REQUIRED','An approved immutable mapping snapshot with the exact WBS source type, company, currency, and required bank-account scope is required');
...
export function validatePostedJournalTrace({draftRequest,postedEvidence}={}){
  ...
  const traceFields=['receipt_id','receipt_ref','receipt_hash','raw_event_id','source_document_id','source_record_id','source_version','source_type','company_key','currency','business_date','accounting_date','direction','mapping_id','mapping_version','mapping_snapshot_hash'];
