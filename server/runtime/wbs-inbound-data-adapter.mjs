...
export function buildStandardDraftRequest({stagingItem,mapping,journal}={}){
  ...
  const journalAmounts=journal.lines.map(line=>({debit:moneyUnits(line?.debit_amount),credit:moneyUnits(line?.credit_amount)}));
  if(journalAmounts.some(line=>line.debit===null||line.credit===null||line.debit<0n||line.credit<0n||line.debit===0n&&line.credit===0n||line.debit>0n&&line.credit>0n))fail('WBS_DRAFT_REQUEST_UNBALANCED','Each Draft request journal line must contain exactly one positive canonical debit or credit amount');
  const debit=journalAmounts.reduce((sum,line)=>sum+line.debit,0n),credit=journalAmounts.reduce((sum,line)=>sum+line.credit,0n);
  if(debit!==credit||debit<=0n)fail('WBS_DRAFT_REQUEST_UNBALANCED','Draft request journal lines must be positive and balanced');
...
export function validateWbsAutoRecG11PostedTrace({reviewRequest,postedJournals}={}){
  ...
  if(policyBound){
    ...
    const policyEffective=[expected.bank_accounting_date,expected.business_accounting_date].every(accountingDate=>mappingEffectiveOn(policy,accountingDate));
    if(!policy||typeof policy!=='object'||!['APPROVED','RETIRED'].includes(text(policy.status))||policyFields.some(field=>!text(policy[field]))||['policy_snapshot_hash','bank_mapping_snapshot_hash','business_mapping_snapshot_hash','receipt_hash'].some(field=>!/^sha256:[0-9a-f]{64}$/.test(text(policy[field])))||!policyEffective||!/^sha256:[0-9a-f]{64}$/.test(text(expected.review_plan_id))||!/^sha256:[0-9a-f]{64}$/.test(text(expected.allocation_edge_id))||!/^sha256:[0-9a-f]{64}$/.test(text(expected.proposal_allocation_edge_id))||requestPolicyMismatch)fail('WBS_AUTOREC_G11_POLICY_TRACE_REQUIRED','A policy-bound AutoRec review must carry one immutable matching-policy receipt effective on both source accounting dates and immutable review-plan and edge identities.');
  }
