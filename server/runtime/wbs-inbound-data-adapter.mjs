...
export function validateWbsAutoRecG11PostedTrace({reviewRequest,postedJournals}={}){
  ...
  const types=new Set(['PAYABLE_INCUR','AUTOC']),byType=new Map(),journalIds=new Set(),auditIds=new Set();
  for(const journal of postedJournals){
    ...
    journalIds.add(text(journal.journal_entry_id));auditIds.add(text(journal.audit_event_id));byType.set(type,journal);
  }
