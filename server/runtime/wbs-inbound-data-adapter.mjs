...
export function validateWbsAutoRecG11PostedTrace({reviewRequest,postedJournals}={}){
  ...
  if(policyBound){
    const policy=expected.matching_policy,policyFields=['policy_id','version','mapping_id','mapping_version','rule_id','rule_version','bank_mapping_id','bank_mapping_version','business_mapping_id','business_mapping_version','date_match_basis','receipt_id','receipt_ref','receipt_hash'];
    const requestPolicyMismatch=text(reviewRequest.allocation_edge_id)!==text(expected.allocation_edge_id)||text(reviewRequest.proposal_allocation_edge_id)!==text(expected.proposal_allocation_edge_id)||canonicalRequestHash(reviewRequest.matching_policy??{})!==canonicalRequestHash(policy);
    if(!policy||typeof policy!=='object'||policyFields.some(field=>!text(policy[field]))||!/^sha256:[0-9a-f]{64}$/.test(text(policy.receipt_hash))||!/^sha256:[0-9a-f]{64}$/.test(text(expected.allocation_edge_id))||!/^sha256:[0-9a-f]{64}$/.test(text(expected.proposal_allocation_edge_id))||requestPolicyMismatch)fail('WBS_AUTOREC_G11_POLICY_TRACE_REQUIRED','A policy-bound AutoRec review must carry one complete approved matching-policy receipt and both immutable edge identities.');
  }
