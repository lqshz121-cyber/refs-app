...
export function buildStandardDraftRequest({stagingItem,mapping,journal}={}){
  ...
  if(text(mapping?.status)!=='APPROVED'||!text(mapping?.mapping_id)||!text(mapping?.version)||text(mapping?.source_type)!==text(stagingItem.source_type)||text(mapping?.company_key)!==text(stagingItem.company_key)||text(mapping?.currency)!==text(stagingItem.currency)||(text(stagingItem.source_type)==='BANK_TRANSACTION'&&text(mapping?.bank_account_ref)!==text(stagingItem.bank_account_ref)))fail('WBS_MAPPING_APPROVED_REQUIRED','An approved mapping with the exact WBS source type, company, currency, and required bank-account scope is required');
  ...
  return Object.freeze({
    ...
    mapping:{mapping_id:mapping.mapping_id,version:mapping.version,source_type:mapping.source_type,company_key:mapping.company_key,currency:mapping.currency,bank_account_ref:mapping.bank_account_ref??null},
