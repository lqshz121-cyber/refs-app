...
| --- | --- | --- | --- |
| Payable Report as business-side input | Receipt-gated Payable -> Raw/Normalized/Staging -> non-dispatchable business-side review candidate; posting/journal/check/clear fields retained only as trace | LOCAL_TESTED + OBSERVED schema | Signed nonempty Payable response with immutable provider key, direction convention and revision/replay semantics |
...
| 12 golden scenarios | Exact, partial, one-to-many, many-to-one, cross-company, tolerance, duplicate, reopen boundary, NUL isolation, invalid date, 291001 trace, report-as-source block | LOCAL_TESTED | De-identified provider sample set and source-vs-target control-total comparison |
