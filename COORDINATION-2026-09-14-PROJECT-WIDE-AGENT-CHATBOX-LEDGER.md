...
| P1 | Codex integration | Cash Transfer PostgreSQL SoD/lifecycle closure completed; next select durable outbox retry/compensation | Fresh PostgreSQL 16 command receipt: `test-postgres-fresh --pattern "Cash Transfer creates a two-bank Draft, applies separated approvals, and posts one retained ledger journal"` | 1/1 pass, zero skips; owned container, network, and volume were removed. Local evidence only. |
