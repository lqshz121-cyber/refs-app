...
| Claude WBS intake review | Complete and acted on | Found missing temporal and cross-category evidence checks plus length guard defect | Length defect is fixed in `96b4a480`; retain review-order and global-distinctness gaps for the next WBS verifier improvement. No claim of 12-sample provider acceptance. |
...
|---|---|---|---|---|
| P0 | Codex integration | Re-run WBS verifier suite after `96b4a480`; decide whether review-before-post and global evidence uniqueness need a forward hardening patch | Fresh command receipt and, only if changed, a committed narrow patch | All named WBS tests pass with zero skips. Never treat a synthetic manifest as provider acceptance. |
| P0 | Claude static reviewer | Review the latest WBS verifier for temporal ordering and global evidence-ID collision semantics, using the integration paths named in the task file | Dated `CLAUDE-TO-CODEX-*` static report with exact lines and limits | Read/reason/write only; no production claims. |
| P1 | Codex integration | Select one controlled business closure still needing fresh evidence: Cash Transfer PostgreSQL SoD/lifecycle or durable outbox retry/compensation | Exact test/fixture receipt with cleanup state | Fresh isolated database run, zero skips. |
