...
}else if(internalReadOnly){
  assert.equal(declaredMode,INTERNAL_TEST_READONLY_MODE,'an internal read-only build must be explicitly marked INTERNAL_TEST_READONLY');
  assert.match(config,/window\.__REFS_OIDC__\s*=\s*null/,'the internal read-only build must not carry OIDC configuration');
