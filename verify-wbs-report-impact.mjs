...
if (impact.postedWbsJEs.length !== 4) fail(`Expected reviewed payable accrual, loan draw, property tax accrual and July amortization mock JEs, got ${impact.postedWbsJEs.length}.`);
...
const insuranceAmortizationJe = impact.postedWbsJEs.find(je => je.ai_rule_id === 'MONTHLY_PREPAID_AMORTIZATION');
if (insuranceAmortizationJe?.je_id !== 'AMORT-AP-INS-12MO-2026-07' || insuranceAmortizationJe.source_document_id !== 'DOC-INS-12MO' || !insuranceAmortizationJe.audit_trail.some(entry => entry.action === 'review-approved')) fail('Only the explicit reviewed July insurance amortization may post into the mock report impact.');
...
if (impact.statement.expenses !== 15000) fail(`Expected property tax plus July insurance expense 15000, got ${impact.statement.expenses}.`);
if (impact.statement.netIncome !== 72500) fail(`Expected net income 72500 after property tax accrual and July insurance amortization, got ${impact.statement.netIncome}.`);
