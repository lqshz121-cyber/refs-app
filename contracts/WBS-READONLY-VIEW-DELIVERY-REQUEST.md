...
- 请提供 ETL 时刻表。REFS 避开全量 delete+insert 窗口；若无法保证一致读，请输出完成标记的 **`WBS_READONLY_SNAPSHOT_V2`** 快照包（含 `snapshot_id`、captured_at、dictionary_version、每视图 content hash、`delivery.extract_started_at`／`extract_completed_at`、`consistency=COMPLETE`、`read_consistency=SNAPSHOT_ISOLATION|REPEATABLE_READ_TRANSACTION`、`pagination=PRIMARY_KEY_SEEK`）；`captured_at` 必须落在声明的提取窗口内。每个视图还须附行数以及首尾主键游标，供 REFS 验证完整分页回执；合法零行须显式标为 `row_count=0` 且两个游标均为 `null`，不得省略视图。
