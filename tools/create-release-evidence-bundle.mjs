...
const requiredCommands = [
  ...
  { name: 'stage1-payable-current-release-readback', command: 'npm.cmd run verify:stage1-payable-current-release-readback', requiredExit: 0, scope: 'same-release authenticated GET-only exact reviewed WBS Payable attachment/source identity→POSTED JE→GL plus AP Aging and financial-statement reads; must pair with stage1-payable-live-chain for offline provider signature, six-role SoD, exact TB and AP Aging amount proofs' },
  ...
  { name: 'stage3-wbs-live-chain', command: 'npm.cmd run verify:stage3-wbs-live-chain -- --provider-trust <pinned-trust.json> --receipt <receipt.json> --request-raw <request.raw> --response-raw <response.raw> --package-raw <package.json> --ingress <ingress.json> --g11 <g11.json> --gl-report <gl-report.json>', requiredExit: 0, scope: 'offline consistency gate for one production V2 provider-signed Payable/Bank AutoRec package through reviewed staging, G11 and tied GL/report files; downstream files are not independently authoritative, so this is neither full Stage 3 completion nor a substitute for same-release authenticated API E2E, and local simulation cannot satisfy it' },
  ...
  { name: 'stage3-bank-exception-live-chain', command: 'npm.cmd run verify:stage3-bank-exception-live-chain', requiredExit: 0, scope: 'same-release authenticated GET-only admitted signed WBS bank statement→exact immutable source/hash/version→BANK_PAYMENT_UNMATCHED finding→open reconciliation worksheet exception; requires no active match, clearance, adjustment Draft, or posted journal and must pair with the offline provider trust/signature/replay gate' },
  ...
  { name: 'stage3-insurance-prepaid-live-chain', command: 'npm.cmd run verify:stage3-insurance-prepaid-live-chain', requiredExit: 0, scope: 'same-release authenticated GET-only provider-signed Insurance source through coverage, independent Review, posted amortization JE, both GL legs, and Prepaid rollforward with exact IDs, hashes, versions, actors, timestamps, and MONEY4' },
  { name: 'stage3-property-rent-live-chain', command: 'npm.cmd run verify:stage3-property-rent-live-chain', requiredExit: 0, scope: 'same-release authenticated GET-only provider-signed Property Rent source through independent Review, posted AUTO JE, both GL legs, and Property P&L with exact IDs, hashes, versions, actors, timestamps, and MONEY4' },
  { name: 'stage3-reporting-live-chain', command: 'npm.cmd run verify:stage3-reporting-live-chain', requiredExit: 0, scope: 'same-release authenticated GET-only Source→POSTED JE→GL→paired TB→BS/IS/Cash matrix with exact snapshot/journal/line/ledger/source IDs and MONEY4; reporting readback only, not provider Insurance/Prepaid or Property Operations/Rent Pickup admission' },
...
const scriptCoverage = Object.fromEntries(
  ['test', 'build', 'test:release-harness', 'test:release-simulation', 'verify:external-release-gate', 'verify:authoritative-runtime-evidence', 'verify:release-s3-scanner', 'verify:release-wbs-receipt', 'verify:stage1-payable-live-acceptance', 'verify:stage1-payable-current-release-readback', 'verify:stage2-bank-live-chain', 'verify:stage3-wbs-live-chain', 'verify:stage3-bank-exception-live-chain', 'verify:stage3-cost-cwip-live-chain', 'verify:stage3-g11-live-chain', 'verify:stage3-insurance-prepaid-live-chain', 'verify:stage3-property-rent-live-chain', 'verify:stage3-reporting-live-chain', 'verify:stage4-report-live-chain']
