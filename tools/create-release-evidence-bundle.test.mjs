...
for (const name of ['root-test', 'release-simulation', 'wbs-e2e-harness', 'external-release-gate-local-sim', 'database-dictionary-live-export', 'server-pg15-fresh', 'server-pg16-fresh', 'server-pg18-fresh', 'live-ui-23-page', 'provider-s3-scanner', 'provider-wbs-receipt', 'stage1-payable-live-chain', 'stage1-payable-current-release-readback', 'stage2-bank-live-chain', 'stage3-wbs-live-chain', 'stage3-bank-exception-live-chain', 'stage3-cost-cwip-live-chain', 'stage3-g11-live-chain', 'stage3-insurance-prepaid-live-chain', 'stage3-property-rent-live-chain', 'stage3-reporting-live-chain', 'stage4-report-live-chain']) {
...
const dictionaryExport=manifest.required_commands.find(row=>row.name==='database-dictionary-live-export');
assert.match(dictionaryExport.command,/REFS_DATABASE_DICTIONARY_DATABASE_URL/);
assert.match(dictionaryExport.scope,/refs_dictionary_reader/);
assert.match(dictionaryExport.scope,/not satisfied by unit tests or local simulation/);
...
assert.equal(manifest.scripts['verify:stage1-payable-current-release-readback'], 'node server/runtime/verify-stage1-authoritative-e2e.mjs');
...
assert.equal(manifest.scripts['verify:stage3-bank-exception-live-chain'], 'node server/runtime/verify-stage3-bank-exception-authoritative-e2e.mjs');
...
const payableReadback=manifest.required_commands.find(row=>row.name==='stage1-payable-current-release-readback');
assert.match(payableReadback.scope,/same-release authenticated GET-only/);
assert.match(payableReadback.scope,/must pair with stage1-payable-live-chain/);
...
assert.equal(manifest.scripts['verify:stage3-insurance-prepaid-live-chain'], 'node server/runtime/verify-stage3-insurance-prepaid-authoritative-e2e.mjs');
assert.equal(manifest.scripts['verify:stage3-property-rent-live-chain'], 'node server/runtime/verify-stage3-property-rent-authoritative-e2e.mjs');
...
const bankException=manifest.required_commands.find(row=>row.name==='stage3-bank-exception-live-chain');
assert.match(bankException.scope,/admitted signed WBS bank statement/);
assert.match(bankException.scope,/BANK_PAYMENT_UNMATCHED finding/);
assert.match(bankException.scope,/no active match, clearance, adjustment Draft, or posted journal/);
...
const insurancePrepaid=manifest.required_commands.find(row=>row.name==='stage3-insurance-prepaid-live-chain');
assert.match(insurancePrepaid.scope,/provider-signed Insurance source/);
assert.match(insurancePrepaid.scope,/posted amortization JE/);
assert.match(insurancePrepaid.scope,/Prepaid rollforward/);
const propertyRent=manifest.required_commands.find(row=>row.name==='stage3-property-rent-live-chain');
assert.match(propertyRent.scope,/provider-signed Property Rent source/);
assert.match(propertyRent.scope,/posted AUTO JE/);
assert.match(propertyRent.scope,/Property P&L/);
...
assert.match(readme, /dedicated-reader live database dictionary export/);
