...
| A8 | Branch protection on `main`, `github-pages` environment approval | GitHub settings (not in repo) | L16 receipt | — | — | **run** 2026-09-17: GitHub API `branches/main` → `protected:false`; PR #582 already merged | **GAP — main unprotected; Owner to add rule** | ☐ |
...
| A12 | Staging read-back (2026-09-17): API 98e3431 ready but crash-looping on idle pg client errors; internal-test-api same; outbox worker suspended since 09-02; static app 12e6a31 (SHA drift); Node 26 runtime; PG 18 | `server/runtime/db.mjs` (pool error listener fix) | `server/tests/db-pool-idle-error.test.mjs`; S31 receipt | fix commit on branch | unit 1/1; posting-sod 7/7 | **run**: 320bcbad deployed to staging 2026-09-17 12:21–12:35 GMT+8 (API + internal-test-api + refs-app); /health/ready release = 320bcbad on both APIs; refs-build.js = 320bcbad; migrations 421/422/423 completed in preDeploy; RELEASE_MISMATCH cleared; crash-loop observation window open | PARTIAL — awaiting 6 h crash-free window, refs-internal-test static still 98e3431, outbox worker suspended | ☐ |
| A13 | CI gate ran on the deployed candidate SHA | `.github/workflows/accounting-kernel-ci.yml` | L17 receipt | 320bcbad | — | **run**: 0 workflow runs for 320bcbad / branch; main ff163552 Kernel Gate = failure | **GAP — staging runs an un-gated build; open PR to trigger** | ☐ |
