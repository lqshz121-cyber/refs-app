...
The unreleased 346 installer additionally takes ACCESS EXCLUSIVE locks on the journal/source/register/binding/posting/source-link tables before function replacement. A two-connection PostgreSQL test proves the installer waits on a real pre-upgrade source writer, then commits and routes the next insertion through the serialization fence. The installation test precedes any native binding fixtures so its protected downgrade is exercised only with empty acquisition history. This installation is a blocking schema operation; a production rollout still needs the final release checks.

## Canonical source attachment identity (347, acceptance pending)

Line-only SOURCE_ATTACHMENT links now resolve their scoped document before the existing source lock and serialization fence. Mismatched line/document identities are rejected. The normalizer follows document-then-line locking and rechecks identity after waiting. Source line tenant, entity, document and UUID identity cannot be reassigned; corrections need a new source version. This identity rule does not yet freeze mutable accounting evidence fields.

Existing ambiguous attachment links are not rewritten. Installation rejects affected acquisition bindings; creation of a new binding rejects ambiguous retained source attachments and rolls back its business artifacts. The independent security context issuance audit remains recorded. Rollback refuses to remove protection while acquisition bindings remain.

Real PostgreSQL tests cover canonicalization, invalid identities, native Draft rollback, historical installation rejection with no partial schema or business changes, protected down and empty down/up, and six attachment/Post transaction variants. Two additional lock-order cases attempt to move a line to another valid document in the same company while attachment insertion runs; the move is rejected and the link remains consistent. PG16 r4 passed seven focused tests before the final down-lock alignment; exact candidate gates remain required. Earlier r2 full fixed-asset suite ran 36 tests with one incorrect total-audit assertion (security issuance was counted as business audit); it is retained as a failed run, not a full pass.

346/347 schema changes require accounting writers to be drained before installation or rollback. The single-writer wait test does not prove safe online migration with all concurrent writer lock orders. Production writer-drain enforcement and release acceptance remain open, along with original classification/provider row-hash validation, evidence-field mutation protection, HTTP/UI commands and the complete platform goal.
